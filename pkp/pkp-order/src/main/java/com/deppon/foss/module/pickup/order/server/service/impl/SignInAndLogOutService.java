 ...
 */
public class SignInAndLogOutService implements ISignInAndLogOutService {
	...
	* @param @return    设定文件
	 */
	@Override
	public List<PdaSignDto> querySignedInfoByPageForLeased(PdaSignDto dto,
			int start, int limit) {
		return pdaSignEntityDao.querySignByDVByPageForLeased(dto, start, limit);
	}
	...
	public int handResolveBind(PdaSignEntity pdaEntity) {
		...
		if(PdaSignStatusConstants.BUNDLE.equals(entity.getStatus())){//14.7.17gcl  防止连续点击多次解绑按钮 重复提交
			...
			//推送状态给oms系统(零担暂时不做推送)；update11-25零担快递都推送
			try {
			...
			//2014.7.17 gcl  解绑成功后如果工作状态没有 执行新增
			...
			expressWorkerStatusParam.setVehicleNo(entity.getVehicleNo());
			expressWorkerStatusParam.setBusinessArea(ExpressWorkerStatusConstants.ORDER_BUSINESS);
			...
			// 结束  2014.7.17 gaochunling 解绑成功后 更新快递员工作状态  native='N'
	...
	public void autoUnbundle() {
		...
		pdaSignDto.setUserType(PdaSignStatusConstants.USER_TYPE_DRIVER);
	 ...
	 * 查询外请车签到信息
	* @author 329757-foss-liuxiangcheng 
	* @date 2016-7-13 下午4:36:07
	* @param @param dto
	* @param @return    设定文件
	 */
	@Override
	public Long querySignedInfoCountForLeased(PdaSignDto dto) {
		return pdaSignEntityDao.querySignedInfoCountForLeased(dto);
	}

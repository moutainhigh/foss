 ...
 */
public class ImportantGoodsNotifyJMSService implements IImportantGoodsNotifyJMSService{
	/**
	 * 日志
	 */
	protected final static Logger logger = 
								LoggerFactory.getLogger(ImportantGoodsNotifyJMSService.class.getName());
	/***
	 * 上报OA接口
	 */ 
	...
	public void processNotifyImportantGoods() {
		List<RecordErrorImportantWaybillResultDto> recordDtoList = recordErrorWaybillDao.findImportantReportOAExceptionList();
		if(CollectionUtils.isNotEmpty(recordDtoList)){
			QmsErrorReportResponseDto resDto = null;
			for(RecordErrorImportantWaybillResultDto recordDto : recordDtoList){
				...
				wentity.setWaybillNo(recordDto.getWaybillNo());
				wentity.setActive("Y");
				//查询签收的签收结果信息
				WaybillSignResultEntity wrentity = waybillSignResultDao.queryWaybillSignResult(wentity);
				//签收时间
				if(null!=wrentity.getSignTime()){
					recordDto.setSignTime(wrentity.getSignTime());
				}else{
					recordDto.setSignTime(null);
				}
				...
				if(waybillEntity!=null){
					...
					if(CollectionUtils.isNotEmpty(deliverbilldtoList) && deliverbilldtoList.get(0) != null){
						...
						//查询员工信息
						EmployeeEntity emp = employeeService.querySimpleEmployeeByEmpCode(deliverbilldtoList.get(0).getDriverCode());
						//获取司机所在部门编码
					...
					//记录日志
					logger.info("*************运单号：" + waybillNo + "上报QMS重大货物异常差错***********start");
					try{
						//获取零担重大货物异常差错上报的json字符串
						String errorInfo = qmsErrorService.getLDImportantErrorQmsInfo(waybillEntity,recordDto);
						//零担差错上报
						resDto = qmsErrorReportService.reportQmsImportantError(errorInfo);
						logger.info("---------------------------------"+resDto);
						//记录日志
						logger.info("*************运单号：" + waybillNo + "上报QMS重大货物异常差错***********end");
						//处理返回结果
						if(resDto != null){
							//处理OA返回数据
						...
						//处理异常
						logger.error("运单号" + waybillNo + "重大货物异常差错上报QMS失败：" + e.getMessage());
						//记录异常日志
					...
					//记录日志
					logger.info("运单号：" + waybillNo + "上报QMS重大货物异常差错失败,该运单不存在。");
					//记录异常日志
					resDto = new QmsErrorReportResponseDto();
					resDto.setMessage("上报QMS重大货物异常差错失败,该运单不存在。");
					this.saveReportOALog(recordDto,resDto,ReportConstants.REPORT_OA_ERROR);
				}
				//销毁对象
				resDto = null;
	 ...
	 * @author 306548-foss-honglujun
	 */
	private void processReturnInfo(RecordErrorImportantWaybillResultDto recordDto,QmsErrorReportResponseDto resDto){
		String isSuccess=StringUtils.EMPTY;
		if (StringUtils.isNotEmpty(resDto.getHandleCode())
						&& resDto.getHandleCode().equals(ReportConstants.QMS_RETURN_SUCCESS)) {
			// 上报成功
			isSuccess=ReportConstants.REPORT_OA_SUCCESS;;
			int flag = saveReportOALog(recordDto,resDto,isSuccess);
			//此数据无需再上报
			recordErrorWaybillDao.updateImportantEntity(recordDto.getId());
			if (flag == 1) {
				...
				logger.info("#####保存上报QMS成功日志失败！######");
						...
						&& resDto.getHandleCode().equals(ReportConstants.QMS_RETURN_REPEAT)) {
			//数据重复上报
	 ...
	 * @author 306548-foss-honglujun
	 */
	private int saveReportOALog(RecordErrorImportantWaybillResultDto recordErrorImportantWaybillResultDto,
		...
		int flag = this.addReportOaLog(recordErrorImportantWaybillResultDto,resDto,msg,isSuccess);
		return flag;
	}
	 ...
	 * @author 306548-foss-honglujun
	 */
	private int addReportOaLog(RecordErrorImportantWaybillResultDto recordErrorImportantWaybillResultDto,
		...
		if(StringUtils.isNotEmpty(isSuccess) && isSuccess.equals(ReportConstants.REPORT_OA_SUCCESS)){
			...
			jobSuccessLogEntity.setJobName(SignConstants.EXCEPTION_IMPORTANT_NOTIFY_PROCESS_JOB);// job名称
			jobSuccessLogEntity.setCreateTime(new Date());
			//调用QMS接口成功返回差错编号
			jobSuccessLogEntity.setSuccessMsg("差错编号:" + resDto.getErrorID() 
					+ ";运单号:" + recordErrorImportantWaybillResultDto.getWaybillNo() + ";" + msg);
			jobSuccessLogEntity.setSuccessId(recordErrorImportantWaybillResultDto.getId());
			...
			jobExceptionLogEntity.setJobName(SignConstants.EXCEPTION_IMPORTANT_NOTIFY_PROCESS_JOB);// job名称
			jobExceptionLogEntity.setCreateTime(new Date());
			//调用QMS接口失败返回失败消息
			jobExceptionLogEntity.setErrorMsg("运单号:" 
					+ recordErrorImportantWaybillResultDto.getWaybillNo() + ";" +msg);
			jobExceptionLogEntity.setErrorId(recordErrorImportantWaybillResultDto.getId());

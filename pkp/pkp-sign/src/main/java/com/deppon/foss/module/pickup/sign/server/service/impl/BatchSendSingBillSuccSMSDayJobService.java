 ...
 */
public class BatchSendSingBillSuccSMSDayJobService implements IBatchSendSMSJobService {
	
	private static final Logger logger = LoggerFactory.getLogger(BatchSendSingBillSuccSMSDayJobService.class);
		...
		if(CollectionUtils.isNotEmpty(resultList)){
			for (CustomerEntity customerEntity : resultList) {
				...
				if(StringUtils.isNotEmpty(cusCode)){
					//queryReceiveAndSignBillGoodsCount
					List<BatchSendSMSDto> signBillGoodsDtoList=waybillDao.querySignBillGoodsCount(cusCode,1);
					List<BatchSendSMSDto> receiveGoodsDtoList=waybillDao.queryReceiveGoodsCount(cusCode,1);
					...
					if(CollectionUtils.isNotEmpty(receiveAndSignBillGoodsDtoList)&&StringUtils.isNotEmpty(unifiedCode)){
						combineSMSData(receiveAndSignBillGoodsDtoList,cusCode,unifiedCode);
					}
				}
			}
		}
			...
			for (BatchSendSMSDto receiveAndSignBill: receiveGoodsDtoList) {
				for (BatchSendSMSDto sendAndReceive : signBillGoodsDtoList) {
					if(StringUtils.equals(sendAndReceive.getCustomerMobile(),receiveAndSignBill.getCustomerMobile())){
						...
						validSendExtracted(map, sendAndReceive);
						validSendExtracted(map, receiveAndSignBill);
					}
				}
			}
		}
	...
	private void validSendExtracted(Map<String, BatchSendSMSDto> map,
			BatchSendSMSDto sendAndReceive) {
		if(map.containsKey(sendAndReceive.getCustomerMobile())){
			if(map.get(sendAndReceive.getCustomerMobile()).getFlag().equals(SignConstants.EXIST)){
				
			}else{
				map.put(sendAndReceive.getCustomerMobile(), sendAndReceive);
				sendAndReceive.setFlag(SignConstants.NOTEXIST);
			}
		}else{
			map.put(sendAndReceive.getCustomerMobile(), sendAndReceive);
			sendAndReceive.setFlag(SignConstants.NOTEXIST);
	...
	private  void combineSMSData(List<BatchSendSMSDto> list,String custCode,String unifiedCode) {
		...
		synchronized (BatchSendSingBillSuccSMSDayJobService.class) {
	...
	private SmsParamDto getTemplateInfo(BatchSendSMSDto batchSendSMSDto,String unifiedCode){
		...
		//<2
		if(batchSendSMSDto.getSignBillSuccessBackCount()==null||batchSendSMSDto.getSignBillSuccessBackCount().intValue()==0){
			dto.setOrgCode(getOrgCode(unifiedCode)!=null?getOrgCode(unifiedCode):"");
			dto.setSmsCode(SignConstants.BATCH_SEND_SIGN_QTY);
			...
			dto.setOrgCode(getOrgCode(unifiedCode)!=null?getOrgCode(unifiedCode):"");
			dto.setSmsCode(SignConstants.BATCH_SEND_SIGN_PROCESS_BILL_RETURN_QTY);
			map.put("signBillSuccessCount", batchSendSMSDto.getSignBillSuccessCount()!=null&&batchSendSMSDto.getSignBillSuccessCount()>0?batchSendSMSDto.getSignBillSuccessCount().toString():"0");

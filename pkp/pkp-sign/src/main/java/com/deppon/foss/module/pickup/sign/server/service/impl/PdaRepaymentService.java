 ...
 * PDA结清货款Service.
 * 
 * @author 309603 yangquang
 * @date 2016-03-04
 * @since
 * @version
 */
public class PdaRepaymentService implements IPdaRepaymentService {
	...
	private static final String SERVICE_CODE = "com.deppon.foss.module.pickup.sign.server.service.impl.PdaRepaymentService";

	/** 日志服务. */
	private static final Logger LOGGER = LoggerFactory
			.getLogger(PdaRepaymentService.class);
	...
	/**
	 * POS端自动结清货款 新增付款信息(内含财务接口调用) 零担业务自用 供PDA运单结清货款
	 * 
	 * @author 309603 yangqiang
	 * @date 2016-3-1
	 * @param CommonQueryParamDto
	 *            dto empCode 员工工号 empName 员工姓名 orgCode 登录部门 isDriver 是否为司机
...
//    			}
				...
				//List<PosCardEntity> posCardEntitys = pdaPosManageService.queryPosCardData(posCardManageDto).getPosCardEntitys();
					...
					for (int i = 0; i < repaymentDtolist.size(); i++) {
						...
						if (amount.compareTo(BigDecimal.ZERO) == 1) {
							MutexElement mutexElement = new MutexElement(repaymentDto.getWaybillDto().getWaybillNo(),"结清货款", MutexElementType.REPAYMENT_NO);
							// 互斥锁定
							...
							if (!isLocked) {
								...
								for (int j = i; j < repaymentDtolist.size(); j++) {
									...
									if (j == i) {
										msgMap.put(repaymentDtolist.get(j).getWaybillDto().getWaybillNo(),"此单据正在操作中，请稍后再试！");
									} else {
										msgMap.put(repaymentDtolist.get(j).getWaybillDto().getWaybillNo(),"请稍后再试！");
									}
								}
								break;
							}
							// 获取还需要支付的代收货款
							BigDecimal receiveableAmount = repaymentDto.getFinancialDto().getReceiveableAmount() == null ? BigDecimal.ZERO : repaymentDto.getFinancialDto().getReceiveableAmount();
							// 获取还需支付的运费
							BigDecimal receiveablePayAmoout = repaymentDto.getFinancialDto().getReceiveablePayAmoout() == null ? BigDecimal.ZERO : repaymentDto.getFinancialDto().getReceiveablePayAmoout();
							// 准备user 数据
							...
							/*try {
								Class c = Class.forName("com.deppon.foss.framework.server.context.SessionContext");
								Constructor c0 = c.getDeclaredConstructor();

								c0.setAccessible(true);
								SessionContext po = (SessionContext) c0.newInstance();
							} catch (Exception e1) {
								e1.printStackTrace();
							}*/
							...
							if (amount.compareTo(receiveableAmount.add(receiveablePayAmoout)) != -1) {
								RepaymentEntity repaymentEntity = increaseRepaymentEntity(dto, repaymentDto, receiveableAmount,receiveablePayAmoout);
								// 新增付款信息
								try {
									//结清货款--判断是否为整车 306579 guoxinru
									if(isVehicle(repaymentEntity.getWaybillNo())){
										org.apache.commons.beanutils.BeanUtils.copyProperties(vtsDto, repaymentEntity);
										//调用整车web结清货款service
										//封装日志实体
										addLog(vtsDto, interfaceEntity,repaymentEntity);
										LOGGER.info("整车PDA结清货款service，运单号："+repaymentEntity.getWaybillNo());
										vtsValidateAndSettlementService.ValidateAndSettlement(vtsDto, currentInfo);
										//foss同步结清状态至vts
										...
										pdaRepaymentService.addPaymentInfo(repaymentEntity, currentInfo,repaymentDto.getWaybillDto());
									}
								} catch (RepaymentException e) {
									errMsgByBusiness(pdaRepaymentDto,wayBillNoList, msgMap,repaymentDtolist, i, e.getErrorCode());
									e.printStackTrace();
									...
									errMsgByBusiness(pdaRepaymentDto, wayBillNoList,msgMap, repaymentDtolist, i, "线路繁忙，请稍后重试");
									e.printStackTrace();
									StringWriter writer = new StringWriter();
						            e.printStackTrace(new PrintWriter(writer));
									// 接口日志添加失败信息
									...
									//插入接口日志表
									boolean logFlag = this.esbInterfaceLogService.addInterfaceLog(interfaceEntity);
									if(logFlag){
										LOGGER.info("接口日志表新增成功！运单号："+repaymentEntity.getWaybillNo());
									}else{
										LOGGER.info("接口日志表新增失败！运单号："+repaymentEntity.getWaybillNo());
									}
								}
								// 可用金额减少
								amount = amount.subtract(receiveableAmount.add(receiveablePayAmoout));
								if ((repaymentEntity.getStorageFee()==null?BigDecimal.ZERO:repaymentEntity.getStorageFee()).compareTo(BigDecimal.ZERO) == 1) {// 保管费不为0；
								...
								if (amount.compareTo(receiveablePayAmoout) != -1) {// 可用余额大于等于运费
									// 准备结清数据
									RepaymentEntity repaymentEntity = increaseRepaymentEntity(dto,repaymentDto,amount.subtract(receiveablePayAmoout),receiveablePayAmoout);
									// 调用结清方法去结清数据
									try {
										//结清货款--判断是否为整车 306579 guoxinru
										if(isVehicle(repaymentEntity.getWaybillNo())){
									...
									amount = amount.subtract(receiveableAmount.add(receiveablePayAmoout));
									// 获取付款信息
									//repaymentDto = repaymentService.queryPaymentByWaybillNo(repaymentEntity.getWaybillNo());
									...
									if (amount.compareTo(BigDecimal.ZERO) == 1) {// 可用余额大于0
										// 可用余额小于运费，可用余额结运费
										repaymentEntity = increaseRepaymentEntity(dto, repaymentDto,BigDecimal.ZERO, amount);
										try {
											//结清货款--判断是否为整车 306579 guoxinru
											if(isVehicle(repaymentEntity.getWaybillNo())){
												org.apache.commons.beanutils.BeanUtils.copyProperties(vtsDto, repaymentEntity);
	...
	private void addLog(SettlementPayToVtsDto vtsDto,
			InterfaceLogEntity interfaceEntity, RepaymentEntity repaymentEntity) {
		/**
		 * 封装接口日志信息实体
		 */
	...
	private void errMsgByBusiness(PdaRepaymentDto pdaRepaymentDto,List<String> wayBillNoList, Map<String, String> msgMap,ArrayList<RepaymentDto> repaymentDtolist, int i, String e) {
	 ...
	 * @param repaymentDto
	 */
	private void unsettled(PdaRepaymentDto pdaRepaymentDto,List<CommonQueryParamDto> commonQueryParamDtoList,RepaymentDto repaymentDto) {
		// 准备返回数据
	...
	private void checkWayBillNo(ArrayList<RepaymentDto> repaymentDtolist,List<String> waybillNoList,String orgCode) {
		// 判空
		...
		for (int i = 0; i < waybillNoList.size(); i++) {
			...
			if (isLack|| repaymentDto.getWaybillDto().getReceiveMethod().indexOf(SignConstants.RECEIVE_METHOD) == -1|| WaybillConstants.MONTH_PAYMENT.equals(repaymentDto.getWaybillDto().getPaidMethod())
					|| ProductEntityConstants.PRICING_PRODUCT_AIR_FREIGHT.equals(repaymentDto.getWaybillDto().getProductCode())|| ProductEntityConstants.PRICING_PRODUCT_PARTIAL_LINE.equals(repaymentDto.getWaybillDto().getProductCode())
					|| ProductEntityConstants.PRICING_PRODUCT_FULL_VEHICLE.equals(repaymentDto.getWaybillDto().getProductCode())|| WaybillConstants.directDetermineIsExpressByProductCode(repaymentDto.getWaybillDto().getProductCode())) {
	...
	public void addPaymentInfo(RepaymentEntity repaymentEntity,CurrentInfo currentInfo, WaybillDto waybilldto) {
		...
		// 判断运单 是否已终止或已作废
		if (actentity != null&& (WaybillConstants.ABORTED.equals(actentity.getStatus()) || WaybillConstants.OBSOLETE.equals(actentity.getStatus()))) {
			// 抛出业务异常
		...
		if (waybillEntity != null&& WaybillConstants.directDetermineIsExpressByProductCode(waybillEntity.getProductCode())) {
			...
			if (waybillExpress != null&& StringUtils.isNotBlank(waybillExpress.getReturnType())&& StringUtils.isNotBlank(waybillExpress.getOriginalWaybillNo())) {
				...
				if (twoInOneWaybillDto != null&& FossConstants.YES.equals(twoInOneWaybillDto.getIsTwoInOne())) { // 是子母件
					// 得到原单 结算应收单信息
					financialDtoOriginal = repaymentService.queryFinanceSign(twoInOneWaybillDto.getMainWaybillNo());
				} else {
					// 得到原单 结算应收单信息
					financialDtoOriginal = repaymentService.queryFinanceSign(waybillExpress.getOriginalWaybillNo());
				}
				// 应收代收款
				...
				if ((!SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__DEBT.equals(repaymentEntity.getPaymentType()))&& (!SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__CREDIT.equals(repaymentEntity.getPaymentType()))
						&& (!repaymentEntity.getCodAmount().equals(receiveableAmount) || !repaymentEntity.getActualFreight().equals(receiveablePayAmoout.add(receiveablePayAmooutOriginal)))) {
					// 不是 抛出业务异常
				...
				twoInOneWaybillDto = waybillRelateDetailEntityService.queryWaybillRelateByWaybillOrOrderNo(params);
				// 判断是否是子母件
				validaEntity(repaymentEntity, currentInfo, waybilldto,
						twoInOneWaybillDto);
			}
		}
		// 创建 付款 对象 充作查询条件
		...
		for (RepaymentEntity repayment : repaymentList) {
			// 财务单据未生成 或 财务单据生成中
			if (RepaymentConstants.STLBILL_NOGENERATE.equals(repayment.getStlbillGeneratedStatus())|| RepaymentConstants.STLBILL_GENERATEING.equals(repayment.getStlbillGeneratedStatus())) {
		...
		if (FossConstants.ACTIVE.equals(actentity.getArriveCentralizedSettlement()) && !isReturnOrders) {
			...
			} else if (repaymentEntity.getActualFreight().compareTo(BigDecimal.ZERO) == 0&& repaymentEntity.getCodAmount().compareTo(BigDecimal.ZERO) > 0) {
		...
		if (twoInOneWaybillDto != null&& FossConstants.YES.equals(twoInOneWaybillDto.getIsTwoInOne())) { // 为子母件
			...
			if (!repaymentEntity.getWaybillNo().equals(twoInOneWaybillDto.getMainWaybillNo())) { // 为子件
				...
				if (!repaymentService.isPayment(twoInOneWaybillDto.getMainWaybillNo())) { // 没结清
					// 得到原单 结算应收单信息
					FinancialDto financialMain = repaymentService.queryFinanceSign(twoInOneWaybillDto.getMainWaybillNo());
					// 应收代收款
	 ...
	 */
	@Transactional
	private void settleUpRepayment(RepaymentEntity repaymentEntity,CurrentInfo currentInfo, BigDecimal receiveableAmount,BigDecimal receiveablePayAmout,TwoInOneWaybillDto twoInOneWaybillDto, WaybillDto waybilldto) {
		// 生成付款编号
		...
		if ((SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__CASH).equals(repaymentEntity.getPaymentType())) {
			...
			if (BigDecimalOperationUtil.compare(receiveableAmount,BigDecimal.ZERO)&& BigDecimalOperationUtil.compare(receiveablePayAmout,BigDecimal.ZERO)) {
			...
			if ((SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__DEBT).equals(repaymentEntity.getPaymentType())|| (SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__CREDIT).equals(repaymentEntity.getPaymentType())) {
				...
				if (SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__CARD
						...
						&& (StringUtil.isBlank(repaymentEntity.getClaimNo()) || !repaymentEntity
								.getClaimNo().matches("[0-9]+"))) {
				...
				if (BigDecimalOperationUtil.compare(receiveableAmount,BigDecimal.ZERO)&& BigDecimalOperationUtil.compare(receiveablePayAmout,BigDecimal.ZERO)&& BigDecimalOperationUtil.compare(
						repaymentEntity.getStorageFee()==null?BigDecimal.ZERO:repaymentEntity.getStorageFee(),BigDecimal.ZERO)) {
					// 更新ActualFreight表中的结清状态为已结清
					updateActualFreightEntity(repaymentEntity);
				} else {
					// 判断是否是子母件
				 ...
				 * @date 2016-2-23
				 */
				if (SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__CARD.equals(repaymentEntity.getPaymentType())&& (!StringUtil.isBlank(repaymentEntity.getClaimNo()) && repaymentEntity.getClaimNo().matches("[0-9]+"))) {
					// 获取流水号内总金额
					PosCardManageDto posCardManageDto = new PosCardManageDto();
	...
	private void validaRepayment(RepaymentEntity repaymentEntity,
		...
		if (twoInOneWaybillDto != null&& FossConstants.YES.equals(twoInOneWaybillDto.getIsTwoInOne())) { // 是子母件
			if (repaymentEntity.getWaybillNo().equals(twoInOneWaybillDto.getMainWaybillNo())) { // 为子母件的母件
				...
				if (SettlementConstants.CONFIRM_PAYMENT_SUCCESS_AND_PAYMENT_SETTLE.equals(message)&& BigDecimalOperationUtil.compare(repaymentEntity.getStorageFee()==null?BigDecimal.ZERO:repaymentEntity.getStorageFee(),BigDecimal.ZERO)) {
					// 更新ActualFreight表中的结清状态为已结清
					updateActualFreightEntity(repaymentEntity);
				}
			} else {
	 ...
	 * @return
	 */
	private RepaymentEntity increaseRepaymentEntity(CommonQueryParamDto dto,RepaymentDto repaymentDto, BigDecimal receiveableAmount,BigDecimal receiveablePayAmoout) {
		...
		repaymentEntity.setPaymentType("CD"); // 付款类型
	...
	private void verification(RepaymentEntity repaymentEntity,CurrentInfo currentInfo, WaybillEntity waybillEntity) {
		 ...
		 * FossConstants.YES.equals(waybillEntity.getIsWholeVehicle())) {
		 * boolean arrival =
		 * FossConstants.YES.equals(waybillEntity.getIsPassOwnDepartment()); if
		 * (!arrival) { //如果当前部门==最终配载部门 if
		 * (currentInfo.getCurrentDeptCode().equals
		 * (waybillEntity.getLastLoadOrgCode())) { arrival=true; } } //update by
		 * foss 231434 2015-6-2
		 * //对整车(三级)产品，若开单时间在2013-08-01之前(不包含2013-08-01)，不做校验 Date date =
		 * DateUtils.convert(dateStr, format); if
		 * (date.before(waybillEntity.getBillTime())) { // 校验运单是否配载是否到达 String
		 * message =
		 * arrivalService.validateWaybillNo(repaymentEntity.getWaybillNo(),
		 * arrival); if(message != null){ throw new RepaymentException(message);
		 * } } } else { // 判断当前部门是否是最终到达部门
		 * if(!currentInfo.getCurrentDeptCode().equals
		 * (waybillEntity.getLastLoadOrgCode())){ throw new
		 * RepaymentException("当前部门与最终到达部门不一致，不能结清货款！"); } }
		 */
			...
			if(SettlementConstants.TYPE_CUBC.equals(vestSystemCode)){
				//调用CUBC物流交易单查询接口校验
					...
					try {
						...
						LOGGER.error("CUBC查询物流交易单接口异常,异常信息为："+e.getMessage());
        ...
        if(SettlementConstants.TYPE_FOSS.equals(vestSystemCode)){
			...
			for (BillReceivableEntity billReceivableEntity : list) {
				if(SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__FREIGHT_COLLECT.equals(billReceivableEntity.getPaymentType())){
					unverifyAmount = unverifyAmount.add(billReceivableEntity.getUnverifyAmount()==null? BigDecimal.ZERO:billReceivableEntity.getUnverifyAmount());
				}
			}
	 ...
	 * @return
	 */
 	public CommonQueryParamDto checkWaybill(String driverCode, String waybill) {
		// 根据工号去查签到表获取车牌号
	...
	private CommonQueryParamDto checkWaybillForAss(String orgCode,String waybill) {
		...
		airAgencyQueryDto.setWaybillNo(waybill);
		airAgencyQueryDto.setProductCode("ALL");
		RepaymentDto repaymentDto = repaymentService.queryPaymentByWaybillNo(waybill);
		if(repaymentDto==null||repaymentDto.getWaybillDto()==null) {
			throw new PdaProcessException("查询不到付款信息。。。");
		}
		airAgencyQueryDto.setIsExpress(repaymentDto.getWaybillDto().getIsExpress());				
		List<AirAgencyQueryDto> list = this.queryAirAgencyQueryDtoList(airAgencyQueryDto,orgCode);
		if (null ==list || list.isEmpty()) {
			throw new PdaProcessException("该运单，请先入库。。。");
		}*/
	...
	private void addCommon(String waybill, CommonQueryParamDto dto) {
		//获取系统归属传给PDA
		String vestSystemCode = SettlementConstants.TYPE_FOSS;
		try {
			List<String> arrayList = new ArrayList<String>();
          	arrayList.add(waybill);
          	CubcGrayRequestEntity requestDto = new CubcGrayRequestEntity(arrayList,
          			SettlementDictionaryConstants.BILL_RECEIVABLE__SOURCE_BILL_TYPE__WAYBILL,SERVICE_CODE+".addCommon",
          			SettlementConstants.TYPE_FOSS);
          	CubcGrayResponseEntity response = (CubcGrayResponseEntity)HttpClientUtils.postMethod(requestDto, new CubcGrayResponseEntity(),grayByWaybillNoUrl);
          	List<VestBatchResult> list1 = response.getVestBatchResult();
          	vestSystemCode = list1.get(0).getVestSystemCode();
		...
		/**
		 *  2016/11/24 优化
		 *  运单号的到达部门(提货网点)在【行政组织业务属性-营业部信息】中的是否加盟网点
		 *  357637 yuanhuijun
		 */
		...
		BigDecimal receiveableAmount = repaymentDto.getFinancialDto().getReceiveableAmount() == null ? BigDecimal.ZERO: repaymentDto.getFinancialDto().getReceiveableAmount();
		BigDecimal receiveablePayAmoout = repaymentDto.getFinancialDto().getReceiveablePayAmoout() == null ? BigDecimal.ZERO: repaymentDto.getFinancialDto().getReceiveablePayAmoout();
		BigDecimal receivedAmount = repaymentDto.getFinancialDto().getReceivedAmount() == null ? BigDecimal.ZERO: repaymentDto.getFinancialDto().getReceivedAmount();
		BigDecimal receivedPayAmount = repaymentDto.getFinancialDto().getReceivedPayAmount() == null ? BigDecimal.ZERO: repaymentDto.getFinancialDto().getReceivedPayAmount();
...
//		dto.setToPayAmount(repaymentDto.getWaybillDto().getToPayAmount());
		...
		dto.setUnPayAmount(receiveableAmount.add(receiveablePayAmoout).add(receivedAmount).add(receivedPayAmount));
	}
	...
	private void checkCustomerPickupOrgCode(String customerpickuporgcode) {
		...
		SaleDepartmentEntity saleDept = saleDepartmentService.querySaleDepartmentInfoByCode(customerpickuporgcode);
		if(null == saleDept){
			throw new SettlementException("运单对应的系统维护的提货部门信息为空!!!");
		}
		if (FossConstants.YES.equals(saleDept.getIsLeagueSaleDept()) || isPTP(customerpickuporgcode,saleDept.getName())) {
	...
	private boolean isPTP(String customerpickuporgcode, String name) {
		...
		if(name.length() < FossConstants.MIN_INDEX){
			throw new SettlementException("运单提货部门编码:"+customerpickuporgcode+"对应的系统维护的到提货门名称:"+name+"异常!!!");
		}
		//截取到达部门的前三位
		String startName = name.substring(FossConstants.START_INDEX, FossConstants.END_INDEX);
		//判断是否为合伙人部门
		if(FossConstants.START_WHITH_H.equals(startName) || FossConstants.START_WHITH_J.equals(startName) || FossConstants.START_WHITH_D.equals(startName)){
			return true;
		}
		return false;
	}
	 ...
	 *            the new arrive sheet mannger service
	 */
	 ...
	 * 
	 * @param paymentSettlementService
	 *            the new payment settlement service
	 */
	 ...
	 *            the new actual freight service
	 */
		...
		if (CollectionUtils.isNotEmpty(twoInOneWaybillDto.getWaybillNoList())) {
			...
			for (int i = 0, len = waybillNos.size(); i < len; i++) {
				if (!repaymentService.isPayment(waybillNos.get(i))) {
					// 得到结算应收单信息
					FinancialDto financialDto = repaymentService.queryFinanceSign(waybillNos.get(i));
					// 应收代收款
					BigDecimal receiveableAmount = financialDto.getReceiveableAmount() == null ? BigDecimal.ZERO: financialDto.getReceiveableAmount();
					// 应收到付款
					BigDecimal receiveablePayAmoout = financialDto.getReceiveablePayAmoout() == null ? BigDecimal.ZERO: financialDto.getReceiveablePayAmoout();
		...
		if (CollectionUtils.isNotEmpty(waybillLists)) {
			...
			for (int i = 0, len = waybillLists.size(); i < len; i++) {
				...
				if (!waybillExpress.getOriginalWaybillNo().equals(twoInOneWaybillDto.getMainWaybillNo())) {
					// 得到结算应收单信息
					FinancialDto financialDto = repaymentService.queryFinanceSign(waybillExpress.getOriginalWaybillNo());
					// 应收代收款
					BigDecimal receiveableAmount = financialDto.getReceiveableAmount() == null ? BigDecimal.ZERO: financialDto.getReceiveableAmount();
					// 应收到付款
					BigDecimal receiveablePayAmoout = financialDto.getReceiveablePayAmoout() == null ? BigDecimal.ZERO: financialDto.getReceiveablePayAmoout();
	...
	private void settleUpOriginalRepayment(RepaymentEntity repaymentEntity,CurrentInfo currentInfo, TwoInOneWaybillDto twoInOneWaybillDto) {
		...
		// 判断运单是否已中止或已作废
		if (actentity != null
				&& (WaybillConstants.ABORTED.equals(actentity.getStatus()) || WaybillConstants.OBSOLETE.equals(actentity.getStatus()))) {
			// 抛出业务异常
		...
		if ((SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__DEBT).equals(paymentType)|| (SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__CREDIT).equals(paymentType)) {
			...
			if (isMainWaybillNo) {		
	            ...
	            try {
                	...
                	CubcGrayRequestEntity requestDto = new CubcGrayRequestEntity(arrayList,
                			SettlementDictionaryConstants.BILL_RECEIVABLE__SOURCE_BILL_TYPE__WAYBILL,SERVICE_CODE+".settleUpOriginalRepayment",
                			SettlementConstants.TYPE_FOSS);
                	CubcGrayResponseEntity response = (CubcGrayResponseEntity)HttpClientUtils.postMethod(requestDto, new CubcGrayResponseEntity(),grayByWaybillNoUrl);
	            	List<VestBatchResult> list = response.getVestBatchResult();
	            	vestSystemCode = list.get(0).getVestSystemCode();		
			...
			// 付款方式为其他方式时,调用结算接口-实收货款
		} else if (SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__CARD.equals(paymentType)|| SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__NOTE.equals(paymentType)
				|| SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__TELEGRAPH_TRANSFER.equals(paymentType)|| SettlementDictionaryConstants.SETTLEMENT__PAYMENT_TYPE__ONLINE.equals(paymentType)
		...
		repaymentEntity.setOperateOrgCode(currentInfo.getCurrentDeptCode());
		// 当实付运费和代收货款同时为0时 更改为无需生成
		if (BigDecimalOperationUtil.compare(repaymentEntity.getCodAmount(),BigDecimal.ZERO)&& BigDecimalOperationUtil.compare(repaymentEntity.getActualFreight(), BigDecimal.ZERO)) {
			// 无需结清 将付款信息置0
			repaymentEntity.setActualFreight(BigDecimal.ZERO);
			// 无需结清 将付款信息置0
			repaymentEntity.setCodAmount(BigDecimal.ZERO);
	...
	public int checkGenerateArriveSheet(ArriveSheetEntity entity,CurrentInfo cinfo) {
		...
		if (printNum > 0) {
			...
			// 可打印件数= 开单件数 - 已生成件数
			addNewArriveSheet(actualFreightEntity.getGoodsQty()- actualFreightEntity.getGenerateGoodsQty(),actualFreightEntity, entity,actualFreightEntity.getGoodsQty(), cinfo);
		}
		businessLockService.unlock(mutexElement);
		return 0;
	}
	...
	private void addNewArriveSheet(Integer printNum,ActualFreightEntity actualFreightEntity, ArriveSheetEntity entity,int generateGoodsQty, CurrentInfo cinfo) {
		// 对重复到达联，再次添加验证
		// 校验是否有效且为“生成”状态的到达联
		List<ArriveSheetEntity> list = arrivesheetDao.queryArriveSheetByWaybillNo(new ArriveSheetEntity(entity.getWaybillNo(), ArriveSheetConstants.STATUS_GENERATE,FossConstants.YES, FossConstants.NO));
		if (CollectionUtils.isNotEmpty(list)) {
	...
	private List<AirAgencyQueryDto> queryAirAgencyQueryDtoList(AirAgencyQueryDto airAgencyQueryDto,String orgCode){
...
//        try {
//        	List<String> arrayList = new ArrayList<String>();
//        	arrayList.add(seriaNo);//TODO 根据流水号查询...T+0数据  ？？？
//        	CubcGrayRequestEntity requestDto = new CubcGrayRequestEntity(arrayList,
...
//        	vestSystemCode = list.get(0).getVestSystemCode();
//		} catch (Exception e) {
//			LOGGER.info("灰度分流失败,"+"流水号："+seriaNo);
//			throw new CUBCGrayException("系统繁忙,灰度分流失败,请稍后重试！");
//		}
//		if(SettlementConstants.TYPE_FOSS.equals(vestSystemCode)){
			posCardEntitys = pdaPosManageService.queryPosCardData(posCardManageDto).getPosCardEntitys();

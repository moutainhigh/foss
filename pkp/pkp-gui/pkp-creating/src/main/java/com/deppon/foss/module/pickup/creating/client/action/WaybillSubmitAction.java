 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/creating/client/action/WaybillSubmitAction.java
 * 
 * FILE NAME        	: WaybillSubmitAction.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
...
import com.deppon.foss.module.pickup.creating.client.common.WaybillValidate;
 ...
 * @date 2012-10-30 下午6:41:15 * * 录入货物信息SUC业务规则 1.
 ...
 *       且保价费率可手动调整；（限保物品从限运物资料中获取）；非限保物品的保价费率不可修改； 2. 实际保险费小于最低保费的按最低保费收取； 3.
 *       保价费 = 保价申明价值*保价费率，不可修改； 4. 保价申明价值默认为3000，可以修改；保价声明价值不封顶； 5.
 ...
 */
public class WaybillSubmitAction implements
	...
	private static final II18n i18n = I18nManager
			.getI18n(WaybillSubmitAction.class);

	IWaybillService waybillService = WaybillServiceFactory.getWaybillService();
	private static final Log LOG = LogFactory.getLog(WaybillSubmitAction.class);
	...
	public void actionPerformed(ActionEvent e) {
		...
		//350909             郭倩云                   如果是营业部开单并且零担轻货上分拣开关已经开启则校验货物是不是零担轻货
	    if(WaybillConstants.WAYBILL_SALE_DEPARTMENT.equals(ui.getWaybillType())&&
				WaybillConstants.YES.equals(CommonUtils.readConfValue(WaybillConstants.LIGHT_GOODS_SORTING))){
			String productCodeFlag = CommonUtils.getSortingFlagFromProductCode(bean.getProductCode().getCode());
			String goodsQtyTotalFlag = CommonUtils.getSortingFlagFromGoodsQtyTotal(bean.getGoodsQtyTotal());
			String goodsWeightTotalFlag = CommonUtils.getSortingFlagFromGoodsWeightTotal(bean.getGoodsWeightTotal(),bean.getGoodsQtyTotal());
			String woodFlag = CommonUtils.getSortingFlagFromWoodAndSalver(bean.getWood());
			String salverFlag = CommonUtils.getSortingFlagFromWoodAndSalver(bean.getSalver());
			String otherPackageFlag = CommonUtils.getSortingFlagFromOtherPackage(bean.getOtherPackage());
			if(WaybillConstants.YES.equals(productCodeFlag)
					&&WaybillConstants.YES.equals(goodsQtyTotalFlag)
					&&WaybillConstants.YES.equals(goodsWeightTotalFlag)
					&&WaybillConstants.YES.equals(woodFlag)
					&&WaybillConstants.YES.equals(salverFlag)
					&&WaybillConstants.YES.equals(otherPackageFlag)){
				bean.setSortingResult(WaybillConstants.YES);
			}else{
				bean.setSortingResult(WaybillConstants.NO);
			}
		}
		...
		if(bean.getIbtg()!=null){
			Date date=new Date();
			//组合数据
			bean.getIbtg().setPictureBillEndTime(date);
			bean.getIbtg().setWaybillNo(bean.getWaybillNo());
			if(bean.getIbtg().getConsignerTelliphoneTime()!=null&&bean.getIbtg().getConsigneeTelliphoneTime()!=null){
				double ccTotleTime=bean.getIbtg().getConsignerTelliphoneTime()+bean.getIbtg().getConsigneeTelliphoneTime();
				ccTotleTime=(new BigDecimal(ccTotleTime)).setScale(NumberConstants.NUMBER_3,BigDecimal.ROUND_HALF_UP).doubleValue();  
				bean.getIbtg().setCcTotleTime(ccTotleTime);
			}
			double bsTotleTime=0;
			if(bean.getIbtg().getCombPaymentModeTime()!=null){
				bsTotleTime=bean.getIbtg().getCombPaymentModeTime();
				bean.getIbtg().setCombPaymentModeTime((new BigDecimal(bean.getIbtg().getCombPaymentModeTime())).setScale(NumberConstants.NUMBER_3,BigDecimal.ROUND_HALF_UP).doubleValue());
			}
			if(bean.getIbtg().getCombPickModeTime()!=null){
				bsTotleTime+=bean.getIbtg().getCombPickModeTime();
				bean.getIbtg().setCombPickModeTime((new BigDecimal(bean.getIbtg().getCombPickModeTime())).setScale(NumberConstants.NUMBER_3,BigDecimal.ROUND_HALF_UP).doubleValue());
			}
			if(bean.getIbtg().getCombProductTypeTime()!=null){
				bsTotleTime+=bean.getIbtg().getCombProductTypeTime();
				bean.getIbtg().setCombProductTypeTime((new BigDecimal(bean.getIbtg().getCombProductTypeTime())).setScale(NumberConstants.NUMBER_3,BigDecimal.ROUND_HALF_UP).doubleValue());
			}
			if(bean.getIbtg().getCombReturnBillTypeTime()!=null){
				bsTotleTime+=bean.getIbtg().getCombReturnBillTypeTime();
				bean.getIbtg().setCombReturnBillTypeTime((new BigDecimal(bean.getIbtg().getCombReturnBillTypeTime())).setScale(NumberConstants.NUMBER_3,BigDecimal.ROUND_HALF_UP).doubleValue());
			}
			bsTotleTime=(new BigDecimal(bsTotleTime)).setScale(NumberConstants.NUMBER_3,BigDecimal.ROUND_HALF_UP).doubleValue();
			bean.getIbtg().setBsTotleTime(bsTotleTime);
			double billTotleTime=(bean.getIbtg().getPictureBillEndTime().getTime()-bean.getIbtg().getPictureBillStartTime().getTime())/NUM_1000;
			billTotleTime=(new BigDecimal(billTotleTime)).setScale(NumberConstants.NUMBER_3,BigDecimal.ROUND_HALF_UP).doubleValue();
			bean.getIbtg().setBillTotleTime(billTotleTime);
			//将其它时间也全部四舍五入，保留三位小数
			if(bean.getIbtg().getConsigneeTelliphoneTime()!=null){
				bean.getIbtg().setConsigneeTelliphoneTime((new BigDecimal(bean.getIbtg().getConsigneeTelliphoneTime())).setScale(NumberConstants.NUMBER_3,BigDecimal.ROUND_HALF_UP).doubleValue());
			}
			if(bean.getIbtg().getConsignerTelliphoneTime()!=null){
				bean.getIbtg().setConsignerTelliphoneTime((new BigDecimal(bean.getIbtg().getConsignerTelliphoneTime())).setScale(NumberConstants.NUMBER_3,BigDecimal.ROUND_HALF_UP).doubleValue());
			}
			if(bean.getIbtg().getRotatePictureTime()!=null){
				bean.getIbtg().setRotatePictureTime((new BigDecimal(bean.getIbtg().getRotatePictureTime())).setScale(NumberConstants.NUMBER_3,BigDecimal.ROUND_HALF_UP).doubleValue());
			}
		}
		String titleName = "";
		if(CommonContents.logToggle){
			IEditor editor = null;
			if(ui.pictureWaybillEditUI !=null){
				editor = ui.pictureWaybillEditUI.getEditor();
			}else{
				editor = ui.getEditor();
			}
			if(editor!=null){
				titleName = editor.getTitle(); 
			}
			LOG.info("运单号："+bean.getWaybillNo()+" 提交开始...");
			CustomerOperaLoggerDto logger = CustomerImpOperLogHandler.setLogger(bean.getWaybillNo());
			logger.setMenuName(titleName);
			//获取schameinfo信息
			logger.setSchameInfo(getSchemaInfo());
...
//			logger.submitStart();
		...
		//设置提交来源
		bean.setGuiTitleName(titleName);
		...
		if(bean.getSpecialValueAddedServiceType()!=null
				&& StringUtil.isNotEmpty(bean.getSpecialValueAddedServiceType().getValueCode())
		...
		*/			//对选择信息进行分装
					...
					{
						if(StringUtils.isNotEmpty(otherChargeVo.getCode()))
						{
							//对安装费和上楼费就行过滤
		...
		if (bool) {
			try {
				...
				if( 	
					...
					bean.setStartCentralizedSettlement(bean.getCustomerCircleEntity().getIsFocusPay());
				}
				//地址校验
				CommonUtils.checkAddress(bean);
				Integer goodsQtyTotal = bean.getGoodsQtyTotal();
				if( (""+goodsQtyTotal).length() > NumberConstants.NUMBER_4){
					throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillsubmitaction.exception.goodsQtyTotalMoreFourDigit"));
				}
				...
				if(StringUtils.isNotBlank(ui.getPictureWaybillType()) && 
					...
					if (!WaybillConstants.INNER_PICKUP.equals(bean.getReceiveMethod().getValueCode())
						...
						//如果图片开单支付方式为空提示空指针异常，修改为正常业务提醒且支付方式获得焦点，151211，杨套红，2016-03-31
					...
					if(StringUtils.isBlank(weight) || new BigDecimal(weight).compareTo(BigDecimal.ZERO) <= 0 
							|| StringUtils.isBlank(volume) || new BigDecimal(volume).compareTo(BigDecimal.ZERO) <= 0){
						
						/**
						 * 此校验针特对图片开单重量体积为0的情况，其他的校验放在计算总运费中
						 */
						validatePcpProductCode(bean);
						
						if(StringUtils.isNotBlank(bean.getCashPay()) && 
								WaybillConstants.YES.equals(bean.getCashPay())){
							MsgBox.showInfo(i18n.get("foss.gui.creating.PictureWaybill.msgBox.iscashPayFlag"));
						}else{
							WaybillValidate validate = new WaybillValidate(ui);
							validate.validate(bean);
							validatePiture(bean);
							if(StringUtils.isBlank(ui.numberPanel.getLblNumber().getText())){
								ui.basicPanel.getTxtWaybillNO().requestFocus();
								throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillDescriptor.waybillNo.exist"));
							}
							if(CommonContents.logToggle){
								CustomerImpOperLogHandler.setLogger(bean.getWaybillNo()).submitEnd();
								LOG.info("运单号："+bean.getWaybillNo()+" 提交结束...");
							}
							if (WaybillConstants.INNER_PICKUP.equals(bean
							...
							ui.pictureWaybillEditUI.setSubmitType(e.getActionCommand());
							SubmitConfirmDialog	dialog = new SubmitConfirmDialog(bean, ui);
							//设置是否打印标签
							this.setLabelPrintEnabled(bean, dialog);
							// 居中显示弹出窗口
						...
						//zxy 20131014 BUG-55269 start 修改：如果label标签运单是空的，则说明填写的运单没触发起来，此时不能提交
						if(StringUtils.isBlank(ui.numberPanel.getLblNumber().getText())){
							ui.basicPanel.getTxtWaybillNO().requestFocus();
							throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillDescriptor.waybillNo.exist"));
						}
						...
						//zxy 20131014 BUG-55269 end 修改：如果label标签运单是空的，则说明填写的运单没触发起来，此时不能提交
						...
						if (!WaybillConstants.INNER_PICKUP.equals(bean
							...
							/*if (!StringUtils.isEmpty(bean.getPromotionsCode())) {
								*//**
								...
								//标示为已经减去优惠费用
								bean.setFlagTakeCoupon("Y");
								// 需要重新计算运费
								CalculateFeeTotalUtils.calculateTotalFee(bean);
								//标示为未减去优惠费用
								bean.setFlagTakeCoupon("N");
							}*/
					...
					validate(bean);
					// 验证该单是否已被图片开单
					validatePitureWaybill(bean);
					// zxy 20131014 BUG-55269 start
					// 修改：如果label标签运单是空的，则说明填写的运单没触发起来，此时不能提交
					if (StringUtils.isBlank(ui.numberPanel.getLblNumber()
							.getText())) {
						ui.basicPanel.getTxtWaybillNO().requestFocus();
						throw new WaybillValidateException(
								i18n.get("foss.gui.creating.waybillDescriptor.waybillNo.exist"));
					}
					if(CommonContents.logToggle){
						CustomerImpOperLogHandler.setLogger(bean.getWaybillNo()).submitEnd();
						LOG.info("运单号："+bean.getWaybillNo()+" 提交结束...");
				...
				//合伙人校验预存款资金池的资金额度 2016年1月22日 08:39:26 葛亮亮
				if(!Boolean.TRUE.equals(bean.getIsWholeVehicle()) && BZPartnersJudge.IS_PARTENER)
				{
				...
				if(ui.getImportWaybillPanelVo()==null){
					//正常提交
					dialog = new SubmitConfirmDialog(bean, ui);
					// 居中显示弹出窗口
					WindowUtil.centerAndShow(dialog);
				}else{
					...
					//正常提交
					dialog = new SubmitConfirmCompareImportDialog(bean, ui);
					// 居中显示弹出窗口
					WindowUtil.centerAndShow(dialog);
				}
			}
			} catch (WaybillValidateException w) {
				//出现异常，将bean中的客户编码设置为实际的客户编码 。
				bean.setDeliveryCustomerCode(bean.getOldDeliveryCustomerCode());
	 ...
	 * @param bean
	 */
	private void validatePcpProductCode(WaybillPanelVo bean){
		//非图片开单不做校验
		if (!WaybillConstants.GUITITLENAME_PICTURE.equals(bean.getGuiTitleName())){
			return;
		}
		ProductEntityVo product = bean.getProductCode();
		// 精准包裹客户
		if(product!=null&&ProductEntityConstants.PRICING_PRODUCT_PCP.equals(product.getCode())
				&&!WaybillConstants.YES.equals(bean.getIsAccuratePackage())) {
	...
	private String getSchemaInfo(){
		...
		try {
			...
			UserEntity userEntity =loginInfo.getUser();
			
			//获取组织信息
			OrgAdministrativeInfoEntity orgAdmin = userEntity.getEmployee().getDepartment();
			
			sb.append(" ")
			.append("操作人:").append(userEntity.getUserName()).append("; ")
			.append("部门名称:").append(orgAdmin.getName()).append("; ")
			.append("部门编码:").append(orgAdmin.getCode()).append("; ");
			
//			System.out.println(userEntity.getEmpName()+"|"+orgAdmin.getName()+"|"+orgAdmin.getCode());
		} catch (Exception e) {
	...
	public void validatePiture(WaybillPanelVo bean) {
		if (StringUtils.isNotBlank(bean.getWaybillNo())) {
			WaybillPictureEntity entity = new WaybillPictureEntity();
			entity.setWaybillNo(bean.getWaybillNo());
			entity.setActive(FossConstants.ACTIVE);
			WaybillPictureEntity waybillPictureEntity = waybillService
					.queryWaybillPictureByEntity(entity);
			if (waybillPictureEntity != null
					&& WaybillConstants.WAYBILL_PICTURE_TYPE_RETURN
							.equals(waybillPictureEntity.getPendgingType())) {
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.WaybillSubmitAction.picture.validate"));
			}
		}
	}
/*	*//**
	 ...
	 * @param bean
	 *//*
	public void valiteMes(WaybillPanelVo bean) {
		Injector injector = GuiceContextFactroy.getInjector();
		isaleDepartmentService= injector.getInstance(SaleDepartmentService.class);
		//canExpressOneMany -一件多票 canCashOnDelivery--是否货到付款 canAgentCollected--是否可代收付款
		String targetOrgCode=bean.getTargetOrgCode();
		UserEntity user = (UserEntity) SessionContext.getCurrentUser();
		OrgAdministrativeInfoEntity dept = user.getEmployee().getDepartment();
		//int total=bean.getGoodsQtyTotal();
		SaleDepartmentEntity saleDepartmentEntity =isaleDepartmentService.querySaleDepartmentByCode(dept.getCode());
		if(targetOrgCode.endsWith("远郊快递营业部"))
		{
			
			if("N".equals(saleDepartmentEntity.getCanExpressOneMany())||saleDepartmentEntity.getCanExpressOneMany()==null||bean.getGoodsQtyTotal()>1)
			{
				//该目的站不能开一票多件
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.WaybillSubmitAction.picture.vallite"));
			}
			if("Y".equals(saleDepartmentEntity.getCanExpressOneMany())&&(saleDepartmentEntity.getCanAgentCollected()==null||"N".equals(saleDepartmentEntity.getCanAgentCollected())))
			{//该目的站不能到快递一票多件代收货款
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.WaybillSubmitAction.picture.vallite1"));
			}
			if("Y".equals(saleDepartmentEntity.getCanExpressOneMany())&&(saleDepartmentEntity.getCanCashOnDelivery()==null||"N".equals(saleDepartmentEntity.getCanCashOnDelivery())))
			{//该目的站不能到快递一票多件货到付款
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.WaybillSubmitAction.picture.vallite2"));
			}
		}
		//saleDepartmentEntity.g
	}
*/
	/**
	 * 驗證司機工號不能為空
	 ...
	 * 
	 */
	private void validatePickupToDoor(WaybillPanelVo bean) {
		if (StringUtils.isBlank(bean.getDriverCode())) {
			throw new WaybillValidateException(
					i18n.get("foss.gui.creating.waybillSubmitAction.exception.nullDriverCode1"));
		} else {
			if (bean.getDriverCode().trim().length() != NumberConstants.NUMBER_6) {
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.waybillSubmitAction.exception.nullDriverCode2"));
			}
		}
	}
	...
	private void validate(WaybillPanelVo bean) {
		...
		// zxy 20131009 BUG-56709 start 新增：总运费和体积不能超过数据库的长度限制
		// 保价费
		if (new Long(bean.getInsuranceFee().multiply(new BigDecimal(NumberConstants.NUMBER_100))
				.longValue()).toString().length() > NumberConstants.NUMBER_8) {
			ui.incrementPanel.getTxtInsuranceValue().requestFocus();
			throw new WaybillValidateException(
					i18n.get("foss.gui.creating.waybillTempSaveAction.msgBox.overMaxInsuranceFee"));
		}
		// 体积
		if (new Long(bean.getGoodsVolumeTotal().multiply(new BigDecimal(NumberConstants.NUMBER_1000))
				.longValue()).toString().length() > NumberConstants.NUMBER_8) {
			if (StringUtils.isNotBlank(ui.getPictureWaybillType())
			...
			throw new WaybillValidateException(
					i18n.get("foss.gui.creating.waybillTempSaveAction.msgBox.overMaxGoodsVolumn"));
		}
		// 开单金额
		if (new Long(bean.getTotalFee().multiply(new BigDecimal(NumberConstants.NUMBER_100))
				.longValue()).toString().length() > NumberConstants.NUMBER_10) {
			throw new WaybillValidateException(
					i18n.get("foss.gui.creating.waybillTempSaveAction.msgBox.overMaxTotalFee"));
		}
		// zxy 20131009 BUG-56709 end 新增：总运费和体积不能超过数据库的长度限制
		// 包装信息不能大于50字符
		...
		// 开单金额校验
		validateAmountLimit(bean);

		// 验证整车业务
		validateVehicleNumber(bean);

		/**
		 * 【新产品放开门到门一些限制】 当运输性质为门到门的时候，提货方式不能选自提
		 * Common.validateDeliverTransportDTD(bean, ui);
		 * 验证运输性质为【门到门】的时候，上门接货的是否已经勾选
		 * Common.validateSubmitDTDSelectedPickUP(bean, ui);
		 */
		//伙伴开单校验
		if(null != bean.getPartnerBilling() && bean.getPartnerBilling()){
			validatePartnerName(bean);
			validatePartnerPhone(bean);
		}
		// 验证客户信息 -- 联系人不能为空
		...
		// validatePdaHandwriteFee(bean); //现在不需要验证手写现付金额，根据BUG-36523修改

		// 手机及电话号码必须录入一个
		validatePhone(bean);
		// 在上门接货的时候 司机工号必填
		//2016年1月15日 09:18:45 葛亮亮 合伙人开单 不用输司机工号
	...
	/*	//校验单号
		valiteMes(bean);*/
		// ===========LianHe/"内部带货"时,校验工号不为空，并校验工号信息和客户信息一致 /2017年1月9日/start=======
		CommonUtils.validateInternalDelivery(bean);
		// ===========LianHe/"内部带货"时,校验工号不为空，并校验工号信息和客户信息一致 /2017年1月9日/end=======

	}

	public void validatePitureWaybill(WaybillPanelVo bean) {
		if (StringUtils.isNotBlank(bean.getWaybillNo())) {
			WaybillPictureEntity entity = new WaybillPictureEntity();
			entity.setWaybillNo(bean.getWaybillNo());
			entity.setActive(FossConstants.ACTIVE);
			...
			if (waybillPictureEntity != null /*
											 * && !WaybillConstants.
											 * WAYBILL_PICTURE_TYPE_RETURN
											 * .equals
											 * (waybillPictureEntity.getPendgingType
											 * ())
											 */) {
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.ValidatePictureWaybillNo"));
			}
			if (StringUtils.isNotBlank(bean.getOrderNo())) {
				entity.setWaybillNo(null);
				entity.setOrderNo(bean.getOrderNo());
				WaybillPictureEntity picture = waybillService
						.queryWaybillPictureByEntity(entity);
				if (picture != null /*
									 * &&
									 * !WaybillConstants.WAYBILL_PICTURE_TYPE_RETURN
									 ...
									 */) {
					throw new WaybillValidateException(
							i18n.get("foss.gui.creating.ValidatePictureOrderNo"));
				}
			}
		}
	}
	...
	private void validateGoodPackage(String goodsPackage) {
		try {
			...
			if (a.length() > NumberConstants.NUMBER_50) {
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.cargoInfoPanel.txtOther.length.notOver50"));
			}
	...
	private void validateDeliveryFee(WaybillPanelVo bean) {
		// 计算送货费
		...
		if (!chargeMode) {
			...
			if (calculatedeliveryGoodsFee != null
					&& deliveryGoodsFee != null
					&& calculatedeliveryGoodsFee.doubleValue() > deliveryGoodsFee
							.doubleValue()) {
				/*
				 * BigDecimal sh=
				 ...
				 * if(PriceEntityConstants
				 * .PRICING_CODE_SH.equals(entity.getCode())){
	...
	private void waybillGoogNameCheck(WaybillPanelVo bean) {
		if (StringUtil.isEmpty(bean.getGoodsName())) {
			if (StringUtils.isNotBlank(ui.getPictureWaybillType())
					&& WaybillConstants.WAYBILL_PICTURE.equals(ui
							.getPictureWaybillType())) {
				bean.setGoodsName(ui.getPictureCargoInfoPanel()
						.getTxtGoodsName().getText());
			} else {
				bean.setGoodsName(ui.getCargoInfoPanel().getTxtGoodsName()
						.getText());
			}
		}
	}
	 ...
	 * 验证预付金额是否为负数
	 * 
	 * @author 105888-foss-zhangxingwang date 2013-9-16 20:11:32
	 * @param bean
	 */
	private void validatePrePayAmountFee(WaybillPanelVo bean) {
		if (bean.getPrePayAmount() != null
				&& bean.getPrePayAmount().doubleValue() < 0) {
			throw new WaybillValidateException(
					i18n.get("foss.gui.creating.waybillsubmitaction.exception.validateprepayamount"));
		}
	}
	...
	private void validateTransportFee(WaybillPanelVo bean) {
		if (bean.getTransportFee() != null
				&& bean.getTransportFee().doubleValue() < 0) {
			throw new WaybillValidateException(
					i18n.get("foss.gui.creating.waybillsubmitaction.exception.validatetransportfee"));
		}
	}
	 ...
	 * 
	 * @author 026123-foss-lifengteng
	 * @date 2013-6-20 上午11:39:37
	 */
	private void validateAmountLimit(WaybillPanelVo bean) {
		if (!bean.getIsWholeVehicle()) {
			if (bean.getTotalFee().intValue() > WaybillConstants.AMOUNT_LIMIT) {
				MsgBox.showInfo(i18n
						.get("foss.gui.creating.waybillSubmitAction.msgBox.amountLimit"));
			}
		}
	}
	...
	private void validateCustomer(WaybillPanelVo bean) {
		...
		if (StringUtils.isNotBlank(bean.getDeliveryCustomerMobilephone())) {
		...
		if (StringUtils.isNotBlank(bean.getReceiveCustomerMobilephone())) {
		...
		// zxy 20131031 BUG-58085 start 新增：提交的时候检查发货人和收获人地址是否为空判断(跟计算总运费逻辑一样)
		// 原件签收单返回
		...
		if (WaybillConstants.DELIVER_NOUP.equals(code)
			...
			if (StringUtils.isBlank(jd.getText())||consigneeAddress == null
					...
					|| StringUtils.isEmpty(consigneeAddress.getProvinceName())
		...
		// zxy 20131031 BUG-58085 end 新增：提交的时候检查发货人和收获人地址是否为空判断(跟计算总运费逻辑一样)
	...
	private void validatePaidMethod(WaybillPanelVo bean) {
		...
		if (WaybillConstants.ONLINE_PAYMENT.equals(bean.getPaidMethod()
			...
			// zxy 20131016 BUG-57455 start 新增：网上支付在无代收货款前提下，不允许有到付
			// 有到付付款方式不能为网上支付
			if (bean.getToPayAmount().compareTo(BigDecimal.ZERO) > 0) {
				if (bean.getCodAmount().compareTo(BigDecimal.ZERO) <= 0)
					throw new WaybillValidateException(
							i18n.get("foss.gui.creating.waybillSubmitAction.exception.paidMethod.noOnline_payment"));
			}
			// zxy 20131016 BUG-57455 end 新增：网上支付在无代收货款前提下，不允许有到付
		}
	...
	//判断能否开月结
	private void validateCharge(WaybillPanelVo bean) {
		/**
	...
	private void validatePack(WaybillPanelVo bean) {
		Integer qtyTotal = bean.getGoodsQtyTotal()==null ? 0 : bean.getGoodsQtyTotal();// 总件数
	...
	private void validateVehicleNumber(WaybillPanelVo bean) {
		if (bean.getIsWholeVehicle()) {
			...
			if (rateLow == null) {
				rateLow = BigDecimal.valueOf(ZERO_POINT_ONE);
			}
			...
			if (bean.getTransportFee().subtract(serviceFee).compareTo(lowLevel) < 0) {
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.listener.Waybill.transportFee.four")
								+ lowLevel.add(serviceFee).setScale(0,
	 ...
	 */
	public void validateVehicleFeeIsProfit(WaybillPanelVo bean) {
		if (bean.getIsWholeVehicle()) {
			// 判断当前运输性质是否正确
			ProductEntityVo productVo = bean.getProductCode();
			if (!ProductEntityConstants.PRICING_PRODUCT_FULL_VEHICLE
			...
			if (null == bean.getWholeVehicleFeeChange()) {
				bean.setWholeVehicleFeeChange(BigDecimal.ZERO);
			}
			// 开单总运费-整车运费冲减
			BigDecimal tempPriceOne = bean.getTotalFee().subtract(
					bean.getWholeVehicleFeeChange());
			// 开单总运费-整车运费冲减-约车报价-装卸费
			BigDecimal tempPriceTwo = tempPriceOne.subtract(
					bean.getWholeVehicleAppfee() == null ? BigDecimal.ZERO
							: bean.getWholeVehicleAppfee()).subtract(
					bean.getServiceFee() == null ? BigDecimal.ZERO : bean
							.getServiceFee());
			// (开单总运费-整车运费冲减)*其他成本参数
			BigDecimal tempPriceThree = tempPriceOne
					.multiply(bean.getWholeVehicleOtherCostParameter() == null ? BigDecimal.ZERO
							: bean.getWholeVehicleOtherCostParameter());
			// 包装费*包装费参数+货物重量*重量参数
			BigDecimal tempPriceFour = ((bean.getPackageFee() == null ? BigDecimal.ZERO
					: bean.getPackageFee())
					.multiply(bean.getWholeVehiclePackageFeeParameter() == null ? BigDecimal.ZERO
							: bean.getWholeVehiclePackageFeeParameter()))
					.add((bean.getGoodsWeightTotal() == null ? BigDecimal.ZERO
							: bean.getGoodsWeightTotal()).multiply(bean
							.getWholeVehicleWeightParameter() == null ? BigDecimal.ZERO
							: bean.getWholeVehicleWeightParameter()));
			// (约车报价*车价参数)-人力成本参数
			BigDecimal tempPriceFive = ((bean.getWholeVehicleAppfee() == null ? BigDecimal.ZERO
					: bean.getWholeVehicleAppfee())
					.multiply(bean.getWholeVehicleCarCostParameter() == null ? BigDecimal.ZERO
							: bean.getWholeVehicleCarCostParameter()))
					.subtract(bean.getWholeVehicleHumanCostParameter() == null ? BigDecimal.ZERO
			...
			if (finalPrice.compareTo(BigDecimal.ZERO) <= 0) {
				/**
				 * 通过校验公式（开单总运费—整车运费冲减）—约车报价—装卸费-（开单总运费—整车运费冲减）*其他成本参数—包装费*包装费参数
				 * —货物重量*重量参数+约车报价*车价参数—人力成本参数=0 计算获得开单总运费
				 */
				totalPrice = (tempPriceFour.subtract(tempPriceFive).add(
						bean.getWholeVehicleAppfee() == null ? BigDecimal.ZERO
								: bean.getWholeVehicleAppfee()).add(bean
						.getServiceFee() == null ? BigDecimal.ZERO : bean
						.getServiceFee()))
						.divide(BigDecimal.ONE.subtract(bean
								.getWholeVehicleOtherCostParameter() == null ? BigDecimal.ZERO
								: bean.getWholeVehicleOtherCostParameter()), 2,
	...
	private void validateDriverNo(WaybillPanelVo bean) {
		if (bean.getPickupToDoor() != null && bean.getPickupToDoor()
			...
			throw new WaybillValidateException(
					i18n.get("foss.gui.creating.waybillSubmitAction.exception.nullDriverCode"));
		}
	}
	...
	private void validateFee(WaybillPanelVo bean) {
		 ...
		 */
		if (!bean.getIsWholeVehicle()) {
			// 如果是送货并且为非免费送货，则送货费不允许为0
			if (WaybillConstants.DELIVER_INGA_AIR.equals(bean
					.getReceiveMethod().getValueCode())
					...
					|| WaybillConstants.DELIVER_UP_AIR.equals(bean
							.getReceiveMethod().getValueCode())) {
				// DEFECT-5051:FOSS开单门到门运输性质，选择送货，点击计算总运费，送货费为0，点击提交按钮，提示‘送货费为0，请检查提货方式’
				if (!ProductEntityConstants.PRICING_PRODUCT_DOOR_TO_DOOR
						.equals(bean.getProductCode().getCode())) {
					// 送货折扣为0的，送货费可以为0
					WaybillDiscountCanvas discountTableModel = (WaybillDiscountCanvas) ui.canvasContentPanel
							.getConcessionsPanel().getTblConcessions()
							.getModel();
					boolean feeIsnotNull = true;
					if (discountTableModel != null) {
						List<WaybillDiscountVo> disdate = discountTableModel
								.getData();
						if (CollectionUtils.isNotEmpty(disdate)) {
							for (WaybillDiscountVo dis : disdate) {
								if ("SH".equals(dis.getFavorableItemCode())) {
									String discount = dis
											.getFavorableDiscount();

									if (StringUtil.isNotEmpty(discount)) {
										BigDecimal discountRate = new BigDecimal(
												dis.getFavorableDiscount());
										if (discountRate
					...
					if (bean.getDeliveryGoodsFee().compareTo(BigDecimal.ZERO) == 0
							&& feeIsnotNull && !StringUtil.equals(bean.getProductCode().getCode(),ProductEntityConstants.PRICING_PRODUCT_PCP)) {
						throw new WaybillValidateException(
								i18n.get("foss.gui.creating.waybillSubmitAction.exception.validateFee.two"));
					}
				}
			}
		}
...
//	}
	...
	/**
	 * 较验是否需要重新打标签
	 * 当涉及到标签信息时(标签信息如下:提货网点、包装(纸木纤托膜其他)、件数、货物类型、运输性质、单号、提货方式时需要重新打标签
	 * )，在FOSS网页端系统综合查询界面中增加标签更换记录，同时校验运单是否是已完成接货
	 * 
	 * @author hehaisu
	 */
	private boolean validateLabel(WaybillPendingEntity pedingEntity,
			WaybillPanelVo bean) {
		return 
			(!stringEqual(pedingEntity.getWaybillNo(), bean.getWaybillNo())) // 验证运单号
			||(!stringEqual(pedingEntity.getCustomerPickupOrgCode(), bean.getCustomerPickupOrgCode().getCode())) // 验证提货网点
			||(pedingEntity.getPaperNum()!=bean.getPaper()) // 纸包装
			||(pedingEntity.getWoodNum()!=bean.getWood())// 木包装
			||(pedingEntity.getFibreNum()!=bean.getFibre())// 纤包装
			||(pedingEntity.getSalverNum()!=bean.getSalver())// 托包装
			||(pedingEntity.getMembraneNum()!=bean.getMembrane())// 膜包装
			||(!stringEqual(pedingEntity.getOtherPackage(), bean.getOtherPackage()))// 其他包装
			||(pedingEntity.getGoodsQtyTotal()!=bean.getGoodsQtyTotal())// 件数
			||(!stringEqual(pedingEntity.getGoodsTypeCode(), bean.getGoodsType()))// 货物类型
			||(!stringEqual(pedingEntity.getProductCode(), bean.getProductCode().getCode()))// 运输性质
			||!stringEqual(pedingEntity.getReceiveMethod(), bean.getReceiveMethod().getValueCode());// 提货方式
	}
	...
	private void setLabelPrintEnabled(WaybillPanelVo bean,
			SubmitConfirmDialog dialog) {
		/**
		 * 通过图片查询结果操作列点击的修改
		 * 
		 */
		// dialog.getChbPrintLabel().setSelected(false);
		dialog.getChbPrintLabel().setEnabled(true);
		if (ui.getImportWaybillPanelVo() != null) {

			WaybillPendingEntity pedingEntity = null;
			if (StringUtils.isNotBlank(bean.getOldWaybillNo())) {
				IWaybillPendingHessianRemoting waybillPendingHessianRemoting = DefaultRemoteServiceFactory
						.getService(IWaybillPendingHessianRemoting.class);

				pedingEntity = waybillPendingHessianRemoting
						.queryWaybillPendingByNo(bean.getOldWaybillNo());
				// 是否需要重新打标答
				if (validateLabel(pedingEntity, bean)) {
					bean.setIsModifyLabel(true);
				}
			}
		}
	}
	 ...
	 * 校验伙伴名称
	 */
	private void validatePartnerName(WaybillPanelVo bean){
		...
		if(!b){
			MsgBox.showInfo(i18n.get("foss.gui.creating.listener.Waybill.validatePartnerName"));
		}
	}
	/**
	 * 校验伙伴电话
	 */
	private void validatePartnerPhone(WaybillPanelVo bean){
		...
		boolean b= matcher.matches();
		if(!b){
			MsgBox.showInfo(i18n.get("foss.gui.creating.listener.Waybill.validatePartnerPhone"));
		}
	}
}
...
public class PendingButtonColumn extends AbstractCellEditor implements  TableCellEditor, TableCellRenderer ,
   ActionListener { 
	//日志
	public static final Logger LOGGER = LoggerFactory.getLogger(PendingButtonColumn.class);
	 ...
	 * 国际化对象
	 */
	private II18n i18n = I18nManager.getI18n(PendingButtonColumn.class); 
	
	//log
	private static Log log = LogFactory
			.getLog(PendingButtonColumn.class);
...
//	private SalesDeptWaybillUI ui;
     ...
     * @param ui
     */
    public PendingButtonColumn(TableColumn tc, SalesDeptWaybillUI ui , LinkTableMode tableModel)   {  
...
//        this.ui = ui;
        ...
        for(DataDictionaryValueEntity data : list){
        	if(WaybillConstants.WAYBILL_STATUS_PC_ACTIVE.equals(data.getValueCode()) 
        			|| WaybillConstants.WAYBILL_STATUS_PENDING_ACTIVE.equals(data.getValueCode())){
    ...
    public Component getTableCellEditorComponent(JTable table,  
        ...
        if(tableModel!=null){
    		...
    		if(ignoreChieseName.contains(statusChinese)){
    		    ...
    		    //272311-sangwenhao
    		    if(StringUtils.equals((String)tableModel.getValueAt(row, NumberConstants.NUMBER_24), WaybillConstants.WAYBILL_STATUS_LTLEWAYBILL)){
    ...
    public void actionPerformed(ActionEvent arg0) {  
    	try {
			...
			String id = dtm.getValueAt(this.row, NumberConstants.NUMBER_22).toString();
			//管理部出发运单业务操作服务
			salesDeptWaybillService = WaybillServiceFactory.getSalesDeptWaybillService();
			//获得待处理运单基本信息对象 
    ...
    private void validateWaybillNo(String pendingType,String waybillNo){
    	if(WaybillConstants.WAYBILL_STATUS_PDA_PENDING.equals(pendingType)){
			//PDA补录
			if (waybillService.checkWaybillNoExceptPDAPending(waybillNo)) {		    			

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/creating/client/ui/WaybillEditUI.java
 * 
 * FILE NAME        	: WaybillEditUI.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
...
import com.deppon.foss.module.pickup.creating.client.ui.editui.BasicPanel;
import com.deppon.foss.module.pickup.creating.client.ui.editui.BillingPayPanel;
...
import com.deppon.foss.module.pickup.creating.client.ui.editui.CanvasPanel;
import com.deppon.foss.module.pickup.creating.client.ui.editui.CargoInfoPanel;
import com.deppon.foss.module.pickup.creating.client.ui.editui.ConsigneePanel;
...
import com.deppon.foss.module.pickup.creating.client.ui.editui.NumberPanel;
import com.deppon.foss.module.pickup.creating.client.ui.editui.PictureCargoInfoPanel;
import com.deppon.foss.module.pickup.creating.client.ui.editui.PictureTransferInfoPanel;
import com.deppon.foss.module.pickup.creating.client.ui.editui.TransferInfoPanel;
 ...
 *
 */
public class WaybillEditUI extends JPanel  implements IApplicationAware{
	
	/**
	 * 国际化对象
	 */
	private II18n i18n = I18nManager.getI18n(WaybillEditUI.class); 
	 ...
	 * 运输信息面板
	 */
	public TransferInfoPanel transferInfoPanel;
	/**
	 * 图片开单运输信息面板
	 */
	public PictureTransferInfoPanel pictureTransferInfoPanel;

	/**
	 * 货物信息面板
	 */
	public CargoInfoPanel cargoInfoPanel;
	/**
	 * 图片开单货物信息面板
	 */
	public PictureCargoInfoPanel pictureCargoInfoPanel;
	 ...
	 * 画布
	 */
	public CanvasPanel canvasPanel = null;
	...
	private WaybillBindingListener listener;//事件监听
	...
	public PictureWaybillEditUI pictureWaybillEditUI;
	/**
	 * 运单状态
	 */
	 ...
	 * 
	 */
	private Boolean isImportOrder = true;
	...
	private WaybillPanelVo importWaybillPanelVo;
	...
	private DefaultComboBoxModel flabelleavemonthModel;//客户分群
	...
	private DefaultComboBoxModel economyGoodsTypeModel;//自提件类型
	
	private DefaultComboBoxModel combActiveInfoModel;//市场营销活动类型	
	//装卸费费率model
	private DefaultComboBoxModel serviceRateModel;
	private DefaultComboBoxModel deliveryTypeModel;//内部发货
	//特殊增值服务model
	private DefaultComboBoxModel specialValueAddedServiceModel;//特殊增值服务
	/**
	 * 参数配置
	 */
	...
	public void setEconomyGoodsTypeModel(DefaultComboBoxModel economyGoodsTypeModel) {
	...
	// 创建打木架窗口
	private EnterYokeInfoDialog dialog = null;
	...
	//简单报价
	private CalculateCostsDialog calculateCostsDialog;
	 ...
	 */
	public void showWoodYokeDialog() {
		if(dialog == null)
		{
			// 创建弹出窗口
			dialog = new EnterYokeInfoDialog(this);
		}
		//图片开单没有重量体积时，木架货物件数和木箱货物件数不可编辑
		if(this.getPictureWaybillType() != null && WaybillConstants.WAYBILL_PICTURE.equals(this.getPictureWaybillType().trim())){
			String weight = this.pictureCargoInfoPanel.getTxtWeight().getText();
			String volume = this.pictureCargoInfoPanel.getTxtVolume().getText();
			if(StringUtils.isNotBlank(weight) && new BigDecimal(weight).compareTo(BigDecimal.ZERO) > 0 
				...
				dialog.getTxtYokeGoodsPieces().setEnabled(true);
				dialog.getTxtBoxGoodsPieces().setEnabled(true);
			} else {
				dialog.getTxtYokeGoodsPieces().setEnabled(false);
				dialog.getTxtBoxGoodsPieces().setEnabled(false);
			}
		}
		// 剧中显示弹出窗口
		WindowUtil.centerAndShow(dialog);
	}
	public EnterYokeInfoDialog getDialog() {
		return dialog;
	}
	 ...
	 * @date:2014-10-16下午19:45
	 */
	private EnterPackingInfoDialog packDialog=null;
	
	public EnterPackingInfoDialog getPackDialog() {
		return packDialog;
	}

	public void setPackDialog(EnterPackingInfoDialog packDialog) {
	 ...
	 * @date 2012-10-29 下午02:23:24
	 */
	public void showPackingYokeDialog() {
		if(packDialog == null)
		{
			// 创建弹出窗口
			packDialog = new EnterPackingInfoDialog(this);
		}
		// 剧中显示弹出窗口
		WindowUtil.centerAndShow(packDialog);
	}
	
	/**
	 * Create the panel.
	 */
	public WaybillEditUI(String waybilltype,String pictureWaybillType) {
	...
	public void  setPictureWaybillEditUI(PictureWaybillEditUI pictureWaybillEditUI)
	{
		this.pictureWaybillEditUI = pictureWaybillEditUI;
	}
	
	public WaybillEditUI() {
	}
	...
	private void initCommonWaybillEditUI() {
		...
		if (!WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())){
			 ...
			 *  参数编码：ConfigurationParamsConstants.PKP_FOSS_IS_OFFLINE_OPEN
			 *  组织机构：FossConstants.ROOT_ORG_CODE
			 */
			ConfigurationParamsEntity conf = 
					configurationParamsService.queryConfigurationParamsByOrgCode(
							DictionaryConstants.SYSTEM_CONFIG_PARM__PKP,
							ConfigurationParamsConstants.PKP_FOSS_IS_OFFLINE_OPEN,
							FossConstants.ROOT_ORG_CODE);
			if(conf != null && !FossConstants.YES.equals(conf.getConfValue())){
				MsgBox.showInfo("当前不允许离线开单");
				return;
			}
		}
		init();
...
//		initDefaultInsurance();
	...
	public void initPicture(){
		...
		waybillBinder.removeBindingListener(listener);
		WaybillPanelVo bean = waybillBinder.getBean();
		if(bean.getProductCode() != null){
			JXTable otherTable = incrementPanel.getTblOther();
			WaybillOtherCharge model = (WaybillOtherCharge) otherTable.getModel();
			List<OtherChargeVo> data = model.getData();
			if(data != null && data.size() > 0){
				data.removeAll(data);
			}
			//incrementPanel.initDataPic();
		}
		WaybillPanelVo waybillPanelVo = new WaybillPanelVo();
		waybillPanelVo.setReceiveCustomerMobilephone("");
		waybillPanelVo.setDeliveryCustomerMobilephone("");
		waybillPanelVo.setGoodsName(" ");
		waybillPanelVo.setTargetOrgCode(" ");
		...
		setInterface();
		try {
			PropertyUtils.copyProperties(bean, waybillPanelVo);
		...
		//BeanUtils.copyProperties(waybillPanelVo, bean);
...
//		initCombPickMode();
	...
	public void setInterface(){
		numberPanel.getLblOrderLabel().setVisible(false);
		numberPanel.getLblOrderNumber().setText(null);
	}
	/**
	 * 
	 * @description:初始化控件
	...
	private void init() {
		setLayout(new FormLayout(new ColumnSpec[] {
			...
			buttonPanel = new ButtonPanel(pictureWaybillType);
...
//		{
			...
			if(this.pictureWaybillType != null && WaybillConstants.WAYBILL_PICTURE.equals(this.pictureWaybillType.trim())){
				pictureCargoInfoPanel = new PictureCargoInfoPanel(/*this.pictureWaybillType*/);
				add(pictureCargoInfoPanel, "2, 12, 3, 1, fill, fill");
				
				pictureTransferInfoPanel = new PictureTransferInfoPanel();
				add(pictureTransferInfoPanel, "2, 10, 3, 1, fill, fill");
			}else{
			...
			incrementPanel = new IncrementPanel(this.pictureWaybillType);
...
//		}
		billingPayPanel = new BillingPayPanel(this.pictureWaybillType);
		add(billingPayPanel, "2, 16, 3, 1, fill, fill");

		canvasContentPanel = new CanvasContentPanel();
...
//		calculateCostsDialog = new CalculateCostsDialog();
	...
	private void bind() {
		...
		listener = new WaybillBindingListener(this);
		waybillBinder.addBindingListener(listener);
	 ...
	 * @return the listener
	 */
	public WaybillBindingListener getListener() {
		return listener;
	}
	...
	private void focusWaybillInit(WaybillPanelVo bean)
	{
		if(WaybillConstants.WAYBILL_FOCUS.equals(waybillType))
		{
			...
			//集中开单隐藏整车报价按钮
//			this.buttonPanel.getBtnVehiclePrice().setEnabled(false);
			...
			{
				if(StringUtils.isNotEmpty(bean.getCreateOrgCode())){
					List<SaleDepartmentEntity> salesDepartList = waybillService.querySaleDepartmentByNameForCentralized(receiveOrgName, bean.getCreateOrgCode());
					if(CollectionUtils.isEmpty(salesDepartList)){
						receiveOrgCode = null;
						receiveOrgName = null;
					}
				}
			...
			if(StringUtils.isNotBlank(this.pictureWaybillType) && 
				...
				this.billingPayPanel.getTxtPackCharge().setEnabled(true);
				//incrementPanel.initDataPic();
			}else{
				/**
				 * 将打包装按钮隐藏
				 * @author:218371-foss-zhaoyanjun
				 * @date:2014-12-8下午16:26
				 */
				//this.cargoInfoPanel.getBtnPacking().setVisible(false);
				/**
				 * 解除“费用明细面板”包装费不可编辑
				 * @author:218371-foss-zhaoyanjun
				 * @date:2014-12-8下午15:00
				 */
				if(this.billingPayPanel!=null){
					this.billingPayPanel.getTxtPackCharge().setEnabled(true);
				}
			}
		}else{
	...
	private void initVo(WaybillPanelVo bean) {
		...
		if(WaybillConstants.WAYBILL_FOCUS.equals(waybillType))
		{
			...
			basicPanel.getCboReceiveModel().setEnabled(false);
		}else{
			bean.setPickupToDoor(false);//是否上门接货			
		}
		//add by 306486 修复图片开单提交并进行下一单免费接货异常
		if(WaybillConstants.WAYBILL_PICTURE.equals(this.getPictureWaybillType()))
		{
			basicPanel.getCboFreePickupGoods().setEnabled(false);
		}	
		...
		//=================设置工号可编辑/lianhe/2016年12月19日11:16:13==================
...
//		}
		//=================设置工号可编辑/lianhe/2016年12月19日11:16:13==================
		//=============FOSS开单优化v0.2:设置发货客户名称和收货客户名称不可编辑/lianhe/2016年12月21日14:16:47/start===
		//发货客户信息面板
		consignerPanel.getTxtConsigerName().setEnabled(Boolean.FALSE);
		//收货客户信息面板
		consigneePanel.getTxtReceiveCustomerName().setEnabled(Boolean.FALSE);
		//=============FOSS开单优化v0.2:设置发货客户名称和收货客户名称不可编辑/lianhe/2016年12月21日14:16:47/start===
...
//		bean.setGoodsWeightTotal(BigDecimal.ZERO);// 重量
		...
		if(bean.getReceiveMethod()!=null){
			...
			if (WaybillConstants.SELF_PICKUP.equals(bean.getReceiveMethod().getValueCode()) || 
				...
				if(StringUtils.isNotBlank(pictureWaybillType) && 
					...
					// 公里数不可编辑
					pictureTransferInfoPanel.getTxtKilometer().setEditable(false);
				}else{
					// 公里数不可编辑
				...
				if(StringUtils.isNotBlank(pictureWaybillType) && 
						WaybillConstants.WAYBILL_PICTURE.equals(pictureWaybillType)){
					// 公里数不可编辑
					pictureTransferInfoPanel.getTxtKilometer().setEditable(true);
				}else{
					// 公里数可编辑
	...
	private void initInsuranceAmount(WaybillPanelVo bean)
	{
		try{
			...
			List<ValueAddDto> list = waybillService
					.queryValueAddPriceList(getQueryParam(bean));
			...
			if(list != null && !list.isEmpty())
			{
				dto = list.get(0);
				//设置最小报价费率
				bean.setMinFeeRate(Common.nullBigDecimalToZero(dto.getMinFeeRate()));
				//设置最大报价费率
				bean.setMaxFeeRate(Common.nullBigDecimalToZero(dto.getMaxFeeRate()));
				//默认保价费率
				...
				if(PriceEntityConstants.PRICING_CODE_BF.equals(dto.getPriceEntityCode()) && CollectionUtils.isNotEmpty(dto.getDiscountPrograms())){
				...
				bean.setInsuranceRateCanvas(Common.nullBigDecimalToZero(feeRate).toString());
			...
			if (StringUtils.isNotEmpty(ex.getMessage())) {
				...
				if(ex.getMessage().indexOf(i18n.get("foss.pkp.creating.itserivce.notconfig.default.insurance"))>=0){
					MsgBox.showITServiceInfo(MessageI18nUtil.getMessage(ex, i18n));
				}else{
	 ...
	 */
	private QueryBillCacilateValueAddDto getQueryParam(WaybillPanelVo bean) {
		UserEntity user = (UserEntity) SessionContext.getCurrentUser();
		OrgAdministrativeInfoEntity dept = user.getEmployee().getDepartment();
		...
		if(bean != null){
			// 出发部门CODE
			if(StringUtil.isNotEmpty(bean.getReceiveOrgCode())){
	...
	private void initComboBox() {
		...
		initComDeliveryMode();
		//特殊增值服务
		initComSpecialValueAddedServiceMode();
		//航班类型
	...
	/*private void initserviceRate(){
		...
		this.incrementPanel.getCombServiceRate().setModel(serviceRateModel);
	}*/
	...
	private void initTxtIndustrySourceCategory() {
		List<DataDictionaryValueEntity> list =  waybillService.queryIndustrySourceCategory4Dict();
		industrySourceCategoryModel = new DefaultComboBoxModel();
		...
		if(StringUtils.isNotBlank(pictureWaybillType) && WaybillConstants.WAYBILL_PICTURE.equals(pictureWaybillType)){
			this.pictureCargoInfoPanel.getTxtIndustrySourceCategory().setModel(industrySourceCategoryModel);
		} else {
			this.cargoInfoPanel.getTxtIndustrySourceCategory().setModel(industrySourceCategoryModel);
		}
	}
	...
	private void initCombFlabelleavemonth() {
		List<DataDictionaryValueEntity> list =  waybillService.queryFlabelleavemonth4Dict();
		flabelleavemonthModel = new DefaultComboBoxModel();
		for (DataDictionaryValueEntity dataDictionary : list) {
			DataDictionaryValueVo vo = new DataDictionaryValueVo();
			ValueCopy.valueCopy(dataDictionary, vo);
			flabelleavemonthModel.addElement(vo);
		}
		this.consignerPanel.getCombFlabelleavemonth().setModel(flabelleavemonthModel);
	}
	/**
	 * 
	 * 设置装卸费费率的默认值
	 */
	public void setServiceRate(WaybillPanelVo bean) {
		for (int i = 0; i < serviceRateModel.getSize(); i++) {
			BigDecimal big = (BigDecimal) serviceRateModel.getElementAt(i);
			//默认设置0
			if (big.compareTo(BigDecimal.ZERO) == 0) {
				bean.setServiceRate(big);
				this.incrementPanel.getCombServiceRate().setSelectedItem(big);
				break;
			}
		}
	}
	private void initCombInvoiceMode() {
		...
		datea.setValueName(WaybillConstants.INVOICE_TYPE_01);
		datea.setValueCode(WaybillConstants.INVOICE_01);
		...
		dateb.setValueName(WaybillConstants.INVOICE_TYPE_02);
		dateb.setValueCode(WaybillConstants.INVOICE_02);
		list.add(datea);
		list.add(dateb);
		DefaultComboBoxModel invoceModeModel = new DefaultComboBoxModel();
		for (DataDictionaryValueEntity dataDictionary : list) {
				DataDictionaryValueVo vo = new DataDictionaryValueVo();
				ValueCopy.valueCopy(dataDictionary, vo);
				invoceModeModel.addElement(vo);
		}
		this.consignerPanel.getCombInvoiceMode().setModel(invoceModeModel);
	}
	/**
	 * 自提件类型
	 */
	private void initEconomyGoodsType(){
		economyGoodsTypeModel = new DefaultComboBoxModel();
		if(WaybillConstants.WAYBILL_SALE_DEPARTMENT.equals(waybillType)){			
			//根据开单日期获取自提件类型列表
			List<MinFeePlanEntity> minFeeList=waybillService.getMinFeePlanEntityByDate(new Date());		
			...
			//是否自提件默认不可编辑
			this.basicPanel.getChbEconomyGoods().setEnabled(false);
			//自提件类型默认不可编辑
			this.basicPanel.getCobEconomyGoodsType().setEnabled(false);
		}		
		this.basicPanel.getCobEconomyGoodsType().setModel(economyGoodsTypeModel);
	}
	...
	{	
		...
		if(isBatchWaybill){
			...
			//根据当前用户所在部门查询部门所属产品
			setProductTypeModel(code);
			this.transferInfoPanel.getCombProductType().setModel(productTypeModel);
		}
		//营业部开单
		else if(WaybillConstants.WAYBILL_SALE_DEPARTMENT.equals(waybillType))
		{
			UserEntity user = (UserEntity) SessionContext.getCurrentUser();
			OrgAdministrativeInfoEntity dept = user.getEmployee().getDepartment();
			productTypeModel = new DefaultComboBoxModel();
		...
		}else if(WaybillConstants.WAYBILL_FOCUS.equals(waybillType))
		{
			productTypeModel = new DefaultComboBoxModel();
			if(StringUtils.isNotBlank(pictureWaybillType) && WaybillConstants.WAYBILL_PICTURE.equals(pictureWaybillType)){
				this.pictureTransferInfoPanel.getCombProductType().setModel(productTypeModel);
			}else{
				this.transferInfoPanel.getCombProductType().setModel(productTypeModel);
			}
		}
	...
	public void setProductTypeModel(String deptCode){
		...
		if(CollectionUtils.isNotEmpty(list)){
			CommonUtils.filterBigGoodsProductEntity(list,false);
	 ...
	 * @date 2015-8-10 下午18:33:17
	 * @see
	 */
	public void initComSpecialValueAddedServiceMode() {
		if(StringUtils.isNotBlank(this.pictureWaybillType) && 
				WaybillConstants.WAYBILL_PICTURE.equals(this.pictureWaybillType)){
				this.basicPanel.getSpecialValueAddedServiceType().setEnabled(false);
		}
		specialValueAddedServiceModel = new DefaultComboBoxModel();
		List<DataDictionaryValueEntity> list = waybillService
				.querySpecialValueAddedServiceType();
		DataDictionaryValueVo nullVo = new DataDictionaryValueVo();
		nullVo.setValueName("");
		specialValueAddedServiceModel.addElement(nullVo);
		for (DataDictionaryValueEntity dataDictionary : list) {
				DataDictionaryValueVo vo = new DataDictionaryValueVo();
				ValueCopy.valueCopy(dataDictionary, vo);
			specialValueAddedServiceModel.addElement(vo);
			}
		this.basicPanel.getSpecialValueAddedServiceType().setModel(specialValueAddedServiceModel);	
		}
	...
	private void initCombPickMode() {	
		...
		for (DataDictionaryValueEntity dataDictionary : list) {
			...
			if(!WaybillConstants.DELIVER_FREE.equals(dataDictionary.getValueCode()) 
					&& !WaybillConstants.DELIVER_FREE_AIR.equals(dataDictionary.getValueCode())){
					//非 批量开单 去除 内部带货送货 -- sangwenhao
					if(WaybillConstants.DELIVER_INNER_PICKUP.equals(dataDictionary.getValueCode()) && !isBatchWaybill
//							&& StringUtils.isNotBlank(waybillType) 
//							&& !WaybillConstants.WAYBILL_SALE_DEPARTMENT.equals(waybillType)
							){
						continue ;
					}
					/*合伙人开单再剔除 内部带货自提， 送货上楼安装（家居）,内部带货送货
					（上面仅当 非营业部开单 剔除了 内部带货送货 ，但是合伙人应该在所有情况下都需要剔除，
					所以在下面单独处理合伙人部分也应该添加） 2016年4月5日 09:32:03 葛亮亮	*/        
		...
		if(StringUtils.isNotBlank(pictureWaybillType) && WaybillConstants.WAYBILL_PICTURE.equals(pictureWaybillType)){
			this.pictureTransferInfoPanel.getCombPickMode().setModel(pikcModeModel);
		} else {
	...
	private void initFlightNumberType() {
		...
		if(StringUtils.isNotBlank(pictureWaybillType) && 
				WaybillConstants.WAYBILL_PICTURE.equals(pictureWaybillType)){
			this.pictureTransferInfoPanel.getCombFlightNumberType().setModel(flightNumberType);
		}else{
	...
	private void initFreightMethod() {
		...
		if(StringUtils.isNotBlank(pictureWaybillType) && WaybillConstants.WAYBILL_PICTURE.equals(pictureWaybillType)){
			this.pictureTransferInfoPanel.getCombFreightMethod().setModel(freightMethod);
		}else{
	...
	private void initCombOutSideRemark() {
		final List<DataDictionaryValueEntity>  list = waybillService
				.queryOuterRemark();
		final DefaultComboBoxModel combOutSideRemarkModel = new DefaultComboBoxModel();
		final JComboCheckBox checkbox;
		if(StringUtils.isNotBlank(this.pictureWaybillType) && 
				WaybillConstants.WAYBILL_PICTURE.equals(this.pictureWaybillType)){
			checkbox = (JComboCheckBox)this.pictureCargoInfoPanel.getCombOutSideRemark();
		}else{
			checkbox = (JComboCheckBox)this.cargoInfoPanel.getCombOutSideRemark();
		}
		final CargoInfoPanel cargoInfoPanel = this.cargoInfoPanel;
		final PictureCargoInfoPanel pictureCargoInfoPanel = this.pictureCargoInfoPanel;
		//针对异步打开装口 需要用swing特殊线程
		
		checkbox.setModel(
				combOutSideRemarkModel);
		for (DataDictionaryValueEntity dataDictionary : list) {
		...
		if(StringUtils.isNotBlank(this.pictureWaybillType) && 
				WaybillConstants.WAYBILL_PICTURE.equals(this.pictureWaybillType)){
			checkbox.addFocusListener(new CheckBoxListMouseHandler(pictureCargoInfoPanel,this.pictureWaybillType));
			checkbox.getRender().addFocusListener(new CheckBoxListMouseHandler(pictureCargoInfoPanel,this.pictureWaybillType));
		}else{
    ...
    protected class CheckBoxListMouseHandler implements FocusListener {
    	 ...
    	 * panel
    	 */
    	private CargoInfoPanel cargoInfoPanel;
    	
    	private PictureCargoInfoPanel pictureCargoInfoPanel;
    	 ...
    	 */
    	public CheckBoxListMouseHandler(CargoInfoPanel cargoInfoPanel){
    		this. cargoInfoPanel = cargoInfoPanel;
    	}
    	
    	public CheckBoxListMouseHandler(PictureCargoInfoPanel pictureCargoInfoPanel,String pictureWaybillType){
    		this. pictureCargoInfoPanel = pictureCargoInfoPanel;
    		this.pictureWaybillType = pictureWaybillType;
    	}
    	...
    	public void focusLost(FocusEvent e) {
    		...
    		if(StringUtils.isNotBlank(pictureWaybillType) &&
    				WaybillConstants.WAYBILL_PICTURE.equals(pictureWaybillType)){
    			originalTxt = pictureCargoInfoPanel.getTxtTransportationRemark().getText();
    		}else{
    			originalTxt = cargoInfoPanel.getTxtTransportationRemark().getText();
    		}
    		...
    		for(int i=0;i<remark.length;i++){
    			...
    			//zxy 20131017 BUG-57397 start 修改：后面增加/ 用来区分对内备注，此方法不能根本解决
    			if(oldData.indexOf(StringUtil.defaultIfNull(i18n.get("foss.gui.creating.woodYokeEnterAction.standGoods")+"/")) != -1){
    				woodTxt = oldData;
    			}
    			//zxy 20131017 BUG-57397 end 修改：后面增加/ 用来区分对内备注，此方法不能根本解决

    			if(oldData.indexOf(StringUtil.defaultIfNull(i18n.get("foss.gui.creating.woodYokeEnterAction.boxGoods"))) != -1){
    				sandTxt = oldData;
    			}

    			//zxy 20131118 ISSUE-4391 start 新增：设置打木托到备注框
    			if(oldData.indexOf(StringUtil.defaultIfNull(i18n.get("foss.gui.creating.woodYokeEnterAction.salverGoods"))) != -1){
    				salverTxt = oldData;
    			}

    			//zxy 20131118 ISSUE-4391 end 新增：设置打木托到备注框
             ...
             * 返单类别
             */
            String returnBillType = waybillBinder.getBean().getReturnBillType().getValueName();
            if(returnBillType!=null && !"".equals(returnBillType)
            		&& !WaybillConstants.NOT_RETURN_BILL.equals(waybillBinder.getBean().getReturnBillType().getValueCode())){         
            ...
            if(StringUtils.isNotBlank(pictureWaybillType) &&
            		WaybillConstants.WAYBILL_PICTURE.equals(pictureWaybillType)){
            	innerSiderRemark = this.pictureCargoInfoPanel.getTxtInsideRemark().getText();
            }else{
            	innerSiderRemark = this.cargoInfoPanel.getTxtInsideRemark().getText();
            }	
            ...
            if(StringUtils.isNotBlank(pictureWaybillType) &&
                 ...
                 * 大车直送
                 */
                if(pictureCargoInfoPanel.getChbCarThrough().isSelected()){
                //zxy 20130927 BUG-56130 start 修改：国际化参数写错
                	sb.append(i18n.get("foss.gui.creating.waybillEditUI.carThrough.name")).append("; ");
                //zxy 20130927 BUG-56130 end 修改：国际化参数写错
                }
            }else{
            	/**
            	 * 大车直送
            ...
            // 外部订单导入的储运事项 BUG-52347
            if(StringUtil.isNotBlank(vo.getTransportationRemarkOfWebOrder())){
            	sb.append(vo.getTransportationRemarkOfWebOrder()).append(";");
            }         
            //自提件类型
            DataDictionaryValueVo economyType=waybillBinder.getBean().getEconomyGoodsType();       
	...
	private void initCombRefundType() {
		...
		//zxy 20131019 BUG-57613 start 修改：valueName=null 个别电脑会报错
		DataDictionaryValueVo nullVo = new DataDictionaryValueVo();
		nullVo.setValueName("");
		combRefundTypeModel.addElement(nullVo);
//		combRefundTypeModel.addElement(new DataDictionaryValueVo());
		//zxy 20131019 BUG-57613 end 修改：valueName=null 个别电脑会报错
	...
	private void combActiveInfoType() {
		...
		if(isStart){
			...
			crmActiveParamVo.setActivityCategory(DictionaryValueConstants.BUSINESS_LTT);
	...
	private void initCombReturnBillType() {
		...
		this.incrementPanel.getCombReturnBillType().setModel(
				combReturnBillTypeModel);
	}
	...
	private void initCombPaymentMode() {
		...
		boolean isPicWaybill = this.getPictureWaybillType() != null && WaybillConstants.WAYBILL_PICTURE.equals(this.getPictureWaybillType().trim());
		//简单去重之后并保证顺序和原读取顺序一致 - DEFECT-15028 -- 272311-sangwenhao
		LinkedHashSet<DataDictionaryValueVo> linkedHashSet = new LinkedHashSet<DataDictionaryValueVo>();
		...
		for (DataDictionaryValueEntity dataDictionary : list) {
				...
				&& !WaybillConstants.CHECK.equals(dataDictionary.getValueCode()))
			{
				DataDictionaryValueVo vo = new DataDictionaryValueVo();
				ValueCopy.valueCopy(dataDictionary, vo);
				if(!(WaybillConstants.ONLINE_PAYMENT.equals(dataDictionary.getValueCode()) && WaybillConstants.WAYBILL_SALE_DEPARTMENT.equals(waybillType))&&BZPartnersJudge.IS_PARTENER ){
					linkedHashSet.add(vo);
				}
				
				//liding add for NCI
				//如果是图片开单,付款方式银行卡屏蔽
				if(isPicWaybill && WaybillConstants.CREDIT_CARD_PAYMENT.equals(dataDictionary.getValueCode())){
					//do nothing
				}else {
				  linkedHashSet.add(vo);
				}
		...
		Iterator<DataDictionaryValueVo> it = linkedHashSet.iterator();
		while(it.hasNext()){
			combPaymentModeModel.addElement(it.next());
		}
		
		this.incrementPanel.getCombPaymentMode()
				.setModel(combPaymentModeModel);
	}
	...
	private void initCombGoodsType() {
		...
		if(StringUtils.isNotBlank(this.pictureWaybillType) && 
				WaybillConstants.WAYBILL_PICTURE.equals(this.pictureWaybillType)){
			this.pictureCargoInfoPanel.getCombGoodsType().setModel(combGoodsType);
		}else{
	...
	private void setCombDefualtValue(WaybillPanelVo bean) {
		...
		setBillingType(bean);
		//设置返单类型默认值
		Common.setReturnBillType(bean,combReturnBillTypeModel);
		//设置空运货物类型默认值
	...
	/**
	 * 为了减少与数据库交互  初始化时对保价费率进行默认设置（不与数据库交互）  只对保险声明价格与数据库交互并默认设置到简单
	 * 保价面板上，因为初始化的时候简单也与数据库进行了同样的交互（可能当时开发的问题）  
	 * @author Foss-278328-hujinyang
	 * @date 2015-12-14 下午02:43:29
	 */

/*	private void initDefaultInsurance()
	{
		WaybillPanelVo bean = waybillBinder.getBean();

		//数据库获取的保价费率
		BigDecimal insuranceAmount = Common.nullBigDecimalToZero(queryInsuranceAmount());
		
		//保险声明价
		bean.setInsuranceAmount(insuranceAmount);
		
		//初始化详细计价面板保价声明值
		bean.setInsuranceAmountCanvas(insuranceAmount+"");
		
//		String insuranceRateRange = "["+defaultMaxFeeRate+","+defaultMinFeeRate+"]";
//		//保价费率
//		bean.setInsuranceRate(defaultFee);
//		//保价费率范围
//		bean.setInsuranceRateRange(insuranceRateRange);
	 ...
	 * 查询默认的保险声明价格
	 * @author Foss-278328-hujinyang
	 * @param 2015-12-14 下午02:49:22
	 * @return
	 */
	public BigDecimal queryInsuranceAmount()
	{
		BigDecimal originnalCost = BigDecimal.ZERO;
		try {
			ConfigurationParamsEntity entity = CommonUtils.getConfigurationParamsEntity(DictionaryConstants.SYSTEM_CONFIG_PARM__PKP,
			          ConfigurationParamsConstants.PKP_PARM__DEFAULT_INSURANCE_AMOUNT, FossConstants.ROOT_ORG_CODE);
	 ...
	 * @功能：增加监听事件
	 * @author:218371-foss-zhaoyanjun
	 * @date:2016-06-03下午18:26
	 */
	public void addListener(){
		WaybillPanelVo bean = waybillBinder.getBean();
		if(pictureWaybillEditUI!=null&&bean.getIbtg()!=null){
			consignerPanel.setBean(bean);
			consigneePanel.setBean(bean);
			pictureTransferInfoPanel.setBean(bean);
			incrementPanel.setIntellligenceBean(bean);
		}
	}
	...
	public WaybillPrintBean getWaybillPrintBean()
		...
		//为配合阿里商城地址栏只打印省市的字段封装
		if(vo.getDeliveryCustomerAreaDto()!=null){
			printBean.setDeliveryCustomerProvice(vo.getDeliveryCustomerAreaDto().getProvinceName());
			printBean.setDeliveryCustomerCity(vo.getDeliveryCustomerAreaDto().getCityName());
		}
...
//		}
	...
	public PictureCargoInfoPanel getPictureCargoInfoPanel() {
		return pictureCargoInfoPanel;
	}
	/**
	 * @return the cargoInfoPanel .
	 */
	public CargoInfoPanel getCargoInfoPanel() {
...
class CanvasPanelAction implements ActionListener {
	...
	public CanvasPanelAction(JPanel contentPanel, JButton btnClose) {
		...
		canvasPanel = new CanvasPanel(contentPanel,
					btnClose);
	}
	 ...
	 */
	public void setConsignerPanel(ConsignerPanel consignerPanel) {
	 ...
	 */
	public void setConsigneePanel(ConsigneePanel consigneePanel) {
	 ...
	 */
	public void setImportWaybillPanelVo(WaybillPanelVo importWaybillPanelVo) {
	...
	public DefaultComboBoxModel getServiceRateModel() {
		return serviceRateModel;
	}

	public void setServiceRateModel(DefaultComboBoxModel serviceRateModel) {
		this.serviceRateModel = serviceRateModel;
	}

	public Boolean getIsImportOrder() {
		return isImportOrder;
	}

	public void setIsImportOrder(Boolean isImportOrder) {
	...
	public WebOrderDialog getWebOrderDialog() {
		return webOrderDialog;
	}

	public void setWebOrderDialog(WebOrderDialog webOrderDialog) {
	...
	public CalculateCostsDialog getCalculateCostsDialog() {
		return calculateCostsDialog;
	}
	
	public void setCalculateCostsDialog(CalculateCostsDialog calculateCostsDialog) {
		this.calculateCostsDialog = calculateCostsDialog;
	}

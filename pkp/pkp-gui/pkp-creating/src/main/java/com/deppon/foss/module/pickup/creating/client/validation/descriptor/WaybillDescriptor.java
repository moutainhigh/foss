 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/creating/client/validation/descriptor/WaybillDescriptor.java
 * 
 * FILE NAME        	: WaybillDescriptor.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 *
 */
public class WaybillDescriptor {
	 ...
	 * 日志
	 */
	private static final Log LOG = LogFactory.getLog(WaybillDescriptor.class);
	...
	private static final II18n i18n = I18nManager
			.getI18n(WaybillDescriptor.class);
	 ...
	 * @see
	 */
	public String validateWaybillNo(String waybillNo, WaybillPanelVo bean) {
		// 已开单、PDA提交，PDA已补录时，不进行校验
		if(WaybillConstants.WAYBILL_STATUS_PC_ACTIVE.equals(bean.getWaybillstatus())
				|| WaybillConstants.WAYBILL_STATUS_PENDING_ACTIVE.equals(bean.getWaybillstatus())){
			return WaybillConstants.SUCCESS;
		}else{	
			if(!StringUtil.isEmpty(waybillNo)){
				if(waybillNo.startsWith("-")){
					return i18n.get("foss.gui.creating.waybillDescriptor.waybillNo.format");
				}				
			}
			if (NumberValidate.checkBetweenLength(waybillNo, EIGHT, NINE)) {
				...
				if(StringUtil.isNotEmpty(bean.getOldWaybillNo())&&!waybillNo.equals(bean.getOldWaybillNo())){
    				...
    				if (bool) {
    					...
    					//根据MANA-749进行优化，校验运单号是否已有状态标识    					
    					if(waybillService.isExistWaybillTransaction(waybillNo)){
    						return i18n.get("foss.gui.creating.waybilldescriptor.waybillno.isbusinessexist");
    					}
    					return WaybillConstants.SUCCESS;
    				}
				}else{
					//PDA待补录运单只判断非PDA待补录的单号存不存在
					bool = waybillService.checkWaybillNoExceptPDAPending(waybillNo);
					if (bool) {
    					LOG.info(i18n.get("foss.gui.creating.waybillDescriptor.waybillNo.exist"));
    					return i18n.get("foss.gui.creating.waybillDescriptor.waybillNo.exist");
    				} else {
	...
	public String validateDriverCode(String driverCode) {
		if(StringUtils.isEmpty(driverCode)){
			return WaybillConstants.SUCCESS;
		}else if (!NumberValidate.checkBetweenLength(driverCode, SIX, SIX)) {
	 ...
	 * @see
	 */
	public String validateDeliveryCustomerMobilephone(String deliveryCustomerMobilephone, WaybillPanelVo bean) {
		if (null != deliveryCustomerMobilephone && !"".equals(deliveryCustomerMobilephone.trim())) {
			...
			if(StringUtils.isNotEmpty(bean.getDeliveryCustomerPhone())){
				if(deliveryCustomerMobilephone.equals(bean.getDeliveryCustomerPhone())){
	 ...
	 */
	public String validateReceiveCustomerMobilephone(String receiveCustomerMobilephone, WaybillPanelVo bean) {
		if (null != receiveCustomerMobilephone && !"".equals(receiveCustomerMobilephone.trim())) {
			...
			if (phoneLength == lengthLimitEleven) {
				if (!"1".equals(receiveCustomerMobilephone.substring(0, 1))) {
					return i18n.get("foss.gui.creating.waybillDescriptor.receiver.mobile.one");
				}
			}
			//判定手机号码与电话号码是否存在一致的情况：存在电话号码组成：0+手机号码这种情况，需要过滤掉
			if(StringUtils.isNotEmpty(bean.getReceiveCustomerPhone())){
				if(receiveCustomerMobilephone.trim().equals(bean.getReceiveCustomerPhone())){
					return i18n.get("foss.gui.creating.listener.Waybill.telephoneInMobilePhoneisExist");
				}
				try {
					...
					if (StringUtils.isNotEmpty(receiveCustomerPhone)//不为空
						...
						if(StringUtils.isNotEmpty(receiveCustomerPhone)
	...
	public String validateTargetOrgCode(String targetOrgCode) {
		...
		if(targetOrgCode != null && !"".equals(targetOrgCode))
		{
	...
	/*// ===========lianhe/TODO 取消工号校验/2017年1月9日/start=======
	...
	* @date 2015-8-19 下午5:46:23 
	...
	//校验意义不大--故暂时去掉
	/*
	public String validateLoadLineName(String loadLineName,WaybillPanelVo bean) {
		try {
			if(bean.getCheckloadLineName()){
				String productCode = bean.getProductCode().getCode();
				//FLF精准卡航  FSF 精准城运
				if(StringUtils.isEmpty(productCode)||
						!(WaybillConstants.TRUCK_FLIGHT.equals(productCode)||
				...
				String targetOrgCode = bean.getCustomerPickupOrgCode().getCode();
				if(StringUtils.isNotEmpty(receiveOrgCode) && StringUtils.isNotEmpty(targetOrgCode)){
					...
					String result = waybillService.checkWarningLine(receiveOrgCode, targetOrgCode);
	...
	*/
	...
	public String validatePartnerPhone(String partnerPhone){
		if(StringUtils.isEmpty(partnerPhone)){
			return WaybillConstants.SUCCESS;
		}
		...
		boolean b= matcher.matches();
		if(!b){
			return i18n.get("foss.gui.creating.listener.Waybill.validatePartnerPhone");
		}
		return WaybillConstants.SUCCESS;
	}
}
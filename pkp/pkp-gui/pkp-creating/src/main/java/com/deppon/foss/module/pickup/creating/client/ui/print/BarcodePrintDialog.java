 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/creating/client/ui/print/BarcodePrintDialog.java
 * 
 * FILE NAME        	: BarcodePrintDialog.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 * 
 */
public class BarcodePrintDialog extends JDialog {
	...
	// 日志
	private static final Log LOG = LogFactory.getLog(BarcodePrintDialog.class);

	/**
	 * 国际化
	 */
	private static II18n i18n = I18nManager.getI18n(BarcodePrintDialog.class);
	 ...
	 * @param waybillNo
	 */
	public BarcodePrintDialog(String waybillNo, String waybillstatus) {
		this();
		...
		if (StringUtils.isNotEmpty(waybillNo)) {
			...
			if(txtWaybillNo.getText().trim().length()>MAXLENGTH){
			...
			boolean isExpressBill = waybillService.queryIsExpressBill(waybillNo);
			if(isExpressBill){
				JOptionPane
				.showMessageDialog(
						null,
	...
	private Boolean validateNum() {
		if (StringUtils.isEmpty(txtBeginGoodNum.getText()) || Integer.parseInt(txtBeginGoodNum.getText()) <= 0) {
			...
			txtBeginGoodNum.setText(String.valueOf(1));
			return false;
			...
			txtEndGoodNum.setText(txtGoodsNum.getText());
			return false;
		} else {
			return true;
		}
	...
	private void addNumListener() {
		...
		txtBeginGoodNum.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent e) {
				//如果单号为空，什么都不做
				if(StringUtils.isEmpty(txtWaybillNo.getText().trim())){
					return;
				}
				if (validateNum()) {
...
//			labeledlist = waybillService.queryAllSerialByWaybillNo(waybillNo);
	...
	private void init() {
		...
		panel2.add(chckbxPrintCheckBox, "2, 6");
		...
		groupLayout.setVerticalGroup(groupLayout.createParallelGroup(Alignment.LEADING).addGroup(groupLayout.createSequentialGroup().addGap(NumberConstants.NUMBER_7).addComponent(panel1, GroupLayout.DEFAULT_SIZE, THREEZEROSEVEN, Short.MAX_VALUE).addContainerGap()));
		// 最终打印流水号

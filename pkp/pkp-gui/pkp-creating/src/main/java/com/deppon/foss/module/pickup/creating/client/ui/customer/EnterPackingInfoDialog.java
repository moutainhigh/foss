...
import com.deppon.foss.module.pickup.creating.client.action.PackingYokeEnterAction;
import com.deppon.foss.module.pickup.creating.client.action.PackingYokeResetAction;
 ...
 * 录入打包装信息
 * 
 * @author：218371-foss-zhaoyanjun creating
...
public class EnterPackingInfoDialog extends JDialog {
	...
	FloatDocument floatDocument1=new FloatDocument(FOUR,2);
	//整数
	...
	PackageNumberDocument sevenInt=new PackageNumberDocument(SEVEN);
	...
	//其他总金额
	@FocusSeq(seq=17)
	private JTextFieldValidate otherTotlePriceTextField;
	 ...
	 * @param waybillEditUI
	 */
	public EnterPackingInfoDialog(WaybillEditUI waybillEditUI) {
...
//		initData(); zxy 20131118 ISSUE-4391 修改：由show函数调用
	...
	private void initFirstData() {
		if(!(
				(bean.getPaperBoxTotlePrice()==null||bean.getPaperBoxTotlePrice().compareTo(BigDecimal.ZERO)==0)&&
				(bean.getFibelBagTotlePrice()==null||bean.getFibelBagTotlePrice().compareTo(BigDecimal.ZERO)==0)&&
				(bean.getOtherTotle()==null||bean.getOtherTotle().compareTo(BigDecimal.ZERO)==0)
				)){
			vo.setPaperBoxTotlePrice(bean.getPaperBoxTotlePrice());
			vo.setFibelBagTotlePrice(bean.getFibelBagTotlePrice());
			vo.setOther(bean.getOtherTotle());
			...
			otherTotlePriceTextField.setText(bean.getOtherTotle()==null?"":bean.getOtherTotle().toString());
			allTotlePriceTextField.setText(bean.getPackingTotle()==null?"":(bean.getPaperBoxTotlePrice().add(bean.getFibelBagTotlePrice().add(bean.getOtherTotle()))).toString());
		}
	...
	private void init() {
		...
		//其他总额
//		JLabel otherTotlePriceLabel = new JLabel("其他：");
//		getContentPane().add(otherTotlePriceLabel, "9, 15, center, center");
//		
//		otherTotlePriceTextField = new JTextFieldValidate();
//		otherTotlePriceTextField.setHorizontalAlignment(SwingConstants.CENTER);
//		getContentPane().add(otherTotlePriceTextField, "10, 15, 2, 1, fill, default");
//		otherTotlePriceTextField.setColumns(10);
//		otherTotlePriceTextField.setDocument(floatDocument);
//		
//		otherTotlePriceTextField.addFocusListener(new FocusAdapter() {
//			@Override
//			public void focusLost(FocusEvent e) {
//				Sum();
//			}
//		});
		...
		allTotlePriceLabel.setFont(new Font("宋体", Font.BOLD, NumberConstants.NUMBER_12));
		...
		resetButton.setFont(new Font("宋体", Font.BOLD, NumberConstants.NUMBER_12));
		getContentPane().add(resetButton, "3, 16, 2, 1, fill, center");
		
		//关闭按钮
		closeButton = new JButton("关闭");
		closeButton.setFont(new Font("宋体", Font.BOLD, NumberConstants.NUMBER_12));
		getContentPane().add(closeButton, "6, 16, 2, 1, fill, center");
		
		//确定按钮
		sureInsertButton = new JButton("确定录入");
		sureInsertButton.setFont(new Font("宋体", Font.BOLD, NumberConstants.NUMBER_12));
	...
	public void sum(){
		if(this.getBufferLabel().getText()==null || ("".equals(this.getBufferLabel().getText()))){
			this.getBufferLabel().setText("0");
		}
		
		if(this.getDiscountLabel().getText()==null || ("".equals(this.getDiscountLabel().getText()))
				//0-0.99折 大于0.99折置为0
				|| (new BigDecimal(this.getDiscountLabel().getText().trim()).compareTo(new BigDecimal("0.99"))==1)){
			this.getDiscountLabel().setText("0");
		}
		//将文本框值放入vo中
		setPackingYokeVo();
		//通过id查询纸纤包装单价信息
...
//		if(this.getOtherTotlePriceTextField().getText()!=null&&!("".equals(this.getOtherTotlePriceTextField().getText()))){
//			vo.setOther(new BigDecimal(this.getOtherTotlePriceTextField().getText().trim()));
//		}else{
...
//		}
	...
	private void setTotle() {
		// TODO Auto-generated method stub
		vo.setTotle(
				vo.getPaperBoxTotlePrice().add(vo.getFibelBagTotlePrice()).add(vo.getBuffer()==null?new BigDecimal(0):vo.getBuffer())
	...
	private void initData() {
		...
		bufferLabel.setText(bean.getBufferPrice()==null?"":bean.getBufferPrice().toString());
		discountLabel.setText(bean.getDiscountRate()==null?"":bean.getDiscountRate().toString());
//		paperBoxTotlePriceTextField.setText(bean.getPaperBoxTotlePrice()==null?"":bean.getPaperBoxTotlePrice().toString());
//		fibelBagTotlePriceTextField.setText(bean.getFibelBagTotlePrice()==null?"":bean.getFibelBagTotlePrice().toString());
//		otherTotlePriceTextField.setText(bean.getOtherTotle()==null?"":bean.getOtherTotle().toString());
//		allTotlePriceTextField.setText(bean.getPackingTotle()==null?"":bean.getPackingTotle().toString());
		sum();
//		if(vo.getPaperBoxTotlePrice()==null||vo.getFibelBagTotlePrice().doubleValue()==0){
...
//------------------------------------------------
	...
	public JTextFieldValidate getOtherTotlePriceTextField() {
		return otherTotlePriceTextField;
	}

	public void setOtherTotlePriceTextField(JTextFieldValidate otherTotlePriceTextField) {
		this.otherTotlePriceTextField = otherTotlePriceTextField;
	}
	...
	public void setWaybillEditUI(WaybillEditUI waybillEditUI) {
		this.waybillEditUI = waybillEditUI;
	}

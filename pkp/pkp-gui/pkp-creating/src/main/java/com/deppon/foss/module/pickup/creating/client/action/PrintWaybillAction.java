 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/creating/client/action/PrintWaybillAction.java
 * 
 * FILE NAME        	: PrintWaybillAction.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
...
打印出发部门名称/出发部门地址/出发部门对外电话号码（如果没有部门对外电话，直接把电话替换成400-830-5555）
 例子：广州白云区营业部/广州市白云区新广从路口（白云心理医院旁）/020-140091 62140092 62140139 62140096

9、	提货网点（对应数据元素：部门信息）

10、	打印到达部门名称/到达部门地址/到达部门对外电话号码（如果没有部门对外电话，直接把电话替换成400-830-5555）
 例子：广州白云区营业部/广州市白云区新广从路口（白云心理医院旁）/020-140091 62140092 62140139 62140096
 ...
 * </p>
 * <p style="display:none">
 * version:V1.0,author:jiangfei,date:2012-10-18 下午1:57:16,content:
 * </p>
 * 
 * @author jiangfei
 * @date 2012-10-18 下午1:57:16
 * @since
 * @version
 *  *  * 录入货物信息SUC业务规则
 ...
 */
public class PrintWaybillAction implements IButtonActionListener<ChoosePrintTypeDialog> {
	// ui object
	private ChoosePrintTypeDialog ui;

	@Inject
	private IWaybillService wayBillService;
	
	// 日志
	private static final Log LOG = LogFactory.getLog(PrintWaybillAction.class);

	// 国际化
	private static final II18n i18n = I18nManager.getI18n(PrintWaybillAction.class);
	// 运单entitiy
	...
	private void dataInsert(JasperContext jctx, WaybillPrintBean resourceBean, WaybillEntity waybillEntity, UserEntity user) {
		...
		/**
		 * 打印运单增设阿里采购商城订单处理的需求=====start
		 * yangxiaolong   220125
		 */
		//注意此处需要与订单来源匹配，当订单来自阿里巴巴商城时，进行如下参数设置
		//测试代码=====注意删除"ALIBABAMRO"
		//waybillEntity.setOrderChannel("ALIBABAMRO");
		if(waybillEntity != null && (WaybillConstants.ALIBABA).equals(waybillEntity.getOrderChannel())){
			jctx.put("deliveryCustomerName", "阿里巴巴(中国)软件有限公司");//发货客户名字    1.对应港版发货公司
			jctx.put("consignor", "阿里巴巴(中国)软件有限公司");//发货人的名字    (大陆版托运人公司)   1.对应大陆全版的托运人/公司
			jctx.put("field1", "400-800-1688#8");   //大陆版联系方式  2.对应大陆全版的联系方式
		    jctx.put("deliveryCustomerContact", "采购商城");//发货联系人（港版）   2.对应港版联系人
		    StringBuffer field2 = new StringBuffer();
			if(StringUtils.isNotEmpty(resourceBean.getDeliveryCustomerProvice())){
				field2.append(resourceBean.getDeliveryCustomerProvice().trim());
			}
			if(StringUtils.isNotEmpty(resourceBean.getDeliveryCustomerCity())){
				field2.append(resourceBean.getDeliveryCustomerCity().trim());
			}
		    jctx.put("consignorAddr",isBeanValueNull(field2));// "$F发货人地址"   4.对应大陆全版地址    以及港版的地址
			jctx.put("payType", "");// "$F付款方式");
			jctx.put("insuranceCharge", "");// "$F保价费");t
			jctx.put("insuranceCharge1", "");// "$F保价费");t
			jctx.put("rate", "");// "$F费率");
			jctx.put("freight", "");// "$F运费");
			jctx.put("otherfee", " ");//"$F其他运费");
			
			jctx.put("noPayAmount","" );// "$F现付费用");
			jctx.put("toPayAmount", "");// "$F到付费用");
			// "$F港版现付费用");
			jctx.put("noPayAmountHK","");// "$F预付费用 * 港币汇率"
			// "$F大陆版到付费用");
			jctx.put("toPayAmountHK", "");// "$F到付费用 * 港币汇率"
			jctx.put("exchangeRate", "");// "$F港币汇率"
			//保价费港版和大陆版应该保持一致，以下为港版
			jctx.put("insuranceHK", " ");// "$F保价申明价值 * 港币汇率"
			jctx.put("insuranceChargeHK"," ");
			jctx.put("promotionsFee", "");// "$F优惠全额");t
		}else{
			jctx.put("deliveryCustomerName", isBeanValueNull(resourceBean.getDeliveryCustomerName()));//发货客户名字    1.对应港版发货公司
			jctx.put("consignor", isBeanValueNull(resourceBean.getDeliveryCustomerContact()));//发货人的名字    (大陆版托运人公司)   1.对应大陆全版的托运人/公司
			jctx.put("field1", isBeanValueNull(deliverCustomConnactWay));   //大陆版联系方式  2.对应大陆全版的联系方式
			jctx.put("deliveryCustomerContact", isBeanValueNull(resourceBean.getDeliveryCustomerContact()));//发货联系人（港版）   2.对应港版联系人
			//发货联系人地址
			StringBuffer field2 = new StringBuffer();
			if(StringUtils.isNotEmpty(ui.getWaybillEditUI().getConsignerPanel().getTxtConsignerArea().getText())){
				field2.append(ui.getWaybillEditUI().getConsignerPanel().getTxtConsignerArea().getText().trim());
			}
			if(StringUtils.isNotEmpty(resourceBean.getDeliveryCustomerAddress())){
				field2.append(resourceBean.getDeliveryCustomerAddress());
			}
			if(StringUtils.isNotEmpty(resourceBean.getDeliveryCustomerAddressNote())){
				field2.append("("+resourceBean.getDeliveryCustomerAddressNote()+")");
			}
			jctx.put("consignorAddr", isBeanValueNull(field2));// "$F发货人地址"   4.对应大陆全版地址    以及港版的地址
			...
			//针对定价体系优化系统--定价体系优化项目POP-561
			jctx.put("promotionsFee", isBeanValueNull(i18n.get("foss.gui.creating.canvasContentPanel.promotionsFee.label1")+resourceBean.getPromotionsFee()));// "$F优惠全额");t
		}
		/**
		 * 打印运单增设阿里采购商城订单处理的需求===end
		 * */
		...
		//jctx.put("field1", isBeanValueNull(deliverCustomConnactWay));// 2.对应大陆全版的联系方式
		...
		if(null != resourceBean.getReceiveCustomerPhone()){
			if(StringUtils.isNotEmpty(resourceBean.getReceiveCustomerPhone())){
		...
		//收货联系人地址
		StringBuffer consignorAddr = new StringBuffer();
		if(StringUtils.isNotEmpty(ui.getWaybillEditUI().getConsigneePanel().getTxtConsigneeArea().getText())){
			consignorAddr.append(ui.getWaybillEditUI().getConsigneePanel().getTxtConsigneeArea().getText().trim());
		}
		if(StringUtils.isNotEmpty(resourceBean.getReceiveCustomerAddress())){
			consignorAddr.append(resourceBean.getReceiveCustomerAddress());
		}
		if(StringUtils.isNotBlank(resourceBean.getReceiveCustomerAddressNote())){
			consignorAddr.append("("+resourceBean.getReceiveCustomerAddressNote()+")");
		}
		jctx.put("field2", isBeanValueNull(receiveCustomConnactWay));// "$F收货人联系方式"
		//jctx.put("consignorAddr", isBeanValueNull(field2));// "$F发货人地址"  4.对应大陆全版地址    以及港版的地址
		jctx.put("addr", isBeanValueNull(consignorAddr));// "$F收货人地址"
...
//		deliveryCustomerName
		...
		if (waybillEntity != null && waybillEntity.getReceiveOrgCode() != null) {
			...
			//添加广告语
			jctx.put("benefitActivity", printUtil.getWaybillDocAd(waybillEntity.getReceiveOrgCode() , DictionaryValueConstants.DEPARTURE_CITY));
	...
	private BigDecimal getConvertFee(BigDecimal rmbAmout,String currentCode,Date date){
		BigDecimal money = waybillService.getExchangedFee(rmbAmout, currentCode, date);
		if (money != null) {
	...
	private BigDecimal getConvertFeeRate(String currentCode,Date date){
		BigDecimal feeRate = waybillService.getExchangedFeeRate(currentCode, date);
		if (feeRate != null) {
			return feeRate;
		}else {
			return BigDecimal.ZERO;
	 ...
	 */
	@Override
	public void setInjectUI(ChoosePrintTypeDialog ui) {
		this.ui = ui;
	}

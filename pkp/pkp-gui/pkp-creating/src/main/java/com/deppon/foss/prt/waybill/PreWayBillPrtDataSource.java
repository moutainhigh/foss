 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/prt/waybill/PreWayBillPrtDataSource.java
 * 
 ...
 *
 */
package com.deppon.foss.prt.waybill;
...
public class PreWayBillPrtDataSource implements JasperDataSource {
	...
	private static final Log LOG = LogFactory
			.getLog(PreWayBillPrtDataSource.class);
	...
	public List<Map<String, Object>> createDetailDataSource(JasperContext jctx) {
		...
		try {
			...
			m.put("deliveryCustomerName", jctx.get("deliveryCustomerName"));// "$F发货客户名称 港版");
			m.put("receiveCustomerName", jctx.get("receiveCustomerName"));// "$F收货客户名称 港版");
			m.put("benefitActivity", jctx.get("benefitActivity"));//出发城市打印广告专用
					                             
			lst.add(m);
		} catch (Exception e) {
			LOG.error("大陆版运单全预览异常",e);
		}
		return lst;
	}

...
import com.deppon.foss.module.pickup.creating.client.ui.popupdialog.CalculateCostsDialog.OtherChargeModel;
 ...
 */
public class ShowCalculateDialogAction  implements IButtonActionListener<CalculateCostsDialog>{

	
	/**
	 * 日志
	 */
	private static final Log LOG = LogFactory.getLog(ShowCalculateDialogAction.class);
	
	/**
	 * 国际化
	 */
	private static final II18n i18n = I18nManager.getI18n(ShowCalculateDialogAction.class);
	
	 /**
	  * 主界面
	  */
	private CalculateCostsDialog ui;
	...
	private void queryLodeDepartmentInfo(WaybillPanelVo bean) {
		...
		try {
			...
			if(bean.getPickupCentralized()!=null&&bean.getPickupCentralized()){
				...
				String pictureWaybillNo = bean.getPictureWaybillNo();
				if(StringUtil.isNotEmpty(waybillNo) && StringUtil.isNotEmpty(pictureWaybillNo)
			...
			if (dto == null 
					/*zxy 20130923 BUG-55629 start 修改：参照开单界面原理[何龙 ：空运时不需要判断，也可以整体把下面这个判断去掉，现在要考虑下，是否会有潜在的影响？？？] */
					/*|| dto.getFreightRoute() == null*/
					) {				
				//zxy 20130923 BUG-55629 end 修改：参照开单界面原理[何龙 ：空运时不需要判断，也可以整体把下面这个判断去掉，现在要考虑下，是否会有潜在的影响？？？] 
...
//				LOG.info("是否可区分AB货:" + dto.getGoodsTypeIsAB());
	...
	/*private void setGoodsTypeAB(WaybillPanelVo bean)
	{
		...
		{
			ui.getRbnA().setEnabled(true);
			ui.getRbnB().setEnabled(true);
		}else
		{
			ui.getRbnA().setEnabled(false);
			ui.getRbnB().setEnabled(false);
		}
	}*/
...
//		Common.setSaveAndSubmitFalse(ui);
	 ...
	 */
	public void calculateOtherCharge(WaybillPanelVo bean) {
		JXTable table = ui.getTblOther();
		OtherChargeModel model = (OtherChargeModel) table.getModel();
	 ...
	 */
	private void queryOtherChargeData(CalculateCostsDialog ui, WaybillPanelVo bean) {

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/creating/client/service/impl/WaybillOffLinePendingService.java
 * 
 * FILE NAME        	: WaybillOffLinePendingService.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 *
 */
public class WaybillOffLinePendingService implements IWaybillOffLinePendingService {
	 ...
	 * 国际化对象
	 */
	private II18n i18n = I18nManager.getI18n(WaybillOffLinePendingService.class);
	 ...
	 * 
	 * @author 025000-FOSS-helong
	 * @date 2012-12-27 上午09:22:57
	 * @param waybillDto
	 */
	@Transactional
	@Override
	public void tempSave(WaybillPendingDto waybillDto) {
		// 对传入的待处理运单信息进行校验
	...
	public void addWaybillPending(WaybillPendingEntity waybillPending,WaybillSystemDto systemDto,boolean isUpdate) {
		...
		if (userEntity == null) {
			throw new IllegalArgumentException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.nullPreUserinfo"));
		}  
		...
		if(waybillPending.getBillTime()==null){
		...
		// 运输类型
		waybillPending.setTransportType(CommonUtils.convertProductCodeToTransType(waybillPending.getProductCode()));
		// 设置是否有效
		...
		if(isUpdate){
    		try {
    			...
    			throw new WaybillPendingSaveException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.failUpdatePendingWaybill") + e.getMessage(), e);
    		}
		}else{
			try {
    			waybillPendingDao.insert(waybillPending);
    		} catch (BusinessException e) {
    			throw new WaybillPendingSaveException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.failSavePendingWaybill") + e.getMessage(), e);
    		}
		}
	}
	...
	private void verifyWaybill(WaybillPendingDto waybillDto){
		if(!waybillDto.getIsPendingImport()){
    		...
    		if (isWayBillExsits(waybillNo)) {
    			throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.one"), i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.two") + waybillNo + i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.three"));
    		}
		}
		...
		if (waybillEntity != null) {
			...
			if (!WaybillConstants.INNER_PICKUP.equals(pickupMode)) {
				if (chargeDetail == null) {
					throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.one"), i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.noEmptyFeeDetail"));
				}
				if (paymentList == null) {
					throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.one"), i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.noEmptyPayDetail"));
				}
				// 提货方式为内部带货自提费用明细必须为空
			} else {
				if (chargeDetail != null) {
					throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.one"), i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.mustEmptyFeeDetail"));
				}
				if (paymentList != null) {
					throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.one"), i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.mustEmptyPayDetail"));
				}
				if (discoutDetail != null) {
					throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.one"), i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.mustEmptyDiscountDetail"));
				}
			}
		} else {
			throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.one"), i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.waybillCheck.noEmptyPendingWaybillBasInfo"));
		}
	}
	...
	public void addWaybillDisDtlPending(List<WaybillDisDtlPendingEntity> discoutDetail, WaybillSystemDto systemDto, boolean isUpdate) {
		...
		try {
			...
			throw new WaybillPendingSaveException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.failSavePendingWaybillDiscountDetail")+ e.getMessage(), e);
		}
	}
	...
	public void addWaybillChDtlPending(List<WaybillCHDtlPendingEntity> chargeDetail, WaybillSystemDto systemDto, boolean isUpdate) {
		...
		try {
			...
			throw new WaybillPendingSaveException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.failSavePendingWaybillFeeDetail") + e.getMessage(), e);
		}
	}
	...
	public void addWaybillPaymentPending(List<WaybillPaymentPendingEntity> paymentList, WaybillSystemDto systemDto, boolean isUpdate) {
		...
		try {
			...
			// e.printStackTrace();
			throw new WaybillPendingSaveException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.failSavePendingWaybillPayDetail") + e.getMessage(), e);
		}
	}
	...
	public void addWoodenRequirePending(WoodenRequirePendingEntity woodenRequire, WaybillSystemDto systemDto, boolean isUpdate) {
		...
		try {
			...
			throw new WaybillPendingSaveException(i18n.get("foss.gui.creating.waybillOffLinePendingService.exception.failSavePendingWaybillWoodPackageInfo") + e.getMessage(), e);
		}
	}
	...
	public WaybillPendingEntity queryPendingByWaybillNoAndOrderNo(String waybillNo, String orderNo){
		return waybillPendingDao.queryPendingByWaybillNoAndOrderNo(waybillNo, orderNo);
	}
	...
	public List<WaybillPendingEntity> queryPendingExpress(
		...
		return waybillPendingDao.queryPendingExpress(waybillPendingDto);
	}
	...
	public List<WaybillOfflineEntity> queryOfflinePending(WaybillPendingDto waybillPendingDto) {
		return waybillPendingDao.queryOfflinePending(waybillPendingDto);
	}
	 ...
	 */
	@Override
	public int insert(WaybillPendingEntity waybillPending) {
		return waybillPendingDao.insert(waybillPending);
	}
	...
	public void deleteByPrimaryKey(String id){
		WaybillPendingEntity pendingEntity = waybillPendingDao.queryByPrimaryKey(id);
		if(pendingEntity != null){
			waybillDisDtlPendingDao.deleteByWaybillNo(pendingEntity.getWaybillNo());
			waybillCHDtlPendingDao.deleteByWaybillNo(pendingEntity.getWaybillNo());
			waybillPaymentPendingDao.deleteByWaybillNo(pendingEntity.getWaybillNo());
			woodenRequirePendingDao.deleteByWaybillNo(pendingEntity.getWaybillNo());
		}
		waybillPendingDao.deleteByPrimaryKey(id);
	}
}
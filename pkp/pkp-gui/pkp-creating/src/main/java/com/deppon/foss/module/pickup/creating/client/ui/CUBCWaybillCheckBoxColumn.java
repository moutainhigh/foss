 ...
 * 选中导出列
 */
public class CUBCWaybillCheckBoxColumn  extends AbstractCellEditor implements  TableCellEditor, TableCellRenderer  {  
    ...
    //TableModel
    private CUBCWaybillManageTableModel dtm; 
    ...
    public CUBCWaybillCheckBoxColumn(TableColumn tc, CUBCWaybillManagerUI ui){  
    ...
    public Component getTableCellEditorComponent(JTable table,  
        ...
        //  当单元格处于编辑状态时
    	if(dtm==null){
    		dtm = (CUBCWaybillManageTableModel)table.getModel();
    	}
        this.row = row;  
	...
	*/
	    ...
	    public void itemStateChanged(ItemEvent e) { 
			...
			if(obj!=null && obj.equals(renderButton)) {
				if(renderButton.isSelected()) {
					...
					Object object = dtm.getValueAt(row, ONE);
					if(object == null){
						return;
					}else{
						waybillId = object.toString();
					}
					if(StringUtils.isNotEmpty(waybillId)){
						if(waybillNoList!=null && !waybillNoList.contains(waybillId)){
							waybillNoList.add(waybillId);
						}
					}
					...
					if(StringUtils.isNotEmpty(waybillId)){
						if(waybillNoList!=null && waybillNoList.contains(waybillId)){
							waybillNoList.remove(waybillId);
						}
					}
				}             
			}           
	 ...
	 */
	public void setCheckBoxPartSelected(CUBCWaybillManagerUI ui,JCheckBox checkBox){
		//获得对象集合
	 ...
	 */
	public void setCheckBoxAllSelected(CUBCWaybillManagerUI ui){

...
public class SalesDepartmentDialog extends JDialog {
	...
	public SalesDepartmentDialog(String deptName,List<SaleDepartmentEntity> saleDepartments) {
		initTable();
		init();
	...
	public SalesDepartmentDialog(String deptName) {
		initTable();
		init();
	}
	...
	public SalesDepartmentDialog(int n, String waybillNo) {
		if(n==1 && StringUtils.isNotBlank(waybillNo)){//图片开单
	 ...
	 * Create the dialog.
	 */
	public SalesDepartmentDialog(boolean needBillGroup) {
	...
	private void  queryDept(String deptName,String waybillNo)
	{
		//获取当前用户
		UserEntity user = (UserEntity) SessionContext.getCurrentUser();
		//当前用户所在部门
		OrgAdministrativeInfoEntity dept = user.getEmployee().getDepartment();
		setData(waybillService.querySaleDepartmentByNameForCentralizedExp(deptName,dept.getCode(),waybillNo));

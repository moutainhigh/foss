 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/creating/client/action/WaybillTempSaveAction.java
 * 
 * FILE NAME        	: WaybillTempSaveAction.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 * @date 2012-10-16 下午02:46:47 * * 录入货物信息SUC业务规则 1.
 ...
 * 
 */
public class WaybillTempSaveAction extends
		AbstractButtonActionListener<WaybillEditUI> {
	// 日志
	public static final Logger LOGGER = LoggerFactory
			.getLogger(WaybillTempSaveAction.class);
	/**
	 * 国际化
	 */
	private II18n i18n = I18nManager.getI18n(WaybillTempSaveAction.class);
	// WAY BILL SERVICE
	...
	public void actionPerformed(ActionEvent e) {
		//获得绑定的VO对象
		WaybillPanelVo bean = Common.getVoFromUI(waybillEditUI);
		...
		bool = validate(bean);
			...
			try {
				...
				Common.requestFocus(waybillEditUI);
				...
				if (bool) {
					if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
						...
						SubmitConfirmDialog panel = new SubmitConfirmDialog(bean, waybillEditUI);
						// 居中显示弹出窗口
	...
	private Boolean validate(WaybillPanelVo bean) {
		if (StringUtils.isEmpty(bean.getReceiveOrgCode())) {
			MsgBox.showInfo(i18n
					.get("foss.gui.creating.waybillTempSaveAction.msgBox.nullReceiveOrgCode"));
			return false;
		} else if (StringUtils.isEmpty(bean.getWaybillNo())) {

			MsgBox.showInfo(i18n
					.get("foss.gui.creating.waybillTempSaveAction.msgBox.nullWaybillNo"));
			return false;
		} else if (StringUtils.isEmpty(bean.getTargetOrgCode())) {

			MsgBox.showInfo(i18n
					.get("foss.gui.creating.waybillTempSaveAction.msgBox.nullTargetOrgCode"));
			return false;
		} else if (bean.getCustomerPickupOrgCode() == null) {
			MsgBox.showInfo(i18n
					.get("foss.gui.creating.waybillTempSaveAction.msgBox.nullCustomerPickupOrgCode"));
			return false;
		} else if (StringUtils.isEmpty(Common.getPackage(bean))) {
			MsgBox.showInfo(i18n
					.get("foss.gui.creating.waybillTempSaveAction.msgBox.nullPackage"));
			return false;
		} else if (bean.getGoodsQtyTotal() == null
				|| bean.getGoodsQtyTotal().intValue() == 0) {
			MsgBox.showInfo(i18n
					.get("foss.gui.creating.waybillTempSaveAction.msgBox.nullGoodsQtyTotal"));
			return false;
		}
		// 配载线路不能为空
		else if (StringUtil.isEmpty(bean.getLoadLineCode())
				|| StringUtil.isEmpty(bean.getLoadLineName())) {
			if (!bean.getIsWholeVehicle()) {
				MsgBox.showInfo(i18n
						.get("foss.gui.creating.waybillSubmitAction.exception.validateFreightRoute.one"));
				return false;
			}
		} else if (bean.getGoodsWeightTotal() == null
				|| bean.getGoodsWeightTotal().intValue() == 0) {
			MsgBox.showInfo(i18n
					.get("foss.gui.creating.cargoInfoPanel.weight.label.isNotNull"));
			return false;
		}

		// 储运事项长度不能超过100字符
		if (!StringUtil.isEmpty(bean.getTransportationRemark())
				&& bean.getTransportationRemark().length() > NumberConstants.NUMBER_100) {
			MsgBox.showInfo(i18n
					.get("foss.gui.creating.cargoInfoPanel.transportationRemark.length.notOver100"));
			return false;
		}

		if (!StringUtils.isEmpty(bean.getGoodsPackage())
				&& getChineseLength(bean.getGoodsPackage()) > NumberConstants.NUMBER_50) {
			MsgBox.showInfo(i18n
					.get("foss.gui.creating.cargoInfoPanel.txtOther.length.notOver50"));
			return false;
		}
		if (waybillService.isExsits(bean.getWaybillNo())) {
			MsgBox.showInfo(i18n
					.get("foss.gui.creating.waybillTempSaveAction.msgBox.waybillNoExist"));
			return false;
		}
		...
		if (new Long(bean.getInsuranceFee().multiply(new BigDecimal(NumberConstants.NUMBER_100))
			...
			MsgBox.showInfo(i18n
					.get("foss.gui.creating.waybillTempSaveAction.msgBox.overMaxInsuranceFee"));
			return false;
		}
		// 体积
		if (bean.getGoodsVolumeTotal() != null
				&& new Long(bean.getGoodsVolumeTotal()
						.multiply(new BigDecimal(NumberConstants.NUMBER_1000)).longValue()).toString()
						.length() > NumberConstants.NUMBER_8) {
			MsgBox.showInfo(i18n
					.get("foss.gui.creating.waybillTempSaveAction.msgBox.overMaxGoodsVolumn"));
			return false;
		}
		// 开单金额
		if (new Long(bean.getTotalFee().multiply(new BigDecimal(NumberConstants.NUMBER_100))
			...
			MsgBox.showInfo(i18n
					.get("foss.gui.creating.waybillTempSaveAction.msgBox.overMaxTotalFee"));
			return false;
		}
	 ...
	 * @date 2013-11-8 17:49:08
	 * @param goodsPackage
	 * @return
	 * @throws Exception
	 */
	private int getChineseLength(String goodsPackage) {
		String a = null;
		try {
			a = new String(goodsPackage.getBytes("UTF-8"), "ISO-8859-1");
	...
	private void validateCustomerPrepayCredit(WaybillPanelVo bean) {
		if (bean.getPaidMethod() != null
				&& (WaybillConstants.TEMPORARY_DEBT.equals(bean.getPaidMethod()
						.getValueCode()) || WaybillConstants.MONTH_PAYMENT
						.equals(bean.getPaidMethod().getValueCode()))) {
			BigDecimal prepayAmountBigDecimal = bean.getPrePayAmount();
			String deliveryCustomerCode = bean.getDeliveryCustomerCode();
			if (prepayAmountBigDecimal == null
					|| StringUtils.isEmpty(deliveryCustomerCode)) {
				return;
			}
			...
			conditions.add(dto);
			List<CustomerQueryConditionDto> resultList = waybillService
					.queryCustomerByConditionList(conditions);
			...
			if (customerPrepayCredit == null) {
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.waybillTempSaveAction.exception.nullCustomerPrepayCredit"));
			}

			if (customerPrepayCredit.doubleValue() < prepayAmountBigDecimal
					.doubleValue()) {
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.waybillTempSaveAction.exception.lackCustomerPrepayCredit.one")
								+ customerPrepayCredit.doubleValue()
								+ i18n.get("foss.gui.creating.waybillTempSaveAction.exception.lackCustomerPrepayCredit.two"));
			}
	...
	private void validateDriverNo(WaybillPanelVo bean) {
		if (bean.getPickupToDoor() != null && bean.getPickupToDoor()
			...
			throw new WaybillValidateException(
					i18n.get("foss.gui.creating.waybillTempSaveAction.exception.nullDriverCode"));
		}
	}
	...
	private void validatePhone(WaybillPanelVo bean) {
		if (StringUtils.isEmpty(bean.getReceiveCustomerMobilephone())
			...
			throw new WaybillValidateException(
					i18n.get("foss.gui.creating.waybillTempSaveAction.exception.nullMobilephoneOrTel"));
		}
		...
		if (WaybillConstants.AIRPORT_PICKUP.equals(bean.getReceiveMethod()
			...
			if (bean.getPaidMethod() != null) {
				if (WaybillConstants.ARRIVE_PAYMENT.equals(bean.getPaidMethod()
						.getValueCode())) {
					throw new WaybillValidateException(
							i18n.get("foss.gui.creating.waybillTempSaveAction.exception.validateAirException.one"));
				}
			}

			if (bean.getToPayAmount().compareTo(BigDecimal.ZERO) > 0) {
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.waybillTempSaveAction.exception.validateAirException.two"));
			}
		}
	}
	...
	private void validateProduct(WaybillPanelVo bean) {
		if (ProductEntityConstants.PRICING_PRODUCT_PARTIAL_LINE.equals(bean
			...
			if (insuranceAmount == null) {
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.waybillTempSaveAction.exception.nullInsuranceAmount"));
			}
			if (maxInsuranceAmount == null) {
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.waybillTempSaveAction.exception.nullMaxInsuranceAmount"));
			}
			if (insuranceAmount.compareTo(maxInsuranceAmount) > 0) {
				bean.setInsuranceAmount(BigDecimal.ZERO);
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.waybillTempSaveAction.exception.overMaxInsuranceAmount")
								+ maxInsuranceAmount);
			}
		}
	}
	...
	private void validateSecretPrepaid(WaybillPanelVo bean) {
		...
		if (bool) {
			if (bean.getPrePayAmount().longValue() == 0) {
				throw new WaybillValidateException(
						i18n.get("foss.gui.creating.waybillTempSaveAction.exception.zeroPrePayAmount"));
			}
		}
	}
	 ...
	 */
	private void tempSaveWaybill(WaybillPanelVo bean) {
		...
		WaybillPendingDto waybill = tempSave.getWaybillDto(bean, waybillEditUI);
	...
	@Override
	public void setIInjectUI(WaybillEditUI waybillEditUI) {

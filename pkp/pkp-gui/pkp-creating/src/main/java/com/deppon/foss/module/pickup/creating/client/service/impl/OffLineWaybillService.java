 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/creating/client/service/impl/OffLineWaybillService.java
 * 
 * FILE NAME        	: OffLineWaybillService.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 * 
 */
public class OffLineWaybillService extends AbstractWaybillService { 

	// 日志
	private static Log log = LogFactory.getLog(OffLineWaybillService.class);
	...
	// 离线提交
	private IWaybillOffLineSubmitService waybillOffLineSubmitService;
	 ...
	 * 国际化对象
	 */
	private II18n i18n = I18nManager.getI18n(OffLineWaybillService.class);
	 ...
	 * 
	 * @author 025000-FOSS-helong
	 * @date 2012-12-20 下午04:58:50
	 */
	public OffLineWaybillService() {
		Injector injector = GuiceContextFactroy.getInjector();

		billCaculateService = injector.getInstance(BillCaculateService.class);

		waybillOfflineDao = injector.getInstance(WaybillOfflineDao.class);

		waybillPendingDao = injector.getInstance(WaybillPendingDao.class);
		
		waybillOffLineSubmitService = injector
				.getInstance(WaybillOffLineSubmitService.class);

		waybillOffLinePendingService = injector
				.getInstance(WaybillOffLinePendingService.class);

		dictionaryValueSerivce = injector.getInstance(DataDictionaryValueService.class);
		
		saleDepartmentService = injector.getInstance(SaleDepartmentService.class);
		
		lineService = injector.getInstance(LineService.class);
		
		expresslineService = injector.getInstance(ExpressLineService.class);
		
		waybillFreightRouteService = injector.getInstance(WaybillFreightRouteService.class);
		
		outfieldDao = injector.getInstance(OutfieldDao.class);
		
		outfieldService = injector.getInstance(OutfieldService.class);
		
		orgInfoService = injector.getInstance(OrgInfoService.class);
		
		goodsAreaService = injector.getInstance(GoodsAreaService.class);
		
		guiBillCaculateService=injector.getInstance(GuiBillCaculateService.class);
		
		billingGroupTransFerService = injector.getInstance(BillingGroupTransFerService.class);
		
		baseDataService = injector.getInstance(BaseDataService.class);
		
		localRuleVerifyService = injector.getInstance(LocalRuleVerifyService.class);
		goodsTypeService = injector.getInstance(com.deppon.foss.module.pickup.pricing.server.service.impl.GoodsTypeService.class);
		
		salesDepartmentService = injector.getInstance(SalesDepartmentService.class);
		
		productDao = injector.getInstance(ProductDao.class);
		
		waybillCHDtlPendingDao = injector.getInstance(WaybillCHDtlPendingDao.class);
		
		priceEntryService = injector.getInstance(PriceEntryService.class);
	...
	public boolean checkWaybillNo(String waybillNo) {
		// 查询本地
		return waybillOffLinePendingService.isWayBillExsits(waybillNo);
	}
	...
	public List<DataDictionaryValueEntity> queryPaymentMode() {
		List<DataDictionaryValueEntity> data = new ArrayList<DataDictionaryValueEntity>();

		List<DataDictionaryValueEntity> dataDictionaryValue = this.dictionaryValueSerivce
				.queryPaymentMode();
		for (DataDictionaryValueEntity entity : dataDictionaryValue) {
			if (WaybillConstants.CASH_PAYMENT.equals(entity.getValueCode())
					|| WaybillConstants.ARRIVE_PAYMENT.equals(entity
							.getValueCode())) {
				data.add(entity);
			}
		}
	...
	public void submitWaybill(WaybillDto waybill) {
		...
		waybillOffLineSubmitService.submitWaybill(waybill);
	}
	...
	public void tempSaveWaybill(WaybillPendingDto waybillDto) {
		...
		waybillOffLinePendingService.tempSave(waybillDto);
	}
	 ...
	 * 
	 * @author 025000-FOSS-helong
	 * @date 2012-12-14 下午02:04:55
	 * @see com.deppon.foss.module.pickup.creating.client.service.IWaybillService#queryProductPriceList(com.deppon.foss.module.pickup.pricing.api.shared.dto.QueryBillCacilateDto)
	 */
	 ...
	 * 
	 * @author 025000-FOSS-helong
	 * @date 2012-12-14 下午02:05:04
	 * @see com.deppon.foss.module.pickup.creating.client.service.IWaybillService#queryValueAddPriceList(com.deppon.foss.module.pickup.pricing.api.shared.dto.QueryBillCacilateValueAddDto)
	 */
	...
	public Date searchPreLeaveTime(String departDeptCode, String specialLine,
		...
		// 准备时间
		int hour = NumberConstants.NUMBER_3;
		// 获得部门信息
		SaleDepartmentEntity department = saleDepartmentService
				.querySaleDepartmentByCode(departDeptCode);
		if (department == null) {
			return null;
		}
		// 可出发的时间
		...
		if (CommonUtils.directDetermineIsExpressByProductCode(productCode)) {//快递或3.60特惠件
			departList = expresslineService
					.queryDepartureStandardListBySourceTarget(departDeptCode,
							specialLine, productCode);
		} else {
			departList = lineService
					.queryDepartureStandardListBySourceTarget(departDeptCode,
						specialLine, productCode);
		}
		...
		for (int i = 0; i < departList.size(); i++) {
			...
			if (leaveTimeString.compareTo(standardTime) <= 0) {
				leaveTime = mergeTime(leaveTime, standardTime);
				return leaveTime;
			}
		}
		...
		leaveTime = mergeTime(calendar.getTime(), departList.get(0)
				.getLeaveTime());
		return leaveTime;
	}
	...
	public EffectiveDto searchPreSelfPickupTime(String departDeptCode,
		...
		List<EffectivePlanDto> list = billCaculateService.searchEffectivePlanDetailList(departDeptCode, arriveDetpCode, productCode, createTime);
		...
		// 判断是否是当天出发,若是则在预计出发时间的基础上加承诺天数，否则加承诺天数-1
		...
		createTime = mergeTime(calendar.getTime(), StringUtil.defaultIfNull(effective.getArriveTime()));
	...
	public EffectiveDto searchPreDeliveryTime(String departDeptCode,
		...
		// 计算承诺派送时间
		Date deliveryDate = mergeTime(calendar.getTime(), deliveryTime);
	...
	public SaleDepartmentEntity queryPickupCentralizedDeptCode(String billingGroupOrgCode){
    	...
    	BillingGroupTransFerEntity transCenterEntity = billingGroupTransFerService.queryTransFerListByBillingGroupCode(billingGroupOrgCode);
    	if(transCenterEntity!=null){
    		String salesDeptCode = saleDepartmentService.queryLeaveStationSaleCodeByTransferCenterCode(transCenterEntity.getTransFerCode()==null?"":transCenterEntity.getTransFerCode());
	...
	public WaybillPendingDto queryPendingWaybill(String waybillNo) {
		return waybillOffLinePendingService.queryPendingWaybillByNo(waybillNo);
	}
	...
	public OrgInfoDto queryLodeDepartmentInfo(boolean pickupCentralized,
			...
			String productCode) {
		return waybillFreightRouteService.queryLodeDepartmentInfoLocal(
				pickupCentralized, orginalOrganizationCode,
				destinationOrganizationCode, productCode);
	}
	
	//根据货物总件数生成流水号。
	private String getPrintSerialnos(int totalGoodsQty){
		StringBuffer serialnos = new StringBuffer();
		String resultSerialnos = null;
		if (totalGoodsQty >0) {
			for(int i = 1; i<=totalGoodsQty; i++){
				serialnos.append(String.format(FORMATSTR,i)).append(COMMA);
			}
			resultSerialnos = serialnos.substring(0, serialnos.length()-1);
		}
		return resultSerialnos;
	}
	
	
	private String getOutfieldCode(String pOrgCode) {
    	OutfieldEntity outfieldEntity  = null;
    	//判断 是否为空运总调
		OrgAdministrativeInfoEntity orgEntity = orgInfoService.queryByCode(pOrgCode);
		if (orgEntity != null) {
			if (FossConstants.YES.equals(orgEntity.getAirDispatch())) {
				...
				//偏线、专线查外场
				outfieldEntity = outfieldDao.queryOutfieldByOrgCode(pOrgCode);
			}
		}
	...
	/*//根据部门编码获取外场编码
	private String getOutfieldCode(String orgCode){
		OutfieldEntity outfieldEntity = outfieldDao.queryOutfieldByOrgCode(orgCode);
		if (outfieldEntity != null) {
			return outfieldEntity.getCode();
		}else {
			return null;
		}
	}*/
	...
	public BarcodePrintLabelDto getLabelPrintInfos(String waybillNo,String waybillstatus) {
		...
		// 离线打印标签
		// 离线查询方式
		WaybillPendingEntity waybillBean = this.queryByWaybillNo(waybillNo);
		if (waybillBean == null) {
			return null;
		}
		// Test
		printLabelBean.setDeliverToDoor(FossConstants.YES.equals(waybillBean.getPickupToDoor()) ? "送" : "自");
		printLabelBean.setWaybillNumber(waybillBean.getWaybillNo());
		
		// 目的站从code转成名字 
		OrgAdministrativeInfoEntity orgAdministrativeInfoEntity = queryByCode(waybillBean.getCustomerPickupOrgCode());
		if (orgAdministrativeInfoEntity != null) {
			//yhj at 2013-05-15 edit 目的站改为简称。统一和在线显示一致
			printLabelBean.setDestination(orgAdministrativeInfoEntity.getOrgSimpleName()); // 目的站(简称) 
		}
		if (ProductEntityConstants.PRICING_PRODUCT_PARTIAL_LINE.equals(waybillBean.getProductCode())
			...
			OuterBranchEntity outerBranchEntity = this.queryAgencyBranchInfo(waybillBean.getCustomerPickupOrgCode());
			if (outerBranchEntity != null) {
				printLabelBean.setDestinationCode(outerBranchEntity.getStationNumber()); // 目的站打印标签编码
			}
		}else{
			SaleDepartmentEntity saleDepartmentEntity = this.querySaleDeptByCode(waybillBean.getCustomerPickupOrgCode());
		...
		ProductEntity productEntity = this.getProductByCache(
				waybillBean.getProductCode(), waybillBean.getBillTime());
		if (productEntity != null && productEntity.getName() != null) {
			...
			printLabelBean.setTranstype(null);
		}

		// 出发城市 始发站  通过  waybillBean.getReceiveOrgCode()收货部门查询城市
		// yhj at 2013-05-15 根据收货市 查询行政区域且带出对应的市区名称
		
		OrgAdministrativeInfoEntity tempOrgAdministrativeInfoEntity = queryByCode(waybillBean.getReceiveOrgCode());
		AdministrativeRegionsEntity receiveRegionsEntity = 	queryAdministrativeRegionsByCode(tempOrgAdministrativeInfoEntity.getCityCode());
		if (receiveRegionsEntity != null ) {
			printLabelBean.setLeavecity(receiveRegionsEntity.getName());
		}
		//printLabelBean.setLeavecity(this.queryAdministrativeRegionsByCode(waybillBean.getReceiveCustomerCityCode()).getName());
		// 重量
		...
		// 总件数
		printLabelBean.setTotalPieces(String.valueOf(waybillBean
				.getGoodsQtyTotal()));

		// 是否异常货物
		printLabelBean.setUnusual(FossConstants.YES.equals(waybillBean.getSpecialShapedGoods() ) ? "异": "");
		// 航班早班 没有写 
		printLabelBean.setPreassembly("");
		//收货人
		printLabelBean.setReceiveCustomerContact(waybillBean.getReceiveCustomerContact());
		
		//设置流水号
		printLabelBean.setPrintSerialnos(getPrintSerialnos(waybillBean.getGoodsQtyTotal()));
		
		UserEntity user = (UserEntity) SessionContext.getCurrentUser();
		if(user!= null && user.getEmployee()!=null){
			//打印人工号
			printLabelBean.setOptuserNum(user.getEmployee().getEmpCode());
		}
		//打印日期
		...
		// 收货部门 ==出发营业部
		String startOrg = waybillBean.getReceiveOrgCode();
		// 根据出发到达 营业部得到 走货路径
		//  调用本地走货路径
		OrgInfoDto orgInfoDto = waybillFreightRouteService.queryLodeDepartmentInfoLocal(true, 
				startOrg, lastLoadOrgCode, waybillBean.getProductCode());
		//货区
		ArrayList<String> tempGoodsAreaNo = new ArrayList<String>();
		String[] routeLines = new String[]{};
		if (orgInfoDto != null) {
			
			//最终外场编码
			OutfieldEntity entity = outfieldDao.queryOutfieldByOrgCode(orgInfoDto.getLastOutLoadOrgCode());
			if (entity != null) {
				printLabelBean.setLastTransCenterNo(entity.getCode());
				// 最终外场ID
				printLabelBean.setFinaloutfieldid(entity.getId());
			}
			
			// 最终外场城市名称 Finaloutname
			/**
			AdministrativeRegionsEntity administrativeRegionsEntity = queryAdministrativeRegionsByCode(orgInfoDto.getLastOutLoadOrgCode());
			if (administrativeRegionsEntity != null) {
				printLabelBean.setLastTransCenterCity(administrativeRegionsEntity.getSimpleName());
			}else {
				printLabelBean.setLastTransCenterCity("空");
			}**/
			
			// 最终外场简称
			OrgAdministrativeInfoEntity lastOutLoadOrg = this.orgInfoService.queryByCode(orgInfoDto.getLastOutLoadOrgCode());
			if (lastOutLoadOrg != null) {
				printLabelBean.setLastTransCenterCity(lastOutLoadOrg.getOrgSimpleName());
			}else {
				printLabelBean.setLastTransCenterCity("空");
			}
			
			if (StringUtils.isNotEmpty(orgInfoDto.getRouteLineCode())) {
				//去掉始发营业部 取走货路径 对应货区
				routeLines = orgInfoDto.getRouteLineCode().split("-");
			}
		}
		if (routeLines.length>=2) {
			String goodsAreaNo = null;
			//去掉始发营业部 获取货区 List
			if (ProductEntityConstants.PRICING_PRODUCT_PARTIAL_LINE.equals(waybillBean.getProductCode())
	    			|| ProductEntityConstants.PRICING_PRODUCT_AIR_FREIGHT.equals(waybillBean.getProductCode())) {
				for(int i=1;i<routeLines.length-1;i++){
					//偏线、空运 货区
					if(i == routeLines.length-2){
	    				...
	    				if (ProductEntityConstants.PRICING_PRODUCT_AIR_FREIGHT.equals(waybillBean.getProductCode())) {
	    					goodsAreaEntities = goodsAreaService.queryGoodsAreaListByType(routeLines[i], DictionaryValueConstants.BSE_GOODSAREA_TYPE_AIRFREIGHT);
						}else {
							//偏线货区
							goodsAreaEntities = goodsAreaService.queryGoodsAreaListByType(routeLines[i], DictionaryValueConstants.BSE_GOODSAREA_TYPE_OTHER);
						}
						
						if (CollectionUtils.isNotEmpty(goodsAreaEntities)) {
							goodsAreaNo = goodsAreaEntities.get(0).getGoodsAreaCode();
						}
					}else{
						 goodsAreaNo = DownLoadDataServiceFactory.getGoodsAreaService().queryCodeByArriveRegionCode(
									routeLines[i],routeLines[i+1]);
					}
					tempGoodsAreaNo.add(goodsAreaNo);
				}
			}else {
				for(int i=1;i<routeLines.length-1;i++){
					 goodsAreaNo = DownLoadDataServiceFactory.getGoodsAreaService().queryCodeByArriveRegionCode(
			...
			// 去掉始发营业部、最后营业部  取最后四个外场
			List<String> newOrgCode = new ArrayList<String>(NumberConstants.NUMBER_4);
			List<String> sortOrgCode = new ArrayList<String>(NumberConstants.NUMBER_4);
			int x = 0;
			if (routeLines.length >=2) {
				for (int i = routeLines.length-2; (i>=1 && x< NumberConstants.NUMBER_4); i--) {
    				newOrgCode.add(routeLines[i]);
    				x++;
				}
				//倒序
			...
			case NumberConstants.NUMBER_4:
				printLabelBean.setAddr4(getOutfieldCode(sortOrgCode.get(NumberConstants.NUMBER_3)));
			case NumberConstants.NUMBER_3:
				printLabelBean.setAddr3(getOutfieldCode(sortOrgCode.get(2)));
			case 2:
				printLabelBean.setAddr2(getOutfieldCode(sortOrgCode.get(1)));
			case 1:
				printLabelBean.setAddr1(getOutfieldCode(sortOrgCode.get(0)));
			default:{
				// printLabelBean.setAddr1("");
				}
			}
		}
		//货区编号 取最后四位
    	if (tempGoodsAreaNo.size()>0) {
    		List<String> goodsAreaCode = new ArrayList<String>(NumberConstants.NUMBER_4);
    		int y = 0;
			for (int n = 0; (n<tempGoodsAreaNo.size() && y<NumberConstants.NUMBER_4); n++) {
				goodsAreaCode.add(tempGoodsAreaNo.get(n));
				y++;
			}
			//Collections.sort(goodsAreaCode);
			
			switch (goodsAreaCode.size()){
			case NumberConstants.NUMBER_4:
				printLabelBean.setLocation4(goodsAreaCode.get(NumberConstants.NUMBER_3));
			case NumberConstants.NUMBER_3:
	 ...
	 * 
	 * @author foss-jiangfei
	 * @date 2012-11-5 上午11:18:28
	 * @param waybillNo
	 * @return
	 * @see com.deppon.foss.module.pickup.creating.client.service.IWaybillService#queryByWaybillNo(java.lang.String)
	 */
	public WaybillPendingEntity queryByWaybillNo(String waybillNo) {
		return waybillPendingDao.queryPendingByNo(waybillNo);
	}
	...
	public int updateWaybillPrintTimes(String waybillNo) {
		...
		return waybillOfflineDao.updateWaybillPrintTimes(waybillNo);
	}
	...
	public boolean isOfflineExsits(String mixNo) {
		WaybillOfflineEntity offineEntity = waybillOfflineDao
				.queryOfflineByNo(StringUtil.defaultIfNull(mixNo));
		if (offineEntity == null) {
			return false;
		} else {
			return true;
		}
...
//			dataInsert(jctx, resourceBean);			
			...
			// 离线 开单
			updateWaybillPrintTimes(waybillNo);
		} catch (Exception exp) {
			log.error("开单打印异常", exp);
	 ...
	 * 
	 * @author 025000-FOSS-helong
	 * @date 2013-1-4 上午09:59:21
	 * @param couponInfo
	 * @return
	 */
	@Override
	public CouponInfoResultDto validateCoupon(CouponInfoDto couponInfo) {
		throw new WaybillValidateException(i18n.get("foss.gui.creating.offLineWaybillService.exception.nullOfflineCoupon"));
	}
	 ...
	 */
	@Override
	public List<CustomerQueryConditionDto> queryHisDeliveryCusByMobileList(
	 ...
	 */
	@Override
	public List<CustomerQueryConditionDto> queryHisDeliveryCusByPhoneList(
	 ...
	 */
	@Override
	public List<CustomerQueryConditionDto> queryHisReceiveCusByMobileList(
			List<String> mobileList) {
		return null;
	}
	 ...
	 */
	@Override
	public List<CustomerQueryConditionDto> queryHisReceiveCusByPhoneList(
			List<String> phoneList) {
		return null;
	}
	 ...
	 * 
	 * @see
	 * com.deppon.foss.module.pickup.creating.client.service.IWaybillService
	 * #checkWaybillNoInterval(java.lang.String)
	 */
	@Override
	public boolean checkWaybillNoInterval(String waybillNo) {
		return false;
	}
	 ...
	 * @see
	 * com.deppon.foss.module.pickup.creating.client.service.IWaybillService
	 * #isExsitsByWaybillAndOrderNo(java.lang.String)
	 */
	@Override
	 ...
	 */
	public Date mergeTime(Date date, String time) {
		if (date == null || time == null) {
			return null;
		}
		if (time.length() != FOUR) {
			throw new IllegalArgumentException(i18n.get("foss.gui.creating.offLineWaybillService.exception.illegalTimeFormat"));
		}
		if (!StringUtil.isNumeric(time)) {
			throw new IllegalArgumentException(i18n.get("foss.gui.creating.offLineWaybillService.exception.illegalTimeCharacter"));
		}

		Calendar oldCal = Calendar.getInstance();
		oldCal.setTime(date);
		oldCal.set(Calendar.HOUR_OF_DAY, Integer.valueOf(time.substring(0, 2)));
		oldCal.set(Calendar.MINUTE, Integer.valueOf(time.substring(2, NumberConstants.NUMBER_4)));
		// 设置秒为0
	...
	public ServiceFeeRateDto queryConfigurationParamsByOrgCode(String orgCode,String config) {
		...
		//根据部门编号获取对应的装卸费率
		String value = property.readData(orgCode);
		if(StringUtils.isEmpty(value))
		{
			dto.setMessage(i18n.get("foss.gui.creating.offLineWaybillService.exception.serviceFee"));
		}else
		{
			dto.setServiceFeeRate(new BigDecimal(value));
		}
		return dto;
	}
	...
	public WaybillPendingEntity queryPendingByNo(String waybillNo){
		return waybillOffLinePendingService.queryPendingByNo(waybillNo);
	}
	 ...
	 * 业务监控
	 * @author foss-sunrui
	 * @date 2013-4-15 下午3:30:04
	 * @param waybillDto 
	 * @see com.deppon.foss.module.pickup.creating.client.service.IWaybillService#businessMonitor(com.deppon.foss.module.pickup.waybill.shared.dto.WaybillDto)
	 */
	@Override
	public void businessMonitor(WaybillDto waybillDto) {
		// TODO Auto-generated method stub
		log.info("进入businessMonitor");
	}
	...
	public OuterBranchEntity queryAgencyBranchInfo(String agencyBranchCode){
		return baseDataService.queryAgencyBranchInfo(agencyBranchCode);
	}
	...
	/**
	 * @是否存在运单更改情况，默认离线暂存不存在更改情况
	 * @author 105888-foss-zhangxingwang
	 * @date 2013-7-9 14:36:18
	 */
	@Override
	public String isExistDestinationAndTodoRfc(WaybillEntity waybillBean,
	...
	public boolean isProhibitGoods(String goodsName) {
		return this.baseDataService.isProhibitGoods(goodsName);
	}
	 ...
	 * 根据件数、体积、保价判断是否贵重物品
	 * @author WangQianJin
	 * @date 2013-7-24 上午11:03:22
	 */
	@Override
	public boolean isValueGoods(String goodsName,int goodsNum, String goodsVolume,
			String goodsValue) {
		boolean bool=false;
		...
		boolean isValueGoods=localRuleVerifyService
	 ...
	 * 是否贵重物品
	 * @author WangQianJin
	 * @date 2013-7-24 上午11:03:58
	 */
	@Override
	public boolean isValuablesGoods(String goodsName) {
		return this.baseDataService.isValuablesGoods(goodsName);
	}
	...
	{
		return salesDescExpandsService.queryByCodeAndPickup(code);
	}
	...
	{
		return salesDescExpandsService.queryByCodeAndDelivery(code);
	}
	...
	public List<BranchQueryVo> queryListByDepartment(SaleDepartmentEntity entity){
		return WaybillServiceFactory.getSalesDepartmentService().queryListByDepartment(entity);
	}
	...
	public List<BranchQueryVo> queryListByBranch(OuterBranchEntity entity){
		return WaybillServiceFactory.getOuterBranchService().queryListByBranch(entity);
	}
	...
	public List<DataDictionaryValueEntity> queryPickUpGoodsAir() {
		return this.dictionaryValueSerivce.queryPickUpGoodsAir();
	}
	...
	public List<DataDictionaryValueEntity> queryPickUpGoodsHighWays() {
		return this.dictionaryValueSerivce.queryPickUpGoodsHighWays();
	}
	...
	public List<DataDictionaryValueEntity> queryAirGoodsType() {
		return this.dictionaryValueSerivce.queryAirGoodsType();
	}
	...
	public List<SalesBillingGroupEntity> querySalesBillGroupByCodeAndBillCode(String code,String billingGroup){
		return salesDepartmentService.querySalesBillGroupByCodeAndBillCode(code,billingGroup);
	}
	...
	public List<SaleDepartmentEntity> querySaleDepartmentByNameForCentralized(String name,String billingGroup) {
		return salesDepartmentService.querySaleDepartmentByNameForCentralized(name,billingGroup);
	}
	...
	public List<SaleDepartmentEntity> querySaleDepartmentByNameForCentralizedExp(String name,String billingGroup,String waybillNo) {
		return salesDepartmentService.querySaleDepartmentByNameForCentralizedExp(name,billingGroup,waybillNo);
	}
	 ...
	 */
	@Override
	public OutfieldEntity queryDefaultTransCodeDept(String saleCode, String productCode) {
		return waybillFreightRouteService.queryDefaultTransDept(saleCode, productCode);
	}
	...
	/* (non-Javadoc)
	 * @see com.deppon.foss.module.pickup.creating.client.service.IWaybillService#queryGuiExpressBillPrice(com.deppon.foss.module.pickup.pricing.api.shared.dto.GuiQueryBillCalculateDto)
	 */
	@Override
	public List<GuiResultBillCalculateDto> queryGuiExpressBillPrice(
			GuiQueryBillCalculateDto billCalculateDto) {
		
		return guiBillCaculateService.queryGuiExpressBillPrice(billCalculateDto);
	}
	@Override
...
//	}
	...
	public boolean isVehicleDirect(String weight, String volume) {
		return localRuleVerifyService.isVehicleDirect(weight, volume);
	}
	...
	@Override
	public WaybillPictureEntity getPictureWaybill(List<String> waybillNos) {
		return null;
	}
	...
	@Override
	public void pushMessage(int pushType, int messageType, int deviceType,
			String deviceId, String title, String description, String value) {
	...
	public boolean submitReturnGoodsApplyToCRM(
			String wayBillNO,String content,WaybillEntity entity) {
		return false;
	}
	...
	@Override
	public List<CusLtDiscountItemDto> iSLtDiscountBackInfo(String customerCode, Date date) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	@Override
	public SynPartenerMonthlyLineDeductResponse partenerMonthlyLineDeduct(
			PartenerDeductDto deductDto) {
		// TODO Auto-generated method stub
		return null;
	}
	
	@Override
	public SynPartenerPrestoreDeductResponse partenerPrestoreDeduct(
	...
	public void commitLTLBatchChangeWeight(
			List<LTLEWaybillChangeWeightDto> expWaybillChangeWeightDto) {
		throw new BusinessException("离线状态不允许导入");
	}

	@Override
	public List<LTLEWaybillChangeWeightDto> queryLTLEWaybillChangeWeightResult(
			LTLEWaybillChangeWeightQueryDto dto) {
		return null;
	}
	...
	public void pushLabelStatus(List<String> waybillNoList){
		throw new BusinessException("离线状态不允许推送标签");
	}
	...
	@Override
	public CustomerCircleNewDto queryCustomerByCusCode(String provinceCode,
			Date date, String active) {
		// TODO Auto-generated method stub
		return null;
	}

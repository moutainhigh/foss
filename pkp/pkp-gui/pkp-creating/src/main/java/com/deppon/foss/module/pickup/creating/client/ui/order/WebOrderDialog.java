 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/creating/client/ui/order/WebOrderDialog.java
 * 
 * FILE NAME        	: WebOrderDialog.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
...
import com.deppon.foss.module.pickup.creating.client.action.WebOrderQueryAction;
import com.deppon.foss.module.pickup.creating.client.action.WebOrderQueryResetAction;
import com.deppon.foss.module.pickup.creating.client.common.Common;
import com.deppon.foss.module.pickup.creating.client.listener.WebOrderComponentListener;
 ...
 * @date 2012-11-13 上午9:39:46 * * 录入货物信息SUC业务规则 1.
 ...
 *       营业部开单时按打完木架后的包装开，即包装中含“木架/木箱”，开单件数为货物打木架/木箱前的实际件数 （防止丢货），尺寸和重量按照以下公式计算：
 *       1.1 整票货物代打时：开单体积=代打货物体积*1.4；开单重量=所有货物重量+代打货物体积*42； 1.2
 ...
 *       打木架要在“对内备注”中备注“代打木架/木箱”，特殊要求（特别是合打情况）必须在对内备注 和A4纸上都注明，例如：货物1、2、3合打成一件等；
 ...
 * 
 */
public class WebOrderDialog extends JFrame {
	 ...
	 * 国际化对象
	 */
	private II18n i18n = I18nManager.getI18n(WebOrderDialog.class);

	// 日志
	private static Log log = LogFactory.getLog(WebOrderDialog.class);
	...
	// @Bind("waybillNo")
	private JTextField txtWaybillNumber;
	...
	@ButtonAction(icon = "", shortcutKey = "", type = WebOrderQueryAction.class)
	private JButton btnQuery;

	@ButtonAction(icon = "", shortcutKey = "", type = WebOrderQueryResetAction.class)
	...
	private WebOrderComponentListener orderComponentListener;
	...
	// 组织信息 Service接口
//	private IOrgAdministrativeInfoService orgAdministrativeInfoService = GuiceContextFactroy
//			.getInjector().getInstance(OrgAdministrativeInfoService.class);
	...
	// 省市区Service
	IAddressServiceHessianRemoting districtService = DefaultRemoteServiceFactory
			.getService(IAddressServiceHessianRemoting.class);

	// 运单开单UI
	private WaybillEditUI ui;

	// dialog
	private WebOrderDialog dialog = this;

	// 登录用户所属部门信息
	private OrgAdministrativeInfoEntity dept;

	// 客户服务接口
	private ICustomerService customerService = GuiceContextFactroy
			.getInjector().getInstance(CustomerService.class);

	/**
	 * LOGGER
	 */
	private Logger logger = Logger.getLogger(WebOrderDialog.class);
	...
	private JLabel lblWaybillNumber;
	...
	public WebOrderDialog(WaybillEditUI ui) {
...
//				.getInstance(OrgAdministrativeInfoService.class);
		baseDataService = GuiceContextFactroy.getInjector().getInstance(
		...
		createListener();
		// 订单号Enter键监控
		EnterKeyForOrder enterOrder = new EnterKeyForOrder(btnQuery);
		txtOrderNo.addKeyListener(enterOrder);
		txtWaybillNumber.addKeyListener(enterOrder);// 运单也添加相应的Enter键的监控

	}
	
	public WebOrderDialog(WaybillEditUI ui,String picture) {
		//super(ApplicationContext.getApplication().getWorkbench().getFrame());
		dataDictionaryValueService = GuiceContextFactroy.getInjector()
				.getInstance(DataDictionaryValueService.class);
//		orgAdministrativeInfoService = GuiceContextFactroy.getInjector()
//				.getInstance(OrgAdministrativeInfoService.class);
	...
	private void init() {
		...
		JLabel labels1 = new JLabel(
				i18n.get("foss.gui.creating.webOrderDialog.datePickerStart.label")
						+ "：");
		getContentPane().add(labels1, "2, 2");

		datePickerStart = new JXDatePicker();

		/**
		 * BUG-29798 网单：AL00012004348879
		 * 是5月21日的单子，有时间限制不能导入，但是把本地时间调整到5月21可以导入开单，输入信息后，点击提交，确定提示找不到对应实体
		 */
		// 服务器时间
		Date serverTime = waybillService.gainServerTime();

		Date stratDate = serverTime;
		Date endDate = serverTime;
		...
		JLabel labele2 = new JLabel(
				i18n.get("foss.gui.creating.webOrderDialog.datePickerEnd.label")
						+ "：");
		getContentPane().add(labele2, "6, 2");
		...
		JLabel label = new JLabel(
				i18n.get("foss.gui.creating.webOrderDialog.orderType.label")
						+ "：");
		// getContentPane().add(label, "10, 2, right, default");
		getContentPane().add(label, "12, 2, right, default");
		...
		getContentPane().add(combOrderType, "14, 2, fill, default");

		// 订单编号
		JLabel label1 = new JLabel(
				i18n.get("foss.gui.creating.webOrderDialog.orderNo.label")
						+ "：");
		// getContentPane().add(label1, "14, 2, right, default");
		getContentPane().add(label1, "16, 2, right, default");

		txtOrderNo = new JTextField();
		// getContentPane().add(txtOrderNo, "16, 2, fill, default");
		getContentPane().add(txtOrderNo, "18, 2, fill, default");
		...
		JLabel label2 = new JLabel(
				i18n.get("foss.gui.creating.webOrderDialog.orderStatus.label")
						+ "：");
		// getContentPane().add(label2, "18, 2, right, default");
		// getContentPane().add(combOrderStatus, "20, 2, fill, default");
		getContentPane().add(label2, "20, 2, right, default");
		getContentPane().add(combOrderStatus, "22, 2, fill, default");

		lblWaybillNumber = new JLabel(
				i18n.get("foss.gui.creating.numberPanel.waybillNo.label") + "：");
		getContentPane().add(lblWaybillNumber, "2, 4, fill, default");
		txtWaybillNumber = new JTextField();
		getContentPane().add(txtWaybillNumber, "4, 4, fill, default");
		txtWaybillNumber.setColumns(TEN);
		// 发货客户
		JLabel label3 = new JLabel(
				i18n.get("foss.gui.creating.webOrderDialog.customerName.label")
						+ "：");
		// getContentPane().add(label3, "2, 4, right, default");
		getContentPane().add(label3, "6, 4, right, default");

		txtCustomerName = new JTextField();
		// getContentPane().add(txtCustomerName, "4, 4, fill, default");
		getContentPane().add(txtCustomerName, "8, 4, fill, default");
		txtCustomerName.setColumns(TEN);

		// 发货联系人
		JLabel label4 = new JLabel(
				i18n.get("foss.gui.creating.webOrderDialog.customerContact.label")
						+ "：");
		getContentPane().add(label4, "6, 4, right, default");
		getContentPane().add(label4, "10, 4, right, default");

		txtCustomerContact = new JTextField();
		// getContentPane().add(txtCustomerContact, "8, 4, fill, default");
		getContentPane().add(txtCustomerContact, "12, 4, fill, default");
		txtCustomerContact.setColumns(TEN);

		// 发货联系人手机
		JLabel label5 = new JLabel(
				i18n.get("foss.gui.creating.webOrderDialog.customerMobile.label")
						+ "：");
		// getContentPane().add(label5, "10, 4, right, default");
		getContentPane().add(label5, "14, 4, right, default");

		txtCustomerMobilephone = new JTextField();
		// getContentPane().add(txtCustomerMobilephone, "12, 4, fill, default");
		getContentPane().add(txtCustomerMobilephone, "16, 4, fill, default");
		txtCustomerMobilephone.setColumns(TEN);

		// 发货联系人电话
		JLabel label6 = new JLabel(
				i18n.get("foss.gui.creating.webOrderDialog.customerPhone.label")
						+ "：");
		// getContentPane().add(label6, "14, 4, right, default");
		getContentPane().add(label6, "18, 4, fill, default");

		txtCustomerPhone = new JTextField();
		// getContentPane().add(txtCustomerPhone, "16, 4, fill, default");
		getContentPane().add(txtCustomerPhone, "20, 4, fill, default");
		txtCustomerPhone.setColumns(TEN);

		JPanel panel = new JPanel();
		// getContentPane().add(panel, "18, 4, 3, 1, fill, fill");
		getContentPane().add(panel, "22, 4, fill, fill");
		...
		// getContentPane().add(scrollPane, "2, 6, 19, 1, fill, fill");
		...
		// getContentPane().add(navigator, "2, 8, 19, 1, fill, fill");
	...
	private void importOrder() {
		WaybillPanelVo panelVo = ui.getBindersMap().get("waybillBinder")
				.getBean();
		int row = table.getSelectedRow();
		if (row >= 0) {
			//存贮原发货客户编码
			String oldDeliveryCustomerCode = null;
			int modelRow = table.convertRowIndexToModel(row);
			CrmOrderInfo webOrder = tableModel.getData().get(modelRow);
			panelVo.setServerType(webOrder.getServiceType());
			// 若导入的订单状态非受理状态，则提示订单不能导入,就是说只有受理中，已退回，接货中的订单能导入
			if (!WaybillConstants.CRM_ORDER_ACCEPT.equals(webOrder
					.getOrderStatus())
					&& !WaybillConstants.CRM_ORDER_GOBACK.equals(webOrder
							.getOrderStatus())
					&& !WaybillConstants.CRM_ORDER_RECEIPTING.equals(webOrder
							.getOrderStatus())) {
				MsgBox.showInfo(i18n.get(
						"foss.gui.creating.webOrderDialog.msgbox.orderStatus",
						webOrder.getOrderStatusName()));
				return;
			}
			
			//零担电子运单不允许导入开单 -- sangwenhao-272311
			if(StringUtils.isNotBlank(webOrder.getOrderNumber())){
				OmsOrderEntity omsOrderEntity = waybillService.queryOmsOrderByOrderNo(webOrder.getOrderNumber());
				if(omsOrderEntity != null){
					MsgBox.showInfo(i18n.get("foss.gui.creating.omsOrderService.exception.isLtlEwaybill"));
					return;
				}
			}
			...
			if(StringUtils.isNotBlank(ui.getPictureWaybillType()) &&
				...
				entity.setOrderNo(webOrder.getOrderNumber());
				List<WaybillPictureEntity>  lists = waybillService.queryWaybillPicturesByEntity(entity);		
				if(lists != null && lists.size() > 0){
					for(WaybillPictureEntity waybillPictureEntiry : lists){
						if(!WaybillConstants.WAYBILL_PICTURE_TYPE_RETURN.equals(waybillPictureEntiry.getPendgingType()) &&
								!WaybillConstants.WAYBILL_PICTURE_TYPE_CANCEL.equals(waybillPictureEntiry.getPendgingType()) &&
								!waybillPictureEntiry.getWaybillNo().equals(panelVo.getWaybillNo())){
							MsgBox.showInfo(i18n.get("foss.gui.creating.webOrderDialog.msgbox.orderRepeat1"));
							return;
						}
					}
				}
				
				if(waybillService.checkWaybillAndPendingOrderNo(webOrder.getOrderNumber())){
					MsgBox.showInfo(i18n.get("foss.gui.creating.webOrderDialog.msgbox.orderRepeat1"));
					return;
				}
				
				if(StringUtils.isNotBlank(panelVo.getOrderNo()) && 
						StringUtils.isNotBlank(webOrder.getOrderNumber()) &&
						!webOrder.getOrderNumber().equals(panelVo.getOrderNo())){
					//MsgBox.showInfo(i18n.get("foss.gui.creating.webOrderDialog.msgbox.orderRepeat2"));
					ImportOrderDialog dialog = new ImportOrderDialog(ui);
					WindowUtil.centerAndShow(dialog);
				}
				if(!ui.getIsImportOrder()){
					return;
				}
			...
			try {
				orderDetailVo = waybillService.importOrder(webOrder
						.getOrderNumber());
			...
			if (StringUtil.isNotEmpty(orderDetailVo.getWaybillNumber())
					&& StringUtil.isNotEmpty(orderDetailVo
				...
				if (!WaybillConstants.OBSOLETE.equals(orderDetailVo
					...
					MsgBox.showInfo(i18n
							.get("foss.gui.creating.webOrderDialog.msgbox.orderAccepted"));
					return;
				}
			}
			...
			if (!accecptDept.equals(loginDept)) {
				if (JOptionPane.YES_OPTION != JOptionPane
					...
					if (panelVo.getPickupCentralized()) {
						orderDetailVo
								.setAcceptDept(panelVo.getReceiveOrgCode());
					} else {
						if (loginOrg != null) {
			 ...
			 * 
			 */
			boolean isSelectedisBigGoods = ui.basicPanel.getChbBigGoods()
					.isSelected();
			// TRUE
			if (isSelectedisBigGoods) {
				if (ProductEntityConstants.PRICING_PRODUCT_DOOR_TO_DOOR
						.equals(webOrder.getTransportModeFossCode())
			...
			//根据发货客户编码查询基础数据资料并赋值给orderDetailVo
			CustomerCircleNewDto customerCircleDto = waybillService.queryCustomerByCusCode(orderDetailVo.getShipperNumber() !=null ?orderDetailVo.getShipperNumber():"", new Date(), "Y");
			if(customerCircleDto !=null && "Y".equals(customerCircleDto.getIsCustCircle())){
				//存贮原发货客户编码
				oldDeliveryCustomerCode=orderDetailVo.getShipperNumber();
			}
			//如果发货客户在客户圈就使用主客户的客户编码进行相关计费
			getDeliveryCustomerCode(panelVo, orderDetailVo,
					customerCircleDto);
			// END
			...
			if(StringUtils.isNotBlank(ui.getPictureWaybillType()) &&
					WaybillConstants.WAYBILL_PICTURE.equals(ui.getPictureWaybillType())){
				panelVo.setGoodsVolumeTotal(null);
				panelVo.setGoodsWeightTotal(null);
			}
			
			//根据获取的网单查询对应保价费率以及保价范围
			if(panelVo.getGoodsVolumeTotal() == null){
				panelVo.setGoodsVolumeTotal(BigDecimal.ZERO);
			}
			if(panelVo.getGoodsWeightTotal() == null){
				panelVo.setGoodsWeightTotal(BigDecimal.ZERO);
			}
			 if(panelVo.getGoodsWeightTotal() != null && panelVo.getGoodsVolumeTotal() != null && panelVo.getCustomerPickupOrgCode() != null){
				 Common.getInsuranceCharge(panelVo, ui);
			 }
			/**
			 ...
			 */
			String invoice=CommonUtils.setInvoice(panelVo.getDeliveryCustomerCode());
			panelVo.setInvoice(invoice);
			if(WaybillConstants.INVOICE_01.equals(invoice)){
				panelVo.setInvoiceTab(WaybillConstants.INVOICE_TYPE_01);
			}else{
				panelVo.setInvoiceTab(WaybillConstants.INVOICE_TYPE_02);
			}
			// 来自官网的订单，导入成功后设置代收和保价不可编辑
			if (WaybillConstants.CRM_ORDER_CHANNEL_ONLINE.equals(webOrder
					.getResource())) {
			 ...
			 */
//			Common.whetherBankCardPayment(panelVo, ui);
			/**
			 * Dmana-9885通过订单来源判断如果是巨商网或者阿里巴巴来源，付款方式不可更改
			 * @author:218371-foss-zhaoyanjun
			 * @date:2015-02-03下午13:55
			 */
			if(WaybillConstants.GIANT_SINK.equals(panelVo.getOrderChannel())
					||WaybillConstants.ALIBABA.equals(panelVo.getOrderChannel())){
			...
			if(customerCircleDto !=null && "Y".equals(customerCircleDto.getIsCustCircle())){
				//存贮原发货客户编码
				panelVo.setDeliveryCustomerCode(oldDeliveryCustomerCode);
			}
			dispose();
		}
	...
	private void getDeliveryCustomerCode(WaybillPanelVo panelVo,
			CrmOrderDetailDto orderDetailVo,
			CustomerCircleNewDto customerCircleDto) {
		if(customerCircleDto !=null && "Y".equals(customerCircleDto.getIsCustCircle()) && 
				customerCircleDto.getCusBargainNewEntity()!=null && 
				customerCircleDto.getCustomerCircleEntity()!=null && 
				customerCircleDto.getCustomerNewEntity()!=null){
			//主客户编码
			orderDetailVo.setShipperNumber(customerCircleDto.getCusBargainNewEntity().getCusCode() !=null ?customerCircleDto.getCusBargainNewEntity().getCusCode():"");
			// 发货客户编码
			if (StringUtils.isNotEmpty(orderDetailVo.getShipperNumber())) {
				panelVo.setDeliveryCustomerCode(orderDetailVo.getShipperNumber());
			} else {
				...
				if (CollectionUtils.isEmpty(contactLis)) {
					panelVo.setDeliveryCustomerCode(null);
				} else {
					for (CustomerQueryConditionDto contactLi : contactLis) {
						if (contactLi != null) {
							panelVo.setDeliveryCustomerCode(contactLi
									.getCustCode());
						}
					}
				}
			}
			panelVo.setIsCustCircle("Y");
			panelVo.setCusBargainNewEntity(customerCircleDto.getCusBargainNewEntity());
			panelVo.setCustomerCircleEntity(customerCircleDto.getCustomerCircleEntity());
			panelVo.setCustomerNewEntity(customerCircleDto.getCustomerNewEntity());
			panelVo.setStartCentralizedSettlement(customerCircleDto.getCustomerCircleEntity().getIsFocusPay() !=null?customerCircleDto.getCustomerCircleEntity().getIsFocusPay():"");
			panelVo.setDeliveryCustomerCode(customerCircleDto.getCusBargainNewEntity().getCusCode());
		}else{
			// 发货客户编码
			if (StringUtils.isNotEmpty(orderDetailVo.getShipperNumber())) {
	 ...
	 */
	public void setOrderToWaybillEditUI(CrmOrderDetailDto orderDetailVo,
		...
		if (StringUtils.isNotEmpty(orderDetailVo.getWaybillNumber())) {
			ui.numberPanel.getLblNumber().setVisible(true);
		...
		try {
			...
			//查询发货客户精准包裹权限
			CusBargainEntity bargainEntity = waybillService.queryCusBargainByCustCode(panelVo.getDeliveryCustomerCode());
			if(bargainEntity!=null){
				panelVo.setIsAccuratePackage(bargainEntity.getIsAccuratePackage());
			}else{
				panelVo.setIsAccuratePackage(FossConstants.INACTIVE);
			}
			...
			if(StringUtils.isNotBlank(panelVo.getDeliveryCustomerCode())){
				...
				customerQueryConditionDto.setCustCode(panelVo
				.getDeliveryCustomerCode());
				deliveryCustomerList.add(customerQueryConditionDto);
				CustomerQueryConditionDto customerQueryConditionDtoDev = new CustomerQueryConditionDto();
				List<CustomerQueryConditionDto> customerQueryConditionDtoList = customerService
				.queryCustomerByConditionList(deliveryCustomerList);
				if (customerQueryConditionDtoList.size() > 0) {
					customerQueryConditionDtoDev = customerQueryConditionDtoList
							.get(0);
					if (FossConstants.ACTIVE.equals(customerQueryConditionDtoDev
							.getIsLargeCustomers())) {
						// 设置大客户标记
						panelVo.setDeliveryBigCustomer(FossConstants.ACTIVE);
						ui.consignerPanel.getLabel2().setIcon(
								CommonUtils.createIcon(
										ui.consignerPanel.getClass(),
										IconConstants.BIG_CUSTOMER, 1, 1));
					} else {
						// 取消大客户标记
						ui.consignerPanel.getLabel2().setIcon(
								CommonUtils.createIcon(
										ui.consignerPanel.getClass(), "", 1, 1));
					}
					//设置客户分群
					String flabelleavemonth	= customerQueryConditionDtoDev.getFlabelleavemonth();
					if(StringUtils.isEmpty(flabelleavemonth)){
						flabelleavemonth="NEWCUST";
					}
					Common.fillFlabelleavemonthData(panelVo, ui, flabelleavemonth);
				}
				
				// 设置是否统一结算wutao
				String invoice = null;
				invoice=CommonUtils.setInvoice(panelVo.getDeliveryCustomerCode());
				if(customerQueryConditionDtoDev!=null){
					...
					if (null != panelVo.getDeliveryCustomerCode()) {
						CustomerQueryConditionDto contact = new CustomerQueryConditionDto();
						contact.setCustCode(panelVo.getDeliveryCustomerCode());
						CusBargainEntity cusBargainEntity = CommonUtils
								.queryCusBargainByCustCode(contact);
						CommonUtils
								.setDevliveryCusomterSettler(
										customerQueryConditionDtoDev, cusBargainEntity,
										panelVo);
					}
				}
			}
			...
			/*
			 * 【新需求：除去门到门勾选上门接货】 是否接货 逻辑： IF:
			 * 如果运输性质为门道门，并且重官网那传递过来的是否接货为False的话，
			 * 那么在开单界面的时候，设置是否接货为True，并且设置为【上门接货】选择框不可编辑 ElSE:
			 * 如果从CRM那边传递过来的【上门接货】为TRUE，那么这么的判断如果运输性质为场到场的话把上门接货选择框设置为不可编辑
			 * if(orderDetailVo.isReceiveGoods()==false &&
			 * ProductEntityConstants
			 * .PRICING_PRODUCT_DOOR_TO_DOOR.equals(orderDetailVo
			 * .getTransportMode())){ panelVo.setPickupToDoor(true);
			 * ui.basicPanel.getCboReceiveModel().setEnabled(false); }else{
			 * if(orderDetailVo.isReceiveGoods() &&
			 * ProductEntityConstants.PRICING_PRODUCT_DOOR_TO_DOOR
			 * .equals(orderDetailVo.getTransportMode())){
			...
			if(StringUtils.isNotBlank(ui.getPictureWaybillType()) && 
					WaybillConstants.WAYBILL_PICTURE.equals(ui.getPictureWaybillType())){
				// 是否接货
				panelVo.setPickupToDoor(true);
			}else{
				// 是否接货
				panelVo.setPickupToDoor(orderDetailVo.isReceiveGoods());
				//zhangchengfu 20150619  新需求：集中开单、图片开单默认勾选上门接货
				if (panelVo.getPickupCentralized()) {
					panelVo.setPickupToDoor(true);
				}
			}
			...
			// }
			// 在线查询
			// IAddressServiceHessianRemoting districtService =
			// DefaultRemoteServiceFactory.getService(IAddressServiceHessianRemoting.class);
			String proviceCode = null;
			// AddressFieldDto dto = null;
			if (orderDetailVo.getContactProvince() != null) {
				// proviceCode =
				// waybillService.queryDistrictCode(DictionaryValueConstants.DISTRICT_PROVINCE,
				// orderDetailVo.getContactProvince(), null);
				...
				// dto =
				// districtService.onlineQueryProviceByName(orderDetailVo.getContactProvince());
				// if(dto != null){
				// proviceCode = dto.getProvinceId();
				// }
				// }
			...
			if (orderDetailVo.getContactCity() != null && proviceCode != null) {
				// cityCode =
				// waybillService.queryDistrictCode(DictionaryValueConstants.DISTRICT_CITY,
				// orderDetailVo.getContactCity(), proviceCode);
				// if(StringUtils.isEmpty(cityCode) &&
				// WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())){
				// dto =
				// districtService.onlineQueryCityByName(orderDetailVo.getContactCity());
				// if(dto != null){
				// cityCode = dto.getCityId();
				// }
				// }
			...
			if (orderDetailVo.getContactArea() != null && cityCode != null) {
				// areaCode =
				// waybillService.queryDistrictCode(DictionaryValueConstants.DISTRICT_COUNTY,
				// orderDetailVo.getContactArea(), cityCode);
				// if(StringUtils.isEmpty(areaCode) &&
				// WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())){
				// dto =
				// districtService.onlineQueryCountyByName(orderDetailVo.getContactArea());
				// if(dto != null){
				// areaCode = dto.getCountyId();
				// }
				// }
			...
			AddressFieldDto deliverAddressDto = Common
					.getAddressFieldInfoByCode(ui, proviceCode, cityCode,
							areaCode);
			ui.getConsignerPanel().getTxtConsignerArea()
					.setAddressFieldDto(deliverAddressDto);
			
			panelVo.setDeliveryCustomerAreaDto(deliverAddressDto);//add by taodongguo 354805
			...
			if (orderDetailVo.getReceiverCustNumber() != null) {
				...
				if (CollectionUtils.isEmpty(contactLis)) {
					...
					for (CustomerQueryConditionDto contactLi : contactLis) {
						if (contactLi != null) {
							panelVo.setReceiveCustomerCode(contactLi
									.getCustCode());
						}
					}
				}
			...
			//设置完客户信息之后设置黑名单
			String receiveCustomerCode = panelVo.getReceiveCustomerCode();
			String deliveryCustomerCode = panelVo.getDeliveryCustomerCode();
			Common.isBlackList(deliveryCustomerCode, receiveCustomerCode,ui);
			...
			if(StringUtils.isNotBlank(panelVo.getReceiveCustomerCode())){
				...
				customerQueryConditionDtoReceiver.setCustCode(panelVo
				.getReceiveCustomerCode());
				receivercCustomerList.add(customerQueryConditionDtoReceiver);
				CustomerQueryConditionDto customerQueryConditionDtoRev = new CustomerQueryConditionDto();
				List<CustomerQueryConditionDto> customerQueryConditionDtoReceivercList = customerService
				.queryCustomerByConditionList(receivercCustomerList);
				/*CustomerQueryConditionDto customerQueryConditionDtoReceiver = new CustomerQueryConditionDto();
				customerQueryConditionDtoReceiver.setCustCode(panelVo
						.getReceiveCustomerCode());
				CustomerQueryConditionDto customerQueryConditionDtoRev = null;
				List<CustomerQueryConditionDto> customerQueryConditionDtoReceivercList = customerService
						.queryCustomerByCondition(customerQueryConditionDtoReceiver);*/
				if (customerQueryConditionDtoReceivercList.size() > 0) {
					customerQueryConditionDtoRev = customerQueryConditionDtoReceivercList
							.get(0);
					if (FossConstants.ACTIVE.equals(customerQueryConditionDtoRev
							.getIsLargeCustomers())) {
						// 设置大客户标记
						panelVo.setReceiveBigCustomer(FossConstants.ACTIVE);
						ui.consigneePanel.getLblNewLabel().setIcon(
								CommonUtils.createIcon(
										ui.consigneePanel.getClass(),
										IconConstants.BIG_CUSTOMER, 1, 1));
					} else {
						// 取消大客户标记
						...
						ui.consigneePanel.getLblNewLabel().setIcon(
								CommonUtils.createIcon(
										ui.consigneePanel.getClass(), "", 1, 1));
					}
				}
				// 设置是否统一结算wutao
				String invoice=CommonUtils.setInvoice(panelVo.getReceiveCustomerCode());
				if(WaybillConstants.INVOICE_01.equals(invoice)){
					customerQueryConditionDtoRev.setInvoiceType(WaybillConstants.INVOICE_01);
				    }else{
				    customerQueryConditionDtoRev.setInvoiceType(WaybillConstants.INVOICE_02);
				}
				if (null != panelVo.getReceiveCustomerCode()) {
					CustomerQueryConditionDto contact = new CustomerQueryConditionDto();
					contact.setCustCode(panelVo.getReceiveCustomerCode());
					CusBargainEntity cusBargainEntity = CommonUtils
							.queryCusBargainByCustCode(contact);
					CommonUtils
							.setReciveCusomterSettler(customerQueryConditionDtoRev,
									cusBargainEntity, panelVo);
				}	
			...
			if (orderDetailVo.getReceiverCustProvince() != null) {
				// proviceCode =
				// waybillService.queryDistrictCode(DictionaryValueConstants.DISTRICT_PROVINCE,
				// orderDetailVo.getReceiverCustProvince(), null);
				// if(StringUtils.isEmpty(proviceCode) &&
				// WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())){
				// dto =
				// districtService.onlineQueryProviceByName(orderDetailVo.getReceiverCustProvince());
				// if(dto != null){
				// proviceCode = dto.getProvinceId();
				// }
				// }
			...
			if (orderDetailVo.getReceiverCustCity() != null
				...
				// cityCode =
				// waybillService.queryDistrictCode(DictionaryValueConstants.DISTRICT_CITY,
				// orderDetailVo.getReceiverCustCity(), proviceCode);
				// if(StringUtils.isEmpty(cityCode) &&
				// WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())){
				// dto =
				// districtService.onlineQueryCityByName(orderDetailVo.getReceiverCustCity());
				// if(dto != null){
				// cityCode = dto.getCityId();
				// }
				// }
			...
			if (orderDetailVo.getReceiverCustArea() != null && cityCode != null) {
				// areaCode =
				// waybillService.queryDistrictCode(DictionaryValueConstants.DISTRICT_COUNTY,
				// orderDetailVo.getReceiverCustArea(), cityCode);
				// if(StringUtils.isEmpty(areaCode) &&
				// WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())){
				// dto =
				// districtService.onlineQueryCountyByName(orderDetailVo.getReceiverCustArea());
				// if(dto != null){
				// areaCode = dto.getCountyId();
				// }
				// }
				if (StringUtils.isNotEmpty(orderDetailVo
						.getReceiverCustAreaCode())) {
					areaCode = orderDetailVo.getReceiverCustAreaCode();
				} else {
					AddressFieldDto dto = districtService
							.onlineQueryCountyByName(orderDetailVo
									.getReceiverCustArea());
					if (dto != null) {
						areaCode = dto.getCountyId();
					}
			...
			// receiverAddressDto.setCountyId(orderDetailVo.getReceiverCustAreaCode());
			panelVo.setReceiveCustomerAreaDto(receiverAddressDto);
			...
			//网上订单精准包裹提示
			if(!WaybillConstants.YES.equals(panelVo.getIsAccuratePackage())
					&&productEntity!=null&&ProductEntityConstants.PRICING_PRODUCT_PCP.equals(productEntity.getCode())){
			...
			/**
			 * FOSS导入开单时
			 * 开单页面保留OMS传给FOSS订单信息中的
			 * 保价，贷收，退款方式，付款方式，交货方式。 
			 * FOSS不做更改
			 */
			try {//add by 3036486
				...
				//add by 3036486
				cleanTargetEmpty(panelVo);
				log.error("ERROR:导入网上订单时效区域异常...",e );
			}
			//
			// 货物名称
			...
			waybillPanelVo.setGoodsName(orderDetailVo.getGoodsName());
			Common.fillIndustrySourceCategoryData(waybillPanelVo, ui);
			// 托运货物总件数
			panelVo.setGoodsQtyTotal(orderDetailVo.getGoodsNumber());

			// zxy 20131118 ISSUE-4391 DEFECT-536 刷新打木托标签列表
			Common.refreshLabeledGood(panelVo, ui);
			// zxy 20131118 ISSUE-4391 DEFECT-536 刷新打木托标签列表
			// 托运货物总重量
			...
			if (orderDetailVo.getReviceMoneyAmount() != null) {
				// 如果代收货款含有小数，则保留两位小数，四舍五入
				BigDecimal bg = orderDetailVo.getReviceMoneyAmount();
				if (orderDetailVo.getReviceMoneyAmount().toString()
						.contains(".")) {
					bg = bg.setScale(2, BigDecimal.ROUND_HALF_UP);
				}
				panelVo.setCodAmount(bg);
				panelVo.setCodAmountCanvas(bg.toString());
			} else {
			...
			//添加国家名称和code - 陶东国 - 2016-11-18 15:18:25
			String deliveryNation = "";
			if(null != deliverAddressDto && StringUtils.isNotBlank(deliverAddressDto.getNationId())){
				panelVo.setDeliveryCustomerNationCode(deliverAddressDto.getNationId());
				if(StringUtils.isNotBlank(deliverAddressDto.getNationName())  && !WaybillConstants.CHINA.equals(deliverAddressDto.getNationName())){
					deliveryNation = deliverAddressDto.getNationName() + "-";
				}
			}
			if (deliveryProvinve != null && deliveryCity != null
					&& deliveryArea != null) {
				panelVo.setDeliveryCustomerArea(deliveryNation + deliveryProvinve + "-"
			...
			//添加国家名称和code - 陶东国 - 2016-11-18 15:18:25
			String receiveNation = "";
			if(null != receiverAddressDto && StringUtils.isNotBlank(receiverAddressDto.getNationId())){
				panelVo.setReceiveCustomerNationCode(receiverAddressDto.getNationId());
				if(StringUtils.isNotBlank(receiverAddressDto.getNationId()) && !WaybillConstants.CHINA.equals(receiverAddressDto.getNationName())){
					receiveNation = receiverAddressDto.getNationName() + "-";
				}
			}
			if (receiveProvinve != null && receiveCity != null
					&& receiveArea != null) {
				panelVo.setReceiveCustomerArea(receiveNation + receiveProvinve + "-"
			...
			//若特殊增值服务不为null,则置空
			if(ui.getBasicPanel().getSpecialValueAddedServiceType()!=null){
				ui.getBasicPanel().getSpecialValueAddedServiceType().setSelectedItem(null);
			}

			// 初始化运输性质
			Common.initCombProductType(panelVo, ui);
			// zxy 20130924 BUG-55905 start:屏蔽，移到上面提前查询
			// 货物运输方式
			// ProductEntityVo productEntity =
			// getProductTypeByCode(orderDetailVo.getTransportMode());
			// panelVo.setProductCode(productEntity);
			// zxy 20130924 BUG-55905 end:屏蔽，移到上面提前查询
			// 产品类型CODE
			String productCode = null;
			if (productEntity != null && productEntity.getCode() != null) {
				productCode = productEntity.getCode();
				// 根据运输性质改变提货方式
				Common.changePickUpMode(panelVo, ui);
				// 空运、偏线以及中转下线无法选择签收单返单
				Common.setReturnBill(panelVo, ui);
				// 偏线与空运不能选择预付费保密
				Common.setSecretPrepaid(panelVo, ui);
			...
			if (orderDetailVo.getShipperId() != null) {
				...
				//巨商汇阿里商城默认月结
				if(WaybillConstants.GIANT_SINK.equals(orderDetailVo.getResource())
						||WaybillConstants.ALIBABA.equals(orderDetailVo.getResource())){
			...
			// 产品类型
			// 重复定义（2928、2934行）：String productCode =
			// getProductTypeByCode(orderDetailVo.getTransportMode()).getCode();
			// 运输类型
			// 根据运输性质获取提货方式
			if (productCode != null) {
			...
			if (panelVo.getReceiveMethod() != null) {
				// 内部带货
				Common.innerPickup(panelVo, ui);
				// 各种自提
				Common.selfPickup(panelVo, ui);

				// 在已经选择了网点的情况下 修改提货方式 需要检查该网点是否支持该提货方式
				Common.validateCustomerPointBySelfPickup(panelVo, ui);
				...
				// 自提件取消上线，设置为不显示
				// 设置能否开经济自提件
				// setEconomyGoodsType(panelVo);
			}
			// 限保物品
			isInsurGoods(panelVo);
			//上面有好几处给付款方式下拉框付值，代码没有研究不敢动，所以将这次特殊的加在下面
			/*合伙人没有“网上支付”,如果导入的订单是网上支付给予提示并将付款方式清空 
			  合伙人导入月结时验证是否可以开“月结”单，如果不能提示并清空 2016年3月4日 14:22:13 葛亮亮*/
			if((BZPartnersJudge.IS_PARTENER && paymentType.equals(WaybillConstants.CRM_ORDER_PAYMENT_ONLINE))
				||(BZPartnersJudge.IS_PARTENER && paymentType.equals(WaybillConstants.CRM_ORDER_PAYMENT_MONTH_PAY))){
				if(paymentType.equals(WaybillConstants.CRM_ORDER_PAYMENT_ONLINE)){//“网上支付”
					panelVo.setOrderPayment(""); //清空 
					panelVo.setPaidMethod(null); //清空  
					MsgBox.showInfo(i18n.get("foss.gui.creating.webOrderDialog.imp.noOnLine"));					
				}else if(paymentType.equals(WaybillConstants.CRM_ORDER_PAYMENT_MONTH_PAY)){//月结
					//是否开单部门的月结客户
					CusBargainVo vo=new CusBargainVo();
					vo.setChargeType(WaybillConstants.MONTH_END);
					vo.setCustomerCode(panelVo.getDeliveryCustomerCode()); //发货客户编号
					vo.setBillDate(new Date()); //当前时间（导入后开单时间）
					//登陆人信息
					UserEntity user = (UserEntity) SessionContext.getCurrentUser();
					//登陆人部门编号
					String partnerDeptCode = user.getEmployee().getDepartment().getCode();
					vo.setBillOrgCode(partnerDeptCode); //开单部门（收货部门）编号,当前登录人的部门（谨防不是可以导入非同一部门的订单）
					boolean isOk = waybillService.isCanPaidMethod(vo);
					if(!isOk){//否
						panelVo.setOrderPayment(""); //清空 单独清空这个界面还是选择了月结
						panelVo.setPaidMethod(null); //清空 
						MsgBox.showInfo(i18n.get("foss.gui.creating.webOrderDialog.imp.noMonthEnd"));
					}//如果是月结客户不用处理了上面的代码已经处理过一次不用再重复处理
				}
			}
			//关掉导入订单界面
			dispose();
			...
			/**
			 * 将是否电子发票传入waybillpanelvo中
			 * 
			 * @author:218371-foss-zhaoyanjun
			 * @date:2014-10-23下午16:12
			 */
			panelVo.setIsElectronicInvoice(orderDetailVo
					.getIsElectronicInvoice());
			/**
			 * 将发票手机号传入waybillpanelvo中
			 * 
			 * @author:218371-foss-zhaoyanjun
			 * @date:2014-10-23下午16:12
			 */
		  ...
		  //根据获取的网单查询对应保价费率以及保价范围
			...
			//报价声明价值
			BigDecimal insuranceAmount =panelVo.getInsuranceAmount();
			//保险生命价值大于等于1000000给出提示
			if(insuranceAmount.intValue()>= NUM_1000000){
				//throw new WaybillValidateException(i18n.get("foss.gui.creating.listener.Waybill.exception.noMonthEndInsuranceOnemillion"));		
	 ...
	 */
	public List<CustomerQueryConditionDto> deliveryCustomerMobilephoneListener(
			WaybillPanelVo bean) {
		// 客户信息列表
	 ...
	 */
	public List<CustomerQueryConditionDto> receiveCustomerMobilephoneListener(
			WaybillPanelVo bean) {
		List<CustomerQueryConditionDto> contactList = null;
	...
	public List<CustomerQueryConditionDto> queryDeliveryCustomer(
		...
		} else if (contactList == null && StringUtils.isNotEmpty(custName)
	 ...
	 * @param panelVo
	 */
	private void setEconomyGoodsType(WaybillPanelVo panelVo) {
			...
			if (minFeeList != null && minFeeList.size() > 0) {
				...
				for (MinFeePlanEntity entity : minFeeList) {
					if (entity != null) {
						...
						if (ecoVo.isResult()) {
							setEconomyGoodsTypeRelevant(panelVo, entity);
							flagEconomy = true;
							break;
						}
					}
		...
		// 如果不符合经济自提件，则还原自提件相关的信息
		if (!flagEconomy) {
			// 初始化是否经济自提件
			initEconomyGoodsTypeRelevant(panelVo);
		}
		// 设置经济自提件可编辑(勾选经济自提件时有验证)
		ui.basicPanel.getChbEconomyGoods().setEnabled(true);
	}

	/**
	 * 初始化是否经济自提件
	 */
	private void initEconomyGoodsTypeRelevant(WaybillPanelVo panelVo) {
		// 设置是否自提件
		panelVo.setIsEconomyGoods(false);
		ui.basicPanel.getChbEconomyGoods().setEnabled(false);
		// 自提件类型
		DataDictionaryValueVo initVo = new DataDictionaryValueVo();
		panelVo.setEconomyGoodsType(initVo);
		// 设置上门接货可编辑
	 ...
	 * 设置经济自提件相关
	 */
	private void setEconomyGoodsTypeRelevant(WaybillPanelVo panelVo,
			MinFeePlanEntity entity) {
		// 设置经济自提件默认勾选
		panelVo.setIsEconomyGoods(true);
		ui.basicPanel.getChbEconomyGoods().setEnabled(true);
		// 经济自提件上门接货不能编辑
		...
		panelVo.setEconomyGoodsType(vo);
		// 设置自提件运输性质
		Common.economyGoodsTypeListener(panelVo, ui);
		// 设置自提件提货方式
		Common.setEconomyReceiveMethod(panelVo);
		if (StringUtils.isNotBlank(ui.getPictureWaybillType())
				&& WaybillConstants.WAYBILL_PICTURE.equals(ui
						.getPictureWaybillType())) {
		...
		// 设置储运事项
		Common.economySetTransportationRemark(panelVo);
	}
	...
	private ProductEntityVo getProductTypeByCode(String productCode)
		...
		if (StringUtil.isNotEmpty(productCode)) {
			ProductEntity entity = waybillService
					.queryTransTypeById(productCode);
	...
	private void bind() {
		...
		// 控件绑定
		binder = BindingFactory.getIntsance().moderateBind(WebOrderVo.class,
				this, null, true);
	}
	...
	private void createListener() {
		orderComponentListener = new WebOrderComponentListener(this);
		// 画面初始化完毕加载数据
		...
		datePickerStart.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// 现在的需求是开始时间和结束时间相差不能超过3天，不与当前时间比较（根据BUG-24239修改），因此以下校验需要注释掉
				// JXDatePicker begin = (JXDatePicker) e.getSource();
				// Calendar selectedDate = Calendar.getInstance();
				// selectedDate.setTime(begin.getDate());
				//
				// Date now = new Date();
				// Calendar nowCal = Calendar.getInstance();
				// nowCal.setTime(now);
				//
				// long day = (nowCal.getTimeInMillis() -
				// selectedDate.getTimeInMillis()) / (24 * 60 * 60 * 1000);
				// if (day > 3) {
				// MsgBox.showInfo(i18n.get("foss.gui.creating.webOrderDialog.msgbox.selectDate"));
				// now.setDate(now.getDate() - 3);
				// datePickerStart.setDate(now);
				// return;
				// }
	...
	public WebOrderComponentListener getOrderComponentListener() {
	 ...
	 * 
	 */
	public class WebOrderSortedTableDataUpdater implements
			CommonSortedTableDataUpdater<WebOrderTableModel> {
		 ...
		 * 查询data
		 */
		public void updateData(int currentPage, int pageSize, int column,
				boolean asc, WebOrderTableModel model) {
		 ...
		 * 查询页数
		 */
		public int getTotalCount(WebOrderTableModel model, int currentPage,
				int pageSize, int column, boolean asc) {
			...
			// 字典ID（取ValueCode）
			queryVo.setAcceptStatus(queryWebOrderVo.getOrderStatus()
					.getValueCode());
			// 订单编号
			...
			// 发货联系人
			queryVo.setShipperLinkman(queryWebOrderVo
					.getDeliveryCustomerContact());
			// 发货人手机
			queryVo.setShipperMobile(queryWebOrderVo
					.getDeliveryCustomerMobilephone());
			// 发货人电话
			...
			if (dialog.getDatePickerEnd().getDate() != null) {
				...
				SimpleDateFormat sdf2 = new SimpleDateFormat(
						"yyyy-MM-dd HH:mm:ss");
				try {
					endTime = sdf2.parse(sdf1.format(dialog.getDatePickerEnd()
							.getDate()) + " 23:59:59");
			...
			try {
				...
				if (infoVos.getTotalCount() == 0) {
					MsgBox.showInfo(i18n
							.get("foss.gui.creating.webOrderDialog.msgbox.noOrder"));
				}
			...
			for (CrmOrderInfo crm : crmOrderInfoList) {
				...
				if (StringUtils.isNotEmpty(orderStatus)) {
					DataDictionaryValueEntity e = dataDictionaryValueService
							.queryDataDictoryValueByCode(
									WaybillConstants.ORDER_STATUS, orderStatus);
					if (e != null) {
				...
				if (StringUtils.isNotEmpty(orderType)) {
					DataDictionaryValueEntity e = dataDictionaryValueService
							.queryDataDictoryValueByCode(
									WaybillConstants.ORDER_TYPE, orderType);
					if (e != null) {
				...
				if (StringUtils.isNotEmpty(tranportMode)) {
					for (CrmTransportTypeEnum tenum : CrmTransportTypeEnum
							.values()) {
				...
				if (StringUtils.isNotEmpty(orgUnifCode)) {
					OrgAdministrativeInfoEntity e = baseDataService
							.queryOrgAdministrativeInfoEntityByUnifiedCode(orgUnifCode);
					if (e != null) {
	...
	class WebOrderTableModel extends CommonSortedTableModel {
		...
		public String getColumnName(int column) {
			...
			case THREE:
				return i18n
						.get("foss.gui.creating.webOrderDialog.column.three");
			case FOUR:
			...
			case SEVEN:
				return i18n
						.get("foss.gui.creating.webOrderDialog.column.seven");
			case EIGHT:
				return i18n
						.get("foss.gui.creating.webOrderDialog.column.eight");
			case NIVE:
			...
			case ELEVEN:
				return i18n
						.get("foss.gui.creating.webOrderDialog.column.eleven");
			case TWELVE:
				return i18n
						.get("foss.gui.creating.webOrderDialog.column.twelve");
			case THIRTEEN:
				return i18n
						.get("foss.gui.creating.webOrderDialog.column.thirteen");
		...
		public Object getValueAt(int row, int column) {
			...
			case TWELVE:
				return data.get(row).getReceivingToPointName();// 提货网点
			case THIRTEEN:
				return data.get(row).getWaybillNumber();// 运单号
			default:
				return "";
			}
		}
	 ...
	 */
	public void setWebOrderSortedTableDataUpdater(
			WebOrderSortedTableDataUpdater webOrderSortedTableDataUpdater) {
	...
	public JTextField getTxtWaybillNumber() {
		return txtWaybillNumber;
	}

	public void setTxtWaybillNumber(JTextField txtWaybillNumber) {
		this.txtWaybillNumber = txtWaybillNumber;
	}

	public JLabel getLblWaybillNumber() {
		return lblWaybillNumber;
	}

	public void setLblWaybillNumber(JLabel lblWaybillNumber) {
		this.lblWaybillNumber = lblWaybillNumber;
	}
}
 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/changingexp/client/validation/descriptor/WaybillInfoDescriptor.java
 * 
...
集中开单小组：21：00-次日4：00
 */
package com.deppon.foss.module.pickup.changingexp.client.validation.descriptor;
...
public class WaybillInfoDescriptor {
	...
	public String validateNewWaybillNo(String waybillNo,
		...
		//单号位数只允许8、9位
			...
			return i18n.get("foss.gui.changingexp.waybillDescriptor.waybillNo.duplicate");
		//单号是否已经存在
		//此判断会覆盖【变更后运单号不能与原运单号重复】场景，为了让上一条规则生效，放在后面校验
			...
			if (isWaybillExsit) {
				LOG.info("运单号已经存在");
				return i18n.get("foss.gui.changingexp.waybillDescriptor.waybillNo.exist");
			} else {
				return WaybillConstants.SUCCESS;
			}
		}
	...
	private boolean checkESCWaybillNo(String waybillNo) {
		try {
			...
			return waybillRfcService.validateWaybillNoIsCorrect(map);
		}catch(BusinessException e){
			LOG.info("校验快递单号是否符合规则出现异常："+e) ;
			return true ;
		} catch (Exception e) {
			LOG.info("校验快递单号是否符合规则出现异常："+e) ;
			return false ;
	...
	public String validateErrorHandlingCode(String errorHandlingCode,
		...
		if (StringUtil.isEmpty(errorHandlingCode)) {
			if (WaybillRfcConstants.RECEIVE_STOCK.equals(waybillInfoVo.getGoodsStatus().getValueCode())) {
				...
				return i18n.get("foss.gui.changingexp.waybillDescriptor.errorHandlingCode.notNull");
			}
		}
	...
	public String validateTransferTargetOrgCode(String transferTargetOrgCode,
		...
		if (waybillInfoVo.getRfcType() == null
    		...
    		if (StringUtil.isEmpty(transferTargetOrgCode)) {
				return i18n.get("foss.gui.changingexp.waybillDescriptor.transferTargetOrgCode.notNull");
    		}
		}
		return WaybillConstants.SUCCESS;
	}
	...
	public String validateGoodsName(String goodsName, WaybillPanelVo bean) {
		...
		if(goodsName!=null&&!"".equals(goodsName)){
    		...
    		if (bool) {
    			return i18n.get("foss.gui.changingexp.waybillDescriptor.embargo.name");
    		}
		}else
		{
			return i18n.get("foss.gui.changingexp.descriptor.Waybill.goodsName.null");
		}
		return WaybillConstants.SUCCESS;
	}
	 ...
	 * @see
	 */
	public String validateDeliveryCustomerMobilephone(
	 ...
	 */
	public String validateReceiveCustomerMobilephone(
	...
	public String validateGoodsSize(String goodsSize) {
		...
		if(goodsSize != null && !"".equals(goodsSize.trim()))
		{
			if (!NumberValidate.checkIsGoodsSize(goodsSize)) {
				StringBuffer str = new StringBuffer(i18n.get("foss.gui.changingexp.waybillDescriptor.size.rule"));
				str.append("(\n").append(i18n.get("foss.gui.changingexp.waybillDescriptor.example"));
				str.append("：0.5*0.5*0.5*2").append(i18n.get("foss.gui.changingexp.waybillDescriptor.or")).append("\n");
				str.append("0.5*0.5*0.5*2+1*1*1*5").append(i18n.get("foss.gui.changingexp.waybillDescriptor.or")).append("\n");
	 ...
	 * @return
	 */
	public String validateGoodsVolumeTotal(BigDecimal goodsVolumeTotal,
			WaybillInfoVo waybillInfoVo) {
...
//	public String validateDeliveryCustomerPhone(String deliveryCustomerPhone) {		
//		if (StringUtil.isNotEmpty(deliveryCustomerPhone)) {
//			int phoneLength = deliveryCustomerPhone.trim().length();			
//			int lengthLimitSeven = SEVEN;
//			int lengthLimitFORTY = FORTY;
//			//如果电话号码长度少于7位数字或者大于40位数字
//			if((phoneLength < lengthLimitSeven || phoneLength > lengthLimitFORTY)){
//				return i18n.get("foss.gui.creating.waybillDescriptor.deliveryCustomerPhone.validateLength");
//			}else if(!NumberValidate.checkPhoneWaybill(deliveryCustomerPhone)){
//				return i18n.get("foss.gui.creating.waybillDescriptor.deliveryCustomerPhone.validateLength");
//			}
//			//如果固定电话号码校验成功,需要判定固话的号码在手机号码里面是否存在
//			//进行数字类型的转换，如果转换成功，则证明这个是纯数字组成，否则走原有一套
//			try {
//				String deliveryCustomerMobilePhone = deliveryCustomerPhone.replaceFirst("^0*", ""); 
//				if (StringUtils.isNotEmpty(deliveryCustomerMobilePhone)//不为空
//						&& (deliveryCustomerMobilePhone.length() == 8//并且满足手机号码的校验条件
//						|| (deliveryCustomerMobilePhone.length() == 11 && deliveryCustomerMobilePhone.startsWith("1")))) {
//					//设置电话号码为手机号，然后进行后台查询，如果查询到，则重复，需要抛错
//					CustomerQueryConditionDto customerQueryConditionDto = new CustomerQueryConditionDto();
//					customerQueryConditionDto.setMobilePhone(deliveryCustomerMobilePhone);
//					//查询参数的封装
...
//					}
//					customerQueryConditionDto.setDeptCode(user.getEmployee().getOrgCode());
//					boolean isExist = waybillRfcService.cheCustomerPhoneExistCustomerMobilePhone(customerQueryConditionDto);
//					//如果存在，需要抛错
//					if(isExist){
...
//	public String validateReceiveCustomerPhone(String receiveCustomerPhone) {		
//		if (StringUtil.isNotEmpty(receiveCustomerPhone)) {
//			int phoneLength = receiveCustomerPhone.trim().length();			
//			int lengthLimitSeven = SEVEN;
//			int lengthLimitFORTY = FORTY;
//			//判定是否少于7位或者大于40位
//			if((phoneLength < lengthLimitSeven || phoneLength > lengthLimitFORTY)){
//				return i18n.get("foss.gui.creating.waybillDescriptor.receiveCustomerPhone.validateLength");
//			}else if(!NumberValidate.checkPhoneWaybill(receiveCustomerPhone)){
//				return i18n.get("foss.gui.creating.waybillDescriptor.receiveCustomerPhone.validateLength");
//			}
//			//如果固定电话号码校验成功,需要判定固话的号码在手机号码里面是否存在
//			//进行数字类型的转换，如果转换成功，则证明这个是纯数字组成，否则走原有一套
//			try {
//				String receiveCustomerMobilePhone = receiveCustomerPhone.replaceFirst("^0*", "");;
//				if (StringUtils.isNotEmpty(receiveCustomerMobilePhone)//不为空
//						&& (receiveCustomerMobilePhone.length() == 8//并且满足手机号码的校验条件
//						|| (receiveCustomerMobilePhone.length() == 11 && receiveCustomerMobilePhone.startsWith("1")))) {
//					//设置电话号码为手机号，然后进行后台查询，如果查询到，则重复，需要抛错
//					CustomerQueryConditionDto customerQueryConditionDto = new CustomerQueryConditionDto();
//					customerQueryConditionDto.setMobilePhone(receiveCustomerMobilePhone);
//					boolean isExist = waybillRfcService.cheCustomerPhoneExistCustomerMobilePhone(customerQueryConditionDto);
//					//如果存在，需要抛错
//					if(isExist){
//						return i18n.get("foss.gui.creating.listener.Waybill.telephoneInMobilePhoneisExist");

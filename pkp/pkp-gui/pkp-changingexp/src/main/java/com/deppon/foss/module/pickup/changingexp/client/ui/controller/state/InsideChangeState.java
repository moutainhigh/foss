 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/changingexp/client/ui/controller/state/InsideChangeState.java
 * 
...
import com.deppon.foss.module.pickup.changingexp.client.ui.internal.BasicPanel;
...
public class InsideChangeState extends AbstractState {
	...
	protected void updateComponents(WaybillRFCUI rfcUI) {
		...
		// String channel = waybillVo.getOrderChannel();
		// if(ExpWaybillConstants.CRM_ORDER_CHANNEL_TAOBAO.equals(channel))
		// {
		// MsgBox.showError(i18n.get("pickup.changingexp.order"));
		// return;
		// }
		if (waybillVo.getIsChangeReason() != 2) {
			...
			rfcUI.getImportPanel().getTxtWaybillNo().setEnabled(false);
			rfcUI.getImportPanel().getBtnImport().setEnabled(false);
			...
			// 设置查询客户不可编辑
			rfcUI.getButtonPanel().getBtnSearchMember().setEnabled(false);
			// 设置查询公布价不可编辑
			rfcUI.getButtonPanel().getBtnSearchPrice().setEnabled(false);
			// 收货联系人
			consigneePanel.getTxtConsigneeLinkMan().setEnabled(true);
			// 运输性质
			if(rfcUI.getOriginWaybill().getProductCode().getCode().equals(ProductEntityConstants.ECOMMERCE_PROMOTIONAL_ENJOY_PACKAGE)){
				transferPanel.getTransportInfoPanel().getCombProductType()
				.setEnabled(false);
			}else{
			transferPanel.getTransportInfoPanel().getCombProductType()
					.setEnabled(true);
			...
			// 目的站
			transferPanel.getTransportInfoPanel().getTxtDestination()
					.setEnabled(true);
			// 放大镜
			transferPanel.getTransportInfoPanel().getBtnQueryBranch()
					.setEnabled(true);
			// 代收货款
			...
			// 现付金额
			billingPayPanel.getTxtCashpayment().setEnabled(true);
			// 收货人电话
			rfcUI.getConsigneePanel().getTxtConsigneePhone().setEnabled(true);
			// 客户名称
			rfcUI.getConsigneePanel().getTxtConsigeeName().setEnabled(true);

			// 收货人手机
			rfcUI.getConsigneePanel().getTxtConsigneeMobile().setEnabled(true);
			// 收货人联系地址
			rfcUI.getConsigneePanel().getTxtConsigneeArea().setEnabled(true);

			rfcUI.getConsigneePanel().getTxtConsigneeAddressNote()
					.setEnabled(true);
			rfcUI.getConsigneePanel().getTxtConsigneeAddress().setEnabled(true);
			// 客户编码
			rfcUI.getUploadVoucherPanel().getTable().setEnabled(true);
			// 搜索按钮
			rfcUI.getConsigneePanel().getBtnQuery().setEnabled(true);
		}
		// 提交、打印按钮
		getWaybillRfcUI().getButtonPanel().getBtnSubmit().setEnabled(false);
		// 退款类型:有代收则代收可大可小，无代收则不可修改
		if (BigDecimal.ZERO.compareTo(waybillVo.getCodAmount()) < 0
				&& !CommonUtils.isTaoBaoOrder(waybillVo)) {
			// 代收货款
			getWaybillRfcUI().getWaybillInfoPanel().getIncrementPanel()
					.getTxtCashOnDelivery().setEnabled(true);
		} else {
			// 代收货款
			getWaybillRfcUI().getWaybillInfoPanel().getIncrementPanel()
					.getTxtCashOnDelivery().setEnabled(false);
		}
		if (waybillVo.getIsChangeReason() != 2) {
		if (waybillVo.getInternalDeliveryType() == null
				|| StringUtils.isBlank(waybillVo.getInternalDeliveryType()
						.getValueCode())
				|| StringUtils.isBlank(waybillVo.getEmployeeNo())) {
			// 若付款方式不为现金或到付，则设置为不可编辑
			if (!WaybillConstants.ARRIVE_PAYMENT.equals(waybillVo
					.getPaidMethod().getValueCode())
					&& !WaybillConstants.CASH_PAYMENT.equals(waybillVo
							.getPaidMethod().getValueCode())
					|| ProductEntityConstants.ECOMMERCE_PROMOTIONAL_ENJOY_PACKAGE
							.equals(waybillVo.getProductCode().getCode())) {

				getWaybillRfcUI().billingPayPanel.getCombPaymentMode()
						.setEditable(false);
				getWaybillRfcUI().billingPayPanel.getCombPaymentMode()
						.setEnabled(false);
			} else {
				// 现付和到付可以互相更改，只能更改付款方式
				getWaybillRfcUI().getWaybillInfoPanel().getBillingPayPanel()
						.getCombPaymentMode().setEnabled(true);
			}
		} else {
			billingPayPanel.getCombPaymentMode().setEnabled(false);// 付款方式
		}}

		rfcUI.getWaybillInfoPanel().getIncrementPanel().getTxtServiceCharge()
				.setEnabled(false);
		// 上传凭证按钮
		getWaybillRfcUI().getUploadVoucherPanel().getBtnAdd().setEnabled(true);
		getWaybillRfcUI().getUploadVoucherPanel().getBtnDelete()
				.setEnabled(true);
		rfcUI.getUploadVoucherPanel().getTable().setEnabled(true);
...
//		Common.paidMethodToTransactionSerialNumber(rfcUI.getBinder().getBean(),
//				rfcUI);
	}

	/**
	 * 初始化为只有现金和到付的下拉框
	 * 注意：此处若使用以下方式获取model则会报"无法将ComboBoxModelAdapt转换为DefaultComboBoxModel类型"的错误
	 * DefaultComboBoxModel model = (DefaultComboBoxModel)this.billingPayPanel.getCombPaymentMode().getModel();
	 * 
	 * @author 026123-foss-lifengteng
	 * @date 2013-8-13 上午10:36:38
	 */
	private void initCombPaymentMode() {
		//获取原下拉框的model
		JComboBox comboBox = (JComboBox)this.billingPayPanel.getCombPaymentMode();
		//下拉个数
		...
		for (int i=0; i<size; i++) {
			//获得下拉元素
			DataDictionaryValueVo vo = (DataDictionaryValueVo) comboBox.getModel().getElementAt(i);
			
			if(WaybillConstants.ARRIVE_PAYMENT.equals(vo.getValueCode()) ||WaybillConstants.CASH_PAYMENT.equals(vo.getValueCode()) ){
				...
				if(modelSize == 0){
					...
					for (int j=0; j<modelSize; j++) {
						DataDictionaryValueVo v = (DataDictionaryValueVo)model.getElementAt(j);
						if(vo.getValueCode().equals(v.getValueCode())){
							flag = true;
						}
					}
					if(!flag){
	...
	private void updateWaybillInfoComonentEnable() {
		...
		if(goodsStatus == null){
			MsgBox.showError(i18n.get("pickup.changingexp.noGoodsStatus"));
			return;
		}
...
//			ui.getWaybillInfoPanel().getTransferPanel().getTransportInfoPanel().getCombReturnBillType().setEnabled(false);
	...
	protected List<UploadVoucherDto> getUploadVoucherDtos() {
		...
		vo.setProofName(i18n.get("pickup.changingexp.waybillOriginal"));
...
//				}
		...
		incrementPanel.getBtnAdd().setEnabled(true);
		incrementPanel.getBtnDelete().setEnabled(true);
		incrementPanel.getTxtServiceCharge().setEnabled(false); // 装卸费
		if(waybillVo.getInternalDeliveryType() == null
				|| StringUtils.isBlank(waybillVo.getInternalDeliveryType().getValueCode())
				|| StringUtils.isBlank(waybillVo.getEmployeeNo())) {
			billingPayPanel.getCombPaymentMode().setEnabled(true);
		} else {
			billingPayPanel.getCombPaymentMode().setEnabled(false);// 付款方式

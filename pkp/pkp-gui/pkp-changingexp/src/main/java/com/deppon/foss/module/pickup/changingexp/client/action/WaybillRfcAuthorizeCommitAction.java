 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/changingexp/client/action/WaybillRfcAuthorizeCommitAction.java
 * 
...
public class WaybillRfcAuthorizeCommitAction implements IButtonActionListener<WaybillRfcAuthorizeDialog>{
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			if(bean.getId() == null){
				...
				//新增				
				rfcVarificationService.addWayBillRfcAgentExp(bean);
			}else{
				//获取状态
				...
				//修改
				rfcVarificationService.updateWaybillRfcAgentExp(bean);
	...
	private void validate(WaybillRfcAgentEntity bean) {
		if(bean.getAgentEmpList().isEmpty()){
			throw new WaybillValidateException(i18n.get("foss.gui.changingexp.waybillRfcAuthorizeCommitAction.emptyAgentEmpList"));
		}
		if(StringUtil.isEmpty(bean.getAgentReason())){
			throw new WaybillValidateException(i18n.get("foss.gui.changingexp.waybillRfcAuthorizeCommitAction.emptyAgentReason"));
		}
		if(bean.getValidTime() == null){
			throw new WaybillValidateException(i18n.get("foss.gui.changingexp.waybillRfcAuthorizeCommitAction.nullValidTime"));
		}
		if(bean.getOverdueTime() == null){
			throw new WaybillValidateException(i18n.get("foss.gui.changingexp.waybillRfcAuthorizeCommitAction.nullOverdueTime"));
		}
		if(bean.getValidTime().after(bean.getOverdueTime())){
			throw new WaybillValidateException(i18n.get("foss.gui.changingexp.waybillRfcAuthorizeCommitAction.afterOverdueTime"));
		}
	}
	...
	private WaybillRfcAgentEntity prepareRfcAgentEntity() {
		...
		bean.setType(WaybillRfcConstants.AGENTTYPE_EXPRESS);
		return bean;
	}

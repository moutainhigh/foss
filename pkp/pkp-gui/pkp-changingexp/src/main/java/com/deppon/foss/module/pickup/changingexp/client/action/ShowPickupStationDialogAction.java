 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/changingexp/client/action/ShowPickupStationDialogAction.java
 * 
...
public class ShowPickupStationDialogAction  implements IButtonActionListener<WaybillRFCUI>{
	...
	private void validate(WaybillInfoVo bean) {
		...
		if(WaybillRfcConstants.RETURN.equals(rfcType)){
			if(bean.getRtnProductCode()==null){// 网点类型标志
				throw new WaybillValidateException(i18n.get("foss.gui.changingexp.showPickupStationDialogAction.exception.nullProductCode"));
			}
			if(bean.getRtnReceiveMethod()==null){// 提货方式
				throw new WaybillValidateException(i18n.get("foss.gui.changingexp.showPickupStationDialogAction.exception.nullReceiveMethod"));
			}
		...
		if(WaybillRfcConstants.RETURN.equals(rfcType)){
			...
			if (bean.getCustomerPickupOrgCode() != null) {	
				*//**
				 * 判断是否需要查询产品时效
				 *//*
				if (isSearchProductTime(bean.getProductCode().getCode())) {
					...
					if(vo.isExist()){
						...
						MsgBox.showInfo(i18n.get("foss.gui.creating.showpickupstationdialogaction.noproducttime",new Object[]{vo.getDepartDeptName(),vo.getArriveDetpName()}));
					}
				}else{
					// 查询始发配载部门、最终配载部门以及线路
					queryLodeDepartmentInfo(bean);
	 ...
	 * 获取时效相关信息
	 */
	private IdentityEffectivePlanVo identityEffectivePlan(WaybillInfoVo bean) {
		...
		if (bean.getPickupCentralized() != null && bean.getPickupCentralized()) {
			SaleDepartmentEntity saleDepartment = waybillRfcService.queryPickupCentralizedDeptCode(bean.getCreateOrgCode());
		...
		outVo.setProductCode(bean.getProductCode().getCode());
		/**
		 * 查询是否有产品时效
		 */
			...
			try{
				if(WaybillRfcConstants.RETURN.equals(rfcType)){
				    ...
				    if(WaybillRfcConstants.INSIDE_CHANGE.equals(rfcType)
				    	...
				    	createOrgCode=getTfrStartOrgCode(createOrgCode);
				    	/**
				    	...
				    	if(bean.getPickupCentralized()!=null&&bean.getPickupCentralized()){
							dto = waybillRfcService.queryLodeDepartmentInfo(bean.getPickupCentralized(),bean.getCreateOrgCode(), bean.getCustomerPickupOrgCode().getCode(), bean.getProductCode().getCode());
						}else{
...
//				setGoodsTypeAB(bean);
	 ...
	 */
	private String getTfrStartOrgCode(String lastLoadOrgCode) {
	...
	private void showPickupStationDialog(WaybillInfoVo bean) {
		...
		dto.setOrgSimpleName(ui.getWaybillInfoPanel().getTransferPanel().getTransportInfoPanel().getTxtDestination().getText());
	    ...
	    if(WaybillRfcConstants.INSIDE_CHANGE.equals(rfcType)
			...
			dto.setDestNetType(bean.getRecordProductCode().getDestNetType());
	...
	public void setDialogData(BranchVo branchVO,WaybillInfoVo bean) {
		...
		//转寄退回 如果更改目的站并且 目的站不在同一行政区域
		if(bean.getPreCustomerPickupTime()!=null){
			BaseDataService baseDataService = baseDataServiceFactory.getBaseDataService() ;
			if(bean.getCustomerPickupOrgCode()!=null && StringUtils.isNotBlank(bean.getCustomerPickupOrgCode().getCode())){
				OrgAdministrativeInfoEntity oldOrgAdmin = baseDataService.queryOrgAdministrativeInfoEntityByCode(bean.getOldReceiveOrgCode());
				OrgAdministrativeInfoEntity orgAdmin = baseDataService.queryOrgAdministrativeInfoEntityByCode(bean.getCustomerPickupOrgCode().getCode());
				if(oldOrgAdmin!=null && orgAdmin!=null ){
					
					if(!StringUtils.equals(oldOrgAdmin.getCityCode(), orgAdmin.getCityCode())){
						if(bean.getOldReceiveOrgCode()!=bean.getCustomerPickupOrgCode().getCode()){
						  //走货路径变更了
						  bean.setChangeType(1);
					   }
						WaybillInfoTempVo vo = bean.getWaybillInfoTempVo() ;
						if(vo == null){
							vo = new WaybillInfoTempVo() ;
						}
						vo.setSameProv(false);
						bean.setWaybillInfoTempVo(vo);
				...
				if(data!=null && !data.isEmpty()){
					for(int i = 0; i < data.size(); i++){
						...
						if(vo.getCode().equals(PriceEntityConstants.PRICING_CODE_ZZ)){
							...
							BigDecimal moeny_ori = new BigDecimal("-1") ;
							WaybillInfoVo waybillInfoVo_ori = ui.getOriginWaybill() ;
							List<OtherChargeVo> otherChargeVos_ori = waybillInfoVo_ori.getOtherChargeVos();
							if(otherChargeVos_ori != null){
								for(OtherChargeVo ori : otherChargeVos_ori){
									if(PriceEntityConstants.PRICING_CODE_ZZ.equals(ori.getCode())){
										moeny_ori = BigDecimal.valueOf(Double.valueOf(StringUtils.defaultIfBlank(ori.getMoney(), "-1")));
										break ;
									}
								}
							}
							if(big_moeny.compareTo(moeny_ori)!=0){
							...
							if(tmpVo != null && tmpVo.getReturnBillFee()!=null ){
								String moeny = StringUtils.defaultIfBlank(vo.getMoney(), "0") ;
								BigDecimal big_moeny = BigDecimal.valueOf(Double.valueOf(moeny));
								BigDecimal mon = big_moeny.subtract(tmpVo.getReturnBillFee()) ;
								vo.setMoney(mon.toPlainString());
								// 累计其他费用合计
								otherSum(data,bean);
							}
	 ...
	 */
	private void canReturnSignBill(BranchVo branchVo, WaybillInfoVo bean) {		
		if(bean.getIsChangeReason()==1||bean.getIsChangeReason()==2){
			bean.getReturnBillType().setValueCode(bean.getOldreturnBillType().getValueCode());
		}else{
	...
	private void otherSum(List<OtherChargeVo> data,WaybillInfoVo bean)
	{
		...
		if (data != null && !data.isEmpty()) {
			for (int i = 0; i < data.size(); i++) {
				...
				if(StringUtils.equals(otherVo.getCode(), PriceEntityConstants.PRICING_CODE_ZYSCZJH)){
					continue ;
				}
	 ...
	 */
	private void queryOtherChargeData(WaybillPanelVo bean) {
		...
		{	
			if(!voList.isEmpty())
			{
				ui.incrementPanel.setChangeDetail(getAllChargeCompare(voList));
	 ...
	 * @date 2013-07-12
	 */
	private List<OtherChargeVo> getAllChargeCompare(List<OtherChargeVo> voList){
		List<OtherChargeVo> allList=voList; 
		JXTable table = ui.incrementPanel.getTable();
		WaybillOtherCharge model = (WaybillOtherCharge) table.getModel();
		List<OtherChargeVo> data = model.getData();
		if(data != null && !data.isEmpty()){			
			for(int i=0;i<data.size();i++){
				OtherChargeVo tableVo = data.get(i);
				//flag用来表示voList中有没有tableVo这个元素，false为没有
				boolean flag=false;
				for(int j=0;j<voList.size();j++){
					OtherChargeVo queryVo = voList.get(j);
		...
		if(list != null)
		{
			for (ValueAddDto dto : list) {
				...
				if(dto.getCandelete() != null && !BooleanConvertYesOrNo.stringToBoolean(dto
						.getCandelete()))
				{
					if(dto.getSubType().equals("ZYSCZJH")){
						continue ;
					}else{
					// 费用编码
...
//		}
		...
		if (isPickup(bean)) {
			effectiveDto = waybillRfcService.searchPreSelfPickupTime(bean.getCreateOrgCode(), bean.getCustomerPickupOrgCode().getCode(), bean.getProductCode().getCode(), bean.getPreDepartureTime(), new Date());
			...
			effectiveDto = waybillRfcService.searchPreDeliveryTime(bean.getCreateOrgCode(), bean.getCustomerPickupOrgCode().getCode(), bean.getProductCode().getCode(), bean.getPreDepartureTime(), new Date());
			if (effectiveDto != null) {
				bean.setLongOrShort(effectiveDto.getLongOrShort());
	...
	private Date getLeaveTime(WaybillPanelVo bean) {
		Date leaveTime = waybillRfcService.searchPreLeaveTime(bean.getCreateOrgCode(),bean.getLoadOrgCode(), bean.getProductCode().getCode(), new Date());
		return leaveTime;
	}
	...
	public static OtherChargeVo getDeliverStorgeCharge(WaybillPanelVo bean,
		...
		if (list != null) {
			if (!list.isEmpty()) {
				...
				throw new WaybillValidateException(i18n.get("foss.gui.changingexp.showPickupStationDialogAction.exception.nullDeliverStorageFee"));
			}
		} else {
			throw new WaybillValidateException(i18n.get("foss.gui.changingexp.showPickupStationDialogAction.exception.nullDeliverStorageFee"));
		}
	...
	public BaseDataServiceFactory getBaseDataServiceFactory() {
		return baseDataServiceFactory;
	}

	public void setBaseDataServiceFactory(
			BaseDataServiceFactory baseDataServiceFactory) {
		this.baseDataServiceFactory = baseDataServiceFactory;
	}
}
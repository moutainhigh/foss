 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/changingexp/client/action/WaybillRfcAuthorizeQueryAction.java
 * 
...
public class WaybillRfcAuthorizeQueryAction implements IButtonActionListener<WaybillRFCAuthorizeUI>{
	...
	public void actionPerformed(ActionEvent e) {
		...
		queryAgentConDto.setType(WaybillRfcConstants.AGENTTYPE_EXPRESS);
		try{
		List<WaybillRfcAgentEntity> rfcAgentEntities = rfcVarificationService.queryWaybillRfcAgent(queryAgentConDto);
		
		//给表格填充数据
		ui.getDataModel().setData(rfcAgentEntities);
		ui.getDataModel().fireTableDataChanged();
		if(rfcAgentEntities.isEmpty()){
			MsgBox.showError(i18n.get("foss.gui.changingexp.waybillRfcAuthorizeQueryAction.msgBox.emptyRfcAgentEntities"));
		}

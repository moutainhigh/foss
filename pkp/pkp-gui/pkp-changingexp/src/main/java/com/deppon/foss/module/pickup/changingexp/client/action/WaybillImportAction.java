 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/changingexp/client/action/WaybillImportAction.java
 * 
 ...
 * 下表为不能起草运单变更的类型及不能起草时系统提示内容
 * 
 * 序号 不能起草运单变更的类型 提示内容 1 运单状态为“已保存待补录”的运单 运单未补录，不能起草运单变更！ 2 运单状态为“作废”、“中止”的运单
 * 运单已作废，不能起草运单变更！ 运单已中止，不能起草运单变更！ 3 运单对应的收货部门与系统当前组织不一致 该单的收货部门不是您部门，不能起草运单变更！
 * 4 运单状态为“已签收”的运单 货物已签收，不能起草运单变更！ 5 变更单状态为“待审核”或“待受理”的运单
 * 该运单有运单变更单还未被审核，不能起草变更运单！ 该运单有运单变更单还未被受理，不能起草变更运单！ 6 运单已进行结清货款操作
 * 该运单已进行结清货款操作，如需更改请联系到达部门进行反结清货款操作！ 7 财务锁定的运单（财务将不允许发起变更的运单进行锁定）不能发起运单变更
 * 该运单已被财务锁定，不能起草变更运单！ 8 偏线运单以外发 偏线运单已中转外发，不能起草运单变更！
 * 
 * 财务单据状态对运单更改限制条件 说明： 以下总共有8大类结算操作会去修改运单的相关状态，当运单处于该状态下任何一种时，都不允许发生更改。 结算单据代码
 * 结算单据名称 结算大类操作代码 结算大类操作名称 结算小类操作代码 结算小类操作名称 运单相关的财务单据状态字段名称 状态内容 描述 备注 应收单
 * 核销应收单 应收冲应付 应收单核销状态 应收冲应付核销中 与运单相关的应收单发生了应收冲应付核销操作，运单的应收单核销状态增加“应收冲应付核销中”
 * 还款冲应收 还款冲应收核销中 与运单相关的应收单发生了还款冲应收核销操作，运单的应收单核销状态增加“还款冲应收核销中” 预收冲应收 预收冲应收核销中
 * 与运单相关的应收单发生了预收冲应收核销操作，运单的应收单核销状态增加“预收冲应收核销中” 坏账冲应收 坏账冲应收核销中
 * 与运单相关的应收单发生了坏账冲应收核销操作，运单的应收单核销状态增加“坏账冲应收核销中” 锁定应收单 锁定应收单 应收单锁定状态 网上支付应收单锁定中
 * 与运单相关的应收单被网上支付时处于锁定状态，运单的应收单锁定状态被置为“网上支付应收单锁定中” 应付单 生成应付单 偏线外发反馈录入 应付单生成状态
 * 偏线外发反馈已录入 该运单已生成了偏线外发成本应付单，运单的应付单生成状态为“偏线外发反馈已录入” 偏线外发反馈录入 生效应付单 生效代收货款应付单
 * 应付单生效状态 代收货款应付单已生效 与运单相关的代收货款应付单生效时，运单的应付单生效状态增加“代收货款应付单已生效” 签收相关 生效装卸费应付单
 * 装卸费应付单已生效 与运单相关的装卸费应付单生效时，运单的应付单生效状态增加“代收货款应付单已生效” 生效整车尾款应付单 整车尾款应付单已生效
...
public class WaybillImportAction implements IButtonActionListener<WaybillRFCUI> {
	...
	public void actionPerformed(ActionEvent e) {
		// 获取UI上需要导入的运单号
		final String waybillNo = waybillRFCUI.getImportPanel().getTxtWaybillNo().getText();
		rfcImportDto = null;
		if (StringUtil.isNotBlank(waybillNo)) {
			// 创建耗时任务
			LongTaskService taskService = new LongTaskService();
			//sonar说匿名内部类太长
			String name = i18n.get("foss.gui.changingexp.waybillImportAction.waybillImport.label");
			// 执行耗时任务
			taskService.execute(name, new ITask() {
				...
				public void execute(ITaskContext context) throws Exception {
					 ...
					 * 原单号不能发更改
					 */
					WaybillExpressEntity exp =rfcService.queryWaybillByOriginalWaybillNo(waybillNo,WaybillConstants.WAYBILL_EXPRESSTYPE_RETURN_CARGO); 
					/**
					 * 伙伴开单不能发更改
					 */
					/*if (null != rfcImportDto && null != rfcImportDto.getWaybillDto() && null != rfcImportDto.getWaybillDto().getActualFreightEntity()) {
						if (FossConstants.YES.equals(rfcImportDto.getWaybillDto().getActualFreightEntity().getPartnerBillingLogo())) {
							MsgBox.showInfo(i18n.get("pickup.changingexp.validatePartnerBillingLogo"));
							return;
						}
					}*/
					/**
					 * 已交接给快递代理后不能再发更改
					 */
					Boolean bool = rfcService.isLdpHandOveredOutBranch(waybillNo);
					/**
					 * 已做了外发录入的不能再发更改
					 */
					Boolean isOuter = rfcService.checkIfExistValidLdpExternalBillForStl(waybillNo,null);
					/**
					 *是否有效的营业部外发 ,建立绑定关系的不能起草更改          
					  */
					Boolean isValid = rfcService.isExternalBillRecords(waybillNo,WaybillConstants.WAYBILLRFC_TRF_METHOD);
					if (bool) {
						MsgBox.showInfo(i18n.get("pickup.changingexp.validateHandOverOuter"));
					}else if(isOuter){
						MsgBox.showInfo(i18n.get("pickup.changingexp.validateOuter"));
					}else if(isValid){
						MsgBox.showInfo(i18n.get("pickup.changingexp.validExternalBillRecords"));
					}else if(exp!=null && exp.getOriginalWaybillNo()!=null){
						MsgBox.showInfo("该单是返货原单号不能更改");
					}
					else {
						// 运单无法导入的时候 不做任何操作
...
//							if(waybillEntity!=null && WaybillConstants.DEPPON_CUSTOMER.equals(waybillEntity.getDeliveryCustomerCode())){
//								MsgBox.showInfo(i18n.get("pickup.changingexp.validateDepponCustomer"));
//								return;
//							}else 
							...
							if (CommonUtils.directDetermineIsExpressByProductCode(waybillEntity.getProductCode())) {
								...
								MsgBox.showInfo(i18n.get("pickup.changingexp.express"));
							}
						}
					}
				}
			...
			if (wdto != null) {
				...
				if (acf != null) {
					...
					if (acf.getStartContractOrgCode() != null
							&& !"".equals(acf.getStartContractOrgCode())) {
					...
					if (acf.getArriveContractOrgCode() != null
							&& !"".equals(acf.getArriveContractOrgCode())) {
...
//					waybillInfoVo.setGoodsTypeIsAB(dto.getGoodsTypeIsAB());// 是否AB货
			...
			if (WaybillConstants.MONTH_PAYMENT.equals(waybillInfoVo.getPaidMethod()
					.getValueCode())) {
				CustomerEntity customerEntity= customerService.queryCustomerInfoByCusCode(deptCode);
			...
			waybillInfoVo.setAccurateCost(isAccurateCost);
		} catch (IllegalAccessException e) {

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/changingexp/client/action/RfcInfoAction.java
 * 
...
public class RfcInfoAction implements IButtonActionListener<WaybillRFCUI> {
	...
	private static final Log LOG = LogFactory
				.getLog(RfcInfoAction.class);
	
	private static final double NUM_ZERO_POINT_ONE = 0.1;
	
	private static final double NUM_ZERO_POINT_SIX = 0.6;
	...
	public void actionPerformed(final ActionEvent e) {
		...
		String consigneeArea=ui.getWaybillInfoPanel().getConsigneePanel().getTxtConsigneeArea().getText();
		
		if(rfcType == null){
			MsgBox.showError(i18n.get("foss.gui.changingexp.rfcInfoAction.msgBox.nullRfcType"));
			return;
		}
		
		
		if(StringUtil.isEmpty(ui.getBinderWaybill().getRfcReason())){
			MsgBox.showError(i18n.get("foss.gui.changingexp.rfcInfoAction.msgBox.nullRfcReason"));
			return;
		}
		
		if(StringUtil.isEmpty(consigneeArea)){
			MsgBox.showError("收货人地址不能为空！");
			return;
		}
		...
		if(WaybillRfcConstants.ABORT.equals(rfcType.getValueCode())){
			...
			if (!WaybillRfcConstants.RECEIVE_STOCK.equals(ui.getBinderWaybill().getGoodsStatus().getValueCode())) {
				...
				if(!ui.getBinder().getBean().isErrorCodeIsTrue()){
					...
					//控件获取焦点
					jText.requestFocus();
//					MsgBox.showError(i18n.get("foss.gui.changingexp.rfcInfoAction.msgBox.errorCode"));
					return;
				}
			}
		...
		//转运、返货、更改
			...
			if(!WaybillRfcConstants.TRANSFER.equals(rfcType.getValueCode()) && 
				...
				if((detailDtos == null || detailDtos.size() == 0) && !ui.getBinderWaybill().isOtherChargeChanged()){
					MsgBox.showError(i18n.get("foss.gui.changingexp.rfcInfoAction.msgBox.nullChangedInfo"));
					return;
				}
			}
			...
			try{
				...
				{
					expPartnerMonthlyPrestoreCheck();
				}
	...
	public void expPartnerMonthlyPrestoreCheck(){
		...
		if (StringUtils.isNotEmpty(entityDate.getConfValue())) {
			...
			try {
				...
				if(!ui.getBinderWaybill().getBillTime().after(parterOnlineDate)){
					return;
				}
		...
		//开单总费用
		BigDecimal limit = ui.getBinderWaybill().getTotalFee() != null ? ui.getBinderWaybill().getTotalFee() : BigDecimal.ZERO;
		//代收货款
		BigDecimal codAmount = ui.getBinderWaybill().getCodAmount() != null ? ui.getBinderWaybill().getCodAmount() : BigDecimal.ZERO ;
				
		//预付金额
		BigDecimal prePayAmount = ui.getBinderWaybill().getPrePayAmount() != null ? ui.getBinderWaybill().getPrePayAmount() : BigDecimal.ZERO;
		// 订单付款方式
		String payCode = ui.getBinderWaybill().getPaidMethod().getValueCode();
	...
	private void validate(WaybillInfoVo bean) {
		if(StringUtil.isEmpty(ui.getWaybillInfoPanel().getIncrementPanel().getTxtCashOnDelivery().getText())){
			throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillSubmitAction.exception.errorCodAmount"));
...
//		validateDriverNo(bean);
		...
		if (WaybillConstants.ONLINE_PAYMENT.equals(bean.getPaidMethod().getValueCode())) {
			if (!WaybillConstants.CRM_ORDER_CHANNEL_ONLINE.equals(bean.getOrderChannel()) && !WaybillConstants.CRM_ORDER_PAYMENT_ONLINE.equals(bean.getOrderPayment())) {
				throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillSubmitAction.exception.errorPaidMethod"));
			}
		}
		 ...
		 * @date:2015-01-23
		 */
//		Common.validateTransactionSerialNumber(ui.getBinderWaybill(), ui);
		//验证代收货款 2016年3月10日 09:56:48 葛亮亮
		Common.cpv(bean);
	}
	...
	public void validatePaidMehtod(WaybillInfoVo bean){
		...
		if (!WaybillConstants.ARRIVE_PAYMENT.equals(bean.getPaidMethod().getValueCode()) && !WaybillConstants.CASH_PAYMENT.equals(bean.getPaidMethod().getValueCode())
	...
	private void validateCustomerPickupOrgName(WaybillInfoVo bean) {
		...
		if(newName!=null && !newName.equals(oldName)){
			...
			if(vo!=null && voOld!=null 
				...
				SaleDepartmentEntity dept = waybillRfcService.querySaleDepartmentByCode(vo.getCode());
				if(dept != null){
					String newName2 = dept.getName();
					if(!newName2.equals(newName)){
						throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillSubmitAction.exception.validatePickupOrg"));
		...
		if (bean.getIsWholeVehicle()!=null && bean.getIsWholeVehicle()) {
			...
			if(bean.getIsPassDept()!=null && !bean.getIsPassDept()){
				if (bean.getToPayAmount().compareTo(BigDecimal.ZERO) > 0) {
					throw new WaybillValidateException(i18n.get("foss.gui.creating.calculateAction.exception.vehicle"));
				}
			}
			
			BigDecimal rateLow = bean.getWholeVehicleActualfeeFlowRangeLow();
			if (rateLow == null) {
				rateLow = BigDecimal.valueOf(NUM_ZERO_POINT_ONE);
			}
			...
			if(bean.getTransportFee().compareTo(lowLevel)<0){
				throw new WaybillValidateException(i18n.get("foss.gui.creating.listener.Waybill.transportFee.three") + lowLevel.doubleValue());
			}
		}
	}
	...
	private void validatePayment(WaybillInfoVo bean) {
		...
		if (WaybillConstants.ONLINE_PAYMENT.equals(bean.getPaidMethod().getValueCode())) {
		    	if(StringUtils.isEmpty(bean.getDeliveryCustomerCode())){
		    	    throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.crmOrderChannelOnlineNoCustNo"));
		    	}
			if (!WaybillConstants.CRM_ORDER_CHANNEL_ONLINE.equals(bean.getOrderChannel()) || !WaybillConstants.CRM_ORDER_PAYMENT_ONLINE.equals(bean.getOrderPayment())) {
			    throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.crmOrderChannelOnline"));
			}
		}
	}
	...
	private void validateCod(WaybillInfoVo bean) {
		...
		if(codAmount == null || codAmount.compareTo(zero) == 0){
			if(bean.getRefundType() != null){
				throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.zeroCodAmount"));
			}
			if (JOptionPane.YES_OPTION != JOptionPane.showConfirmDialog(ui, i18n.get("foss.gui.changingexp.rfcInfoAction.confirmDialog.validateCod"), i18n.get("foss.gui.changingexp.waybillRFCUI.common.prompt"), JOptionPane.YES_NO_OPTION)) {
				throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.nullCodAmount"));
			}
		}else{
			if(bean.getRefundType() == null){
				throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.nullRefundType"));
			}else{
				if(!WaybillConstants.REFUND_TYPE_VALUE.equals(bean.getRefundType().getValueCode())){
					if(StringUtils.isEmpty(bean.getAccountName())||
						...
						throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.nullAccountDetail"));
					}
				}
			}
		}
	...
	protected void validateDriverNo(WaybillPanelVo bean) {
		if(bean.getPickupToDoor()!=null 
			...
			throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.nullDriverCode"));
		}
	}
	...
	private void validateReceiveMethod(WaybillInfoVo bean) {
		...
		if (WaybillConstants.INNER_PICKUP.equals(bean.getReceiveMethod().getValueCode())) {
			if(isCustomerChange(bean)){
				throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.notInnerPickup"));
			}
			//在中转库存的运单不能修改内部带货自提
			if(ui.getOriginWaybill().getReceiveMethod()!= null &&
				!WaybillConstants.INNER_PICKUP.equals(ui.getOriginWaybill().getReceiveMethod().getValueCode())
				&& !WaybillRfcConstants.RECEIVE_STOCK.equals(bean.getGoodsStatus().getValueCode()) ){
					throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.notReceiveStock"));	
			}
		}
		
		//客户要求不能选择免费自提
		if (WaybillConstants.AIR_PICKUP_FREE.equals(bean.getReceiveMethod().getValueCode())) {
			if(isCustomerChange(bean)){
				throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.notFreeAirPickup"));
			}
	    	//在中转库存的运单不能修改免费自提
	    	if (ui.getOriginWaybill().getReceiveMethod()!= null &&
					!WaybillConstants.AIR_PICKUP_FREE.equals(ui.getOriginWaybill().getReceiveMethod().getValueCode())
					&&!WaybillRfcConstants.RECEIVE_STOCK.equals(bean.getGoodsStatus().getValueCode())) {
	    		throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.notFreeStock"));
	    	}
		}
	}
	...
	private void validateTransportPath(WaybillInfoVo bean) {
		if(bean.getIsWholeVehicle()!= null && Boolean.TRUE.equals(bean.getIsWholeVehicle())){
			//整车没有走货路径
			//此处无需判断
			return;
		}
		...
		if(isNeedCheck ){
			...
			if(dto == null || dto.getFreightRoute() == null){
				throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.nullFreightRoute"));
			}
		}
		}
	...
	private void valiadateGoodQtyAndWoodQty(WaybillInfoVo bean) {
		...
		int hasGood = 0;
		int hasWood = 0;
		List<LabeledGoodChangeHistoryDto> list = bean.getLabeledGoodChangeHistoryDtoList();
		...
		//只有在改变件数的情况下 才做这个检查
		if(oldTotal!=null && goodQtyTotalNew!=null && goodQtyTotalNew.intValue()!=oldTotal.intValue()){
			if(hasGood!=goodTotal ){
				throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.wrongGoodTotal"));			
			}
			if(hasWood>wood ){
				throw new WaybillValidateException(i18n.get("foss.gui.changingexp.rfcInfoAction.exception.overHasWood"));
			}	
		}
	...
	private void validateFee(WaybillPanelVo bean){
		if(bean.getTotalFee().compareTo(BigDecimal.ZERO) < 0){
			throw new WaybillValidateException(i18n.get("foss.gui.creating.waybillSubmitAction.exception.validateFee.one"));
		}
	}
	...
	private void validateTransportFee(WaybillPanelVo bean) {

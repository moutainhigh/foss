 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/changing/client/ui/internal/BillingPayPanel.java
 * 
 * FILE NAME        	:java
 * 
 * AUTHOR			: FOSS接送货系统开发组
...
import com.deppon.foss.module.pickup.changing.client.action.CalculateAction;
...
public class BillingPayPanel extends JPanel {
	 ...
	 */
	private void init() {
		setBorder(new JDTitledBorder(i18n.get("pickup.changing.billingPayPanel")));
		...
		JLabel lblIncrementCharge = new JLabel(
				i18n.get("pickup.changing.incrementCharge"));
		...
		txtIncrementCharge.setColumns(NumberConstants.NUMBER_10);

		JLabel lblPromotionCharge = new JLabel(
				i18n.get("pickup.changing.promotionCharge"));
		...
		txtPromotionCharge.setColumns(NumberConstants.NUMBER_10);
		
		lblAdjustCharge = new JLabel(i18n.get("pickup.changing.adjustCharge")+"：");
		add(lblAdjustCharge, "14, 1, right, default");
		
		txtAdjustCharge = new JTextFieldValidate();
		add(txtAdjustCharge, "16, 1, fill, default");
		txtAdjustCharge.setColumns(NumberConstants.NUMBER_10);

		JLabel lblTotalCharge = new JLabel(
				i18n.get("pickup.changing.totalCharge"));
		add(lblTotalCharge, "18, 1, right, default");

		txtTotalCharge = new JTextFieldValidate();
		add(txtTotalCharge, "20, 1, fill, default");
		txtTotalCharge.setDocument(new NumberDocumentUtils(TEN));
		txtTotalCharge.setColumns(NumberConstants.NUMBER_10);

		JLabel lblPaymentMode = new JLabel(
				i18n.get("pickup.changing.paymentMode"));
		 ...
		 * @date:2015-01-24
		 */
		lblNewLabel_1 = new JLabel("交易流水号：");
		add(lblNewLabel_1, "6, 3, right, default");
		...
		transactionSerialNumber.setColumns(NumberConstants.NUMBER_10);
		transactionSerialNumber.setEnabled(false);
		transactionSerialNumber.setDocument(number);

		JLabel lblArrivePayment = new JLabel(
				i18n.get("pickup.changing.arrivePayment"));
		add(lblArrivePayment, "10, 3, right, default");

		txtArrivePayment = new JTextFieldValidate();
		add(txtArrivePayment, "12, 3, fill, default");
		txtArrivePayment.setDocument(new NumberDocumentUtils(NumberConstants.NUMBER_9));
		txtArrivePayment.setColumns(NumberConstants.NUMBER_10);

		JLabel lblCashPayment = new JLabel(
				i18n.get("pickup.changing.cashPayment"));
		add(lblCashPayment, "14, 3, right, default");

		txtCashpayment = new JTextFieldValidate();
		add(txtCashpayment, "16, 3, fill, default");
		txtCashpayment.setDocument(new NumberDocument(TEN));
		txtCashpayment.setColumns(NumberConstants.NUMBER_10);

		panel = new JPanel();
		add(panel, "18, 3, 3, 1, fill, fill");
		panel.setLayout(new FlowLayout(FlowLayout.LEFT, 0, 0));

		chbSecrecy = new JCheckBox(i18n.get("pickup.changing.secrecy"));
		...
		//计算总运费
		btnCalculate = new JButton(i18n.get("pickup.changing.calculateTotalFee"));

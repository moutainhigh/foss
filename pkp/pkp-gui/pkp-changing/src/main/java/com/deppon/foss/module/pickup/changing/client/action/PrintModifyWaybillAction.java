 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/changing/client/action/PrintModifyWaybillAction.java
 * 
...
import com.deppon.foss.module.pickup.changing.client.ui.dialog.RFCPrintChoiceDiaglogNew;
...
public class PrintModifyWaybillAction implements IButtonActionListener<RFCPrintChoiceDiaglogNew> {
	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			if (ui.list.getSelectedIndex() == 0) {
				...
				// 提示请选择打印模版
				JOptionPane.showMessageDialog(null, i18n.get("foss.gui.changing.printModifyWaybillAction.choosePrintTemplates.exception"), i18n.get("foss.gui.changing.printModifyWaybillAction.choosePrintTemplates.inputError"),
	...
	private void dataInsert(JasperContext jctx, WaybillRfcPrintVo resourceBean) {
		...
		if (waybillInfo.getPaper() != null && waybillInfo.getPaper() > 0) {
			packageInfo += waybillInfo.getPaper() + i18n.get("foss.gui.changing.printModifyWaybillAction.packageInfo.papper");
		}
		//获得多少木包装
		if (waybillInfo.getWood() != null && waybillInfo.getWood() > 0) {
			packageInfo += waybillInfo.getWood() + i18n.get("foss.gui.changing.printModifyWaybillAction.packageInfo.wood");
		}
		//获得多少 纤包装
		...
		if (waybillInfo.getSalver() != null && waybillInfo.getSalver() > 0) {
			packageInfo += waybillInfo.getSalver() + i18n.get("foss.gui.changing.printModifyWaybillAction.packageInfo.salver");
		}
		//获得多少膜包装
		if (waybillInfo.getMembrane() != null && waybillInfo.getMembrane() > 0) {
			packageInfo += waybillInfo.getMembrane() + i18n.get("foss.gui.changing.printModifyWaybillAction.packageInfo.membrane");
		}
		//其他
		if (waybillInfo.getOtherPackage() != null) {
			packageInfo += waybillInfo.getOtherPackage() + i18n.get("foss.gui.changing.printModifyWaybillAction.packageInfo.otherPackage");
		}
...
//		jctx.put("size", (waybillInfo.getGoodsSize() != null) ? waybillInfo.getGoodsSize() : "-");// 货物尺寸
		...
		//新增需求
		if(resourceBean.getWaybillRfcChangeDetailDtos().size()<= EIGHT){
			jctx.put("halfTipLine", i18n.get("foss.gui.changing.printModifyWaybillAction.halfTipLine.label"));
		}else{
			jctx.put("halfTipLine", "");
		}

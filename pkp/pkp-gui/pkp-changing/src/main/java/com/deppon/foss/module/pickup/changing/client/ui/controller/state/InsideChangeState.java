 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/changing/client/ui/controller/state/InsideChangeState.java
 * 
 * FILE NAME        	: InsideChangeState.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class InsideChangeState extends AbstractState {
	/**
	 *  国际化
	 */
	...
	public InsideChangeState(WaybillRfcStateContext context) {
		super(context);
		//初始化内部页面
		init();
	}
	 ...
	 * 初始化内部页面
	 * @author 102246-foss-shaohongliang
	 * @date 2012-12-24 下午9:04:27
	 */
	private void init() {
		//运单基础信息面板
		basicPanel = getWaybillRfcUI().getWaybillInfoPanel().getBasicPanel();
		//收货客户信息面板
	...
	protected void updateComponents(WaybillRFCUI rfcUI) {
		...
		rfcUI.getImportPanel().getTxtRfcReason().setVisible(true);
		rfcUI.getImportPanel().getTxtCustomerRfcReason().setVisible(false);
...
//			getWaybillRfcUI().getWaybillInfoPanel().getBasicPanel().getCboFreePickupGoods().setEnabled(false);
		...
		//设置运输信息目的站不可编辑
		if(BZPartnersJudge.IS_PARTENER && BZPartnersJudge.DETINATION_STATION_RFC){
			rfcUI.getWaybillInfoPanel().getTransferPanel().getTransportInfoPanel().getTxtDestination().setEnabled(true);
		}else{
			rfcUI.getWaybillInfoPanel().getTransferPanel().getTransportInfoPanel().getTxtDestination().setEnabled(false);
		}
...
//		}
		 ...
		 * 225131
		 */
		basicPanel.getInternalDeliveryType().setEnabled(true);
		basicPanel.getTxtStaffNumber().setEnabled(true);
		
		//发货联系人不可编辑，置灰
		consignerPanel.getTxtConsignerLinkMan().setEnabled(false);
		//安装费可用
		incrementPanel.getBtnInstall().setEnabled(true);
...
//				ui.getWaybillInfoPanel().getTransferPanel().getTransportInfoPanel().getCombReturnBillType().setEnabled(false);
			...
			//zxy 20140626 JZDP start 新增:内部更改 开单部门库存可以修改精准大票，
			//						非开单部门库存，原精准可修改为非精准大票，原非精准大票不能修改精准大票
			//getWaybillRfcUI().getWaybillInfoPanel().getBasicPanel().getCboBigGoodsModel().setEnabled(waybillVo.getIsBigGoods());
			...
			if (ProductEntityConstants.PRICING_PRODUCT_LONG_DISTANCE_FAST_FREIGHT.equals(procuctCode)
				...
				getWaybillRfcUI().getWaybillInfoPanel().getTransferPanel().getTransportInfoPanel().getCombReturnBillType().setEnabled(true);
				//BUG BUG-6320
			...
			//若为特殊增值服务，则不允许修改运单号
			if(waybillVo!=null&&waybillVo.getSpecialValueAddedServiceType()!=null&&waybillVo.getSpecialValueAddedServiceType().getValueCode()!=null){
				basicPanel.getCboRfcWaybillNo().setEnabled(false); // 变更单号
		}
			if(waybillVo!=null&&waybillVo.getSpecialValueAddedServiceType()==null){
				basicPanel.getSpecialValueAddedServiceType().setEnabled(false);
			}else{
				basicPanel.getSpecialValueAddedServiceType().setEnabled(true);
		    }
		}
	}
	...
	protected JComponent getTransferInjectPanel() {
		...
		if(waybillVo.getTransferRecordList().size()>0 || waybillVo.getReturnRecordList().size()>0){
			//根据BUG-6430有变更记录，增加运输信息变更、变更记录页签
			return getWaybillRfcUI().getWaybillInfoPanel().getTransferPanel().getTransportRecordTabPanel();
		}else{
	...
	protected List<UploadVoucherDto> getUploadVoucherDtos() {
		...
		vo.setProofName(i18n.get("pickup.changing.waybillOriginal"));
	 ...
	 * 	2	单号	可以变更	不能变更	
	 * 发货客户信息	3	客户名称	可以变更	可以变更	
	 * 	4	发货联系人	可以变更	可以变更	
	 * 	5	手机	可以变更	不能变更	和发货联系人绑定变化，不能单独修改
	 * 	6	电话	可以变更	不能变更	和发货联系人绑定变化，不能单独修改
	 * 	7	地址	可以变更	不能变更	和发货联系人绑定变化，不能单独修改
	 ...
	 * 	16	对外备注	可以变更	可以变更	
	 * 	17	对内备注	可以变更	可以变更	
	 * 货物信息	18	名称	可以变更	可以变更	
	 * 	19	包装	可以变更	可以变更	
	 * 	20	件数	可以变更	可以变更	
	 * 	21	重量	可以变更	可以变更	
	 * 	22	体积	可以变更	可以变更	
	 * 增值业务信息	23	保价声明价值	可以变更	可以变更	
	 * 	24	代收货款	可以变更	可以变更	
	...
	private void insideFullVehicleReceiveOut(WaybillInfoVo waybillVo) {
		consignerPanel.getTxtConsigerName().setEnabled(true); // 客户名称
...
//		goodsPanel.getTxtPaper().setEnabled(true); // 包装
		...
		//来自官网的订单才能能设置
		if(!WaybillConstants.CRM_ORDER_CHANNEL_ONLINE.equals(waybillVo.getWaybillDto().getWaybillEntity().getOrderChannel())){
...
//		incrementPanel.getTxtPayee().setEnabled(true); // 收款人
//		incrementPanel.getTxtBankAccount().setEnabled(true); // 收款账号
...
//                    || WaybillConstants.AIR_SELF_PICKUP.equals(code)
		...
		//zxy 20131113 BUG-58342 start 修改：整车业务“其他费用” 不能修改
		if (waybillVo.getIsWholeVehicle()!=null && waybillVo.getIsWholeVehicle()){
			incrementPanel.getBtnAdd().setEnabled(false);
			incrementPanel.getBtnDelete().setEnabled(false);
			if(waybillVo.getSpecialValueAddedServiceType()!=null)
			{
				incrementPanel.getBtnInstall().setEnabled(false);
			}
			
		}else{
			incrementPanel.getBtnAdd().setEnabled(true);
			incrementPanel.getBtnDelete().setEnabled(true);
			if(waybillVo.getSpecialValueAddedServiceType()!=null)
			{
				incrementPanel.getBtnInstall().setEnabled(true);
			}
		}
		//zxy 20131113 BUG-58342 end 修改：整车业务“其他费用” 不能修改
		billingPayPanel.getCombPaymentMode().setEnabled(true); // 付款方式
		billingPayPanel.getTxtArrivePayment().setEnabled(true); // 到付金额
		billingPayPanel.getTxtCashpayment().setEnabled(true); // 预付金额
		billingPayPanel.getBillingPayBelongPanel().getTxtPublicCharge().setEnabled(true); // 运费
	 ...
	 * 
	 * @author 102246-foss-shaohongliang
	 * @date 2012-12-24 下午9:01:49
	 */
	private void insideFullVehicleReceiveIn(WaybillInfoVo waybillVo) {
		insideFullVehicleReceiveOut(waybillVo);
		
		basicPanel.getCboRfcWaybillNo().setEnabled(true); // 变更单号
	 ...
	 * 偏线出库规则
	 * 
	 * 收货部门		可以变更		接货开单组可更改，非集中接货开单组在任何环节都不能发起变更
	 * 变更单号	不能变更			单号变更生效后红冲财务数据，变更后生成新的财务数据并于新单号进行绑定
	 * 上门接货	可以变更			只能由非上门接货更改为上门接货
	 * 客户名称	可以变更
	 * 联系人	可以变更
	 * 手机		不能变更			和发货联系人绑定变化，不能单独修改
	 * 电话		不能变更			和发货联系人绑定变化，不能单独修改
	 * 地址		不能变更			和发货联系人绑定变化，不能单独修改
	 * 联系人	可以变更
	 * 手机		可以变更
	 * 电话		可以变更
	 * 地址		可以变更
	...
	private void insidePartialReceiveOut(WaybillInfoVo waybillVo) {
		...
		//集中接货开单组可更改 
		if (waybillVo.getPickupCentralized()) {
			basicPanel.getBtnSearch().setEnabled(true); // 收货部门
		}else if(waybillVo.getPickupToDoor()==null || !waybillVo.getPickupToDoor()){
			//只能由非上门接货改为上门接货
			basicPanel.getCboReceiveModel().setEnabled(true); // 上门接货
		}
		consignerPanel.getTxtConsigerName().setEnabled(true); // 客户名称
		consignerPanel.getBtnQuery().setEnabled(true); // 客户名称查询
		...
		//如果有运输变更记录，只有返单类别，对外备注，对内备注能修改
	
		if(waybillVo.getReturnRecordList().size()> 0 || waybillVo.getTransferRecordList().size()> 0){
			transferPanel.getTransportInfoPanel().getCombReturnBillType().setEnabled(true); // 返单类别
			transferPanel.getTransportInfoPanel().getCombOutSideRemark().setEnabled(true); // 对外备注
			transferPanel.getTransportInfoPanel().getTxtInsideRemark().setEnabled(true); // 对内备注	
		}else{
			// 除货物状态其余均可编辑
			enableUI(transferPanel);
			transferPanel.getTransportInfoPanel().getCombGoodsStatus().setEnabled(false);
			...
			transferPanel.getTransportInfoPanel().getTxtPickBranch().setEnabled(false);
		}
...
//                    || WaybillConstants.AIR_SELF_PICKUP.equals(code)
	 ...
	 * 
	 * 收货部门	可以变更			接货开单组可更改，非集中接货开单组在任何环节都不能发起变更
	 * 变更单号	可以变更			单号变更生效后红冲财务数据，变更后生成新的财务数据并于新单号进行绑定
	 * 上门接货	可以变更			只能由非上门接货更改为上门接货
	 * 客户名称	可以变更
	 * 联系人	可以变更		
	 * 手机		可以变更
	...
	private void insideAirReceiveOut(WaybillInfoVo waybillVo) {
		...
		//如果有运输变更记录，只有返单类别，对外备注，对内备注能修改
		if(waybillVo.getTransferRecordList().size()>0 || waybillVo.getReturnRecordList().size()>0){
			transferPanel.getTransportRecordTabPanel().getTransferInfoPanel().getCombTransferPredictFlight().setEnabled(true);
			transferPanel.getTransportRecordTabPanel().getTransferInfoPanel().getCombFreightMethod().setEnabled(true);
		}else{
	...
	private void insideTruckReceiveOut(WaybillInfoVo waybillVo) {
		...
		//如果有运输变更记录，只有返单类别，对外备注，对内备注能修改
		if(waybillVo.getTransferRecordList().size()>0 || waybillVo.getReturnRecordList().size()>0){
			transferPanel.getTransportRecordTabPanel().getTransferInfoPanel().getCombTransferPredictFlight().setEnabled(false);
			transferPanel.getTransportRecordTabPanel().getTransferInfoPanel().getCombFreightMethod().setEnabled(false);
		}else{
			transferPanel.getTransportInfoPanel().getCombPredictFlight().setEnabled(false);
			transferPanel.getTransportInfoPanel().getCombFreightMethod().setEnabled(false);
			transferPanel.getTransportInfoPanel().getTxtFlightShift().setEnabled(false);
	 ...
	 * 客户名称		可以变更
	 * 联系人		可以变更
	 * 手机			可以变更		和发货联系人绑定变化，不能单独修改
	 * 电话			可以变更		和发货联系人绑定变化，不能单独修改
	 * 地址			可以变更		和发货联系人绑定变化，不能单独修改
	 * 联系人		可以变更
	 * 手机			可以变更
	 * 电话			可以变更
	 * 地址			可以变更

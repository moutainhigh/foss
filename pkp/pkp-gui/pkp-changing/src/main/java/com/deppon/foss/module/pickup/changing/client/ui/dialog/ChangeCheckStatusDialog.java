 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/changing/client/ui/dialog/ChangeCheckStatusDialog.java
 * 
...
public class ChangeCheckStatusDialog extends JDialog {
		...
		btnAgree.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (isChangeOk) {
						MsgBox.showInfo(i18n.get("foss.gui.changing.changeCheckStatusDialog.msgBox.auditSuccess"));
						setVisible(false);
						waybillRFCCheckUI
								.waybillRFCCheckUITableRefresh(selectedRow);
					} else {
						MsgBox.showInfo(i18n.get("foss.gui.changing.changeCheckStatusDialog.msgBox.auditFailure"));
					}
				} catch (Exception e1) {
	 ...
	 */
	private void init() {
		setTitle(i18n.get("foss.gui.changing.changeCheckStatusDialog.init.auditRFCBill.label"));
		...
		JLabel lblNewLabel = new JLabel(i18n.get("foss.gui.changing.changeCheckStatusDialog.init.auditRemark.label"));
	...
	private void setComponentState() {
		...
		if (StringUtil.isEmpty(waybillRfcChangeDto.getCheckStatus())) {
			textField.setText("");
		} else {

 ...
 *
 */
public class ExpLoadPickupStationAction  implements IButtonActionListener<ExpWaybillEditUI>{

	
	/**
	 * 日志
	 */
	private static final Log LOG = LogFactory.getLog(ExpLoadPickupStationAction.class);
	
	/**
	 * 国际化
	 */
	private static final II18n i18n = I18nManager.getI18n(ExpLoadPickupStationAction.class);
...
//				setGoodsTypeAB(bean);
	...
	private void validateProduct(WaybillPanelVo bean,BranchQueryVo branchQueryVo)
	{
		...
		}else if(CommonUtils.directDetermineIsExpressByProductCode(productVo.getCode())){
			//这里要对小件做判断
	...
	private BranchVo getBranchVo(BranchQueryVo branchQueryVo){
		...
		if (WaybillConstants.ZX.equals(branchQueryVo.getBranchType())) {
			SaleDepartmentEntity saleDepartment = WaybillServiceFactory.getSalesDepartmentService().querySaleDepartmentByCode(branchQueryVo.getCode());
			BeanUtils.copyProperties(saleDepartment, vo);
			...
			//查询代理信息
			OuterBranchEntity outerBranch = WaybillServiceFactory.getOuterBranchService().queryAgencyBranchInfo(branchQueryVo.getCode());
			//代理编号
	...
	private void setCanAgentCollectedOrServiceFee(BranchVo branchVO,WaybillPanelVo bean) {
		 ...
		 */
		if(branchVO.getCityCode()!=null&&"810100".equals(branchVO.getCityCode()))
		{
			 ...
			 */
		}else if(bean.getReceiveOrgProvCode()!=null&&"810000".equals(bean.getReceiveOrgProvCode()))
		{
			setCanAgentCollectedOrServiceFeeFalse(bean);
		}
	}

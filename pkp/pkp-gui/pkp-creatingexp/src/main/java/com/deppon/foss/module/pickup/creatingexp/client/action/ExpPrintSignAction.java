 ...
 * @version
 */
public class ExpPrintSignAction implements IButtonActionListener<ExpPrintSignUI> {

	// 日志
	private static final Log LOG = LogFactory.getLog(ExpPrintSignAction.class);

	// 国际化
	private static final II18n i18n = I18nManager.getI18n(ExpPrintSignAction.class);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (WaybillConstants.NOT_RETURN_BILL.equals(waybill.getReturnBillType()) || StringUtils.isEmpty(waybill.getReturnBillType())) {
		...
		/*
		 * if
		 * (!user.getEmployee().getOrgCode().equals(waybill.getReceiveOrgCode(
		 * ))) { MsgBox.showInfo(i18n.get(
		 * "foss.gui.creating.printSignAction.MsgBox.notOrgWaybill")); return; }
		 */
		...
		try {
			String inputValue = JOptionPane.showInputDialog(i18n.get("foss.gui.creating.printSignAction.MsgBox.inputPrintCounts"));
			ctx.put("prtpiece", inputValue);
			ctx.put("destination", ui.getTxtDestnationOrg().getText());
			...
			// ctx.put("username", user.getEmployee().getDepartment().getName()+
			// user.getEmpName());
			...
			JOptionPane.showMessageDialog(null, exp.toString(), i18n.get("foss.gui.creating.waybillEditUI.common.error"), JOptionPane.WARNING_MESSAGE);
		}
	}

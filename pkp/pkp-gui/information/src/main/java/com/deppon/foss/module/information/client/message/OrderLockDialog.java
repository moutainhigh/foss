 ...
 * @version
 */
public class OrderLockDialog extends JDialog {
	private JButton btnUnlock;
	private JLabel lbContext;
	private static OrderLockDialog instance;
	private boolean status=false;//0为解锁，1为锁屏
	
	private static final II18n i18n = I18nManager.getI18n(OrderLockDialog.class);
	
	public OrderLockDialog() {
    ...
    public OrderLockDialog(Frame owner, boolean modal,long n,final String orgCode) {
    	
     	super(ApplicationContext.getApplication().getWorkbench().getFrame(), true);
     	JPanel panel = new JPanel();
		getContentPane().add(panel, BorderLayout.SOUTH);
		btnUnlock = new JButton("申请解锁");
		panel.add(btnUnlock);
		JPanel panel1 = new JPanel();
		getContentPane().add(panel1, BorderLayout.CENTER);
		
		lbContext = new JLabel(i18n.get("com.deppon.foss.module.information.client.message.lock.message", new Object[]{n}));
        ...
        String isUnlock="Y";
        try{
			 ConfigurationParamsEntity unLockCF= getConfigurationParamsEntity(ConfigurationParamsConstants.PKP_FOSS_CAN_UNLOCK);//是否可以解锁
	      if(unLockCF!=null&&StringUtils.isNotEmpty(unLockCF.getConfValue()))
			 {
	    	  isUnlock = unLockCF.getConfValue();
			 }
        ...
        lbContext.setText(i18n.get("com.deppon.foss.module.information.client.message.lock.message", new Object[]{n}));
        if(FossConstants.ACTIVE.equals(isUnlock)){
		...
		btnUnlock.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String unifiedCode =orderLockHessianRemoting.queryUnifiedCode(orgCode);
				OrderLockEntity orderLockEntity =orderLockHessianRemoting.queryOrderLockByDeptCodeForSyn(unifiedCode);
	...
	public static OrderLockDialog getInstance(Frame owner, boolean modal,long n,String orgCode){
		if(instance==null){
			instance = new OrderLockDialog(owner,modal,n,orgCode);

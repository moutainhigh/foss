 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvMakEventExOrgSaver.java
 * 
 * FILE NAME        	: SrvMakEventExOrgSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvMakEventExOrgSaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return MakEventExOrgEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<MakEventExOrgEntity> list = (List<MakEventExOrgEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for(MakEventExOrgEntity goodsType: list){
				MakEventExOrgService makEventExOrgService = DownLoadDataServiceFactory
						.getMakEventExOrgService();
				makEventExOrgService.saveOrUpdate(goodsType);
				Long version = goodsType.getVersionNo();
				//获得最大版本号

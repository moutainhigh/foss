 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/service/impl/RuleVerifyService.java
 * 
 * FILE NAME        	: RuleVerifyService.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 * </p>
 * <p style="display:none">
 * version:V1.0,author:Bobby,date:2012-10-16 上午11:31:45,
 * </p>
 * 
 * @author Bobby
 * @date 2012-10-16 上午11:31:45
 * @since
 * @version
 *  *  * 录入货物信息SUC业务规则
 ...
 *
 */
public class RuleVerifyService implements IRuleVerifyService {

	/**
	 * 1000
	 */
	private static final int ONETHSOUSAND = 1000;
	 ...
	 * 国际化
	 */
	private static final II18n i18n = I18nManager.getI18n(RuleVerifyService.class);	
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-10-23 下午4:29:28
	 * @param weight
	 * @param volume
	 * @return
	 * @see com.deppon.foss.module.pickup.common.client.service.IDataDictionaryService#isVehicleDirect(java.lang.String,
	 *      java.lang.String)
	 */
	public boolean isVehicleDirect(String weight, String volume) {
		...
		if (weight != null && weight.length() > 0) {
			...
			sysConfig = sysConfigService.queryByConfCode(confCode, null);
			if (sysConfig != null) {
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-10-24 下午3:10:44
	 * @param goodsNum
	 * @param goodsVolume
	 * @param goodsValue
	 * @return
	 * @see com.deppon.foss.module.pickup.common.client.service.IDataDictionaryService#isValueGoods(int,
	...
	public boolean isValueGoods(int goodsNum, String goodsVolume,String goodsValue) {
		...
		ConfigurationParamsEntity sysConfig = sysConfigService.queryByConfCode(WaybillConstants.VALUEGOODS_WAYBILL_PRICE, null);
		/**
		 * 判断单票货物平均单件保价大于等于10万元；
		 */
		if (sysConfig != null && goodsNum != 0 && (Double.parseDouble(goodsValue)/goodsNum) >= Double.parseDouble(sysConfig.getConfValue())) {
			retVal = true;
		} else {
			if (goodsNum == 1) {
				confCodes[0] = WaybillConstants.VALUEGOODS_SINGLE_VOLUME;
				confCodes[1] = WaybillConstants.VALUEGOODS_SINGLE_PRICE;
				sysConfigs = sysConfigService.queryByConfCodeArray(confCodes, null);
				if (sysConfigs != null && sysConfigs.size() > 0) {
					...
					if (volume != null&& value != null&& Double.parseDouble(goodsVolume) <= Double.parseDouble(volume)&& Double.parseDouble(goodsValue) >= Double.parseDouble(value)) {
						retVal = true;
					}
				}
			} else {
				...
				if (sysConfigs != null && sysConfigs.size() > 0) {
					...
					if (volume != null&& value != null && (Double.parseDouble(goodsVolume) / goodsNum) <= Double.parseDouble(volume) && (Double.parseDouble(goodsValue) / goodsNum) >= Double.parseDouble(value)) {
						retVal = true;
					}
				}
			}
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-10-29 上午11:27:28
	 * @param goodsWeight
	 * @param goodsVolume
	 * @return
	 * @see com.deppon.foss.module.pickup.common.client.service.IRuleVerifyService#isWeightByVolume(java.lang.String,
	 *      java.lang.String, java.lang.String)
	 */
	public boolean isWeightByVolume(String goodsWeight, String goodsVolume) {
		boolean retVal = false;
		String confValueBegin;
		String confValueEnd;
		if ((goodsWeight != null && goodsWeight.length() > 0)
				&& (goodsVolume != null && goodsVolume.length() > 0)) {
			confValueBegin = queryConfValue("weightbyvolumebegin");
			confValueEnd = queryConfValue("weightbyvolumeend");
			if (confValueBegin != null && confValueEnd != null
					&&Double.parseDouble(confValueBegin) < (Double.parseDouble(goodsWeight) / Double.parseDouble(goodsVolume))
					&& Double.parseDouble(confValueEnd) > (Double.parseDouble(goodsWeight) / Double.parseDouble(goodsVolume))) {
					retVal = true;
	 ...
	 * 
	 * <p>
	 * 判断单票货物或单件货物体积和重量是否在到达营业部允许的承运范围内
	 * </p>
	 * 
	 * @author foss-sunrui
	 * @date 2012-10-29 下午1:54:06
	 * @param salesDeptId
	 * @param goodsNum
	 * @param goodsWeight
	 * @param goodsVolume
	 * @return
	 * @see com.deppon.foss.module.pickup.common.client.service.IRuleVerifyService#verifyGoodsByDept(java.lang.String,
	 *      int, java.lang.String, java.lang.String)
	 */
	public ResultDto verifyGoodsByDept(String salesDeptId, int goodsNum,
			String goodsWeight, String goodsVolume) {
		String code = "1";
		String msg = "";
		SaleDepartmentEntity saleDpet = salesDepartmentDao
				.queryByCode(salesDeptId);
		if ((saleDpet.getSingleBillLimitkg().doubleValue() / ONETHSOUSAND) < Double
				.parseDouble(goodsWeight)) {
			code = "0";
			msg = i18n.get("foss.gui.common.ruleVerifyService.message.overGoodsWeight");
		} else if ((saleDpet.getSingleBillLimitvol().doubleValue() / ONETHSOUSAND) < Double
				.parseDouble(goodsVolume)) {
			code = "0";
			msg = i18n.get("foss.gui.common.ruleVerifyService.message.overGoodsVolume");
		} else if ((saleDpet.getSinglePieceLimitkg().doubleValue() / ONETHSOUSAND) < (Double
				.parseDouble(goodsWeight) / goodsNum)) {
			code = "0";
			msg = i18n.get("foss.gui.common.ruleVerifyService.message.overSingleGoodsWeight");
		} else if ((saleDpet.getSinglePieceLimitvol().doubleValue() / ONETHSOUSAND) < (Double
				.parseDouble(goodsVolume) / goodsNum)) {
			code = "0";
			msg = i18n.get("foss.gui.common.ruleVerifyService.message.overSingleGoodsVolume");
		}
	 ...
	 * 
	 * <p>
	 * 通过常量定义的key值获取系统配置项的值
	 * </p>
	 * 
	 * @author foss-sunrui
	 * @date 2012-10-29 上午11:36:32
	 * @param keyValue
	 * @return
	 * @see
	 */
	private String queryConfValue(String keyValue) {
		//注入远程Remoting方法
		waybillHessianRemoting = DefaultRemoteServiceFactory.getService(IWaybillHessianRemoting.class);
		String confCode = "";
		if("weightbyvolumebegin".equals(keyValue)){
			confCode = WaybillConstants.WEIGHT_BY_VOLUME_BEGIN;
		}else if("weightbyvolumeend".equals(keyValue)){
			confCode = WaybillConstants.WEIGHT_BY_VOLUME_END;
		}
		ConfigurationParamsEntity sysConfig = null;
		// 在线开单从远程取，离线开单从本地取 by 卢飞飞
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			sysConfig = waybillHessianRemoting.queryByConfCode(confCode, null);
		}else{
			sysConfig = sysConfigService.queryByConfCode(confCode, null);			
		}

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvEffectivePlanSaver.java
 * 
 * FILE NAME        	: SrvEffectivePlanSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvEffectivePlanSaver extends AbstractSyncDataSaver {
	//区域id
	...
	public Class<?> getSaveType() {
		return EffectivePlanEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		...
		List<Map<String, List<EffectivePlanEntity>>> list = (List<Map<String, List<EffectivePlanEntity>>>) data;
		if (list != null && !list.isEmpty()) {
			Map<String, List<EffectivePlanEntity>> map =  (Map)list.get(0);
			for (Entry<String, List<EffectivePlanEntity>> entry : map.entrySet()) {
				List<EffectivePlanEntity> dataList = (List<EffectivePlanEntity>) entry
						.getValue();
				EffectivePlanService effePlanService = DownLoadDataServiceFactory
				...
				for (EffectivePlanEntity effectivePlan : dataList) {
					effePlanService.saveOrUpdate(effectivePlan);
					regionID = effectivePlan.getDeptRegionId();
					Long version = effectivePlan.getVersionNo();
					//获得最大版本号

...
import com.deppon.foss.module.pickup.common.client.service.ISrvPriceArriveRegionService;
 ...
 * 
 * 
 * @ClassName: SrvPriceArriveRegionSaver
 * 
 * @Description: BUG-55198 到达区域实体
 * 
 ...
 * 
 * @date 2013-10-8 上午11:03:12
 * 
 * 
 */
public class SrvPriceArriveRegionSaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return PriceRegionArriveEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<PriceRegionArriveEntity> list = (List<PriceRegionArriveEntity>) data;
		if (list != null && !list.isEmpty()) {
			ISrvPriceArriveRegionService srvPriceArriveRegionService = DownLoadDataServiceFactory
					.getSrvPriceArriveRegionService();
			for (PriceRegionArriveEntity entity : list) {
				if (FossConstants.YES.equals(entity.getActive())) {
					srvPriceArriveRegionService.saveOrUpdate(entity);
				} else {
					srvPriceArriveRegionService.deleteID(entity.getId());
				}

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasUserOrgRoleSaver.java
 * 
 * FILE NAME        	: BasUserOrgRoleSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasUserOrgRoleSaver  extends AbstractSyncDataSaver{
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return UserOrgRoleEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<UserOrgRoleEntity> list = (List<UserOrgRoleEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			UserOrgRoleService userOrgRoleService = DownLoadDataServiceFactory
					.getUserOrgRoleService();
			for(UserOrgRoleEntity userOrgRole: list){
				if(	FossConstants.YES.equals(userOrgRole.getActive())){	
					userOrgRoleService.saveOrUpdate(userOrgRole);
					
				}else{
					userOrgRoleService.delete(userOrgRole);
					
				}
				
				Long version = userOrgRole.getVersion();

 ...
 *
 */
public class BasSalesDescExpandSaver  extends AbstractSyncDataSaver   {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return SalesDescExpandEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<SalesDescExpandEntity> list = (List<SalesDescExpandEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for(SalesDescExpandEntity descEntity: list){
				SalesDescExpandsService service = DownLoadDataServiceFactory
						.getSalesDescExpandsService();
				if(	FossConstants.YES.equals(descEntity.getActive())){	
					service.saveOrUpdate(descEntity);
				}else{
					service.delete(descEntity);
					
				}
			
				Long version = descEntity.getVersionNo();

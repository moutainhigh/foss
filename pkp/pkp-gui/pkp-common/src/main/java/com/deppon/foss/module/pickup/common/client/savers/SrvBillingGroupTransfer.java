...
import com.deppon.foss.module.pickup.common.client.service.impl.BillingGroupTransFerService;
 ...
 *
 */
public class SrvBillingGroupTransfer  extends AbstractSyncDataSaver{
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return BillingGroupTransFerEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		...
		List<BillingGroupTransFerEntity> list =  (List<BillingGroupTransFerEntity>) data;
		long mxVersion = 0;
		if(CollectionUtils.isNotEmpty(list)){
			BillingGroupTransFerService billingGroupTransFerService = DownLoadDataServiceFactory.getBillingGroupTransFerService();
			
			for(BillingGroupTransFerEntity  entity : list){
				if(	FossConstants.YES.equals(entity.getActive())){
					billingGroupTransFerService.saveOrUpdate(entity);
					
				}else{
					billingGroupTransFerService.delete(entity);
				}

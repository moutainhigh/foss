 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/service/impl/CustomerService.java
 * 
...
public class CustomerService implements ICustomerService {
	...
	private ICustomerHessianRemoting customerRemotingService = DefaultRemoteServiceFactory.getService(ICustomerHessianRemoting.class);
	...
	public List<CustomerContactDto> queryCustomerInfo(CustomerQueryConditionDto condition) {
		return customerRemotingService.queryCustomerInfo(condition);
	}
	
	/**
	 * 将CustomerContactDto转换为QueryMemberDialogVo类型
	 * @author 026123-foss-lifengteng
	 * @date 2012-11-28 下午10:42:01
	 */
	public List<QueryMemberDialogVo> convertToVo(List<CustomerContactDto> customerList) {
		// 判空操作
		if (customerList == null) {
			return null;
		}

		List<QueryMemberDialogVo> dialogList = new ArrayList<QueryMemberDialogVo>();
		// 对返回的值根据业务需要进行封装
		for (CustomerContactDto customerDto : customerList) {
			// 客户信息
			QueryMemberDialogVo dialogVo = new QueryMemberDialogVo();
			dialogVo.setCustomerCode(customerDto.getCustCode());
			dialogVo.setCustomerName(customerDto.getCustName());
			dialogVo.setCustId(StringUtil.defaultIfNull(customerDto.getCustId()));//客户ID

			Boolean chargeMode = DictionaryValueConstants.CLEARING_TYPE_MONTH.equals(customerDto.getChargeMode()) ? true : false;
			dialogVo.setChargeMode(Boolean.valueOf(chargeMode));
			dialogVo.setCreditAmount(customerDto.getCreditAmount());

			// 联系人信息
			dialogVo.setMobilePhone(customerDto.getMobilePhone());
			dialogVo.setPhone(customerDto.getOfficePhone());
			dialogVo.setAddress(customerDto.getAddress());
			
			//所属部门名称
			dialogVo.setDeptName(customerDto.getDeptName());

			// 将数据封装到list中
			dialogList.add(dialogVo);
		}
		return dialogList;
	}
	...
	public List<CustomerContactDto> convertToDto(CustomerDto customer) {
		...
		if(CollectionUtils.isNotEmpty(contactList)){
			for (ContactEntity contact : contactList) {
				//客户ID
				customerContact.setCustId(customer.getId());
				//客户地址
				...
				//客户所属部门名称
				customerContact.setDeptName(customer.getDeptname());
	...
	public List<CustomerContactDto> queryCustInfoByPhone(CustomerQueryConditionDto conditionDto) {
		// 返回联系人信息
		List<CustomerContactDto> contactList = new ArrayList<CustomerContactDto>();
		// 手机
		String mobilePhone = conditionDto.getMobilePhone();
		// 电话
		String telePhones = conditionDto.getContactPhone();
		// 解析电话号码
		List<String> telePhoneList = StrUtils.resolvePhone(telePhones);
		...
		// 判断：若电话号码与手机号码同时有时，忽略电话号码的查询条件
		if ((!"".equals(mobilePhone.trim()) && !"".equals(telePhones.trim()))
				||!"".equals(mobilePhone)) {
			CustomerDto customer = customerRemotingService.queryCustInfoByMobile(mobilePhone,null);
			if (customer != null) {
				contactList = convertToDto(customer);
			}
		} else if (!"".equals(telePhones.trim())) {
			List<CustomerDto> list = customerRemotingService.queryCustInfoByPhone(telePhoneList,null);
			if (list == null || list.size() == 0) {
				return null;
			}
			for (CustomerDto customerDto : list) {
				boolean isSucceed = contactList.addAll(convertToDto(customerDto));
				if (!isSucceed) {
					return null;
				}
			}
	...
	public List<CustomerContactDto> queryHisDeliveryCusByPhone(CustomerQueryConditionDto conditionDto) {
		List<HisDeliveryCusEntity> entityList = customerRemotingService.queryHisDeliveryCusByPhone(conditionDto);
		...
		for (HisDeliveryCusEntity entity : entityList) {
			...
			dto.setOfficePhone(entity.getDeliveryCustomerPhone());
			dtoList.add(dto);
		}
		return dtoList;
	}
	...
	public List<CustomerContactDto> queryHisReceiveCusByPhone(CustomerQueryConditionDto conditionDto) {
		List<HisReceiveCusEntity> entityList = customerRemotingService.queryHisReceiveCusByPhone(conditionDto);
		if (entityList == null || entityList.size() == 0) {
			return null;
		}
		...
		for (HisReceiveCusEntity entity : entityList) {
			...
			dto.setOfficePhone(entity.getReceiveCustomerPhone());
			dtoList.add(dto);
		}
	 ...
	 */
	public List<QueryMemberDialogVo> queryCustInfoByCode(String deptCode){
		CustomerDto custDto = customerRemotingService.queryCustInfoByCode(deptCode);
		if(custDto == null){
			return null;
		}else{
			return convertToVo(convertToDto(custDto));
		}
	}
	 ...
	 */
	@Override
	public String queryAccurateCollectionByCustCode(String custCode) {
		CustomerDto custDto = customerRemotingService.queryCustInfoByCode(custCode);
		if(custDto==null)
		     return null;
		return custDto.getAccurateCollection()==null ? FossConstants.NO : custDto.getAccurateCollection();
	}
	...
	public List<CusAccountEntity> queryBankAccountByCode(String customerCode) {
		//从CRM正式客户中查询开户行信息
		CustomerDto custDto = customerRemotingService.queryCustInfoByCode(customerCode);
		//判断是否为空
		if(custDto == null){
			//散客帐户集合
			List<NonfixedCusAccountEntity> nonfixAccountList = customerRemotingService.queryBankAccountByCode(customerCode);
			//判空操作
	...
	public List<CustomerContactDto> queryCustInfoByName(String custName){
		List<CustomerDto> custDto = customerRemotingService.queryCustInfoByName(custName);
		if (CollectionUtils.isNotEmpty(custDto)) {
			List<CustomerContactDto> dtoList = new ArrayList<CustomerContactDto>();
			for (CustomerDto dto : custDto) {
				dtoList = convertToDto(dto);
			}
			return dtoList;
		} else {
			return null;
    ...
    public List<CustomerQueryConditionDto> queryHisDeliveryCusByMobileList(List<String> mobileList){
    	...
    	if(CollectionUtils.isNotEmpty(mobileList)){
    		//查询出历史发货客户信息
    		List<HisDeliveryCusEntity> entityList = customerRemotingService.queryHisDeliveryCusByMobileList(mobileList);
    		return convertDeliveryCustToConditionDto(entityList);
    	}else{
    		return null;
    	}
    }
	...
	public List<CustomerQueryConditionDto> queryHisDeliveryCusByPhoneList(List<String> phoneList){
    	...
    	if(CollectionUtils.isNotEmpty(phoneList)){
    		//查询出历史发货客户信息
    		List<HisDeliveryCusEntity> entityList = customerRemotingService.queryHisDeliveryCusByPhoneList(phoneList);
    		return convertDeliveryCustToConditionDto(entityList);
    	}else{
    		return null;
    	}
	...
	public List<CustomerQueryConditionDto> queryHisReceiveCusByMobileList(List<String> mobileList) {
		...
		if (CollectionUtils.isNotEmpty(mobileList)) {
			// 查询出历史收货客户信息
			List<HisReceiveCusEntity> entityList = customerRemotingService.queryHisReceiveCusByMobileList(mobileList);
			// 查询出历史收货客户信息
			return convertReceiveCustToConditionDto(entityList);
		} else {
			return null;
		}
	}
	...
	public List<CustomerQueryConditionDto> queryHisReceiveCusByPhoneList(List<String> phoneList){
    	...
    	if(CollectionUtils.isNotEmpty(phoneList)){
    		//查询出历史收货客户信息
    		List<HisReceiveCusEntity> entityList = customerRemotingService.queryHisReceiveCusByPhoneList(phoneList);
    		return convertReceiveCustToConditionDto(entityList);
    	}else{
    		return null;
    	}
	 ...
	 * @author 026123-foss-lifengteng
	 * @date 2013-1-12 下午4:28:10
	 */
	public List<CustomerQueryConditionDto> convertDeliveryCustToConditionDto(List<HisDeliveryCusEntity> entityList) {
	 ...
	 * @date 2013-1-12 下午4:28:10
	 */
	public List<CustomerQueryConditionDto> convertReceiveCustToConditionDto(List<HisReceiveCusEntity> entityList) {
		List<CustomerQueryConditionDto> dtoList = null;
		// 判断是否为空
		if (CollectionUtils.isNotEmpty(entityList)) {
			dtoList = new ArrayList<CustomerQueryConditionDto>();
	...
	public List<CustomerQueryConditionDto> queryCustomerByCondition(CustomerQueryConditionDto condition) {
		return customerRemotingService.queryCustomerByCondition(condition);
	}
	...
	public CustomerPaginationDto queryCustomerByConditionByPage(CustomerPaginationDto condition) {
		...
		return customerRemotingService.queryCustomerByConditionByPage(condition);
	}
	...
	public List<CustomerQueryConditionDto> queryCustomerByConditionList(List<CustomerQueryConditionDto> conditions){
		return customerRemotingService.queryCustomerByConditionList(conditions);
	}
	...
	public List<EmployeeEntity> queryEmployeeByCodeList(List<String> codes) {
		return customerRemotingService.queryEmployeeByCodeList(codes);
	}
	...
	public List<EmployeeEntity> queryEmployeeNameAndCodeByCodeList(List<String> codes){
		return customerRemotingService.queryEmployeeNameAndCodeByCodeList(codes);
	}
	...
	public EmployeeEntity queryEmployeeByEmpCode(String code){
		return customerRemotingService.queryEmployeeByEmpCode(code);
	}
	...
	public CusContractTaxEntity queryCurrentUseInvoiceMark(CustomerQueryConditionDto condition, Date date){
		return customerRemotingService.queryCurrentUseInvoiceMark(condition,date);
	}
	
	/**
	 * Dmana-10888根据客户编码查询客户信息和发票标记
	 * @author 218371-foss-zhaoyanjun
	 * @date 2012-11-28 下午10:25:07
	 */
	public String queryInvoiceTypeByCode(String custCode){
		CustomerDto custDto = customerRemotingService.queryCustInfoByCode(custCode);
		if(custDto == null){
			return null;
		}else{
			return custDto.getInvoiceType();
		}
	}
    
	/** 
	 * <p>根据客户编码只查询发票标记，不查询客户其他信息</p> 
	 * @author Foss-151211-yangtaohong 
	 * @date 2016-5-12 下午9:39:33
	 * @param custCode
	 * @return 
	 * @see com.deppon.foss.module.pickup.common.client.service.ICustomerService#queryInvoiceTypeByCusCode(java.lang.String)
	 */
	public String queryInvoiceTypeByCusCode(String custCode){
		CustomerDto custDto = customerRemotingService.queryInvoiceTypeByCusCode(custCode);
		if(custDto == null){
			return null;
		}else{
			return custDto.getInvoiceType();
	...
	public boolean cheCustomerPhoneExistCustomerMobilePhone(CustomerQueryConditionDto customerQueryConditionDto) {
		return customerRemotingService.cheCustomerPhoneExistCustomerMobilePhone(customerQueryConditionDto);
	}
	@Override
	public  CustomerEntity queryCustomerInfoByCusCode(String code){
		return customerRemotingService.queryCustomerInfoByCusCode(code);
	}

	@Override
	public int queryOneticketornotBycode(String code) {
		return customerRemotingService.queryOneticketornotBycode(code);
	}
}
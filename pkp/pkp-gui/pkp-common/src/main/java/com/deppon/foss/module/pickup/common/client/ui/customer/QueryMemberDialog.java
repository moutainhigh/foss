 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/ui/customer/QueryMemberDialog.java
 * 
 * FILE NAME        	: QueryMemberDialog.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class QueryMemberDialog extends JDialog {
	/**
	 * 定义日志静态类 
	 * 通过日志工厂类获得该类的日志对象
	 *  使用该日志类的静态方法记录日志
...
//	private JTextField txtAddress;
	 ...
	 */
	public QueryMemberDialog(WaybillPanelVo bean) {
		...
		EnterKeyForQueryConsignee enterCombine=new EnterKeyForQueryConsignee(this);		
	...
	private class QueryHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try{
    			...
    			validateData();
    			// 查询客户综合信息    		
    			List<CustomerQueryConditionDto> dtoList = customerService.queryCustomerByCondition(gainQueryCondition());
    			// 往表格中设值
    			if (CollectionUtils.isNotEmpty(dtoList)) {
    				List<QueryMemberDialogVo> list = CommonUtils.convertToMemberVo(dtoList);
    				setTableData(list);
...
//			txtAddress.setText("");
	...
	private void obtainCustomer() {
		...
		// 总条数
		int totalRow = tblCombine.getRowCount();
		if(totalRow>0){
			if (row < 0) {
				//将异常信息抛出到前台
				//throw new WaybillValidateException(i18n.get("foss.gui.common.queryConsigneeDialog.msgBox.nullSelectedRow"));
				MsgBox.showInfo(i18n.get("foss.gui.common.queryConsigneeDialog.msgBox.nullSelectedRow"));
			} else {

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/ui/addressfield/popup/AddressSelecterExtender.java
 * 
 * FILE NAME        	: AddressSelecterExtender.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 * <p style="display:none">version:V1.0,author:102246-foss-shaohongliang,date:2012-10-24 下午6:39:49, </p>
 * @author 102246-foss-shaohongliang
 * @date 2012-10-24 下午6:39:49
 * @since
 * @version
 *  *  * 录入货物信息SUC业务规则
 ...
 *	@update date 2014-3-8 下午3:26:25
 */
public class AddressSelecterExtender {
	
	private static final int THREE = 3;
	/**
	 * 国际化
	 */
	private II18n i18n = I18nManager.getI18n(AddressSelecterExtender.class);
    /**
     * 绑定的文本组件
     */
    ...
    private List<AddressFieldDto> hotCityEntityData;
    private List<AddressFieldDto> provinceEntityData;
    private List<AddressFieldDto> nationEntityData;
    
    private LinkedPageGridPanel hotCityEntityPanel;
    private LinkedPageGridPanel nationEntityPanel;
    private LinkedPageGridPanel provinceEntityPanel;
    private LinkedPageGridPanel cityEntityPanel;
    private LinkedPageGridPanel countyEntityPanel;
    private CommitListener commitListener;
    private IAddressService addressService;
    //在线查询
	IAddressServiceHessianRemoting onlineAddressService;
    
    public AddressSelecterExtender(JTextComponent textComponent, List<AddressFieldDto> hotCityEntityData, List<AddressFieldDto> provinceEntityData,List<AddressFieldDto> nationEntityData) {
    	 if (textComponent == null) {
             /**
              * 确保绑定的插件不为null
              */
             throw new IllegalArgumentException(i18n.get("foss.gui.common.autoCompleteExtender.exception.nullTextComponent"));
         }
    	 if (hotCityEntityData == null) {
             /**
              * 确保绑定的插件不为null
              */
             throw new IllegalArgumentException(i18n.get("foss.gui.common.addressSelecterExtender.exception.nullHotCityEntityData"));
         }
    	 if (provinceEntityData == null) {
             /**
              * 确保绑定的插件不为null
              */
             throw new IllegalArgumentException(i18n.get("foss.gui.common.addressSelecterExtender.exception.nullProvinceEntityData"));
         }
    	 if (nationEntityData == null) {
    		 /**
    		  * 确保绑定的插件不为null
    		  */
    		 throw new IllegalArgumentException(i18n.get("foss.gui.common.addressSelecterExtender.exception.nullNationEntityData"));
    	 }
         this.textComponent = textComponent;
         this.hotCityEntityData = hotCityEntityData;
         this.provinceEntityData = provinceEntityData;
         this.nationEntityData = nationEntityData;

 		addressService = AddressServicesFactory.getAddressService();
 		//地址服务在线查询
 		onlineAddressService = DefaultRemoteServiceFactory.getService(IAddressServiceHessianRemoting.class);
        /**
         * 初始化数据
         */
        resetAll();
    ...
    private synchronized void initResultTab() {
        /**
         * list
         */
        if (resultTab != null) {
        	...
        	resultTab = new JTabbedPane(JTabbedPane.TOP, JTabbedPane.SCROLL_TAB_LAYOUT);
        	resultTab.setPreferredSize(new Dimension(NumberConstants.NUMBER_450, NumberConstants.NUMBER_200));
        }
        hotCityEntityPanel = new LinkedPageGridPanel(hotCityEntityData, hotCityEntityClickListener);
        nationEntityPanel = new LinkedPageGridPanel(nationEntityData, nationEntityClickListener);
        provinceEntityPanel = new LinkedPageGridPanel(provinceEntityData, provinceEntityClickListener);
        cityEntityPanel = new LinkedPageGridPanel(null, cityEntityClickListener);
        countyEntityPanel = new LinkedPageGridPanel(null, countyEntityClickListener);
        String hotCity = i18n.get("foss.gui.common.AddressSelecterExtender.addressTab.hotCity");
        String nation = i18n.get("foss.gui.common.AddressSelecterExtender.addressTab.nation");
        String province = i18n.get("foss.gui.common.AddressSelecterExtender.addressTab.province");
        String city = i18n.get("foss.gui.common.AddressSelecterExtender.addressTab.city");
        String area = i18n.get("foss.gui.common.AddressSelecterExtender.addressTab.area");
        resultTab.add("<html><body><table align=center width='60'>"+ hotCity +"</table></body></html>", hotCityEntityPanel);
        resultTab.add("<html><body><table align=center width='60'>"+ nation +"</table></body></html>", nationEntityPanel);
        resultTab.add("<html><body><table align=center width='60'>"+ province +"</table></body></html>", provinceEntityPanel);
        resultTab.add("<html><body><table align=center width='60'>"+ city +"</table></body></html>", cityEntityPanel);
        resultTab.add("<html><body><table align=center width='60'>"+ area +"</table></body></html>", countyEntityPanel);
        // 允许提示框
        ToolTipManager.sharedInstance().registerComponent(resultTab);
...
//        originalEditText = textComponent.getText();
	...
	private void initTextComponent() {
		textComponent.addMouseListener(defaultTextFieldMouseAdapter);
	...
	private void commitValue(AddressFieldDto value) {
		String str = "";
		if(StringUtil.isNotBlank(value.getNationName()) && !WaybillConstants.CHINA.equals(value.getNationName().trim())){
			str = value.getNationName() + "-" ;
		}
		textComponent.setText(str+value.getProvinceName()+"-"+value.getCityName()+"-"+value.getCountyName());
     ...
     * 默认提供的文本编辑框获得焦点事件
     */
    private final MouseAdapter defaultTextFieldMouseAdapter = new MouseAdapter() {
    	@Override
     ...
     * 默认提供的文本编辑框获得焦点事件
     */
    private final CommitListener hotCityEntityClickListener = new CommitListener() {
    	@Override
    	public void commit(AddressFieldDto value) {
    		//切至城市选择页签
    		resultTab.setSelectedIndex(4);
    		List<AddressFieldDto> countyEntitys = null;
    		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
    			//在线查询
    			countyEntitys = onlineAddressService.queryCountyListByCityId(value.getCityId());
    		} else {
    			countyEntitys = addressService.queryCountyListByCityId(value.getCityId());
    		}
    	    
    		countyEntityPanel.setData(countyEntitys);
    	}
    };
     ...
     * @date 2016-10-11 15:59:38
     */
    private final CommitListener nationEntityClickListener = new CommitListener() {
    	@Override
    	public void commit(AddressFieldDto value) {
    		//切至省份选择页签
    		resultTab.setSelectedIndex(2);
    		List<AddressFieldDto> provinceEntitys = null;
    		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
    			//在线查询
    			provinceEntitys = onlineAddressService.queryProvinceListByNationId(value.getNationId());
    		} else {
    			provinceEntitys = addressService.queryProvinceListByNationId(value.getNationId());
    		}
    		
    		provinceEntityPanel.setData(provinceEntitys);
    	}
    };
     ...
     * 默认提供的文本编辑框获得焦点事件
     */
    private final CommitListener provinceEntityClickListener = new CommitListener() {
    	@Override
    	public void commit(AddressFieldDto value) {
    		//切至城市选择页签
    		resultTab.setSelectedIndex(THREE);
    		List<AddressFieldDto> cityEntitys = null;
    		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
    			//在线查询
    			cityEntitys = onlineAddressService.queryCityListByProvinceId(value.getProvinceId());
    		} else {
    			//离线查询
    			cityEntitys = addressService.queryCityListByProvinceId(value.getProvinceId());
    		}
    		cityEntityPanel.setData(cityEntitys);
    	}
    };
    /**
     * 默认提供的文本编辑框获得焦点事件
     */
    private final CommitListener cityEntityClickListener = new CommitListener() {
    	@Override
    	public void commit(AddressFieldDto value) {
    		//切至区县选择页签
    		resultTab.setSelectedIndex(4);
     ...
     * 默认提供的文本编辑框获得焦点事件
     */
    private final CommitListener countyEntityClickListener = new CommitListener() {
    	@Override
    	public void commit(AddressFieldDto value) {
    		//切至区域选择页签
    		hidePopup();
	...
	public synchronized void showCoutysByCity(String cityId) {
		List<AddressFieldDto> countyEntitys = null;
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			//在线查询
			countyEntitys = onlineAddressService.queryCountyListByCityId(cityId);
		} else {
			//离线查询
			countyEntitys = addressService.queryCountyListByCityId(cityId);
		}
		countyEntityPanel.setData(countyEntitys);
		showPopup();
		resultTab.setSelectedIndex(NumberConstants.NUMBER_4);
	}
	 ...
	 * @param provinceId
	 */
	public synchronized void showCityByProvince(String provinceId) {
		List<AddressFieldDto> cityEntitys = null;
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			//在线查询
			cityEntitys = onlineAddressService.queryCityListByProvinceId(provinceId);
		} else {
			//离线查询
			cityEntitys = addressService.queryCityListByProvinceId(provinceId);
		}
		cityEntityPanel.setData(cityEntitys);
		showPopup();
		resultTab.setSelectedIndex(NumberConstants.NUMBER_3);
	}

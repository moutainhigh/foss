...
import com.deppon.foss.module.pickup.common.client.service.ISrvValueaddRegionService;
import com.deppon.foss.module.pickup.pricing.api.shared.domain.PriceRegionValueAddEntity;
import com.deppon.foss.util.define.FossConstants;

/**
 * 
 * @ClassName: SrvValueaddRegionSaver
 * 
 * @Description:BUG-55198 增值区域
 * 
 ...
 * 
 * @date 2013-10-8 上午9:57:28
 * 
 * 
 */
public class SrvValueaddRegionSaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return PriceRegionValueAddEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<PriceRegionValueAddEntity> list = (List<PriceRegionValueAddEntity>) data;
		if (list != null && !list.isEmpty()) {
			ISrvValueaddRegionService srvValueaddRegionService = DownLoadDataServiceFactory
					.getSrvValueaddRegionService();
			for (PriceRegionValueAddEntity entity : list) {
				if (FossConstants.YES.equals(entity.getActive())) {
					srvValueaddRegionService.saveOrUpdate(entity);

				} else {
					srvValueaddRegionService.deleteID(entity.getId());
				}

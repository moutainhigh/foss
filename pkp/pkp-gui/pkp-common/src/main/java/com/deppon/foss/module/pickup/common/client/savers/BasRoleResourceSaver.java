 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasRoleResourceSaver.java
 * 
 * FILE NAME        	: BasRoleResourceSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasRoleResourceSaver extends AbstractSyncDataSaver   {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<RoleResourceEntity> list = (List<RoleResourceEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for(RoleResourceEntity roleResource: list){
				RoleResourceService roleResourceService = DownLoadDataServiceFactory
						.getRoleResourceService();
				
				if(	FossConstants.YES.equals(roleResource.getActive())){
				...
				Long version = roleResource.getVersionNo();

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvPriceValuationFreightSaver.java
 * 
 * FILE NAME        	: SrvPriceValuationFreightSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvPriceValuationFreightSaver extends AbstractSyncDataSaver {
	//区域id
	...
	public Class<?> getSaveType() {
		return PriceValuationFreightDto.class;
	}
	...
	public Date saveData(List<?> data) {
		...
		List<PriceValuationFreightDto> list = (List<PriceValuationFreightDto>)data;
		if(list != null && !list.isEmpty()){
			Map<String, List<PriceValuationFreightDto>> map =  (Map)list.get(0);
			for (Entry<String, List<PriceValuationFreightDto>> entry : map.entrySet()) {
			...
			for(PriceValuationFreightDto priceValuationFreightDto : entry.getValue()){
				priceValuationService.saveOrUpdate(priceValuationFreightDto);
				detailList = priceValuationFreightDto.getCriteriaDetailEntities();
				if(detailList != null){
				...
				Long version = priceValuationFreightDto.getVersionNo();
				//取新价格区域ID
				regionID = priceValuationFreightDto.getDeptRegionId();
				//获得最大版本号

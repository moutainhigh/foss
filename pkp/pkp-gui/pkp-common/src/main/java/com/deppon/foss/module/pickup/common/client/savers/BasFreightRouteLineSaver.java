 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasFreightRouteLineSaver.java
 * 
 * FILE NAME        	: BasFreightRouteLineSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 * @date 2012-10-24 下午3:54:08
 */
public class BasFreightRouteLineSaver extends AbstractSyncDataSaver{
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return FreightRouteLineEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<FreightRouteLineEntity> freightRouteLineEntitys =  (List<FreightRouteLineEntity>) data;
		if(freightRouteLineEntitys != null && !freightRouteLineEntitys.isEmpty()){
			FreightRouteLineService freightRouteLineService = DownLoadDataServiceFactory
					.getFreightRouteLineService();
			for (FreightRouteLineEntity freightRouteLineEntity : freightRouteLineEntitys) {
				
				if(	FossConstants.YES.equals(freightRouteLineEntity.getActive())){
					freightRouteLineService.saveOrUpdate(freightRouteLineEntity);
				}else{
					freightRouteLineService.delete(freightRouteLineEntity);
				}
				
				Long version = freightRouteLineEntity.getVersion();

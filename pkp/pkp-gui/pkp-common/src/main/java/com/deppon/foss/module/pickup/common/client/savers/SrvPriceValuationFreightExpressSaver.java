 ...
 *
 */
public class SrvPriceValuationFreightExpressSaver  extends AbstractSyncDataSaver {
	//区域id
	...
	public Class<?> getSaveType() {
		return PriceValuationFreightExpressDto.class;
	}
	...
	public Date saveData(List<?> data) {
		...
		List<PriceValuationFreightExpressDto> list = (List<PriceValuationFreightExpressDto>)data;
		if(list != null && !list.isEmpty()){
			Map<String, List<PriceValuationFreightExpressDto>> map =  (Map)list.get(0);
			for (Entry<String, List<PriceValuationFreightExpressDto>> entry : map.entrySet()) {
			...
			for(PriceValuationFreightExpressDto priceValuationFreightDto : entry.getValue()){

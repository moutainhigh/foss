 ...
 * 
 * 
 * @ClassName: SrvValueaddRegionOrgSaver
 * 
 ...
 * 
 * @date 2013-10-8 上午10:47:58
 * 
 * 
 */
public class SrvValueaddRegionOrgSaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return PriceRegionOrgValueAddEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<PriceRegionOrgValueAddEntity> list = (List<PriceRegionOrgValueAddEntity>) data;
		if (list != null && !list.isEmpty()) {
			ISrvValueaddRegionOrgService srvValueaddRegionOrgService = DownLoadDataServiceFactory
					.getSrvValueaddRegionOrgService();
			for (PriceRegionOrgValueAddEntity entity : list) {
				if (FossConstants.YES.equals(entity.getActive())) {
					srvValueaddRegionOrgService.saveOrUpdate(entity);
				} else {
					srvValueaddRegionOrgService.deleteID(entity.getId());
				}

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasOutfieldSaver.java
 * 
 * FILE NAME        	: BasOutfieldSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasOutfieldSaver extends AbstractSyncDataSaver {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<OutfieldEntity> list =  (List<OutfieldEntity>) data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for (OutfieldEntity outfield : list) {
				OutfieldService outfieldService = DownLoadDataServiceFactory
						.getOutfieldService();
				if(	FossConstants.YES.equals(outfield.getActive())){
					outfieldService.saveOrUpdate(outfield);
					
				}else{
					outfieldService.delete(outfield);
				}
				

				Long version = outfield.getVersionNo();

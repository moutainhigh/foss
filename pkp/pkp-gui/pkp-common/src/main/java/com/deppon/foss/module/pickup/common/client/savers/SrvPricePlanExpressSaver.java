 ...
 *
 */
public class SrvPricePlanExpressSaver extends AbstractSyncDataSaver {
	//区域id
	...
	public Class<?> getSaveType() {
		return PricePlanExpressDto.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<Map<String, List<PricePlanExpressDto>>> list = (List<Map<String, List<PricePlanExpressDto>>>) data;
		long mxVersion = 0;
		boolean needDelete = true;
		if (list != null && !list.isEmpty()) {
			Map<String, List<PricePlanExpressDto>> map =  (Map)list.get(0);
			for (Entry<String, List<PricePlanExpressDto>> entry : map.entrySet()) {
				List<PricePlanExpressDto> dataList = (List<PricePlanExpressDto>) entry
						.getValue();
				...
				for (PricePlanExpressDto pricePlan : dataList) {

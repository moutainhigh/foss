 ...
 *
 */
public class SrvExpressRegionSaver  extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return PriceRegionExpressEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<PriceRegionExpressEntity> list = (List<PriceRegionExpressEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			
			Injector injector = GuiceContextFactroy.getInjector();
			PriceRegionExpressService priceRegionService =  injector.getInstance(PriceRegionExpressService.class);
			
//			DownLoadDataServiceFactory
//					.getPriceRegionExpressService();
			for(PriceRegionExpressEntity priceRegion : list){

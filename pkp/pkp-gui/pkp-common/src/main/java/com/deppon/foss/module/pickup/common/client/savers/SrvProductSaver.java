 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvProductSaver.java
 * 
 * FILE NAME        	: SrvProductSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvProductSaver extends AbstractSyncDataSaver {
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<ProductEntity> list = (List<ProductEntity>)data;
		if(list != null && !list.isEmpty()){
			ProductService productService = DownLoadDataServiceFactory
					.getProductService();
			for(ProductEntity product : list){
				if(	FossConstants.YES.equals(product.getActive())){	
				...
				Long version = product.getVersionNo();
				//获得最大版本号

 ...
 *
 */
public class BasCityExpressSaver  extends AbstractSyncDataSaver   {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return ExpressCityEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<ExpressCityEntity> list = (List<ExpressCityEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for(ExpressCityEntity e: list){
				Injector injector = GuiceContextFactroy.getInjector();
				ExpressCityService service = injector.getInstance(ExpressCityService.class);

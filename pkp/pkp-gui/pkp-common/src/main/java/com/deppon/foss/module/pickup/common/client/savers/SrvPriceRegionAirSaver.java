 ...
 */
public class SrvPriceRegionAirSaver extends AbstractSyncDataSaver {
	//区域id
	...
	public Class<?> getSaveType() {
		return PriceRegionAirEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<PriceRegionAirEntity> list = (List<PriceRegionAirEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			PriceRegionAirService priceRegionAirService = DownLoadDataServiceFactory
					.getPriceRegionAirService();
			for(PriceRegionAirEntity priceRegionAir : list){

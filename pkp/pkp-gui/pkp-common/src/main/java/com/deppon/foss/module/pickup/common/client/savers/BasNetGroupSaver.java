 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasNetGroupSaver.java
 * 
 * FILE NAME        	: BasNetGroupSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasNetGroupSaver extends AbstractSyncDataSaver{
	/**
	 * 
	 * 功能：映射实体类
	...
	public Date saveData(List<?> data) {
		...
		List<NetGroupEntity> list =  (List<NetGroupEntity>) data;
		long mxVersion = 0;
		if(CollectionUtils.isNotEmpty(list)){
			NetGroupService netGroupService = DownLoadDataServiceFactory.getNetGroupService();
			for(NetGroupEntity  netGroupEntity : list){
				if(	FossConstants.YES.equals(netGroupEntity.getActive())){
					netGroupService.saveOrUpdate(netGroupEntity);
					
				}else{
					netGroupService.delete(netGroupEntity);
				}

 ...
 * 
 * @date 2013-10-8 上午11:01:06
 * 
 * 
 */
public class SrvOuterPriceSaver extends AbstractSyncDataSaver {
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<OuterPriceEntity> list = (List<OuterPriceEntity>) data;
		if (list != null && !list.isEmpty()) {
			ISrvOuterPriceService srvOuterPriceService = DownLoadDataServiceFactory
					.getSrvOuterPriceService();
			for (OuterPriceEntity entity : list) {
				if (FossConstants.YES.equals(entity.getActive())) {
					srvOuterPriceService.saveOrUpdate(entity);

				} else {
					srvOuterPriceService.deleteID(entity.getId());

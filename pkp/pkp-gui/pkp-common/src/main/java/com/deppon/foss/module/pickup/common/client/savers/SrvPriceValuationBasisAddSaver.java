 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvPriceValuationBasisAddSaver.java
 * 
 * FILE NAME        	: SrvPriceValuationBasisAddSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvPriceValuationBasisAddSaver extends AbstractSyncDataSaver {
	//区域id
	...
	public Class<?> getSaveType() {
		return PriceValuationBasisAddDto.class;
	}
	...
	public Date saveData(List<?> data) {
		...
		long mxVersion = 0;
		List<PriceValuationBasisAddDto> list = (List<PriceValuationBasisAddDto>)data;
		if(list != null && !list.isEmpty()){
			Map<String, List<PriceValuationBasisAddDto>> map =  (Map)list.get(0);
			for (Entry<String, List<PriceValuationBasisAddDto>> entry : map.entrySet()) {
			...
			for(PriceValuationBasisAddDto priceValuationBasisAddDto : entry.getValue()){
				priceValuationService.saveOrUpdate(priceValuationBasisAddDto);
				detailList = priceValuationBasisAddDto.getCriteriaDetailEntities();
				if(detailList != null){
				...
				Long version = priceValuationBasisAddDto.getVersionNo();
				//取新价格区域ID
				regionID = priceValuationBasisAddDto.getDeptRegionId();
				//获得最大版本号

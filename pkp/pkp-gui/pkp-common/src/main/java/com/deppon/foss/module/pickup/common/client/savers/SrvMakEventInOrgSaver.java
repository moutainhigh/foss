 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvMakEventInOrgSaver.java
 * 
 * FILE NAME        	: SrvMakEventInOrgSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvMakEventInOrgSaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return MakEventInOrgEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<MakEventInOrgEntity> list = (List<MakEventInOrgEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for(MakEventInOrgEntity makEventInOrg: list){
				MakEventInOrgService makEventInOrgService = DownLoadDataServiceFactory
						.getMakEventInOrgService();
				makEventInOrgService.saveOrUpdate(makEventInOrg);
				Long version = makEventInOrg.getVersionNo();
				//获得最大版本号

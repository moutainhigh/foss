 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasSalesProductSaver.java
 * 
 * FILE NAME        	: BasSalesProductSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasSalesProductSaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return SalesProductEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<SalesProductEntity> list = (List<SalesProductEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for(SalesProductEntity salesProduct: list){
				SalesProductService salesProductService = DownLoadDataServiceFactory
						.getSalesProductService();
				if(	FossConstants.YES.equals(salesProduct.getActive())){	
					salesProductService.saveOrUpdate(salesProduct);
					
				}else{
					salesProductService.delete(salesProduct);
					
				}
				Long version = salesProduct.getVersionNo();

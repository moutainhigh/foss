 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasGoodsAreaSaver.java
 * 
 * FILE NAME        	: BasGoodsAreaSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasGoodsAreaSaver  extends AbstractSyncDataSaver {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<GoodsAreaEntity> list =  (List<GoodsAreaEntity>) data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			GoodsAreaService goodsAreaService = DownLoadDataServiceFactory
					.getGoodsAreaService();
			for(GoodsAreaEntity  goodsAreaEntity : list){
			
				if(	FossConstants.YES.equals(goodsAreaEntity.getActive())){
					goodsAreaService.saveOrUpdate(goodsAreaEntity);
				}else{
					goodsAreaService.delete(goodsAreaEntity);
				}
				Long version = goodsAreaEntity.getVersion();

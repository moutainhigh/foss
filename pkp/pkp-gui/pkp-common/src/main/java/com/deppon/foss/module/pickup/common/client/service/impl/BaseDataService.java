 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/service/impl/BaseDataService.java
 * 
 ...
 * </p>
 * <p style="display:none">
 * version:V1.0,author:Bobby,date:2012-10-15 上午11:07:05,
 * </p>
 * 
 * @author foss-sunrui
 * @date 2012-10-15 上午11:07:05
 * @since
 * @version
 *  *  * 录入货物信息SUC业务规则
...
public class BaseDataService implements IBaseDataService {
	...
	// 日志
	private static final Log LOG = LogFactory.getLog(BaseDataService.class);

	// 限保物品
	private IInsurGoodsDao insurGoodsDao;
	...
	// 获得基础服务的远程HessionRemoting接口
	private IBaseDataHessianRemoting baseDataRemotingService = null;
	...
	public BaseDataService(){
		...
		//获得基础服务的远程HessionRemoting接口
		baseDataRemotingService = DefaultRemoteServiceFactory.getService(IBaseDataHessianRemoting.class);
	}
	 ...
	 */
	@Inject
	public void setInsurGoodsDao(IInsurGoodsDao insurGoodsDao) {
		this.insurGoodsDao = insurGoodsDao;
	}
	/**
	 * 设置对象
	 * @param outerBranchDao
	 */
	@Inject
	public void setProhibitGoodsDao(IProhibitGoodsDao prohibitGoodsDao) {
		this.prohibitGoodsDao = prohibitGoodsDao;
	}
	/**
	 * 设置对象
	 * @param outerBranchDao
	 ...
	 */
	@Inject
	public void setOuterBranchDao(IOuterBranchDao outerBranchDao) {
		this.outerBranchDao = outerBranchDao;
	}
	/**
	 * 设置对象
	 ...
	 */
	@Inject
	public void setSalesBillingGroupService(ISalesBillingGroupService salesBillingGroupService) {
	...
	public BillInspectionRemindEntity queryBillInspectionRemindByRegionCode(
		...
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			return baseDataRemotingService.queryBillInspectionRemindByRegionCode(entity, regionCode, regionLevCode);
		}else{
	 ...
	 */
	 public OuterBranchEntity queryOuterBranchByBranchCode(String branchCode,
		...
		//判断是否是在线登陆
			if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
				return baseDataRemotingService.queryOuterBranchByBranchCode(branchCode,branchType);
			}else{
	 ...
	 */
	 public List<BargainAppOrgEntity> queryAppOrgByBargainCrmId(BigDecimal crmId,List<String> unifiedCodeList){
		 if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())){
			 return baseDataRemotingService.queryAppOrgByBargainCrmId(crmId,unifiedCodeList);
		 }else{
	 ...
	 public OrgAdministrativeInfoEntity queryOrgAdministrativeInfoByUnifiedCodeClean(String unifiedCode){
		 if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())){
			 return baseDataRemotingService.queryOrgAdministrativeInfoByUnifiedCode(unifiedCode);
		 }else{
	 ...
	 public PreferentialInfoDto queryPreferentialInfoForBubbleRate(String customerCode,Date date, String code){
		 if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())){
			 return baseDataRemotingService.queryPreferentialInfoForBubbleRate(customerCode,date,code);
		 }else{
	 ...
	 * @see
	 */
	public OrgAdministrativeInfoEntity queryOrgAdministrativeInfoEntityByCode(String code) {
		LOG.debug(code);
		//判断是否是在线登陆
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
	 ...
	 */
	@Override
	public OrgAdministrativeInfoEntity queryOrgAdministrativeInfoEntityByCode(String code,Date billTime){
		//添加日志信息
		LOG.debug(code);
		...
		//查询远程数据时，必须处于在线状态
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			//根据billTime是否为空来判断是否需要根据billTime查询远程历史部门信息
			if(null != billTime){
				//在线则调用在线远程接口方法
				orgInfo = waybillRemotingService.queryHisOrgInfoByCode(code, billTime);
			}else{
				//根据编码从本地获取对象
				orgInfo = queryOrgAdministrativeInfoEntityByCode(code);
			}
	 ...
	 */
	@Override
	public List<OrgAdministrativeInfoEntity> queryOrgInfoByNameOnline(String name){
		//添加日志信息
		LOG.debug("根据部门名称在线查询部门信息");
		...
		if(CommonUtils.isOnline()){
			//在线则调用在线远程接口方法
			return baseDataRemotingService.queryOrgInfoByName(name);
		}else{
			return orgInfoService.queryOrgInfoByName(name);
		}
	}
	 ...
	 * @see
	 */
	public List<LimitedWarrantyItemsEntity> queryAllInsurGoods() {
		return insurGoodsDao.queryAllActive();
	}
	...
	public LimitedWarrantyItemsEntity isInsurGoods(String goodsName) {
		return insurGoodsDao.isInsurGoods(goodsName);
	}
	...
	public String queryLimitAmount(String goodsName) {
		return insurGoodsDao.queryLimitAmount(goodsName);
	}
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-10-16 下午3:19:49
	 * @param goodsName
	 * @return
	 * @see com.deppon.foss.module.pickup.creating.client.service.IBaseDataService#isProhibitGoods(java.lang.String)
	 */
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-10-19 上午9:37:07
	 * @param goodsName
	 * @return
	 * @see com.deppon.foss.module.pickup.creating.client.service.IBaseDataService#isValuablesGoods(java.lang.String)
	 */
	public boolean isValuablesGoods(String goodsName) {
		boolean retValue = false;
		List<DataDictionaryValueEntity> list = dataDictionaryValueService
				.queryByTermsCode(WaybillConstants.VALUE_GOODS);
		if (list.size() > 0) {
			for (DataDictionaryValueEntity entity : list) {
				if (goodsName.equals(entity.getValueName().trim())) {
					retValue = true;
					break;
				}
			}
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-10-16 下午3:19:31
	 * @param degree
	 * @return
	 * @see com.deppon.foss.module.pickup.creating.client.service.IBaseDataService#selectByDegree(java.lang.String)
	 */
	public List<AdministrativeRegionsEntity> queryByDegree(String degree) {
		return districtDao.queryByDegree(degree);
	}
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-10-16 下午3:19:35
	 * @param parentDistCode
	 * @return
	 * @see com.deppon.foss.module.pickup.creating.client.service.IBaseDataService#selectByParentDistCode(java.lang.String)
	 */
	public List<AdministrativeRegionsEntity> queryByParentDistCode(
			String parentDistCode) {
		return districtDao.queryByParentDistCode(parentDistCode);
	}
	 ...
	 * 
	 * @author Bobby
	 * @date 2012-10-17 上午10:48:51
	 * @param saleDepartmentId
	 * @return
	 * @see com.deppon.foss.module.pickup.creating.client.service.IBaseDataService#queryDepartureNetGrp(java.lang.String)
	 */
	public NetGroupEntity queryDepartureNetGrp(String saleDepartmentId) {
		Map<String, String> map = new HashMap<String, String>();
		map.put("saleDepartmentId", saleDepartmentId);
		map.put("type", WaybillConstants.DEPARTURE_TYPE);
		map.put("active", FossConstants.ACTIVE);
		return netGroupDao.queryBySaleDept(map);
	}
	 ...
	 * @see
	 */
	public NetGroupEntity queryArrivalNetGrp(String saleDepartmentId) {
		Map<String, String> map = new HashMap<String, String>();
		map.put("saleDepartmentId", saleDepartmentId);
		map.put("type", WaybillConstants.ARRIVAL_TYPE);
		map.put("active", FossConstants.ACTIVE);
		return netGroupDao.queryBySaleDept(map);
	}
	 ...
	 * 
	 * @author Bobby
	 * @date 2012-10-17 上午11:04:33
	 * @param transType
	 *            运输性质
	 * @param departureSaleDept
	 *            出发营业部
	 * @param arrivalSaleDept
	 *            到达营业部
	 * @return
	 * @see com.deppon.foss.module.pickup.creating.client.service.IBaseDataService#queryBySaleDept(java.lang.String,
	 *      java.lang.String)
	 */
	public FreightRouteEntity queryRouteBySaleDept(String transType,
			String departureSaleDept, String arrivalSaleDept) {
		NetGroupEntity departure = queryDepartureNetGrp(departureSaleDept);
		NetGroupEntity arrival = queryArrivalNetGrp(arrivalSaleDept);
		if (departure != null && arrival != null) {
			Map<String, String> map = new HashMap<String, String>();
			map.put("departureNetGrp", departure.getId());
			map.put("arrivalNetGrp", arrival.getId());
			map.put("active", FossConstants.ACTIVE);
			map.put("transType", transType);
			return freightRouteDao.queryBySaleDept(map);
		} else {
			return null;
		}
	}
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-10-25 下午2:57:05
	 * @param salesDeptId
	 * @return
	 * @see com.deppon.foss.module.pickup.common.client.service.IBaseDataService#queryTransType(java.lang.String)
	 */
	public List<ProductEntity> queryTransType(String salesDeptId) {		
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			//查询远程数据时，必须处于在线状态
			return waybillRfcHessianRemoting.queryBySalesDept(salesDeptId, "3");
		}else{
			//查询本地数据
			return productDao.queryBySalesDept(salesDeptId, "3");
		}
	}
	 /**
     * 
	...
	public List<ProductEntity> queryByArriveDeptProduct(String arriveDept) {
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			...
			//查询本地数据
			return productDao.queryByArriveDeptProduct(arriveDept, "3");
		}	
	}
	 ...
	 * @param orgCode
	 * @return
	 * @see com.deppon.foss.module.pickup.creating.client.service.IBaseDataService#queryByTermsCode()
	 */
	public ConfigurationParamsEntity queryByConfCode(String confCode,
			String orgCode) {
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			//查询远程数据时，必须处于在线状态
			return waybillRemotingService.queryByConfCode(confCode, orgCode);
		}else{
			//查询本地数据
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-10-30 下午2:50:22
	 * @return
	 * @see com.deppon.foss.module.pickup.common.client.service.IBaseDataService#queryGoodsPackingVolume()
	 */
	public String queryGoodsPackingVolume() {
		UserEntity user = (UserEntity) SessionContext.getCurrentUser();
		String confCode = WaybillConstants.GOODS_PACKING_VOLUME;
		ConfigurationParamsEntity conf = this.queryByConfCode(confCode, user.getEmployee().getDepartment().getCode());
		if (conf != null) {
	 ...
	 * <p>通过产品ID查产品</p> 
	 * @author foss-sunrui
	 * @date 2012-12-25 下午1:00:39
	 * @param transTypeId
	 * @return 
	 * @see com.deppon.foss.module.pickup.common.client.service.IBaseDataService#queryTransTypeById(java.lang.String)
	 */
	public ProductEntity queryTransTypeById(String transTypeId) {
		if(CommonUtils.isOnline()){
			return waybillRemotingService.queryProductByCode(transTypeId);
		}else{
			return productDao.queryByPrimaryKey(transTypeId);
		}
	}
	...
	public List<SaleDepartmentEntity> querySaleDepartmentExactByEntity(
		...
		return salesDepartmentDao.querySaleDepartmentExactByEntity(entity,
				start, limit);
	}
	...
	public SaleDepartmentEntity querySaleDepartmentByCode(String code,Date billTime) {
		...
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			...
			if(null != billTime){
				//在线则调用在线远程接口方法
				saleDept = waybillRemotingService.queryHisSaleDeptByCode(code, billTime);
			}else{
				//根据编码从本地获取对象
				saleDept = querySaleDepartmentByCode(code);
			}			
		}else{
			//根据编码从本地获取对象
			saleDept = querySaleDepartmentByCode(code);
	...
	public OuterBranchEntity queryOuterBranchByCode(String code,Date billTime){
		...
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			...
			if(null != billTime){
				//在线则调用在线远程接口方法
				agentDept = waybillRemotingService.queryHisOuterBranchByCode(code,billTime);
			}else{
				agentDept = queryAgencyBranchInfo(code);
			}		
		}else{
			//若为离线状态，则返回空
			agentDept = queryAgencyBranchInfo(code);
	 ...
	 */
	@Override
	public List<OrgAdministrativeInfoEntity> queryBillingGroupListBySalesCode(String salesCode){ 
		//定义对象接收数据集合
		List<OrgAdministrativeInfoEntity> orgList = null;
		//查询集合对象
		List<SalesBillingGroupEntity> billGroups = null;
		//当前是否在线
		if(CommonUtils.isOnline()){
			billGroups = baseDataRemotingService.queryBillingGroupListBySalesCode(salesCode);
		} else {
			billGroups = salesBillingGroupService.queryBillingGroupListBySalesCode(salesCode);
		}
		//集合非空判断
		if(CollectionUtils.isNotEmpty(billGroups)){
			//实例化对象
			orgList = new ArrayList<OrgAdministrativeInfoEntity>();
			//查询组织对象
			for (SalesBillingGroupEntity entity : billGroups) {
				//此处不用使用queryByCode(code,date)方法，因为若部门已失效则无法登陆的
				OrgAdministrativeInfoEntity org = orgInfoService.queryByCode(StringUtil.defaultIfNull(entity.getBillingGroupCode()));
				if(null != org){
					orgList.add(org);
				}
			}
	 ...
	 */
	@Override
	public List<SaleDepartmentEntity> querySalesListByBillingGroupCode(String billingGroupCode){
		//定义对象接收数据集合
		...
		//当前是否在线
		if(CommonUtils.isOnline()){
			billGroups = baseDataRemotingService.querySalesListByBillingGroupCode(billingGroupCode);
		} else {
			billGroups = salesBillingGroupService.querySalesListByBillingGroupCode(billingGroupCode);
		}
		...
		if(CollectionUtils.isNotEmpty(billGroups)){
			...
			for (SalesBillingGroupEntity entity : billGroups) {
				//此处不用使用queryByCode(code,date)方法，因为若部门已失效则无法登陆的
				SaleDepartmentEntity saleDept = querySaleDepartmentByCodeOnline(StringUtil.defaultIfNull(entity.getSalesDeptCode()));
				if(null != saleDept){
     ...
     */
	@Override
	public SaleDepartmentEntity querySaleDepartmentByCodeOnline(String code) {
		//若为在线登陆，则查询在线数据
		if(CommonUtils.isOnline()){
			return waybillRemotingService.querySaleDepartmentByCode(code);
		}else{
			return querySaleDepartmentByCode(code);
		}
	...
	/**
	 * <p>根据编码查询营业部信息，不关联组织名称等只查询基本信息</p> 
	 * @author Foss-151211-yangtaohong 
	 * @date 2016-5-12 下午9:02:33
	 * @param code
	 * @return
	 * @see
	 */
	@Override
	public SaleDepartmentEntity querySaleDepartmentNoAttachOnline(String code) {
		//若为在线登陆，则查询在线数据
		if(CommonUtils.isOnline()){
			return waybillRemotingService.querySaleDepartmentNoAttach(code);
		}else{
			return querySaleDepartmentByCode(code);
		}
    ...
    public List<ProductEntity> queryBySalesDept(String salesDeptCode, String productLevel){    	
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			//查询远程数据时，必须处于在线状态
			return waybillRfcHessianRemoting.queryBySalesDept(salesDeptCode, productLevel);
		}else{
			//查询本地数据
			return productDao.queryBySalesDept(salesDeptCode, productLevel);
		}
    }
    ...
    public List<ProductEntity> queryByArriveDept(String salesDeptCode, String productLevel){    	
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			//查询远程数据时，必须处于在线状态
			return waybillRfcHessianRemoting.queryByArriveDept(salesDeptCode, productLevel);
		}else{
			//查询本地数据
			return productDao.queryByArriveDept(salesDeptCode, productLevel);
		}
    }
	 ...
	 * @date 2014-3-5
	 */
    public List<ProductEntity> searchByArriveDept(String salesDeptCode, String productLevel){    	
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			//查询远程数据时，必须处于在线状态
			return waybillRfcHessianRemoting.searchByArriveDept(salesDeptCode, productLevel);
		}else{
			//查询本地数据
			return productDao.queryByArriveDept(salesDeptCode, productLevel);
		}
	 ...
	 */
    @Override
	public List<SaleDepartmentEntity> querySaleDepartmentByEntityOnline(SaleDepartmentEntity entity) {
		//非空判断
		if(null == entity){
			return null;
		}else{
			//当前是否在线
			if(CommonUtils.isOnline()){
				return baseDataRemotingService.querySaleDepartmentByEntity(entity);
			}else{
				return salesDepartmentDao.querySaleDepartmentExactByEntity(entity, 0, NumberConstants.NUMBER_100);
			}
		}
	}
    ...
    public ProductEntity getProductByCache(String productCode, Date billDate){
    	return baseDataRemotingService.getProductByCache(productCode, billDate);
    }
    ...
    @Override
    public OrgAdministrativeInfoEntity queryOrgAdministrativeInfoEntityByUnifiedCode(String unifiedCode) {
    	//添加日志信息
		LOG.debug(unifiedCode);		
		OrgAdministrativeInfoEntity orgInfo=null;		
		//查询远程数据时，必须处于在线状态
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			//在线则调用在线远程接口方法
			orgInfo = baseDataRemotingService.queryOrgAdministrativeInfoByUnifiedCode(unifiedCode);			
		}else{
			//根据编码从本地获取对象
			orgInfo = orgInfoService.queryOrgByUnifiedCode(unifiedCode);
		}		
		return orgInfo;
	}
	...
	public CarloadPricePlanDto queryByConfCode(Date date, String invoceType,
		...
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			//查询远程数据时，必须处于在线状态
			return waybillRemotingService.queryByConfCode(date, invoceType,code);
		}else{
			return null;
		}
	}
    ...
    public int queryCountByCodeAndProduct(SalesProductEntity entity){
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			//查询远程数据时，必须处于在线状态
			return waybillRemotingService.queryCountByCodeAndProduct(entity);
		}else{
	...
	public PreferentialInfoDto queryPreferentialInfo(String customerCode,
		...
		//查询远程数据时，必须处于在线状态
		return baseDataRemotingService.queryPreferentialInfo(customerCode, date, productCode);		
	}
	...
	public List<CarloadPricePlanDto> selectCarloadPricePlanDetailByOrganizationCode(
		...
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			return baseDataRemotingService.selectCarloadPricePlanDetailByOrganizationCode(carloadPricePlanDto);
		}else{
			return null;
		}
	...
	public List<OrgAdministrativeInfoEntity> queryOrgAdministrativeInfoEntityAllUpNOCache(
			String code){
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			return baseDataRemotingService.queryOrgAdministrativeInfoEntityAllUpNOCache(code);
		}else {
			return null;
		}
	}
	...
	public List<ExhibitionKeywordEntity> queryExhibitionKeywordListByTargetOrgName(
		...
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			return baseDataRemotingService.queryExhibitionKeywordListByTargetOrgName(condition);
		}else {
			return null;
		}
	...
	public List<ExhibitionKeywordEntity> queryExhibitionKeyword(
		...
		// TODO Auto-generated method stub
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			return baseDataRemotingService.queryExhibitionKeyword(exhibitionKeyword);
		}else {
			return null;
		}
	...
	public String queryUnifiedCodeByCode(String code){
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())){
			OrgAdministrativeInfoEntity entity = waybillRemotingService.queryOrgInfoByCode(code);
			return entity == null ? null : entity.getUnifiedCode();
		 }else{
			 return orgAdministrativeInfoService.queryUnifiedCodeByCode(code);
		 }
	}
	@Override
	public  List<CusLtDiscountItemDto> queryCusLtdiscountafterByCondition(CusLtdiscountafterEntity entity) {
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())) {
			return baseDataRemotingService.queryCusLtdiscountafterByCondition(entity);
		}else {
			return null;
		}
	...
	@Override
	public List<ProductEntity> getAllProductInfoByLevels1(String transTypeCode) {
		if (WaybillConstants.ONLINE_LOGIN.equals(SessionContext.get(WaybillConstants.LOGIN_TYPE).toString())){
			return waybillRemotingService.getAllExpressOrCargoProductInfo(transTypeCode);
		 }else{
			//对应一级产品类型
			List<String> productCodeList = new ArrayList<String>();
			//物流标示
			if(WaybillConstants.TYPE_OF_CARGO.equals(transTypeCode)){
				//物流一级汽运
				...
				//快递标示
			}else if(WaybillConstants.TYPE_OF_EXPRESS.equals(transTypeCode)){
				//快递一级汽运
				productCodeList.add(PricingConstants.ProductEntityConstants.PRICING_PRODUCT_C1_C20003);
			}else if(WaybillConstants.TRANS_VEHICLE.equals(transTypeCode)){
				//物流一级汽运
				productCodeList.add(PricingConstants.ProductEntityConstants.PRICING_PRODUCT_C1_C20001);
			}else if(WaybillConstants.AIR_FREIGHT.equals(transTypeCode)){
				//物流一级空运
				productCodeList.add(PricingConstants.ProductEntityConstants.PRICING_PRODUCT_C1_C20002);
			}
			...
			return productDao.getLevel3ForProductInfo(maps);
		 }
	}
	...
	public SaleDepartmentEntity querySimpleSaleDepartmentByCodeCache(String code) {
		//若为在线登陆，则查询在线数据
		if(CommonUtils.isOnline()){
			return waybillRemotingService.querySimpleSaleDepartmentByCodeCache(code);
		}else{
			return querySaleDepartmentByCode(code);
		}

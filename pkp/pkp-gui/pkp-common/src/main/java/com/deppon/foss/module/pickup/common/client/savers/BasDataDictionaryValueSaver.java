 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasDataDictionaryValueSaver.java
 * 
 * FILE NAME        	: BasDataDictionaryValueSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasDataDictionaryValueSaver extends AbstractSyncDataSaver {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return DataDictionaryValueEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<DataDictionaryValueEntity> dataDictionaryValues =  (List<DataDictionaryValueEntity>) data;
		if(dataDictionaryValues != null && !dataDictionaryValues.isEmpty()){
			for (DataDictionaryValueEntity dataDictionaryValue : dataDictionaryValues) {
				DataDictionaryValueService dataDictionaryValueService = DownLoadDataServiceFactory
						.getDataDictionaryValueService();
				
				if(	FossConstants.YES.equals(dataDictionaryValue.getActive())){
					dataDictionaryValueService.saveOrUpdate(dataDictionaryValue);
					
				}else{
					dataDictionaryValueService.delete(dataDictionaryValue);
				}
				
				Long version = dataDictionaryValue.getVersionNo();
				//获得最大版本号

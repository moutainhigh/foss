 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvEffectiveRegionSaver.java
 * 
 * FILE NAME        	: SrvEffectiveRegionSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvEffectiveRegionSaver extends AbstractSyncDataSaver{
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return EffectiveRegionEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<EffectiveRegionEntity> list = (List<EffectiveRegionEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			EffectiveRegionService effectiveRegionService = DownLoadDataServiceFactory
					.getEffectiveRegionService();
			for(EffectiveRegionEntity effectiveRegion : list){
				effectiveRegionService.saveOrUpdate(effectiveRegion);
				Long version = effectiveRegion.getVersionNo();
				//获得最大版本号

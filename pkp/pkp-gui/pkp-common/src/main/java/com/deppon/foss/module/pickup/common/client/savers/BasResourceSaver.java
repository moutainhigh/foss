 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasResourceSaver.java
 * 
 * FILE NAME        	: BasResourceSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasResourceSaver extends AbstractSyncDataSaver {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<ResourceEntity> list = (List<ResourceEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for(ResourceEntity resource: list){
				ResourceService resourceService = DownLoadDataServiceFactory
						.getResourceService();
				if(	FossConstants.YES.equals(resource.getActive())){
				...
				Long version = resource.getVersionNo();

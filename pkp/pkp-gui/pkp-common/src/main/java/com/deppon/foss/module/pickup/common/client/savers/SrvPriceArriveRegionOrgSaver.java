 ...
 * 
 * 
 * @ClassName: SrvPriceArriveRegionOrgSaver
 * 
 ...
 * 
 * @date 2013-10-8 上午11:02:05
 * 
 * 
 */
public class SrvPriceArriveRegionOrgSaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		...
		return PriceRegionOrgArriveEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<PriceRegionOrgArriveEntity> list = (List<PriceRegionOrgArriveEntity>) data;
		if (list != null && !list.isEmpty()) {
			ISrvPriceArriveRegionOrgService srvPriceArriveRegionOrgService = DownLoadDataServiceFactory
					.getSrvPriceArriveRegionOrgService();
			for (PriceRegionOrgArriveEntity reqionOrg : list) {
				if (FossConstants.YES.equals(reqionOrg.getActive())) {
					srvPriceArriveRegionOrgService.saveOrUpdate(reqionOrg);
				} else {
					srvPriceArriveRegionOrgService.deleteID(reqionOrg.getId());
				}

				Long version = reqionOrg.getVersionNo();
				// 获得最大版本号

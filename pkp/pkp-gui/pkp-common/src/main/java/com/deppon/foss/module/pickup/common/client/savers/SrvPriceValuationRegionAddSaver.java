 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvPriceValuationRegionAddSaver.java
 * 
 * FILE NAME        	: SrvPriceValuationRegionAddSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvPriceValuationRegionAddSaver extends AbstractSyncDataSaver {
	//区域id
	...
	public Class<?> getSaveType() {
		return PriceValuationRegionAddDto.class;
	}
	...
	public Date saveData(List<?> data) {
		...
		long mxVersion = 0;
		List<PriceValuationRegionAddDto> list = (List<PriceValuationRegionAddDto>)data;
		if(list != null && !list.isEmpty()){
			Map<String, List<PriceValuationRegionAddDto>> map =  (Map)list.get(0);
			for (Entry<String, List<PriceValuationRegionAddDto>> entry : map.entrySet()) {
			...
			for(PriceValuationRegionAddDto priceValuationRegionAddDto : entry.getValue()){

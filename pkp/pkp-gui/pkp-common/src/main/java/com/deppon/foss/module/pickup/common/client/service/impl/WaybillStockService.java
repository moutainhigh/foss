 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/service/impl/WaybillStockService.java
 * 
...
public class WaybillStockService implements IWaybillStockService {
	 ...
	 * 国际化
	 */
	private II18n i18n = I18nManager.getI18n(WaybillStockService.class);
	@Inject
	IOrgInfoService orgInfoService;// 组织信息
	@Inject
	ISalesDepartmentDao salesDepartmentDao;// 营业部门
	...
	public String queryStartStocksDepartmentService(WaybillEntity waybillEntry) {
		if (waybillEntry == null) {
			throw new WaybillStoreException(i18n.get("foss.gui.common.waybillStockService.exception.nullWaybillEntity"));
		}
		if (waybillEntry.getCreateOrgCode() == null) {
			throw new WaybillStoreException(i18n.get("foss.gui.common.waybillStockService.exception.nullCreateOrgCode"));
		}
		
		//BUG-51697 徐州贾汪区营业部开不了单
		waybillHessianRemoting = DefaultRemoteServiceFactory.getService(IWaybillHessianRemoting.class);
		OrgAdministrativeInfoEntity orgAdministrativeInfoEntity = waybillHessianRemoting.queryByCodeAndServerTime(waybillEntry.getCreateOrgCode());
//		OrgAdministrativeInfoEntity orgAdministrativeInfoEntity = orgInfoService.queryByCodeAndServerTime(waybillEntry.getCreateOrgCode());

		if (orgAdministrativeInfoEntity == null) {
			throw new WaybillStoreException(i18n.get("foss.gui.common.waybillStockService.exception.nullOrgAdministrativeInfoEntity"));
		}

		// 在集中开单的情况下 配载部门取法不同
		if (FossConstants.YES.equals(waybillEntry.getPickupCentralized())) {
			if (waybillEntry.getLoadOrgCode() == null) {
				throw new WaybillStoreException(i18n.get("foss.gui.common.waybillStockService.exception.nullLoadOrgCode"));
			}
			...
			if (FossConstants.YES.equals(orgAdministrativeInfoEntity.getSalesDepartment())){
				SaleDepartmentEntity saleDepartmentEntity = baseDataService.querySaleDepartmentByCodeOnline(waybillEntry.getCreateOrgCode());
				if (saleDepartmentEntity == null) {
					throw new WaybillStoreException(i18n.get("foss.gui.common.waybillStockService.exception.nullSaleDepartmentEntity"));
				}
				// 是否驻地部门
				...
				throw new WaybillStoreException(i18n.get("foss.gui.common.waybillStockService.exception.notSalesDepartment"));
			}
		}
	}

	@Override
	public String queryEndStocksDepartmentService(WaybillEntity waybillEntry) {
		if (waybillEntry == null) {
			throw new WaybillStoreException(i18n.get("foss.gui.common.waybillStockService.exception.nullWaybillEntity"));
		}
		if (waybillEntry.getLastLoadOrgCode() == null) {
			throw new WaybillStoreException(i18n.get("foss.gui.common.waybillStockService.exception.nullLastLoadOrgCode"));
		}
		SaleDepartmentEntity saleDepartmentEntity = salesDepartmentDao

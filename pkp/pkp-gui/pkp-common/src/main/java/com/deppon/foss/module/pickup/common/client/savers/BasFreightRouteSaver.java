 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasFreightRouteSaver.java
 * 
 * FILE NAME        	: BasFreightRouteSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasFreightRouteSaver extends AbstractSyncDataSaver{
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return FreightRouteEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<FreightRouteEntity> freightRouteEntitys =  (List<FreightRouteEntity>) data;
		long mxVersion = 0;
		if(freightRouteEntitys != null && !freightRouteEntitys.isEmpty()){
			FreightRouteService freightRouteService = DownLoadDataServiceFactory
					.getFreightRouteService();
			for (FreightRouteEntity freightRouteEntity : freightRouteEntitys) {
				
				if(	FossConstants.YES.equals(freightRouteEntity.getActive())){
					freightRouteService.saveOrUpdate(freightRouteEntity);
				}else{
					freightRouteService.delete(freightRouteEntity);
				}
				
				Long version = freightRouteEntity.getVersion();

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvPriceRegioOrgSaver.java
 * 
 * FILE NAME        	: SrvPriceRegioOrgSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvPriceRegioOrgSaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return PriceRegioOrgnEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<PriceRegioOrgnEntity> list = (List<PriceRegioOrgnEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			PriceRegioOrgService priceRegioOrgService = DownLoadDataServiceFactory
					.getPriceRegioOrgService();
			for(PriceRegioOrgnEntity priceRegioOrgn : list){
				priceRegioOrgService.saveOrUpdate(priceRegioOrgn);

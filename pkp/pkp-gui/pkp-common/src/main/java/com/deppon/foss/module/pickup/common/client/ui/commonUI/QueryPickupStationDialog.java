 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/ui/commonUI/QueryPickupStationDialog.java
 * 
 * FILE NAME        	: QueryPickupStationDialog.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 * </p>
 * <p style="display:none">
 * version:V1.0,author:foss-jiangfei,date:2012-10-31 上午9:00:18,content
 * </p>
 * 
 * @author foss-jiangfei
 * @date 2012-10-31 上午9:00:18
 * @since
 * @version
 *  *  * 录入货物信息SUC业务规则
 ...
 *
 */
public class QueryPickupStationDialog extends JDialog {
	 ...
	 * 国际化
	 */
	private II18n i18n = I18nManager.getI18n(QueryPickupStationDialog.class);
	 ...
	 * @param targetOrg
	 */
	public QueryPickupStationDialog(QueryPickupPointDto dto) {
		...
		String simpleName= dto.getOrgSimpleName();
		if(StringUtils.isNotEmpty(simpleName)){
			simpleName = StringUtils.substringBeforeLast(simpleName, "市");
		}
		this.initData2(false,simpleName);
		dto.setOrgSimpleName(simpleName);
		this.queryPickupStatonInfo(dto);
		this.addTableListener();
		this.addButtonListener();
	
		//监听Enter键
		EnterKeyForQueryPickup enter=new EnterKeyForQueryPickup(btnNewButton);		
		textField.addKeyListener(enter);
		EnterKeyForQueryPickup enterTabel=new EnterKeyForQueryPickup(tbltable,this);		
	 ...
	 */
	public  QueryPickupStationDialog(List<BranchVo> depts,WaybillPanelVo bean) {
		super(ApplicationContext.getApplication().getWorkbench().getFrame());
		//非空判断
	...
	private void queryPickupStatonInfo(QueryPickupPointDto dto) {
		...
		if (!queryType) { // queryType is false
			...
			if (FossConstants.YES.equals(dto.getIsHome())) {
				temp.setIsHome(FossConstants.YES);
				temp.setPickUpDoor(null);
			}
	...
	private  void setBranchVo()
	{
		...
		if(queryType)
		{
			...
			//是否支持快递快递返单签收
			branchVO.setCanExpressReturnSignBill(selectedSaleDepartmentInfo.getCanExpressReturnSignBill());
			//是否加盟网点
			branchVO.setIsLeagueSaledept(selectedSaleDepartmentInfo.getIsLeagueSaleDept());
			//是否二级网点
			branchVO.setIsTwoLevelNetwork(selectedSaleDepartmentInfo.getIsTwoLevelNetwork());
			//网点模式
			branchVO.setNetworkModel(selectedSaleDepartmentInfo.getNetworkModel());
		}else{
			//代理编号
	...
	private void initData2(boolean isPickupOrg,String orgName) {
		if(!isPickupOrg){
			if (!queryType) {
	 ...
	 */
	private void initGui(String orgName) {
		setTitle(i18n.get("foss.gui.common.queryPickupStationDialog.pickUpStation.title"));
		setSize(NumberConstants.NUMBER_700, NumberConstants.NUMBER_450);
		...
		//ISSUE-4430 开单增加虚拟营业部校验
		//1、开单送货根据城市筛选出快递营业部，如：输入城市为"北京"，
		//则虚拟营业部筛选出来的只有"北京快递营业部"和"北京远郊营业部"，但是还是支持输入关键字重新搜索后选择其他虚拟营业部
		if(StringUtils.isNotEmpty(orgName)){
			textField.setText(orgName);
		}
...
class OuterBranchEntityModel extends DefaultTableModel {
	...
	@Override
	public int getColumnCount() {
		return NumberConstants.NUMBER_4;
	}

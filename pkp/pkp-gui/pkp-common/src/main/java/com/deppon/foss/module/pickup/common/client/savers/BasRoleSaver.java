 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasRoleSaver.java
 * 
 * FILE NAME        	: BasRoleSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasRoleSaver extends AbstractSyncDataSaver  {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<RoleEntity> list = (List<RoleEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for(RoleEntity role: list){
				RoleService roleService = DownLoadDataServiceFactory
						.getRoleService();
				

				if(	FossConstants.YES.equals(role.getActive())){
				...
				Long version = role.getVersionNo();

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvDiscountProgramSaver.java
 * 
 * FILE NAME        	: SrvDiscountProgramSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvDiscountProgramSaver extends AbstractSyncDataSaver {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return DiscountProgramEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<DiscountProgramEntity> list = (List<DiscountProgramEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for(DiscountProgramEntity discountProgram: list){
				DiscountProgramService discountProgramService = DownLoadDataServiceFactory
						.getDiscountProgramService();
				discountProgramService.saveOrUpdate(discountProgram);
				Long version = discountProgram.getVersionNo();
				//获得最大版本号

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvMarketingEventSaver.java
 * 
 * FILE NAME        	: SrvMarketingEventSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvMarketingEventSaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return MarketingEventEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<MarketingEventEntity> list = (List<MarketingEventEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			MarketingEventService marketingEventService = DownLoadDataServiceFactory
					.getMarketingEventService();
			for(MarketingEventEntity marketingEvent: list){
				marketingEventService.saveOrUpdate(marketingEvent);
				Long version = marketingEvent.getVersionNo();

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvDiscountOrgSaver.java
 * 
 * FILE NAME        	: SrvDiscountOrgSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvDiscountOrgSaver extends AbstractSyncDataSaver {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return DiscountOrgEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<DiscountOrgEntity> list = (List<DiscountOrgEntity>)data;
		long mxVersion = 0;
		List<PriceCriteriaDetailEntity> detailList;
		if(list != null && !list.isEmpty()){
			DiscountOrgService discountOrgService = DownLoadDataServiceFactory
					.getDiscountOrgService();
			PriceValuationService priceValuationService = DownLoadDataServiceFactory
					.getPriceValuationService();
			PriceCriteriaDetailService priceCriteriaDetailServic = DownLoadDataServiceFactory
					.getPriceCriteriaDetailService();
			for(DiscountOrgEntity discountOrg: list){
				discountOrgService.saveOrUpdate(discountOrg);
				PriceValuationEntity priceValuation = discountOrg.getPriceValuationEntity();
				if(priceValuation != null){
					priceValuationService.saveOrUpdate(priceValuation);
					detailList = priceValuation.getCriteriaDetailEntities();
					if(detailList != null){
						for(PriceCriteriaDetailEntity detailEntity :detailList){
							priceCriteriaDetailServic.saveOrUpdate(detailEntity);
						}
					}
				}
				
				Long version = discountOrg.getVersionNo();
				//获得最大版本号

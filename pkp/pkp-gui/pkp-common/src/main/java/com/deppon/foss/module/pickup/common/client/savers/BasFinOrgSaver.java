 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasFinOrgSaver.java
 * 
 * FILE NAME        	: BasFinOrgSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 * @return 
 */
public class BasFinOrgSaver extends AbstractSyncDataSaver {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return FinancialOrganizationsEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<FinancialOrganizationsEntity> financialOrganizations =  (List<FinancialOrganizationsEntity>) data;
		if(financialOrganizations != null && !financialOrganizations.isEmpty()){
			FinOrgService finOrgService = DownLoadDataServiceFactory
					.getFinOrgService();
			for (FinancialOrganizationsEntity entity : financialOrganizations) {
				
				if(	FossConstants.YES.equals(entity.getActive())){
					finOrgService.saveOrUpdate(entity);
				}else{
					finOrgService.delete(entity);
				}

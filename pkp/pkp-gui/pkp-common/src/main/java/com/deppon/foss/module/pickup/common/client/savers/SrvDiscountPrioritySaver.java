 ...
 * 
 * 
 * @ClassName: SrvDiscountPrioritySaver
 * 
 * @Description: BUG-55198 优惠方案类型
 * 
 ...
 * 
 */
public class SrvDiscountPrioritySaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return DiscountPriorityEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<DiscountPriorityEntity> list = (List<DiscountPriorityEntity>) data;
		if (list != null && !list.isEmpty()) {
			ISrvDiscountPriorityService srvDiscountPriorityService = DownLoadDataServiceFactory
					.getSrvDiscountPriorityService();
			for (DiscountPriorityEntity entity : list) {
				srvDiscountPriorityService.saveOrUpdate(entity);

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasBusinessPartnerSaver.java
 * 
 * FILE NAME        	: BasBusinessPartnerSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasBusinessPartnerSaver extends AbstractSyncDataSaver  {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<BusinessPartnerEntity> list = (List<BusinessPartnerEntity>)data;

		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			BusinessPartnerService businessPartnerService = DownLoadDataServiceFactory
					.getBusinessPartnerService();
			for(BusinessPartnerEntity businessPartner: list){
				if(	FossConstants.YES.equals(businessPartner.getActive())){
					businessPartnerService.saveOrUpdate(businessPartner);
				}else{
					businessPartnerService.delete(businessPartner);
				}
				
				
				Long version = businessPartner.getVersionNo();
				//获得最大版本号

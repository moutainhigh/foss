 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvProductItemSaver.java
 * 
 * FILE NAME        	: SrvProductItemSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvProductItemSaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return ProductItemEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<ProductItemEntity> list = (List<ProductItemEntity>)data;
		if(list != null && !list.isEmpty()){
			ProductItemService productItemService = DownLoadDataServiceFactory
					.getProductItemService();
			for(ProductItemEntity productItem : list){
				
				if(	FossConstants.YES.equals(productItem.getActive())){	
					productItemService.saveOrUpdate(productItem);
				...
				Long version = productItem.getVersionNo();
				//获得最大版本号

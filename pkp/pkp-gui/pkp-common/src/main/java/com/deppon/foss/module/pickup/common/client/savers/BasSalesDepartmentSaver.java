 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasSalesDepartmentSaver.java
 * 
 * FILE NAME        	: BasSalesDepartmentSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasSalesDepartmentSaver extends AbstractSyncDataSaver   {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return SaleDepartmentEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<SaleDepartmentEntity> list = (List<SaleDepartmentEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for(SaleDepartmentEntity saleDepartment: list){
				SalesDepartmentService salesDepartmentService = DownLoadDataServiceFactory
						.getSalesDepartmentService();
				if(	FossConstants.YES.equals(saleDepartment.getActive())){	
					salesDepartmentService.saveOrUpdate(saleDepartment);
				}else{
					salesDepartmentService.delete(saleDepartment);
					
				}
			
				Long version = saleDepartment.getVersionNo();

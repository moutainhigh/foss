 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/ui/addressfield/autocomplete/AutoCompleteExtender.java
 * 
 * FILE NAME        	: AutoCompleteExtender.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 *
 */
package com.deppon.foss.module.pickup.common.client.ui.addressfield.autocomplete;
...
import com.deppon.foss.module.pickup.common.client.ui.addressfield.autocomplete.match.DataMatchHelper;
import com.deppon.foss.module.pickup.common.client.ui.addressfield.autocomplete.match.DefaultDataMatchHelper;
import com.deppon.foss.module.pickup.common.client.ui.addressfield.autocomplete.match.MatchedType;
import com.deppon.foss.module.pickup.common.client.ui.addressfield.autocomplete.match.SearchElement;
import com.deppon.foss.module.pickup.common.client.ui.addressfield.autocomplete.provider.DataProvider;
import com.deppon.foss.module.pickup.common.client.ui.addressfield.autocomplete.provider.DefaultDataProvider;

/**
 * 
 * 自动完成扩展工具
 * <p style="display:none">modifyRecord</p>
 * <p style="display:none">version:V1.0,author:DP-shaohongliang,date:2012-10-18 下午4:54:21,content: </p>
 * @author DP-shaohongliang
 * @date 2012-10-18 下午4:54:21
 * @since
 * @version
 *  *  * 录入货物信息SUC业务规则
...
public class AutoCompleteExtender {
	 ...
	 * 国际化
	 */
	private II18n i18n = I18nManager.getI18n(AutoCompleteExtender.class);
	/**
	 * 显示的列数
	 */
     ...
     */
    public AutoCompleteExtender(JTextComponent textComponent, List<AddressFieldDto> data) {
        ...
        if (data != null) {
            for (AddressFieldDto value : data) {
    ...
    public synchronized void setCommitListener(CommitListener commitListener) {
     ...
     * @return
     */
    public synchronized SearchElement getSelectedValue() {
        return (SearchElement)resultList.getSelectedValue();
    }

    /**
     * 确定指定的文本为最终选定
     * update -354805-taodongguo
     * @updateDate 2016-10-11 15:48:43
     * @param element
     */
    public synchronized void commitValue(SearchElement element) {
    	AddressFieldDto value = element.getValue();
    	originalEditText = value.toString();
        StringBuffer sbuf = new StringBuffer();
        if(StringUtil.isNotBlank(value.getNationName()) && !WaybillConstants.CHINA.equals(value.getNationName().trim())){
        	sbuf.append(value.getNationName()).append("-");
        }
        if(value.getDegree() == 1){//检索省份
        	sbuf.append(value.getProvinceName());
        }else if(value.getDegree() == 2){ //检索城市
        	sbuf.append(value.getProvinceName()).append("-").append(value.getCityName());
        }else{ //检索区县
            sbuf.append(value.getProvinceName()).append("-").append(value.getCityName()).append("-").append(value.getCountyName());
        }
        textComponent.setText(sbuf.toString());
    ...
    public synchronized boolean doMatch() {
        ...
        if (dataMatchHelper != null && !dataMatchHelper.isMatchTextAccept(keyWord)) {
            return false;
    ...
    public synchronized void setMatchDataAsync(boolean async) {
        if (this.matchDataAsync != async) {
            ...
            if (async) {
                ...
                executor = new ThreadPoolExecutor(2, NumberConstants.NUMBER_10, NumberConstants.NUMBER_20, TimeUnit.SECONDS, queue);
            } else {
                if (queue != null) {
                    queue.clear();
                }
     ...
     * @param text
     */
    public synchronized void showToolTips(String text) {
        if (text != null) {
    ...
    private void doMatchInner(String matchText) {
        if (dataProvider != null) {
            ...
            for (AddressFieldDto value : dataProvider.toArray()) {
                if (dataMatchHelper != null) {
                	MatchedType matchedType = dataMatchHelper.isDataMatched(matchText, value);
                    switch (matchedType) {
						case NoMatch:
							break;
						case FullMatch:
		                	listModel.addElement(new SearchElement(matchText,value,MatchedType.FullMatch));
							break;
						case ShortMatch:
		                	listModel.addElement(new SearchElement(matchText,value,MatchedType.ShortMatch));
							break;
						default:
		                	listModel.addElement(new SearchElement(matchText,value));
							break;
					} 
                } else {
                    // 直接添加
    ...
    private void commitTextBySelectedValue() {
    	SearchElement value = getSelectedValue();
        if (value != null) {
            commitValue(value);
        }
        collapse();
     ...
     * 转移焦点到文本编辑框，并关闭结果列表
     */
    private synchronized void changeFocusToTextField() {
        // 取消选择
        removeSelectionInterval();
        // 转移焦点到文本框
        setFocusOnTextComponent();
    ...
    /**
     * 设置当前选中项的值到文本框
     * @updateAuthor 354805-taodongguo
     * @updateDate 2016-10-11 15:50:38
     */
    private synchronized void showCurrentSelectedValue() {
    	SearchElement element = getSelectedValue();
        if (element != null) {
        	AddressFieldDto value = element.getValue();
            StringBuffer sbuf = new StringBuffer();
            if(StringUtil.isNotBlank(value.getNationName()) && !WaybillConstants.CHINA.equals(value.getNationName().trim())){
    ...
    private synchronized void updateExpandListUI() {
        ...
        int preferredHeight = Math.min(resultList.getVisibleRowCount(), dataSize) * resultList.getFixedCellHeight() + NumberConstants.NUMBER_3; // 多预留一些空间，这个值可自己调整不是很准的
    ...
    private final KeyAdapter defaultTextFieldKeyAdapter = new KeyAdapter() {
        ...
        public void keyReleased(KeyEvent e) {
            ...
            if (keyCode == KeyEvent.VK_UP
                    ...
                    || keyCode == KeyEvent.VK_ENTER /*VK_BACK_SPACE*/) {
                return;
            }
            /**
             * 打开结果列表

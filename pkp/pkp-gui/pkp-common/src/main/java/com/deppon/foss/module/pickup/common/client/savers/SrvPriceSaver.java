 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvPriceSaver.java
 * 
 * FILE NAME        	: SrvPriceSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvPriceSaver extends AbstractSyncDataSaver {
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<PriceEntity> list = (List<PriceEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			PriceService priceService = DownLoadDataServiceFactory
					.getPriceService();
			for(PriceEntity price : list){
				priceService.saveOrUpdate(price);
				Long version = price.getVersionNo();
				//获得最大版本号

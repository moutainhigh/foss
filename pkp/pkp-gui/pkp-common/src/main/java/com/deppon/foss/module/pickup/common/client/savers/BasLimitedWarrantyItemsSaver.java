 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasLimitedWarrantyItemsSaver.java
 * 
 * FILE NAME        	: BasLimitedWarrantyItemsSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasLimitedWarrantyItemsSaver extends AbstractSyncDataSaver  {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return LimitedWarrantyItemsEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<LimitedWarrantyItemsEntity> limitedWarrantyItemsEntitys =  (List<LimitedWarrantyItemsEntity>) data;
		if(limitedWarrantyItemsEntitys != null && !limitedWarrantyItemsEntitys.isEmpty()){
			for (LimitedWarrantyItemsEntity limitedWarrantyItemsEntity : limitedWarrantyItemsEntitys) {
				LimitedWarrantyItemsService limitedWarrantyItemsService = DownLoadDataServiceFactory
						.getLimitedWarrantyItemsService();
				
				if(	FossConstants.YES.equals(limitedWarrantyItemsEntity.getActive())){
					limitedWarrantyItemsService.saveOrUpdate(limitedWarrantyItemsEntity);
					
				}else{
					limitedWarrantyItemsService.delete(limitedWarrantyItemsEntity);
				}
				
				
				Long version = limitedWarrantyItemsEntity.getVersionNo();

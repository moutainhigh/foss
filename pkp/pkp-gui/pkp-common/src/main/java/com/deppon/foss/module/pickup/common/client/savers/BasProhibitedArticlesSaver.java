 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasProhibitedArticlesSaver.java
 * 
 * FILE NAME        	: BasProhibitedArticlesSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
...
public class BasProhibitedArticlesSaver extends AbstractSyncDataSaver  {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return ProhibitedArticlesEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<ProhibitedArticlesEntity> list = (List<ProhibitedArticlesEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for(ProhibitedArticlesEntity prohibitedArticlesEntity: list){
				ProhibitedArticlesService prohibitedArticlesService = DownLoadDataServiceFactory
						.getProhibitedArticlesService();
				if(	FossConstants.YES.equals(prohibitedArticlesEntity.getActive())){
					prohibitedArticlesService.saveOrUpdate(prohibitedArticlesEntity);
					
				}else{
					prohibitedArticlesService.delete(prohibitedArticlesEntity);
				}
				
				
				Long version = prohibitedArticlesEntity.getVersionNo();

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasConfigSaver.java
 * 
 * FILE NAME        	: BasConfigSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasConfigSaver extends AbstractSyncDataSaver  {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Class<?> getSaveType() {
		return ConfigurationParamsEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<ConfigurationParamsEntity> list = (List<ConfigurationParamsEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			for(ConfigurationParamsEntity config: list){
				ConfigService configService = DownLoadDataServiceFactory
						.getConfigService();
				
				if(	FossConstants.YES.equals(config.getActive())){
				...
				configService.saveOrUpdate(config);
				Long version = config.getVersionNo();
				//获得最大版本号

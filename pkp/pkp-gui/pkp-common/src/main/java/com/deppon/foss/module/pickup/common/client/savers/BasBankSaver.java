 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasBankSaver.java
 * 
 * FILE NAME        	: BasBankSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasBankSaver extends AbstractSyncDataSaver {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<BankEntity> list = (List<BankEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			BankService bankService = DownLoadDataServiceFactory
					.getBankService();
			for(BankEntity bankEntity: list){
				bankService.saveOrUpdate(bankEntity);
				Long version = bankEntity.getVersionNo();
				//获得最大版本号

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvEffectiveRegionOrgSaver.java
 * 
 * FILE NAME        	: SrvEffectiveRegionOrgSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvEffectiveRegionOrgSaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return EffectiveRegionOrgEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<EffectiveRegionOrgEntity> list = (List<EffectiveRegionOrgEntity>)data;
		if(list != null && !list.isEmpty()){
			EffectiveRegionOrgService effectiveRegionOrgService = DownLoadDataServiceFactory
					.getEffectiveRegionOrgService();
			for(EffectiveRegionOrgEntity priceOrg : list){
				effectiveRegionOrgService.saveOrUpdate(priceOrg);
				
				Long version = priceOrg.getVersionNo();
				//获得最大版本号

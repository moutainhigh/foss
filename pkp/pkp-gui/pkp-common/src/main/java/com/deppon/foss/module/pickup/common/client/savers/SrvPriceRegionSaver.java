 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvPriceRegionSaver.java
 * 
 * FILE NAME        	: SrvPriceRegionSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvPriceRegionSaver extends AbstractSyncDataSaver {
	...
	public Class<?> getSaveType() {
		return PriceRegionEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<PriceRegionEntity> list = (List<PriceRegionEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			PriceRegionService priceRegionService = DownLoadDataServiceFactory
					.getPriceRegionService();
			for(PriceRegionEntity priceRegion : list){
				priceRegionService.saveOrUpdate(priceRegion);
				Long version = priceRegion.getVersionNo();
				//获得最大版本号

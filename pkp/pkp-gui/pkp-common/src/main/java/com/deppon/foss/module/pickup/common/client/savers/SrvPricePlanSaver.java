 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvPricePlanSaver.java
 * 
 * FILE NAME        	: SrvPricePlanSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvPricePlanSaver extends AbstractSyncDataSaver {
	//区域id
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<Map<String, List<PricePlanEntity>>> list = (List<Map<String, List<PricePlanEntity>>>) data;
		long mxVersion = 0;
		boolean needDelete = true;
		if (list != null && !list.isEmpty()) {
			Map<String, List<PricePlanEntity>> map =  (Map)list.get(0);
			for (Entry<String, List<PricePlanEntity>> entry : map.entrySet()) {
				List<PricePlanEntity> dataList = (List<PricePlanEntity>) entry
						.getValue();
				...
				for (PricePlanEntity pricePlan : dataList) {
					pricePlanService.saveOrUpdate(pricePlan);
					Long version = pricePlan.getVersionNo();
					regionID = pricePlan.getPriceRegionId();
					//获得最大版本号

 ...
 */
public class SrvPriceRegionOrgAirSaver extends AbstractSyncDataSaver {
	//区域id
	...
	public Class<?> getSaveType() {
		return PriceRegionOrgAirEntity.class;
	}
	...
	public Date saveData(List<?> data) {
		Date date = null;
		List<PriceRegionOrgAirEntity> list = (List<PriceRegionOrgAirEntity>)data;
		long mxVersion = 0;
		if(list != null && !list.isEmpty()){
			PriceRegionOrgAirService priceRegionAirService = DownLoadDataServiceFactory
					.getPriceRegionOrgAirService();
			for(PriceRegionOrgAirEntity priceRegionAir : list){
				priceRegionAirService.saveOrUpdate(priceRegionAir);

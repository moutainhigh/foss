 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/SrvPriceValuationProductAddSaver.java
 * 
 * FILE NAME        	: SrvPriceValuationProductAddSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SrvPriceValuationProductAddSaver extends AbstractSyncDataSaver {
	//区域id
	...
	public Class<?> getSaveType() {
		return PriceValuationProductAddDto.class;
	}
	...
	public Date saveData(List<?> data) {
		...
		long mxVersion = 0;
		List<PriceValuationProductAddDto> list = (List<PriceValuationProductAddDto>)data;
		if(list != null && !list.isEmpty()){
			Map<String, List<PriceValuationProductAddDto>> map =  (Map)list.get(0);
			for (Entry<String, List<PriceValuationProductAddDto>> entry : map.entrySet()) {
			...
			for(PriceValuationProductAddDto PriceValuationProductAddDto : entry.getValue()){
				priceValuationService.saveOrUpdate(PriceValuationProductAddDto);
				detailList = PriceValuationProductAddDto.getCriteriaDetailEntities();
				if(detailList != null){
				...
				Long version = PriceValuationProductAddDto.getVersionNo();
				//取新价格区域ID
				regionID = PriceValuationProductAddDto.getDeptRegionId();
				//获得最大版本号

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/common/client/savers/BasLineItemSaver.java
 * 
 * FILE NAME        	: BasLineItemSaver.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class BasLineItemSaver extends AbstractSyncDataSaver {
	/**
	 * 
	 * 功能：映射实体类
	...
	public Date saveData(List<?> data) {
		Date date = null;
		long mxVersion = 0;
		List<LineItemEntity> lineItems =  (List<LineItemEntity>) data;
		if(lineItems !=null && !lineItems.isEmpty()){
			LineItemService lineItemService = DownLoadDataServiceFactory
					.getLineItemService();
			for (LineItemEntity lineItemEntity : lineItems) {
				
				if(	FossConstants.YES.equals(lineItemEntity.getActive())){
					lineItemService.saveOrUpdate(lineItemEntity);
					
				}else{
					lineItemService.delete(lineItemEntity);
				}
				
				Long version = lineItemEntity.getVersion();

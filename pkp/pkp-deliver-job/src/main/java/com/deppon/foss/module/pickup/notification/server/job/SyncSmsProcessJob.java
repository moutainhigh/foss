...
/**
 * 
 * 同步客户通知模块短信、语音通知结果JOB
 * 
 * @author ibm-wangfei
 * @date 2012-11-28 15:07:03
 */
public class SyncSmsProcessJob extends GridJob implements StatefulJob{
	private static final Logger LOGGER = LoggerFactory.getLogger(SyncSmsProcessJob.class);
	...
	protected void doExecute(JobExecutionContext arg0) throws JobExecutionException {
		try {
//			LOGGER.info("SyncSmsPreprocessJob begin");
//			INotifyCustomerService notifyCustomerService = getBean("notifyCustomerService", INotifyCustomerService.class);
//			// 调用短信同步方法
//			notifyCustomerService.syncSmsStatus();
//			LOGGER.info("SyncSmsPreprocessJob End");
		} catch (Exception e) {
			LOGGER.error("同步客户通知模块短信、语音通知结果JOB error", e);
//			throw new JobExecutionException("同步客户通知模块短信、语音通知结果JOB error", e);
		}
	}
}
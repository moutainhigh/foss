...
pricing.showDept = function(regionId){
	var grid = Ext.getCmp('T_pricing-indexExpress_content').getAreaGrid();
...
pricing.showEndTimeWindow = function(id){
	var grid = Ext.getCmp('T_pricing-indexExpress_content').getAreaGrid().getRegionUpdateWindow().getDeptEditGrid();
 ...
 * 区域与部门关联MODEL
 */
Ext.define('Foss.pricing.priceRegionExpress.RegionDeptModel', {
 ...
 * 区域MODEL
 */
Ext.define('Foss.pricing.priceRegionExpress.AreaModel', {
	extend : 'Ext.data.Model',
	fields : [ {
		name : 'id',
		type : 'string'
 ...
 * 区域Store
 */
Ext.define('Foss.pricing.priceRegionExpress.AreaStore', {
	extend : 'Ext.data.Store',
	model : 'Foss.pricing.priceRegionExpress.AreaModel',
	pageSize:20,
	proxy : {
		type : 'ajax',
		actionMethods : 'post',
		url : "../pricing/searchRegionExpressByCondition.action",
		reader : {
			type : 'json',
 ...
 * 区域查询表单
 */
Ext.define('Foss.pricing.priceRegionExpress.QueryRegionForm', {
	...
	constructor : function(config) {
		...
		me.items = [{
		    ...
		    valueField: 'key',
		    editable:false,
		    store:pricing.getStore('Foss.pricing.priceRegionExpress.AreaStatusStore',null,['key','value']
			...
			items : [{
				...
				disabled: !pricing.region.isPermission('indexExpress/indexQueryButton'),
				hidden: !pricing.region.isPermission('indexExpress/indexQueryButton'),
				handler : function() {
 ...
 * 区域列表
 */
Ext.define('Foss.pricing.priceRegionExpress.AreaGridPanel', {
	...
	getRegionWindow : function(){
		if(Ext.isEmpty(this.regionWindow)){
			this.regionWindow = Ext.create('Foss.pricing.priceRegionExpress.RegionWindow');
		}
	...
	getEndTimeWindow : function(){
		if(Ext.isEmpty(this.endTimeWindow)){
			this.endTimeWindow = Ext.create('Foss.pricing.priceRegionExpress.EndTimeWindow',{
				type:'BEGIN'
			});
		}
	...
	getImmediatelyActiveWindow: function(){
		if(Ext.isEmpty(this.immediatelyActiveWindow)){
			this.immediatelyActiveWindow = Ext.create('Foss.pricing.priceRegionExpress.ImmediatelyActiveTimeWindow');
	...
	getImmediatelyStopWindow: function(){
		if(Ext.isEmpty(this.immediatelyStopWindow)){
			this.immediatelyStopWindow = Ext.create('Foss.pricing.priceRegionExpress.ImmediatelyStopEndTimeWindow');
	...
	getRegionUpdateWindow : function(){
		if(Ext.isEmpty(this.regionUpdateWindow)){
			this.regionUpdateWindow = Ext.create('Foss.pricing.priceRegionExpress.RegionUpdateWindow');
		}
	...
	getShowDeptWindow : function(){
		if(Ext.isEmpty(this.showDeptWindow)){
			this.showDeptWindow = Ext.create('Foss.pricing.priceRegionExpress.PricingDepartmentWindow');
		}
	...
	deleteArea: function() {
		...
		pricing.showQuestionMes(pricing.region.i18n('foss.pricing.isDeleteTheseUnActiveRegion'),function(e){//是否要删除这些未激活的区域？
			if(e=='yes'){//询问是否激活，是则发送请求
				var params = {'regionExpressVo':{'regionIds':regionIds,'regionNature':pricing.region.PRICING_REGION}};
	...
	constructor : function(config) {
		...
		me.columns = [{xtype: 'rownumberer',
			...
			items: [{
                ...
                disabled: !pricing.region.isPermission('indexExpress/indexUpdateButton'),
				width:42,
                handler: function(grid, rowIndex, colIndex) {
                	//获取选中的数据
    				var record=grid.getStore().getAt(rowIndex);
                	var id= record.get('id');//区域编码
    				var params = {'regionExpressVo':{'regionEntity':{'id':id,'regionNature':pricing.region.PRICING_REGION}}};
    				var successFun = function(json){
    					var updateWindow = me.getRegionUpdateWindow();//获得修改窗口
    					updateWindow.selection = new Foss.pricing.priceRegionExpress.AreaModel(json.regionExpressVo.regionEntity);//区域
                ...
                disabled: !pricing.region.isPermission('indexExpress/indexDeleteQueryButton'),
				width:42,
			...
			renderer:function(value){
				if(value==pricing.region.PRESCRIPTION_REGION){//时效区域
					...
					return  pricing.region.i18n('i18n.pricingRegion.pricingAreaExpress');
				}else{
					return '';
				}
			}
		}];
		me.store = Ext.create('Foss.pricing.priceRegionExpress.AreaStore',{
			autoLoad : false,
			pageSize : 20,
			listeners: {
				beforeload: function(store, operation, eOpts){
					var queryForm = Ext.getCmp('T_pricing-indexExpress_content').getQueryForm();
					if(queryForm!=null){
						Ext.apply(operation,{
							params : {
								...
								'regionExpressVo.regionEntity.regionNature':pricing.region.PRICING_REGION
							}
						});	
					}
				}
		...
		me.tbar = [{
			...
			disabled: !pricing.region.isPermission('indexExpress/indexAddButton'),
			hidden: !pricing.region.isPermission('indexExpress/indexAddButton'),
			...
			disabled: !pricing.region.isPermission('indexExpress/indexImmediatelyActiveButton'),
			hidden: !pricing.region.isPermission('indexExpress/indexImmediatelyActiveButton'),
			handler :function(){
				me.immediatelyActive(this);
			} 
		},'-', {
			text : pricing.region.i18n('foss.pricing.immediatelyStopProgram'),//立即中止
			disabled: !pricing.region.isPermission('indexExpress/indexImmediatelyStopButton'),
			hidden: !pricing.region.isPermission('indexExpress/indexImmediatelyStopButton'),
			handler :function(){
			...
			disabled: !pricing.region.isPermission('indexExpress/indexDeleteQueryButton'),
			hidden: !pricing.region.isPermission('indexExpress/indexDeleteQueryButton'),
			handler :function(){
				me.deleteArea();
			} 
		}];
...
Ext.onReady(function() {
	Ext.QuickTips.init();
	if (Ext.getCmp('T_pricing-indexExpress_content')) {
		return;
	}
	pricing.haveServerNowTime();
	var queryForm = Ext.create('Foss.pricing.priceRegionExpress.QueryRegionForm');//查询FORM
	var areaGrid = Ext.create('Foss.pricing.priceRegionExpress.AreaGridPanel');//查询结果GRID
	Ext.getCmp('T_pricing-indexExpress').add(Ext.create('Ext.panel.Panel', {
	  	id : 'T_pricing-indexExpress_content',
 ...
 * 区域管理-区域部门关联显示弹窗
 */
Ext.define('Foss.pricing.priceRegionExpress.PricingDepartmentWindow',{
	...
	listeners:{
		...
		beforeshow:function(me){//根据区域ID和区域性质，查询区域关联部门
			var params = {'regionExpressVo':{priceRegioOrgnEntity:{'priceRegionId':me.regionId,'regionNature':pricing.region.PRICING_REGION}}};
			var successFun = function(json){
    ...
    getDeptShowGrid : function(){
    	if(Ext.isEmpty(this.deptShowGrid)){
    		this.deptShowGrid = Ext.create('Foss.pricing.priceRegionExpress.PricingDepartmentShowGridPanel');
    	}
 ...
 * 区域管理-区域部门关系显示-部门表格
 */
Ext.define('Foss.pricing.priceRegionExpress.PricingDepartmentShowGridPanel', {
	extend: 'Ext.grid.Panel',
	title : pricing.region.i18n('i18n.pricingRegion.deptDetailed'),
	frame: true,
	flex:1,
	...
	constructor : function(config) {
		var me = this, cfg = Ext.apply({}, config);
		me.store = pricing.getStore('Foss.pricing.priceRegionExpress.DepartmentShowGridStore','Foss.pricing.priceRegionExpress.RegionDeptModel',null,[]);
		me.listeners = {
	    	scrollershow: function(scroller) {
	    		if (scroller && scroller.scrollEl) {
	    				scroller.clearManagedListeners(); 
 ...
 * 区域管理-新增弹窗
 */
Ext.define('Foss.pricing.priceRegionExpress.RegionWindow',{
	extend : 'Ext.window.Window',
	...
	getEditForm:function(){
		if(this.editForm==null){
    		this.editForm = Ext.create('Foss.pricing.priceRegionExpress.RegionEditFormPanel');
    ...
    getDeptEditGrid : function(){
    	if(Ext.isEmpty(this.deptEditGrid)){
    		this.deptEditGrid = Ext.create('Foss.pricing.priceRegionExpress.DepartmentGridPanel');
    ...
    commintRegion:function(){
    	...
    	if(form.isValid()){//表单校验
    		var regionModel = new Foss.pricing.priceRegionExpress.AreaModel()
        	...
        	var successFun = function(json){
        		...
        		me.close();//隐藏窗口
        		Ext.getCmp('T_pricing-indexExpress_content').getAreaGrid().getPagingToolbar().moveFirst();//重新加载数据
        		me.getEl().unmask();
        	};
        	var failureFun = function(json){
        		me.getEl().unmask();
 ...
 * 区域管理-修改弹窗
 */
Ext.define('Foss.pricing.priceRegionExpress.RegionUpdateWindow',{
	extend : 'Ext.window.Window',
	...
	listeners:{
		...
		beforeshow:function(me){
			...
			me.getEditForm().items.items[5].setReginValue(selection.get('nationName'), selection.get('nationCode'), 0);
			me.getEditForm().items.items[5].setReginValue(selection.get('proName'), selection.get('proCode'), 1);
			me.getEditForm().items.items[5].setReginValue(selection.get('cityName'), selection.get('cityCode'),2);
			me.getEditForm().items.items[5].setReginValue(selection.get('countyName'), selection.get('countyCode'), 3);
			...
			if(!Ext.isEmpty(selection.get('regionType'))&&selection.get('regionType')==pricing.region.DEPT){//如果是类型是部门
				...
				var params = {'regionExpressVo':{priceRegioOrgnEntity:{'priceRegionId':selection.get('id'),'regionNature':pricing.region.PRICING_REGION}}};
				var successFun = function(json){
    ...
    commintRegion:function(){
    	...
    	if(form.isValid()){//符合填写条件
    		...
    		/*if(regionModel.get('regionType') == pricing.region.ADMINISTRATIVE){//行政区域
    			var proCode = me.getEditForm().items.items[5].items.items[0].getValue();
    			var cityCode = me.getEditForm().items.items[5].items.items[1].getValue();
    			var countyCode = me.getEditForm().items.items[5].items.items[2].getValue();
    			regionModel.set('proCode',proCode);
    			regionModel.set('cityCode',cityCode);
    			regionModel.set('countyCode',countyCode);
    		}*///修改的时候不需要向后台传省市区县，只要显示就够了
        	...
        	var successFun = function(json){
        		pricing.showInfoMes(json.message);
        		me.close();
        		Ext.getCmp('T_pricing-indexExpress_content').getAreaGrid().getPagingToolbar().moveFirst();
        	};
        	var failureFun = function(json){
        		if(Ext.isEmpty(json)){
    				pricing.showErrorMes(pricing.region.i18n('i18n.pricingRegion.requestTimeOut'));
 ...
 * 区域管理-新增-部门表格
 */
Ext.define('Foss.pricing.priceRegionExpress.DepartmentGridPanel',{
	extend: 'Ext.grid.Panel',
	...
	getDepartmentAddWindow:function(){
		if(Ext.isEmpty(this.departmentAddWindow)){
    		this.departmentAddWindow = Ext.create('Foss.pricing.priceRegionExpress.DepartmentAddWindow');
    	}
    	...
    	if(Ext.isEmpty(me.orgWindow)){
    		me.orgWindow = Ext.create('Foss.baseinfo.commonSelector.orgSelectorWindow',{
    			...
    			commitFun:function(){
    				...
    				var depts = new Array();
    				var grid = Ext.getCmp('T_pricing-indexExpress_content').getAreaGrid().getRegionWindow().getDeptEditGrid();//获取区域关联部门的GRID
	...
	constructor : function(config) {
		...
		if(me.type=='ADD'){//新增和修改获取两个完全不同的STORE
			me.store = pricing.getStore('Foss.pricing.priceRegionExpress.DepartmentEditGridStoreForAdd','Foss.pricing.priceRegionExpress.RegionDeptModel',null,[]);
		}else if(me.type=='UPDATE'){
			me.store = pricing.getStore('Foss.pricing.priceRegionExpress.DepartmentEditGridStoreForUpdate','Foss.pricing.priceRegionExpress.RegionDeptModel',null,[]);
		}
		me.columns = [{xtype: 'rownumberer',
			width:40,
			text : pricing.region.i18n('i18n.pricingRegion.num')//序号
 ...
 * 区域管理-新增/修改-FORM
 */
Ext.define('Foss.pricing.priceRegionExpress.RegionEditFormPanel', {
	extend : 'Ext.form.Panel',
	...
	items :[{
	    ...
	    editable:false,
	    readOnly:true,
	    store:pricing.getStore('Foss.pricing.priceRegionExpress.RegionNatureStore',null,['key','value'],[{'key':pricing.region.PRESCRIPTION_REGION,'value':pricing.region.i18n('i18n.pricingRegion.prescriptionArea')},{'key':pricing.region.PRICING_REGION,'value':pricing.region.i18n('i18n.pricingRegion.pricingAreaExpress')}]),
		 ...
		 items:[{
	        ...
	        listeners:{
	        	afterrender:function(comb) {
	        		...
	        		if(value==pricing.region.DEPT){//部门组织，新增加部门按钮可以操作，省市区县不可选
        		...
        		}else if(value==pricing.region.ADMINISTRATIVE){//行政组织
 ...
 */
Ext.define('Foss.pricing.priceRegionExpress.EndTimeWindow',{
	extend : 'Ext.window.Window',
	...
	getStopFormPanel : function() {
		if (Ext.isEmpty(this.stopFormPanel)) {
			this.stopFormPanel = Ext.create('Foss.pricing.priceRegionExpress.ImmediatelySuspendFormPanel');
		}
 ...
 * 立即中止功能FormPanel
 */
Ext.define('Foss.pricing.priceRegionExpress.ImmediatelySuspendFormPanel',{
	extend : 'Ext.form.Panel',
	layout:'column',
	 //设置失效日期
 ...
 * 立即激活价格方案Window
 */
Ext.define('Foss.pricing.priceRegionExpress.ImmediatelyActiveTimeWindow',{
	extend: 'Ext.window.Window',
	...
	getImmediatelyActiveFormPanel : function(){
    	if(Ext.isEmpty(this.immediatelyActiveFormPanel)){
    		this.immediatelyActiveFormPanel = Ext.create('Foss.pricing.priceRegionExpress.ImmediatelyActiveFormPanel');
    	}
 ...
 * 立即激活功能Form
 */
Ext.define('Foss.pricing.priceRegionExpress.ImmediatelyActiveFormPanel',{
	extend : 'Ext.form.Panel',
	layout:'column',
	activetion:function(){
		var me = this;
		...
		if(selectedDate<nowDate){
			pricing.showErrorMes(pricing.region.i18n('foss.pricing.activeTimeIsMustBeforeStopTime'));
			return false;
		}
		...
		if(form.isValid()){
			...
			var params = {'regionExpressVo':{'regionEntity':priceRegion,'regionNature':pricing.region.PRICING_REGION,'beginTime':beginTime}};
 ...
 * 立即中止价格区域方案 Window
 */
Ext.define('Foss.pricing.priceRegionExpress.ImmediatelyStopEndTimeWindow',{
		extend: 'Ext.window.Window',
		...
		getStopFormPanel : function(){
	    	if(Ext.isEmpty(this.stopFormPanel)) {
	    		this.stopFormPanel = Ext.create('Foss.pricing.priceRegionExpress.ImmediatelyStopFormPanel');
	    	}
	    	return this.stopFormPanel;
	    },
	    listeners:{
 ...
 * 立即中止价格区域功能FormPanel
 */
Ext.define('Foss.pricing.priceRegionExpress.ImmediatelyStopFormPanel',{
	extend : 'Ext.form.Panel',
	layout:'column',
	stop:function(){
		var me = this;
    	...
    	if(form.isValid()){
    		var priceRegion = me.up('window').priceRegionEntity;
    		var endTime = Ext.Date.parse(form.findField('endTime').getValue(), 'Y-m-d H:i:s');
    		var params = {'regionExpressVo':{'regionEntity':priceRegion,'regionNature':pricing.region.PRICING_REGION,'endTime':endTime}};

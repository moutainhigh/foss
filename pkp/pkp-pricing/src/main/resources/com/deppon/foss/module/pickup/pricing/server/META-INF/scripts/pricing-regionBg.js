...
pricing.showDept = function(regionId){
	var grid = Ext.getCmp('T_pricing-regionBgIndex_content').getAreaGrid();
...
pricing.showEndTimeWindow = function(id){
	var grid = Ext.getCmp('T_pricing-regionBgIndex_content').getAreaGrid().getRegionUpdateWindow().getDeptEditGrid();
...
//时效区域
pricing.regionBg.PRESCRIPTION_REGION = 'EFFECTIVE_REGION';
//价格区域
pricing.regionBg.PRICING_REGION = 'PRICING_REGION';
//行政区域
pricing.regionBg.ADMINISTRATIVE = 'ADMINISTRATIVE';
//网点
pricing.regionBg.DEPT = 'DEPT';
//服务器明天时间
pricing.regionBg.tomorrowTime = null;
//服务器今天天时间
pricing.regionBg.currentDay = null;
//获取服务当前时间
pricing.haveServerNowTime = function(){
	Ext.Ajax.request({
		...
		success:function(response){
			...
			pricing.regionBg.currentDay = new Date(result.pricingValuationVo.nowTime);
			pricing.regionBg.tomorrowTime = today.setDate(today.getDate()+1);
		},
		failure:function(response){
			var result = Ext.decode(response.responseText);
			if(Ext.isEmpty(result)){
				pricing.showErrorMes(pricing.regionBg.i18n('i18n.pricingRegion.requestTimeOut'));
			}else{
 ...
 * 区域与部门关联MODEL
 */
Ext.define('Foss.pricing.regionBg.RegionDeptModel', {
 ...
 * 区域MODEL
 */
Ext.define('Foss.pricing.regionBg.AreaModel', {
	extend : 'Ext.data.Model',
	fields : [ {
		name : 'id',
		type : 'string'
 ...
 * 区域Store
 */
Ext.define('Foss.pricing.regionBg.AreaStore', {
	extend : 'Ext.data.Store',
	model : 'Foss.pricing.regionBg.AreaModel',
	pageSize:20,
	proxy : {
		type : 'ajax',
		actionMethods : 'post',
		url : "../pricing/searchRegionBgByCondition.action",
		reader : {
			type : 'json',
			root : 'regionBgVo.regionEntityList',
 ...
 * 区域查询表单
 */
Ext.define('Foss.pricing.regionBg.QueryRegionForm', {
	extend : 'Ext.form.Panel',
	title: pricing.regionBg.i18n('i18n.pricingRegion.searchCondition'),
	frame: true,
	collapsible: true,
    defaults : {
    	columnWidth : .3,
	...
	constructor : function(config) {
		...
		me.items = [{
			name: 'regionName',
	        fieldLabel: pricing.regionBg.i18n('i18n.pricingRegion.regionName'),//区域名称
	        xtype : 'textfield'
		},{
			name: 'active',
			queryMode: 'local',
		    ...
		    valueField: 'key',
		    editable:false,
		    store:pricing.getStore('Foss.pricing.regionBg.AreaStatusStore',null,['key','value']
		    ,[{'key':'N','value':pricing.regionBg.i18n('i18n.pricingRegion.unActive')},{'key':'Y','value':pricing.regionBg.i18n('i18n.pricingRegion.active')}
		    ,{'key':'ALL','value':pricing.regionBg.i18n('i18n.pricingRegion.all')}]),
	        fieldLabel: pricing.regionBg.i18n('i18n.pricingRegion.regionStatus'),//区域状态
	        xtype : 'combo'
		},{
			xtype : 'container',
			margin : '0 0 0 0',
			items : [{
				...
				width:70,
				cls:'yellow_button',
				text : pricing.regionBg.i18n('i18n.pricingRegion.search'),
				disabled: !pricing.regionBg.isPermission('regionBg/indexQueryButton'),
				hidden: !pricing.regionBg.isPermission('regionBg/indexQueryButton'),
				handler : function() {
 ...
 * 区域列表
 */
Ext.define('Foss.pricing.regionBg.AreaGridPanel', {
	extend: 'Ext.grid.Panel',
	title : pricing.regionBg.i18n('i18n.pricingRegion.searchResults'),
	frame: true,
	cls: 'autoHeight',
	bodyCls: 'autoHeight', 
	...
	emptyText: pricing.regionBg.i18n('foss.pricing.theQueryResultIsEmpty'),//查询结果为空
	//得到bbar
	pagingToolbar : null,
	...
	getRegionWindow : function(){
		if(Ext.isEmpty(this.regionWindow)){
			this.regionWindow = Ext.create('Foss.pricing.regionBg.RegionWindow');
		}
	...
	getEndTimeWindow : function(){
		if(Ext.isEmpty(this.endTimeWindow)){
			this.endTimeWindow = Ext.create('Foss.pricing.regionBg.EndTimeWindow',{
				type:'BEGIN'
			});
		}
	...
	getImmediatelyActiveWindow: function(){
		if(Ext.isEmpty(this.immediatelyActiveWindow)){
			this.immediatelyActiveWindow = Ext.create('Foss.pricing.regionBg.ImmediatelyActiveTimeWindow');
	...
	getImmediatelyStopWindow: function(){
		if(Ext.isEmpty(this.immediatelyStopWindow)){
			this.immediatelyStopWindow = Ext.create('Foss.pricing.regionBg.ImmediatelyStopEndTimeWindow');
	...
	getRegionUpdateWindow : function(){
		if(Ext.isEmpty(this.regionUpdateWindow)){
			this.regionUpdateWindow = Ext.create('Foss.pricing.regionBg.RegionUpdateWindow');
		}
	...
	getShowDeptWindow : function(){
		if(Ext.isEmpty(this.showDeptWindow)){
			this.showDeptWindow = Ext.create('Foss.pricing.regionBg.PricingDepartmentWindow');
		}
	...
	immediatelyActive: function(btn) {
		...
		if(selections.length<1){
			pricing.showErrorMes(pricing.regionBg.i18n('foss.pricing.selectOneRecordToOp'));
			return;
		}
		if(selections.length > 1){
	 		pricing.showWoringMessage(pricing.regionBg.i18n('foss.pricing.selectOneRecordToOp'));
			return;
	 	}
		if(selections[0].get('active')=='Y'){
	 		pricing.showWoringMessage(pricing.regionBg.i18n('foss.pricing.selectOneUnActiveRecordToOp'));
	 		return;
	 	}else{
	...
	immediatelyStop:function(){
    	var me = this;
	 	...
	 	if(selections[0].get('active')!='Y'){
	 		pricing.showWoringMessage(pricing.regionBg.i18n('foss.pricing.selectOneActiveRecordToOp'));
	 		return;
	 	}else{
	 		var priceRegionEntity = selections[0].data;
	...
	deleteArea: function() {
		...
		var selections = me.getSelectionModel().getSelection();
		if(selections.length<1){
			pricing.showErrorMes(pricing.regionBg.i18n('foss.pricing.pleaseSelectUnActiveRegionDelete'));
			return;
		}
		...
		pricing.showQuestionMes(pricing.regionBg.i18n('foss.pricing.isDeleteTheseUnActiveRegion'),function(e){//是否要删除这些未激活的区域？
			if(e=='yes'){//询问是否激活，是则发送请求
				var params = {'regionBgVo':{'regionIds':regionIds,'regionNature':pricing.regionBg.PRICING_REGION}};
				...
				var failureFun = function(json){
					if(Ext.isEmpty(json)){
						pricing.showErrorMes(pricing.regionBg.i18n('foss.pricing.requestTimedOut'));//请求超时
					}else{
						pricing.showErrorMes(json.message);
					}
				};
				var url = pricing.realPath('deleteRegionsBg.action');
	...
	constructor : function(config) {
		...
		me.columns = [{xtype: 'rownumberer',
			width:40,
			text : pricing.regionBg.i18n('i18n.pricingRegion.num')//序号
		},{
			
			align : 'center',
			xtype : 'actioncolumn',
			text : pricing.regionBg.i18n('i18n.pricingRegion.opra'),//操作
			items: [{
				iconCls: 'deppon_icons_edit',
                tooltip: pricing.regionBg.i18n('foss.pricing.update'),//修改
                disabled: !pricing.regionBg.isPermission('regionBg/indexUpdateButton'),
				width:42,
                handler: function(grid, rowIndex, colIndex) {
                	//获取选中的数据
    				var record=grid.getStore().getAt(rowIndex);
                	var id= record.get('id');//区域编码
    				var params = {'regionBgVo':{'regionEntity':{'id':id,'regionNature':pricing.regionBg.PRICING_REGION}}};
    				var successFun = function(json){
    					var updateWindow = me.getRegionUpdateWindow();//获得修改窗口
    					updateWindow.selection = new Foss.pricing.regionBg.AreaModel(json.regionBgVo.regionEntity);//区域
    				...
    				var url = pricing.realPath('searchRegionBgByID.action');
    				pricing.requestJsonAjax(url,params,successFun,failureFun);
                }
			},{
				iconCls: 'deppon_icons_delete',
                tooltip: pricing.regionBg.i18n('foss.pricing.delete'),//删除
                disabled: !pricing.regionBg.isPermission('regionBg/indexDeleteQueryButton'),
				width:42,
                ...
                handler: function(grid, rowIndex, colIndex) {
    				...
    				pricing.showQuestionMes(pricing.regionBg.i18n('foss.pricing.isDeleteThisUnActiveRegion'),function(e){//是否要删除这个未激活的区域？
            			if(e=='yes'){//询问是否激活，是则发送请求
            				var params = {'regionBgVo':{'regionIds':regionIds,'regionNature':pricing.regionBg.PRICING_REGION}};
            				var successFun = function(json){
            					pricing.showInfoMes(json.message);
			...
			text : pricing.regionBg.i18n('i18n.pricingRegion.regionNum'),//区域编号
			dataIndex : 'regionCode'
		},{
			text : pricing.regionBg.i18n('i18n.pricingRegion.regionName'),//区域名称
			dataIndex : 'regionName'
		},{
			text : pricing.regionBg.i18n('i18n.pricingRegion.regionType'),//区域类型
			dataIndex : 'regionType',
			renderer:function(value,obj,record){
				if(!Ext.isEmpty(value)&&value==pricing.regionBg.DEPT){//部门组织
					var regionId = "'"+record.get('id')+"'";
					//点击部门时触发的方法
					return '<a href="javascript:pricing.showDept('+regionId+');">'+pricing.regionBg.i18n('foss.pricing.sectorOrganizations')+'</a>';
				}else if(!Ext.isEmpty(value)&&value==pricing.regionBg.ADMINISTRATIVE){//行政组织
					return pricing.regionBg.i18n('foss.pricing.administrativeOrganization');
				}else{
					return '';
				}
			}
		},{
			text : pricing.regionBg.i18n('i18n.pricingRegion.lastCreateTime'),//最后修改时间
			...
			text : pricing.regionBg.i18n('i18n.pricingRegion.lastCreateUser'),//最后修改人
			dataIndex : 'modifyUserName'
		},{
			text : pricing.regionBg.i18n('i18n.pricingRegion.regionStatus'),//是否激活（区域状态）
			dataIndex : 'active',
			renderer:function(value){
				if(value=='Y'){
					return pricing.regionBg.i18n('i18n.pricingRegion.active');
				}else if(value=='N'){
					return  pricing.regionBg.i18n('i18n.pricingRegion.unActive');
				}else{
					return '';
				}
			}
		},{
			text : pricing.regionBg.i18n('i18n.pricingRegion.regionNature'),//区域性质
			dataIndex : 'regionNature',
			renderer:function(value){
				if(value==pricing.regionBg.PRESCRIPTION_REGION){//时效区域
					return pricing.regionBg.i18n('i18n.pricingRegion.prescriptionArea');
				}else if(value==pricing.regionBg.PRICING_REGION){//价格区域
					return  pricing.regionBg.i18n('i18n.pricingRegion.pricingArea');
				}else{
					return '';
				}
			}
		}];
		me.store = Ext.create('Foss.pricing.regionBg.AreaStore',{
			autoLoad : false,
			pageSize : 20,
			listeners: {
				beforeload: function(store, operation, eOpts){
					var queryForm = Ext.getCmp('T_pricing-regionBgIndex_content').getQueryForm();
					if(queryForm!=null){
						Ext.apply(operation,{
							params : {
								'regionBgVo.regionEntity.regionName' : "%"+queryForm.getForm().findField('regionName').getValue()+"%",//区域名称
								'regionBgVo.regionEntity.active' : queryForm.getForm().findField('active').getValue(),//是否激活
								'regionBgVo.regionEntity.regionNature':pricing.regionBg.PRICING_REGION
							}
						});	
					}
				}
		...
		me.tbar = [{
			text : pricing.regionBg.i18n('i18n.pricingRegion.add'),//新增
			disabled: !pricing.regionBg.isPermission('regionBg/indexAddButton'),
			hidden: !pricing.regionBg.isPermission('regionBg/indexAddButton'),
			//iconcls:'deppon_icons_newOnBtn',
			handler :function(){
				me.addArea(this);
			} 
		},'-', {
			text : pricing.regionBg.i18n('foss.pricing.immediatelyActivationProgram'),//立即激活
			hidden: !pricing.regionBg.isPermission('regionBg/indexImmediatelyActiveButton'),
			handler :function(){
				me.immediatelyActive(this);
			} 
		},'-', {
			text : pricing.regionBg.i18n('foss.pricing.immediatelyStopProgram'),//立即中止
			disabled: !pricing.regionBg.isPermission('regionBg/indexImmediatelyStopButton'),
			hidden: !pricing.regionBg.isPermission('regionBg/indexImmediatelyStopButton'),
			handler :function(){
				me.immediatelyStop(this);
			} 
		},'-', {
			text : pricing.regionBg.i18n('foss.pricing.delete'),//删除
			disabled: !pricing.regionBg.isPermission('regionBg/indexDeleteQueryButton'),
			hidden: !pricing.regionBg.isPermission('regionBg/indexDeleteQueryButton'),
			handler :function(){
				me.deleteArea();
			} 
		}];
...
Ext.onReady(function() {
	Ext.QuickTips.init();
	if (Ext.getCmp('T_pricing-regionBgIndex_content')) {
		return;
	}
	pricing.haveServerNowTime();
	var queryForm = Ext.create('Foss.pricing.regionBg.QueryRegionForm');//查询FORM
	var areaGrid = Ext.create('Foss.pricing.regionBg.AreaGridPanel');//查询结果GRID
	Ext.getCmp('T_pricing-regionBgIndex').add(Ext.create('Ext.panel.Panel', {
	  	id : 'T_pricing-regionBgIndex_content',
 ...
 * 区域管理-区域部门关联显示弹窗
 */
Ext.define('Foss.pricing.regionBg.PricingDepartmentWindow',{
	extend : 'Ext.window.Window',
	title : pricing.regionBg.i18n('i18n.pricingRegion.deptDetailed'),
	closable : true,
	modal : true,
	resizable:false,
	closeAction : 'hide',
	...
	listeners:{
		...
		beforeshow:function(me){//根据区域ID和区域性质，查询区域关联部门
			var params = {'regionBgVo':{priceRegioOrgnEntity:{'priceRegionId':me.regionId,'regionNature':pricing.regionBg.PRICING_REGION}}};
			var successFun = function(json){
				me.getDeptShowGrid().getStore().add(json.regionBgVo.priceRegioOrgnEntityList);
			};
			var failureFun = function(json){
				if(Ext.isEmpty(json)){
					pricing.showErrorMes(pricing.regionBg.i18n('i18n.pricingRegion.requestTimeOut'));
			...
			var url = pricing.realPath('searchRegionBgDept.action');
			pricing.requestJsonAjax(url,params,successFun,failureFun);
		}
	},
	//得到区域关联部门的表格
    getDeptShowGrid : function(){
    	if(Ext.isEmpty(this.deptShowGrid)){
    		this.deptShowGrid = Ext.create('Foss.pricing.regionBg.PricingDepartmentShowGridPanel');
    	}
    ...
    constructor : function(config) {
		...
		me.fbar = [{
			text : pricing.regionBg.i18n('i18n.pricingRegion.returnGrid'),//返回列表，其实就是隐藏窗口
			handler :function(){
				me.close();
			} 
		}];
 ...
 * 区域管理-区域部门关系显示-部门表格
 */
Ext.define('Foss.pricing.regionBg.PricingDepartmentShowGridPanel', {
	extend: 'Ext.grid.Panel',
	title : pricing.regionBg.i18n('i18n.pricingRegion.deptDetailed'),
	frame: true,
	flex:1,
	...
	columns : [{xtype: 'rownumberer',
		...
		text : pricing.regionBg.i18n('i18n.pricingRegion.nationProCityCounty'),//所属国家/省份/市/区-县
		...
		text : pricing.regionBg.i18n('i18n.pricingRegion.deptName'),
		dataIndex : 'includeOrgName',
		flex:2
	}],
	constructor : function(config) {
		var me = this, cfg = Ext.apply({}, config);
		me.store = pricing.getStore('Foss.pricing.regionBg.DepartmentShowGridStore','Foss.pricing.regionBg.RegionDeptModel',null,[]);
		me.listeners = {
	    	scrollershow: function(scroller) {
	    		if (scroller && scroller.scrollEl) {
	    				scroller.clearManagedListeners(); 
		    ...
		    view.tip = Ext.create('Ext.tip.ToolTip', {
		        ...
		        listeners: {
		            beforeshow: function updateTipBody(tip) {
		                tip.update(pricing.regionBg.i18n('i18n.pricingRegion.deptName')//部门名称
		                		+': '+ view.getRecord(tip.triggerElement).get('includeOrgName')
		                		+pricing.regionBg.i18n('foss.pricing.semicolon')
		                		+pricing.regionBg.i18n('i18n.pricingRegion.nationProCityCounty')//所属国家/省份/市/区-县
		                		+': '
 ...
 * 区域管理-新增弹窗
 */
Ext.define('Foss.pricing.regionBg.RegionWindow',{
	extend : 'Ext.window.Window',
	title : pricing.regionBg.i18n('i18n.pricingRegion.addRegion'),
	closable : true,
	modal : true,
	resizable:false,
	closeAction : 'hide',
	...
	getEditForm:function(){
		if(this.editForm==null){
    		this.editForm = Ext.create('Foss.pricing.regionBg.RegionEditFormPanel');
    ...
    getDeptEditGrid : function(){
    	if(Ext.isEmpty(this.deptEditGrid)){
    		this.deptEditGrid = Ext.create('Foss.pricing.regionBg.DepartmentGridPanel');
    ...
    commintRegion:function(){
    	...
    	if(form.isValid()){//表单校验
    		var regionModel = new Foss.pricing.regionBg.AreaModel()
        	form.updateRecord(regionModel);
    		if(regionModel.get('regionType') == pricing.regionBg.ADMINISTRATIVE){//行政区域
    			...
    			if(Ext.isEmpty(proCode)||Ext.isEmpty(nationCode)){
    				pricing.showWoringMessage(pricing.regionBg.i18n('foss.pricing.countriesAndProvincesRequired'));//国家和省份必填！
					return;    				
    			}
        	...
        	var params = {'regionBgVo':{'regionEntity':regionEntity,'addPriceRegioOrgnEntityList':addPriceRegioOrgnEntityList}};
        	var successFun = function(json){
        		pricing.showInfoMes(json.message);//提示信息
        		me.close();//隐藏窗口
        		Ext.getCmp('T_pricing-regionBgIndex_content').getAreaGrid().getPagingToolbar().moveFirst();//重新加载数据
        		me.getEl().unmask();
        	};
        	var failureFun = function(json){
        		me.getEl().unmask();
        	...
        	var url = pricing.realPath('addRegionBg.action');
        	me.getEl().mask();
        	pricing.requestJsonAjax(url,params,successFun,failureFun);
    	}
    ...
    constructor : function(config) {
		...
		me.fbar = [{
			text :pricing.regionBg.i18n('i18n.pricingRegion.returnGrid'),//返回列表
			handler :function(){
				me.close();
			} 
		},{
			text : pricing.regionBg.i18n('i18n.pricingRegion.commit'),//提交
			margin:'0 0 0 560',
			cls:'yellow_button',
			handler :function(){
				me.commintRegion();
 ...
 * 区域管理-修改弹窗
 */
Ext.define('Foss.pricing.regionBg.RegionUpdateWindow',{
	extend : 'Ext.window.Window',
	title : pricing.regionBg.i18n('i18n.pricingRegion.updateRegion'),
	closable : true,
	modal : true,
	resizable:false,
	closeAction : 'hide',
	...
	listeners:{
		...
		beforeshow:function(me){
			if(Ext.isEmpty(me.selection)){//如果没有选择所要修改的数据，则提示选择
				pricing.showErrorMes(pricing.regionBg.i18n('i18n.pricingRegion.notSelectOneToUpdate'));
				return;
			}
			...
			//判断区域类型
			if(!Ext.isEmpty(selection.get('regionType'))&&selection.get('regionType')==pricing.regionBg.DEPT){//如果是类型是部门
				me.getDeptEditGrid().getAddButton().setDisabled(false);//新增按钮打开，查询该区域关联的部门
				var params = {'regionBgVo':{priceRegioOrgnEntity:{'priceRegionId':selection.get('id'),'regionNature':pricing.regionBg.PRICING_REGION}}};
				var successFun = function(json){
					me.getDeptEditGrid().getStore().add(json.regionBgVo.priceRegioOrgnEntityList);
				};
				var failureFun = function(json){
					if(Ext.isEmpty(json)){
						pricing.showErrorMes(pricing.regionBg.i18n('i18n.pricingRegion.requestTimeOut'));
    ...
    commintRegion:function(){
    	...
    	if(form.isValid()){//符合填写条件
    		...
    		regionModel.set('regionNature',pricing.regionBg.PRICING_REGION);//区域性质
        	...
        	var successFun = function(json){
        		pricing.showInfoMes(json.message);
        		me.close();
        		Ext.getCmp('T_pricing-regionBgIndex_content').getAreaGrid().getPagingToolbar().moveFirst();
        	};
        	var failureFun = function(json){
        		if(Ext.isEmpty(json)){
    				pricing.showErrorMes(pricing.regionBg.i18n('i18n.pricingRegion.requestTimeOut'));
        	...
        	var url = pricing.realPath('updateRegionBg.action');
        	pricing.requestJsonAjax(url,params,successFun,failureFun);
    	}
    },
    constructor : function(config) {
		...
		me.fbar = [{
			...
			text : pricing.regionBg.i18n('i18n.pricingRegion.commit'),//提交区域信息
			cls:'yellow_button',
			margin:'0 0 0 560',
			handler :function(){
				me.commintRegion();
 ...
 * 区域管理-新增-部门表格
 */
Ext.define('Foss.pricing.regionBg.DepartmentGridPanel',{
	extend: 'Ext.grid.Panel',
	title : pricing.regionBg.i18n('i18n.pricingRegion.deptInfo'),
	frame: true,
	flex:1,
	autoScroll:true,
    sortableColumns:false,
	...
	getDepartmentAddWindow:function(){
		if(Ext.isEmpty(this.departmentAddWindow)){
    		this.departmentAddWindow = Ext.create('Foss.pricing.regionBg.DepartmentAddWindow');
    	}
    	...
    	//if(Ext.isEmpty(me.orgWindow)){
    		me.orgWindow = Ext.create('Foss.baseinfo.commonSelector.orgSelectorWindow',{
    			...
    			commitFun:function(){
    				...
    				if(selections.length<1){
    					pricing.showWoringMessage(pricing.regionBg.i18n('foss.pricing.pleaseSelectLeastOneData'));//请至少选择一条数据！
    					return;
    				}
    				var depts = new Array();
    				var grid = Ext.getCmp('T_pricing-regionBgIndex_content').getAreaGrid().getRegionWindow().getDeptEditGrid();//获取区域关联部门的GRID
    				...
    				for(var i=0;i<realySelections.length;i++){
    					if(Ext.isEmpty(selection)){//me.selection为空，则表示是新增，不需要设置关联的区域id/code/name
    						...
    						var data = {'includeOrgId':realySelections[i].get('id')
    			    				...
    			    				,'regionNature':pricing.regionBg.PRICING_REGION
    			    				};
    						depts.push(data);
    					}
	...
	constructor : function(config) {
		...
		if(me.type=='ADD'){//新增和修改获取两个完全不同的STORE
			me.store = pricing.getStore('Foss.pricing.regionBg.DepartmentEditGridStoreForAdd','Foss.pricing.regionBg.RegionDeptModel',null,[]);
		}else if(me.type=='UPDATE'){
			me.store = pricing.getStore('Foss.pricing.regionBg.DepartmentEditGridStoreForUpdate','Foss.pricing.regionBg.RegionDeptModel',null,[]);
		}
		me.columns = [{xtype: 'rownumberer',
			width:40,
			text : pricing.regionBg.i18n('i18n.pricingRegion.num')//序号
		},{
			text : pricing.regionBg.i18n('i18n.pricingRegion.deptStandCode'),//部门编号
			dataIndex : 'includeOrgCode',
			flex:3
		},{
			text : pricing.regionBg.i18n('i18n.pricingRegion.deptName'),//部门名称
			dataIndex : 'includeOrgName'
		},{
			text : pricing.regionBg.i18n('i18n.pricingRegion.belongCity'),//所属城市
			dataIndex : 'cityName'
		},{
			text : pricing.regionBg.i18n('i18n.pricingRegion.effectiveStartTime'),//有效期是日期
			dataIndex : 'beginTime',
			renderer:function(value){
				if(Ext.isEmpty(value)){
					return new Date(pricing.regionBg.currentDay).format('yyyy-MM-dd');//服务器时间的明天
				}else{
					return new Date(value).format('yyyy-MM-dd');
				}
			}
		},{
			text : pricing.regionBg.i18n('i18n.pricingRegion.effectiveEndTime'),//有效截止日期
			dataIndex : 'endTime',
			renderer:function(value){
				if(Ext.isEmpty(value)){
					return '2999-12-31';
 ...
 * 区域管理-新增/修改-FORM
 */
Ext.define('Foss.pricing.regionBg.RegionEditFormPanel', {
	extend : 'Ext.form.Panel',
	title: pricing.regionBg.i18n('i18n.pricingRegion.regionInfoSet'),
	frame: true,
	height:280,
	collapsible: true,
	//isSearchComb:true,
	...
	items :[{
	    ...
	    displayField: 'value',
	    valueField: 'key',
	    value:pricing.regionBg.PRICING_REGION,
	    editable:false,
	    readOnly:true,
	    store:pricing.getStore('Foss.pricing.regionBg.RegionNatureStore',null,['key','value'],[{'key':pricing.regionBg.PRESCRIPTION_REGION,'value':pricing.regionBg.i18n('i18n.pricingRegion.prescriptionArea')},{'key':pricing.regionBg.PRICING_REGION,'value':pricing.regionBg.i18n('i18n.pricingRegion.pricingArea')}]),
        fieldLabel: pricing.regionBg.i18n('i18n.pricingRegion.regionNature'),
        xtype : 'combo'
	},{
		name: 'regionName',//区域名称
		allowBlank:false,
		...
		regexText:pricing.regionBg.i18n('foss.pricing.areaNameNotContainSpaceCharacter'),//区域名称不能包含空格符！
        fieldLabel: pricing.regionBg.i18n('i18n.pricingRegion.regionName'),
        xtype : 'textfield'
	},{
		...
		regexText: pricing.regionBg.i18n('foss.pricing.regionCodeOnlyNumberLetter'),
        fieldLabel: pricing.regionBg.i18n('i18n.pricingRegion.regionCode'),
        xtype : 'textfield'
	},{
		 ...
		 name:'active',
		 width:160,
		 fieldLabel:pricing.regionBg.i18n('i18n.pricingRegion.isActive'),
		 items:[{
		 	 xtype:'radio',
		     boxLabel:pricing.regionBg.i18n('i18n.pricingRegion.ye'),
		     name:'active',
		     inputValue:'Y'
	     },{
	    	 xtype:'radio',
		     boxLabel:pricing.regionBg.i18n('i18n.pricingRegion.no'),
		     name:'active',
		     inputValue:'N'
		     }]
   	   },{
		    ...
		    editable:false,
		    allowBlank:false,
		    value:pricing.regionBg.ADMINISTRATIVE,
		    store:pricing.getStore(null,null,['key','value'],[{'key':pricing.regionBg.DEPT,'value':pricing.regionBg.i18n('foss.pricing.netWork')}
		    ,{'key':pricing.regionBg.ADMINISTRATIVE,'value':pricing.regionBg.i18n('foss.pricing.administrativeRegions')}]),
	        fieldLabel:pricing.regionBg.i18n('i18n.pricingRegion.regionType'),//区域类型
	        xtype : 'combo',
	        listeners:{
	        	change:function(comb,newValue,oldValue){
	        		//if(comb.up('form').isSearchComb){
	        			if(newValue==pricing.regionBg.DEPT){//部门组织，新增加部门按钮可以操作，省市区县不可选
		        		...
		        		}else if(newValue==pricing.regionBg.ADMINISTRATIVE){//行政组织
   		   ...
   		   //hidden:true,
   		   allowBlank:false,
   		   fieldLabel:pricing.regionBg.i18n('i18n.pricingRegion.belongAddress')//所在地
   	   }]
});
 ...
 */
Ext.define('Foss.pricing.regionBg.EndTimeWindow',{
	extend : 'Ext.window.Window',
	title: pricing.regionBg.i18n('foss.pricing.immediatelySupendPricePriceScheme'),
	closable : true,
	modal : true,
	resizable:false,
	closeAction : 'hide',
	...
	getStopFormPanel : function() {
		if (Ext.isEmpty(this.stopFormPanel)) {
			this.stopFormPanel = Ext.create('Foss.pricing.regionBg.ImmediatelySuspendFormPanel');
		}
	...
	listeners : {
		beforeshow : function(me) {
			...
			var showendTime = Ext.Date.format(new Date(me.selection
					.get('endTime')), 'Y-m-d');
			var value = pricing.regionBg.i18n('foss.pricing.showleftTimeInfo')
					+ showbeginTime
					+ pricing.regionBg.i18n('foss.pricing.showmiddleTimeInfo')
					+ showendTime
					+ pricing.regionBg
 ...
 * 立即中止功能FormPanel
 */
Ext.define('Foss.pricing.regionBg.ImmediatelySuspendFormPanel',{
	extend : 'Ext.form.Panel',
	layout:'column',
	 //设置失效日期
	...
	constructor: function(config) {
		...
		me.items = [{
				...
				fieldLabel :pricing.regionBg.i18n('foss.pricing.suspendTime') ,//'中止日期',
				name : 'endTime',
				...
				width:70,
				columnWidth:.15,
				text : pricing.regionBg.i18n('i18n.pricingRegion.determine'),//"确认",
				handler : function() {
					...
					if(result<0)
					{
						Ext.ux.Toast.msg(pricing.regionBg.i18n('foss.pricing.promptMessage'),'中止时间不能小于当前时间','error', 3000);
				...
				width:70,
				columnWidth:.15,
				text : pricing.regionBg.i18n('i18n.pricingRegion.cancel'),//"取消",
				handler : function() {
					me.up('window').hide();
				}
			}];
 ...
 * 立即激活价格方案Window
 */
Ext.define('Foss.pricing.regionBg.ImmediatelyActiveTimeWindow',{
	extend: 'Ext.window.Window',
	title: pricing.regionBg.i18n('foss.pricing.immediatelyActiveationPriceScheme'),//"立即激活方案",
	width:380,
	height:152,
	closeAction: 'hide' ,
	...
	getImmediatelyActiveFormPanel : function(){
    	if(Ext.isEmpty(this.immediatelyActiveFormPanel)){
    		this.immediatelyActiveFormPanel = Ext.create('Foss.pricing.regionBg.ImmediatelyActiveFormPanel');
    	}
    ...
    listeners:{
    	beforeshow:function(me){
    		...
    		var value = pricing.regionBg.i18n('foss.pricing.showleftTimeInfo')
			  +showbeginTime+pricing.regionBg.i18n('foss.pricing.showmiddleTimeInfo')
			  +showendTime+pricing.regionBg.i18n('foss.pricing.showrightEndTimeInfo');
 ...
 * 立即激活功能Form
 */
Ext.define('Foss.pricing.regionBg.ImmediatelyActiveFormPanel',{
	extend : 'Ext.form.Panel',
	layout:'column',
	activetion:function(){
		var me = this;
    	...
    	if(form.isValid()){
			...
			var params = {'regionBgVo':{'regionEntity':priceRegion,'regionNature':pricing.regionBg.PRICING_REGION,'beginTime':beginTime}};
			var url = pricing.realPath('activeRegionsBg.action');
    		var successFun = function(json){
    			pricing.showInfoMes(json.message);
    			me.up('window').hide();
	...
	constructor: function(config) {
		...
		me.items = [{
				...
				fieldLabel:pricing.regionBg.i18n('foss.pricing.availabilityDate'),//'生效日期',
				name : 'beginTime',
				xtype : 'datetimefield_date97',
				editable:false,
				time : true,
 ...
 * 立即中止价格区域方案 Window
 */
Ext.define('Foss.pricing.regionBg.ImmediatelyStopEndTimeWindow',{
		extend: 'Ext.window.Window',
		title: pricing.regionBg.i18n('foss.pricing.immediatelySupendPricePriceScheme'),//"立即中止方案",
		width:380,
		height:152,
		closeAction: 'hide' ,
		stopFormPanel:null,
		...
		getStopFormPanel : function(){
	    	if(Ext.isEmpty(this.stopFormPanel)) {
	    		this.stopFormPanel = Ext.create('Foss.pricing.regionBg.ImmediatelyStopFormPanel');
	    	}
	    	return this.stopFormPanel;
	    },
	    listeners:{
	    	beforeshow:function(me){
	    		...
	    		var value = pricing.regionBg.i18n('foss.pricing.showleftTimeInfo')
				  +showbeginTime+pricing.regionBg.i18n('foss.pricing.showmiddleTimeInfo')
				  +showendTime+pricing.regionBg.i18n('foss.pricing.showstopRightEndTimeInfo');
	    		me.getStopFormPanel().down('displayfield').setValue(value);
	    	},
	    	beforehide:function(me){
	    		me.getStopFormPanel().getForm().reset();
 ...
 * 立即中止价格区域功能FormPanel
 */
Ext.define('Foss.pricing.regionBg.ImmediatelyStopFormPanel',{
	extend : 'Ext.form.Panel',
	layout:'column',
	stop:function(){
		var me = this;
    	...
    	if(form.isValid()){
    		var priceRegion = me.up('window').priceRegionEntity;
    		var endTime = Ext.Date.parse(form.findField('endTime').getValue(), 'Y-m-d H:i:s');
    		var params = {'regionBgVo':{'regionEntity':priceRegion,'regionNature':pricing.regionBg.PRICING_REGION,'endTime':endTime}};
    		var url = pricing.realPath('immedietelyStopRegionBg.action');
    		var successFun = function(json){
    			pricing.showInfoMes(json.message);
    			me.up('window').hide();
    			me.up('window').parent.getPagingToolbar().moveFirst();

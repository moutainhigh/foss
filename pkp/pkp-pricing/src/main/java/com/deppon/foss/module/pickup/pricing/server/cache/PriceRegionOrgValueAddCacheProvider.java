 ...
 * @version V1.0
 */
public class PriceRegionOrgValueAddCacheProvider implements ITTLCacheProvider<List<PriceRegionOrgValueAddEntity>> {

	/**
	 * 
	 */
	private static final Logger log = Logger.getLogger(PriceRegionOrgValueAddCacheProvider.class);
	/**
	 * 区域DAO
	 */
    @Inject
     ...
     */
    @Override
    public List<PriceRegionOrgValueAddEntity> get(String key) {
    	log.debug("PriceRegionOrgAirCacheProvider cacheKey is :"+ key);
    	if (StringUtils.isBlank(key)) {
    	    return null;
    	}
    	return regionValueAddDao.searchRegionOrgByDeptNo(key, PricingConstants.VALUEADD_REGION);
    }
}
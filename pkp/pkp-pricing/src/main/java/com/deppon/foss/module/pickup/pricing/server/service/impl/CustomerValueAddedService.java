...
/**
 * 
 * @Description: 客户增值服务SERVICE
 * 
  dp-foss-dongjialing
  225131
 */
@Transactional
public class CustomerValueAddedService implements ICustomerValueAddedService {
	...
	public void addValueAdded(PriceValuationEntity priceValuationEntity,
		...
		if (FossConstants.ACTIVE.equals(priceValuationEntity.getActive())) {
			...
			//客户编码
			searchModel.setCustomerCode(priceValuationEntity.getCustomerCode());   
			//根据规则类型,开始时间
...
//                }
	...
	public List<PriceValuationEntity> selectByCodition(
		...
		if(PricingConstants.VALUATION_TYPE_CUSTOMERVALUEADDED.equals(record.getType())){
			for(PriceValuationEntity priceValuationEntity:priceValuationEntityList){
	...
	public void activeFast(PriceValuationEntity priceValuationEntity) {
		...
		//客户编码
		seletpriceValuationEntity.setCustomerCode(tempPriceValuationEntity.getCustomerCode());
...
/**
	...
	public List<CustomerIndustryEntity> queryAllSecProfession(String valuationId){
	 if(StringUtil.isEmpty(valuationId)){
		 ...
		 List<PriceIndustryEntity> priceIndustryEntityList =  
				 priceValuationDao.selectPriceIndustryEntityByValueAddedId(
						 valuationId, TABLE_NAME_VALUEADD);

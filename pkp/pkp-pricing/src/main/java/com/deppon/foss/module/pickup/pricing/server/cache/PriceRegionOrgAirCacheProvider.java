 ...
 * @version V1.0
 */
public class PriceRegionOrgAirCacheProvider implements ITTLCacheProvider<List<PriceRegionOrgAirEntity>> {

	/**
	 * 
	 */
	private static final Logger log = Logger.getLogger(PriceRegionOrgAirCacheProvider.class);
	/**
	 * 区域DAO
	 */
    @Inject
	 ...
	 * 设置 区域DAO.
	 *
	 * @param regionAirDao the new 区域DAO
	 */
     ...
     */
    @Override
    public List<PriceRegionOrgAirEntity> get(String key) {
    	log.debug("PriceRegionOrgAirCacheProvider cacheKey is :"+ key);
    	if (StringUtils.isBlank(key)) {
    	    return null;
    	}
    	return regionAirDao.searchRegionOrgByDeptNo(key, PricingConstants.PRICING_REGION);
    }
}
 ...
 */
@Service
public class WaybillExpressPriceChangeServiceForECS implements IWaybillExpressPriceChangeServiceForECS {
	//日志对象 
	protected final static Logger LOG = LoggerFactory.getLogger(WaybillExpressPriceChangeServiceForECS.class.getName());
...
//		}
	...
	public void executeCoupon(EscWayBillRequestEntity requestEntity, EscWayBillMiddleEntity escWayBillMiddleEntity){
		...
		if (couponInfoDto != null) {
			CouponInfoResultDto dto = priceCrmOrderService.validateCoupon(couponInfoDto);
			if (dto != null) {
				// 判断：该优惠券是否已被使用
				if (!dto.isCanUse()) {
...
//		waybillInfo.setLeavePhone(bean.getDeliveryCustomerPhone());
	...
	private void calculateFee(EscWayBillResponseEntity escWayBillResponseEntity,
		...
		//快递没有 返货和转运		
		if (PricingConstants.CUSTOMER_CHANGE.equals(rfcType)) { //客户原因
			//货物状态
			String inventory = requestEntity.getInventory();
			//未出第一外场返货从第一外场开始
			if (PricingConstants.RECEIVE_STOCK.equals(inventory)){
...
//			}else{
//				delOtherFee(escWayBillMiddleEntity, PricingConstants.RETURNBILLTYPE_ORIGINAL);
//			}	
//			//重新累加其他费用
...
//			throw new BillCaculateServiceException("输入的快递装卸费超过限制，请重新输入！");
//		}
	}

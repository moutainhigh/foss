 ...
 *
 */
public class SyncPriceRegionExpress extends iBatis3DaoImpl implements ISyncDataDao<PriceRegionExpressEntity> {
	/**
	 *  价格 下载服务
	 */
	...
	public List getSyncData(Date fromDate,
		...
		List<PriceRegionExpressEntity> list = new ArrayList<PriceRegionExpressEntity>();
		...
		data = priceDownLoadExpressService.downPriceRegionExpressServerData(clientInfo);
		if(data != null){
			if(data.getObject()!=null){
				list = ((List<PriceRegionExpressEntity>)data.getObject());
			}
		}
		return  list;
	}

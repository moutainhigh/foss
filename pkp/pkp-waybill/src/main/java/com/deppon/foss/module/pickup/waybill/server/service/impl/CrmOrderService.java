 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/service/impl/CrmOrderService.java
 * 
 * FILE NAME        	: CrmOrderService.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
...
import com.deppon.foss.esb.crm.client.CouponStateRequest;
import com.deppon.foss.esb.crm.client.CouponStateResponse;
import com.deppon.foss.esb.crm.client.CouponWaybillInfo;
import com.deppon.foss.esb.crm.client.ModifyOrderLockInfoRequest;
import com.deppon.foss.esb.crm.client.ModifyOrderLockInfoResponse;
import com.deppon.foss.esb.crm.client.Order;
import com.deppon.foss.esb.crm.client.OrderInfo;
import com.deppon.foss.esb.crm.client.QueryOrderListRequest;
import com.deppon.foss.esb.crm.client.QueryOrderListResponse;
import com.deppon.foss.esb.crm.client.SearchOrderRequest;
...
import com.deppon.foss.module.pickup.waybill.shared.request.QueryOmsOrderRequest;
import com.deppon.foss.module.pickup.waybill.shared.response.QueryOmsOrderInfoResponse;
import com.deppon.foss.module.pickup.waybill.shared.response.QueryOmsOrderListResponse;
 ...
 * </p>
 * <p style="display:none">
 * version:V1.0,author:foss-sunrui,date:2012-11-13 下午3:17:52,content:TODO
 * </p>
 * 
 * @author foss-sunrui
 * @date 2012-11-13 下午3:17:52
 * @since
 * @version
 * 
...
public class CrmOrderService implements ICrmOrderService {
	 ...
	 * 日志
	 */
	protected final static Logger logger = LoggerFactory.getLogger(CrmOrderService.class.getName());
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2013-3-11 上午11:20:23
	 * @see
	 */
	private void setInvokeTimeOut() {
		String connectTimeOut = PropertiesUtil
				.getKeyValue("ws.pkp.connection.timeout");
		if (StringUtil.isEmpty(connectTimeOut)) {
			connectTimeOut = DEFAULT_CONNECTION_TIMEOUT;
		}
		String receiveTimeOut = PropertiesUtil
				.getKeyValue("ws.pkp.receive.timeout");
		if (StringUtil.isEmpty(receiveTimeOut)) {
			receiveTimeOut = DEFAULT_RECEIVE_TIMEOUT;
		}

		Client client = ClientProxy.getClient(crmService);
		HTTPConduit http = (HTTPConduit) client.getConduit();

		HTTPClientPolicy httpClientPolicy = new HTTPClientPolicy();

		httpClientPolicy.setConnectionTimeout(Long.parseLong(connectTimeOut));
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-11-13 下午4:56:49
	 * @param query
	 * @return
	 * @see com.deppon.foss.module.pickup.waybill.api.server.service.ICrmOrderService#searchOrder(com.deppon.foss.module.pickup.waybill.shared.dto.CrmOrderQueryDto)
	 */
	public CrmOrderInfoDto searchOrder(CrmOrderQueryDto query) {
		/**
		 * @author 305082
		 * 注释，变更访问方式，从CRM变至为OMS
		createHeader();
		esbHeader.value.setRequestId(UUIDUtils.getUUID());
		esbHeader.value.setEsbServiceCode("ESB_FOSS2ESB_QUERYORDER");
		CrmOrderInfoDto crmOrderInfoDto = new CrmOrderInfoDto();
		QueryOrderListRequest request = convertRequestBean(query);
		QueryOrderListResponse reponse = null;
		List<CrmOrderInfo> crmOrderInfos = null;
		try {
			//setInvokeTimeOut();
			reponse = crmService.queryOrder(request, esbHeader);
			if (reponse != null && reponse.getTotalCount() > 0) {
				crmOrderInfos = new ArrayList<CrmOrderInfo>();
				CrmOrderInfo crmOrderInfo = null;
				List<OrderInfo> orderInfos = reponse.getOrderLists();
				int expCount = 0;
				for (OrderInfo orderInfo : orderInfos) {
					//过滤掉快递订单信息
					if(waybillExpressService.onlineDetermineIsExpressByProductCode(orderInfo.getTransportMode(), new Date())){
						expCount = expCount + 1;
					}else{
						crmOrderInfo = new CrmOrderInfo();
						BeanUtils.copyProperties(crmOrderInfo, orderInfo);
						crmOrderInfo.setReceivingToPointName(orderInfo.getLadingStationName());//到达网点名称
						crmOrderInfos.add(crmOrderInfo);
					}
				}
				//减去快递单子数量
				crmOrderInfoDto.setTotalCount(reponse.getTotalCount()-expCount);
				crmOrderInfoDto.setCrmOrderInfoList(crmOrderInfos);
			}
		} catch (CommException e) {
			logger.info("调用CRM系统查询订单失败：" + e.getMessage(), e);
			throw new CrmOrderImportException(
					CrmOrderImportException.CRM_ORDER_SERVICE_FAIL, e);
		} catch (IllegalAccessException e) {
			logger.info("查询CRM订单失败：" + e.getMessage(), e);
			throw new CrmOrderImportException(
					CrmOrderImportException.CRM_ORDER_QUERY_FAIL, e);
		} catch (InvocationTargetException e) {
			logger.info("查询CRM订单失败：" + e.getMessage(), e);
			throw new CrmOrderImportException(
					CrmOrderImportException.CRM_ORDER_QUERY_FAIL, e);
		}
		return crmOrderInfoDto;
		 */
		/**
		 * 查询OMS订单列表
		 */
		...
		try {
			//oms_request
			QueryOmsOrderRequest queryOmsRequest = new QueryOmsOrderRequest();
			//oms_response
			QueryOmsOrderListResponse queryOmsResponse = new QueryOmsOrderListResponse();
			//对数据进行填充没有封装进去的数据
			queryOmsRequest.setOrderType(request.getOrderType());
			queryOmsRequest.setOrderNumber(request.getOrderNumber());
		    queryOmsRequest.setAcceptStatus(request.getAcceptStatus());
		    queryOmsRequest.setWaybillNumber(request.getWaybillNumber());
		    queryOmsRequest.setPageSize(request.getPageSize());
			queryOmsRequest.setShipperName(request.getShipperCust());
			queryOmsRequest.setContactName(request.getShipperLinkman());
			queryOmsRequest.setContactMobile(request.getShipperMobile());
			queryOmsRequest.setContactPhone(request.getShipperPhone());
			queryOmsRequest.setDeptCode(request.getDeptCode());
			queryOmsRequest.setCurrentPage(request.getPageNum());
			queryOmsRequest.setTransType(request.getTransType());
			if(request.getBeginTime()!=null&&request.getEndTime()!=null){
				queryOmsRequest.setBeginTime(DateUtils.convert(request.getBeginTime()));
				queryOmsRequest.setEndTime(DateUtils.convert(request.getEndTime()));
			}
			// 这个方法是用来调用OMS的接口服务
			logger.info("调用OMS的接口,查询订单信息searchOrder,订单号:"+request.getOrderNumber()+",开始...");
			long begin = System.nanoTime() ;
			queryOmsResponse = simpleHttp(queryOmsOrderListUrl,queryOmsRequest,QueryOmsOrderListResponse.class);
			logger.info("调用OMS的接口,查询订单信息searchOrder,订单号:"+request.getOrderNumber()+",结束,耗时："+((System.nanoTime()-begin)/(NumberConstants.NUMBER_1000*NumberConstants.NUMBER_1000)));
			if (queryOmsResponse != null && queryOmsResponse.getData() != null && queryOmsResponse.getData().getTotalRows() != null && queryOmsResponse.getData().getTotalRows() > 0 && queryOmsResponse.getData().getModel() != null) {
				crmOrderInfos = new ArrayList<CrmOrderInfo>();
				CrmOrderInfo crmOrderInfo = null;
				//获取数据列表
				List<QueryOmsOrderInfoResponse> orderList =  queryOmsResponse.getData().getModel();
				int expCount = 0;
				for (QueryOmsOrderInfoResponse responseOrder : orderList) {
					//定义OrderInfo
					OrderInfo orderInfo = new OrderInfo();
					com.deppon.foss.framework.shared.util.classes.BeanUtils.copyProperties(responseOrder,orderInfo);
					//转换Oms系统中与FOSS系统对于不上的值 297064
					orderInfo.setLadingStation(responseOrder.getReceivingtoPointid());
					orderInfo.setLadingStationName(responseOrder.getReceivingToPointName());
					orderInfo.setStartStation(responseOrder.getDepartureId());
					//过滤掉快递订单信息
					if(waybillExpressService.onlineDetermineIsExpressByProductCode(orderInfo.getTransportMode(), new Date())){
						expCount = expCount + 1;
					}else{
						crmOrderInfo = new CrmOrderInfo();
						if(responseOrder.getServiceType() != null){
							crmOrderInfo.setServiceType(responseOrder.getServiceType());
						}
						BeanUtils.copyProperties(crmOrderInfo, orderInfo);
						crmOrderInfo.setReceivingToPointName(orderInfo.getLadingStationName());//到达网点名称
						crmOrderInfos.add(crmOrderInfo);
				...
				//减去快递单子数量
				crmOrderInfoDto.setTotalCount((int)(queryOmsResponse.getData().getTotalRows() - expCount));
				crmOrderInfoDto.setCrmOrderInfoList(crmOrderInfos);
			}
		} catch (IllegalAccessException e) {
			logger.info("查询OMS订单失败：" + e.getMessage(), e);
			throw new CrmOrderImportException(
					CrmOrderImportException.CRM_ORDER_QUERY_FAIL, e);
		} catch (InvocationTargetException e) {
			logger.info("查询OMS订单失败：" + e.getMessage(), e);
			...
			logger.info("调用OMS系统查询订单失败：" + e.getMessage(), e);
			throw new CrmOrderImportException(
						CrmOrderImportException.CRM_ORDER_SERVICE_FAIL, e);
		}
		return crmOrderInfoDto;
	...
	public CrmOrderInfoDto searchExpressOrder(CrmOrderQueryDto query) {
			...
			if (reponse != null && reponse.getTotalCount() > 0) {
				...
				for (OrderInfo orderInfo : orderInfos) {
					...
					if(StringUtils.isNotBlank(query.getProductCode())){
						if(StringUtils.equals(query.getProductCode(), orderInfo.getTransportMode())){
							crmOrderInfo = new CrmOrderInfo();
							BeanUtils.copyProperties(crmOrderInfo, orderInfo);
							crmOrderInfos.add(crmOrderInfo);
						}	
				...
				crmOrderInfoDto.setTotalCount(crmOrderInfos.size());
				crmOrderInfoDto.setCrmOrderInfoList(crmOrderInfos);
			}
		} catch (CommException e) {
			// TODO Auto-generated catch block
			logger.info("调用CRM系统查询订单失败：", e.getMessage());
			throw new CrmOrderImportException("调用CRM系统查询订单失败：",e);		
		} catch (IllegalAccessException e) {
			// TODO Auto-generated catch block
			logger.info("处理系统订单失败：", e.getMessage());
			throw new CrmOrderImportException("处理系统订单失败：",e);
		} catch (InvocationTargetException e) {
			// TODO Auto-generated catch block
			logger.info("处理系统订单失败：", e.getMessage());
			throw new CrmOrderImportException("处理系统订单失败：",e);
		...
		*/
		...
		try {
			 ...
			 * 2016年4月15日09:27:08
			 * 323098
			 * 由于对返回的数据进行了封装，然后把需要的值赋值给真正的返回接受对象
			 */
			if(queryOmsResponse!=null&&queryOmsResponse.getData()!=null && queryOmsResponse.getData().getTotalRows() > 0){
				//定义返回的列表
				crmOrderInfos = new ArrayList<CrmOrderInfo>();
				//定义展示类
				CrmOrderInfo crmOrderInfo = null;
				//设置页面总条数
				reponse=new QueryOrderListResponse();
				//对reponse的总数量属性进行赋值
				reponse.setTotalCount(new Integer(queryOmsResponse.getData().getTotalRows().toString()));
				//获取数据列表
				List<QueryOmsOrderInfoResponse> orderList =  queryOmsResponse.getData().getModel();
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-11-13 下午4:57:03
	 * @param orderNumber
	 * @return
	 * @see com.deppon.foss.module.pickup.waybill.api.server.service.ICrmOrderService#importOrder(java.lang.String)
	 */
	public CrmOrderDetailDto importOrder(String orderNumber) {
		CrmOrderDetailDto crmOrderDetail = null;
		/*
		createHeader();
		esbHeader.value.setRequestId(UUIDUtils.getUUID());
		esbHeader.value.setEsbServiceCode("ESB_FOSS2ESB_IMPORT");
		*/
		SearchOrderRequest request = new SearchOrderRequest();
		request.setOrderNumber(orderNumber);
		Order reponse = null;
		logger.info("调用'导入OMS系统订单'接口 开始,订单号："+orderNumber) ;
		long begin = System.nanoTime() ;
		try {
			//setInvokeTimeOut();
			//reponse = crmService.searchOrder(request, esbHeader);
			reponse = simpleHttp(importOmsOrderUrl,request,Order.class);
			if (reponse != null) {
				crmOrderDetail = convertResponseBean(reponse);
			} else {
				throw new CrmOrderImportException(CrmOrderImportException.VALID_CRM_ORDER_DATA_NULL);
			}
		}catch(CrmOrderImportException ce){
			logger.info("导入OMS系统订单失败：" + ce.getMessage(), ce);
			throw new CrmOrderImportException(CrmOrderImportException.CRM_ORDER_IMPORT_FAIL, ce);
		}catch(Exception e) {
			logger.info("导入OMS系统订单失败：" + e.getMessage(), e);
			throw new CrmOrderImportException(CrmOrderImportException.CRM_ORDER_IMPORT_FAIL, e);
		}finally{
			logger.info("调用'导入OMS系统订单'接口 完成,订单号："+orderNumber+",耗时："+((System.nanoTime()-begin)/(NumberConstants.NUMBER_1000*NumberConstants.NUMBER_1000))) ;
		}

		String waybillNo = waybillManagerService.getWayBillNoExsitsByOrderNo(orderNumber);
		//该订单已与运单绑定，但还需要判断是否为中止或作废运单
		if (StringUtil.isNotEmpty(waybillNo)) {
			ActualFreightEntity af = actualFreightService.queryByWaybillNo(waybillNo);
			if (af != null) {
				WaybillEntity wayEntry = waybillManagerService.queryWaybillBasicByNo(waybillNo);
				//已补录或已开单
				if (!WaybillConstants.WAYBILL_STATUS_PDA_PENDING.equals(wayEntry.getPendingType())
						&& !WaybillConstants.WAYBILL_STATUS_PC_PENDING.equals(wayEntry.getPendingType())) {
					//有效运单
					if (WaybillConstants.EFFECTIVE.equals(af.getStatus()) || af.getStatus() == null) {
						logger.info("选择的订单编号已经导入开单");
						throw new CrmOrderImportException(CrmOrderImportException.VALID_CRM_ORDER_NO_NULL);
					} 
					//已作废或已中止（此时该订单可再使用）
					else {
						crmOrderDetail.setActualFreightStatus(af.getStatus());
					}
				} 
			} 
		}
	 ...
	 * @see com.deppon.foss.module.pickup.waybill.api.server.service.ICrmOrderService#importOrder(java.lang.String)
	 */
	public String searchServiceType(String orderNumber) {
		String serviceType="";
		SearchOrderRequest request = new SearchOrderRequest();
		request.setOrderNumber(orderNumber);
		try {
			serviceType = simpleHttpAdd(importOmsOrderUrl,request);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return serviceType;
...
//							PriceEntityConstants.PRICING_CODE_ZHXX};
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-11-15 下午5:57:26
	 * @param couponNumber
	 * @return
	 * @see com.deppon.foss.module.pickup.waybill.api.server.service.ICrmOrderService#effectCouponState(java.lang.String)
	 */
	public boolean effectCouponState(String couponNumber) {
		boolean retVal = false;
		if (couponNumber != null && !couponNumber.equals("")) {
			createHeader();
			esbHeader.value.setRequestId(UUIDUtils.getUUID());
			esbHeader.value.setEsbServiceCode("ESB_FOSS2ESB_COUPON_STATE");
			CouponStateRequest request = new CouponStateRequest();
			request.setCouponNumber(couponNumber);
			try {
				//setInvokeTimeOut();
				CouponStateResponse response = crmService.effectCouponState(
						request, esbHeader);
				if (response != null && response.isIsSuccess()) {
					retVal = true;
				}
			} catch (CommException e) {
				logger.info("调用优惠劵使用状态退回接口失败：" + e.getMessage(), e);
				throw new CrmOrderImportException(
						CrmOrderImportException.COUPON_STATE_FAIL, e);
			}
		}
		return retVal;
	}
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-11-13 下午4:54:21
	 * @param order
	 * @return
	 * @see
	 */
	private QueryOrderListRequest convertRequestBean(CrmOrderQueryDto order) {
		QueryOrderListRequest request = new QueryOrderListRequest();
		if (order != null) {
			request.setAcceptStatus(order.getAcceptStatus());
			request.setBeginTime(order.getBeginTime());
			request.setEndTime(order.getEndTime());
			request.setOrderNumber(order.getOrderNumber());
			if(StringUtils.isNotEmpty(order.getWaybillNumber())){
				request.setWaybillNumber(order.getWaybillNumber());//运单号
			}
			request.setOrderType(order.getOrderType());
			request.setPageNum(order.getPageNum());
			request.setPageSize(order.getPageSize());
			request.setShipperCust(order.getShipperCust());
			request.setShipperLinkman(order.getShipperLinkman());
			request.setShipperMobile(order.getShipperMobile());
			request.setShipperPhone(order.getShipperPhone());
			//如果订单号为空，再查询运单号是否为空
			if (StringUtil.isEmpty(order.getOrderNumber())) {
				if(StringUtils.isEmpty(order.getWaybillNumber())){
					request.setDeptCode(order.getSalesDept());
				}				
			}
		}
		return request;
	 ...
	 * 
	 * @author foss-sunrui
	 * @date 2012-11-13 下午4:54:51
	 * @param order
	 * @return
	 * @see
	 */
	private CrmOrderDetailDto convertResponseBean(Order order) {
		CrmOrderDetailDto detail = new CrmOrderDetailDto();
		detail.setAcceptDept(order.getAcceptDept());
		detail.setAcceptDeptName(order.getAcceptDeptName());
		detail.setAccepterPersonMobile(order.getAccepterPersonMobile());
		detail.setBeginAcceptTime(order.getBeginAcceptTime());
		detail.setChannelCustId(order.getChannelCustId());
		detail.setChannelNumber(order.getChannelNumber());
		detail.setChannelType(order.getChannelType());
		detail.setContactAddress(order.getContactAddress());
		detail.setContactArea(order.getContactArea());
		detail.setContactCity(order.getContactCity());
		detail.setContactProvince(order.getContactProvince());
		detail.setContactAreaCode(order.getContactAreaCode());
		detail.setContactCityCode(order.getContactCityCode());
		detail.setContactProvinceCode(order.getContactProvinceCode());
		detail.setContactManId(order.getContactManId());
		detail.setContactMobile(order.getContactMobile());
		detail.setContactName(order.getContactName());
		detail.setContactPhone(order.getContactPhone());
		detail.setDealPerson(order.getDealPerson());
		detail.setDeliveryMode(order.getDeliveryMode());
		detail.setEndAcceptTime(order.getEndAcceptTime());
		detail.setFeedbackInfo(order.getFeedbackInfo());
		detail.setGoodsName(order.getGoodsName());
		detail.setGoodsNumber(order.getGoodsNumber());
		detail.setGoodsType(order.getGoodsType());
		detail.setHastenCount(order.getHastenCount());
		detail.setInsuredAmount(order.getInsuredAmount());
		detail.setLastHastenTime(order.getLastHastenTime());
		detail.setMemberType(order.getMemberType());
		detail.setOnlineName(order.getOnlineName());
		detail.setOrderAcceptTime(order.getOrderAcceptTime());
		detail.setOrderNumber(order.getOrderNumber());
		detail.setOrderPerson(order.getOrderPerson());
		detail.setOrderStatus(order.getOrderStatus());
		detail.setOrderTime(order.getOrderTime());
		detail.setPacking(order.getPacking());
		detail.setPaymentType(order.getPaymentType());
		detail.setReceiveGoods(order.isIsReceiveGoods());
		detail.setReceiver(order.getReceiver());
		detail.setReceiverCustAddress(order.getReceiverCustAddress());
		detail.setReceiverCustArea(order.getReceiverCustArea());
		detail.setReceiverCustCity(order.getReceiverCustCity());
		detail.setReceiverCustProvince(order.getReceiverCustProvince());
		detail.setReceiverCustAreaCode(order.getReceiverCustAreaCode());
		detail.setReceiverCustCityCode(order.getReceiverCustCityCode());
		detail.setReceiverCustProvinceCode(order.getReceiverCustProvinceCode());
		detail.setReceiverCustcompany(order.getReceiverCustcompany());
		detail.setReceiverCustId(order.getReceiverCustId());
		detail.setReceiverCustMobile(order.getReceiverCustMobile());
		detail.setReceiverCustName(order.getReceiverCustName());
		detail.setReceiverCustNumber(order.getReceiverCustNumber());
		detail.setReceiverCustPhone(order.getReceiverCustPhone());
		detail.setReceivingToPoint(order.getReceivingToPoint());
		detail.setReceivingToPointName(order.getReceivingToPointName());
		detail.setReciveLoanType(order.getReciveLoanType());
		detail.setResource(order.getResource());
		detail.setReturnBillType(order.getReturnBillType());
		detail.setReviceMoneyAmount(order.getReviceMoneyAmount());
		detail.setSendmessage(order.isIsSendmessage());
		detail.setShipperId(order.getShipperId());
		detail.setShipperName(order.getShipperName());
		detail.setShipperNumber(order.getShipperNumber());
		detail.setStartStation(order.getStartStation());
		detail.setStartStationName(order.getStartStationName());
		detail.setTotalVolume(order.getTotalVolume());
		detail.setTotalWeight(order.getTotalWeight());
		detail.setTransNote(order.getTransNote());
		detail.setTransportMode(order.getTransportMode());
		detail.setWaybillNumber(order.getWaybillNumber());
		detail.setCouponNumber(order.getCouponNumber());
		detail.setContactAddrRemark(order.getContactAddrRemark());
		detail.setReceiverCustAddrRemark(order.getReceiverCustAddrRemark());
		/**
		 * 将order中是否电子发票的字段传入detail中
		 * @author:218371-foss-zhaoyanjun
		 * @date:2014-10-23下午17:04
		 */
		detail.setIsElectronicInvoice(order.getIfEInvoice());
		/**
		 * 将order发票手机号的字段传入detail中
		 * @author:218371-foss-zhaoyanjun
		 * @date:2014-10-23下午17:04
		 */
		detail.setInvoiceMobilePhone(order.getInvoicePhone());
		/**
		 * 将order发票邮箱字段传入detail中
		 * @author:218371-foss-zhaoyanjun
		 * @date:2014-10-23下午17:04
		 */
		detail.setEmail(order.getInvoiceMail());
		/**
		 * Dmana-9885将order里的CRM运费传入detail中
		 * @author:218371-foss-zhaoyanjun
		 * @date:2015-02-02下午16:55
		 */		
		detail.setCrmTransportFee(new BigDecimal(order.getFreight()));

		
//		整合代码时请保留此段注释
//		/**
//		 * Dmana-9885将order里的CRM重量传入detail中
//		 * @author:218371-foss-zhaoyanjun
//		 * @date:2015-02-02下午16:55
//		 */
//		detail.setCRMWeight(null);
//		/**
//		 * Dmana-9885将order里的CRM体积传入detail中
//		 * @author:218371-foss-zhaoyanjun
//		 * @date:2015-02-02下午16:55
//		 */
//		detail.setCRMVolume(null);
		return detail;
	}
	...
	private ValidateCouponRequest convertValidateCouponRequest(
		...
		if (couponInfo != null) {
			...
			if (couponInfo.getWaybillInfo() != null) {
				...
				waybillInfo.setCreateOrChange(waybill.getCreateOrChange());
	 ...
	 * @date 2013-8-7 下午4:56:49
	 * @return
	 */
	@Override
	public ModifyOrderLockInfoResponse modifyOrderLockInfo(String deptCode) {
		/**
		 * @author 321519
		 * 注释掉之前采用SOAP协议访问CRM
		 * 因CRM接口迁至OMS,现做如下修改，1:访问方式，采用Http协议，2:RestFul风格，访问OMS 
		createHeader();
		esbHeader.value.setRequestId(UUIDUtils.getUUID());
		esbHeader.value.setEsbServiceCode("ESB_FOSS2ESB_MODIFY_ORDER_LOCK_INFO");
		ModifyOrderLockInfoRequest modifyOrderLockInfoRequest = new ModifyOrderLockInfoRequest();
	
		ModifyOrderLockInfoResponse reponse;
		modifyOrderLockInfoRequest.setDeptCode(deptCode);
		//setInvokeTimeOut();
		try {
			reponse = crmService.modifyOrderLockInfo(modifyOrderLockInfoRequest, esbHeader);
		} catch (CommException e) {
			// TODO Auto-generated catch block
			logger.info("调用crm接口失败：" + e.getMessage(), e);
			throw new CrmOrderImportException(
					CrmOrderImportException.LOCK_STATE_FAIL, e);
		}
		return reponse;
		*/
		
		logger.info("调用OMS查询接口>>"+modifyOrderLockInfoUrl+";"+"传递当前操作部门的部门标杆编码>>"+deptCode);
		ModifyOrderLockInfoRequest modifyOrderLockInfoRequest = new ModifyOrderLockInfoRequest();
		ModifyOrderLockInfoResponse reponse=null;
		modifyOrderLockInfoRequest.setDeptCode(deptCode);
		try {
			// 采用 ResultFull 数据格式
			reponse = simpleHttp(modifyOrderLockInfoUrl, modifyOrderLockInfoRequest, ModifyOrderLockInfoResponse.class);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			logger.info("调用OMS接口失败：" + e.getMessage(), e);
			e.printStackTrace();
		}
		return reponse;
	}
	...
	public boolean searchOrder(String orderNumber) {
		...
		*/
		SearchOrderRequest request = new SearchOrderRequest();
		request.setOrderNumber(orderNumber);
		//297064 注释，直接使用Order进行修改
		//SearchOrderResponse reponse = null;
		Order reponse = null;
		long begin = System.nanoTime() ;
		try {
			//setInvokeTimeOut();
			...
			/**
			reponse = crmService.searchOrder(request, esbHeader);
			if (reponse != null && reponse.getOrder() != null) {
				return true;
			} else {
				logger.info("处理订单界面查询CRM系统订单失败");
				return false;
			}
			*/
			logger.info("调用OMS的接口,根据订单号查询订单信息:"+orderNumber+",开始...");
			reponse = simpleHttp(importOmsOrderUrl,request,Order.class);
			if (reponse != null) {
				return true;
			} else {
				logger.info("处理订单界面查询OMS系统订单失败");
				return false;
			}
		}catch(CommException e1){
			logger.info("处理订单界面查询OMS订单失败：" + e1.getMessage(), e1);
			return false;
		}catch (Exception e) {
			logger.info("处理订单界面查询OMS订单失败：" + e.getMessage(), e);
			return false;
		}finally{
			logger.info("调用OMS的接口,根据订单号查询订单信息:"+orderNumber+",结束,耗时："+((System.nanoTime()-begin)/(NumberConstants.NUMBER_1000*NumberConstants.NUMBER_1000)));
		}
	}
	...
	/**
	 * @author 297064
	 * @discrption 封装Json数据的Http请求
	 */
	public <T>T simpleHttp(String targetUrl,Object requestEntity,Class<T> clazz)throws Exception{
		//
		int status = 0 ;
		...
		String jsonParam = JSONObject.toJSONString(requestEntity);
		...
		try {
			...
			try{
				...
				exceptionMsg = "url:"+targetUrl+";请求内容："+jsonParam+";method.releaseConnection exception:"+e ;
				throw new Exception("接口请求异常",e);
			}
			/**
			 * 判断返回结果
			 */
			if(StringUtils.isNotEmpty(responseBody)){
				try{
					result = JSONObject.parseObject(responseBody,clazz);
				}catch(Exception e){
					exceptionMsg = "返回内容："+responseBody+";method.JSONObject.parseObject exception:"+e ;
					logger.error("查询或导入异常: " + e.getMessage(),e);
				}
			}
			times = (System.nanoTime()-begin)/(NumberConstants.NUMBER_1000*NumberConstants.NUMBER_1000) ;
		} catch (Exception e) {
			logger.error("simpleHttp 请求异常:" + e.getMessage(),e);
			throw new Exception("接口请求异常",e);
		}finally{
			try {
				// sangwenhao-272311
				...
				//传递内容
				logEntity.setSendMsg(jsonParam);
				//接口返回参数异常信息为空表示成功，插入Y，否则N
				logEntity.setSuccessFlag(StringUtils.isEmpty(exceptionMsg) ? "Y" : "N");
				//日志记录时间
				logEntity.setCreateTime(new Date());
				//返回信息
				logEntity.setResponeMsg(responseBody);
				//响应时间
				logEntity.setResponseTime(times);
				//返回状态
				logEntity.setColumn1(status+"");
				logEntity.setColumn2(targetUrl);
				//异常信息
				logEntity.setExceptionInfo(exceptionMsg);
				//系统调用关系
				logEntity.setSysRelation("OMSOrderService");
				pushFoss2EcsWaybillNoLogService.insertWaybillNoLog(logEntity);
			} catch (Exception e) {
				logger.error("推送日志信息至日志表(pushFoss2EcsWaybillNoLog)失败：" + e.getMessage());
			}
	 ...
	 * @discrption 封装Json数据的Http请求
	 */
	public String simpleHttpAdd(String targetUrl,Object requestEntity)throws Exception{
		String serviceType  = "";
		String responseBody = null;
		if(requestEntity==null){
			throw new Exception("请求参数不能为空");
		...
		if(StringUtils.isNotEmpty(responseBody)){
			try{
				...
				if(result.containsKey("serviceType")){
					serviceType = (String) result.get("serviceType");
				}
			}catch(Exception e){
				logger.error("查询或导入异常: " + e.getMessage(),e);
			}

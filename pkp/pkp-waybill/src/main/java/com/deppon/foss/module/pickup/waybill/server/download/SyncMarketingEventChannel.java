 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncMarketingEventChannel.java
 * 
 * FILE NAME        	: SyncMarketingEventChannel.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncMarketingEventChannel extends iBatis3DaoImpl implements ISyncDataDao<MarketingEventChannelEntity> {
	/**
	 *  价格 下载服务
	 */
	...
	public List<MarketingEventChannelEntity> getSyncData(Date fromDate,
			String empCode, String regionID,String pagionation, int page) {
		List<MarketingEventChannelEntity> list = new ArrayList<MarketingEventChannelEntity>();
		...
		data = priceDownLoadService.downMarketingEventChannel(clientInfo);
		if(data != null){
			if(data.getObject()!=null){
				list = ((List<MarketingEventChannelEntity>)data.getObject());
			}
		}
		return  list;
	}

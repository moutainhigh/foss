 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/service/impl/GisQueryService.java
 * 
 * FILE NAME        	: GisQueryService.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
...
import com.deppon.foss.module.pickup.waybill.server.utils.GisInterfaceUtil;
...
import com.deppon.foss.module.pickup.waybill.shared.exception.SendSmsVoiceException;
import com.deppon.foss.module.pickup.waybill.shared.request.HisSegMatchRequest;
import com.deppon.foss.module.pickup.waybill.shared.response.GisServiceResponse;
 ...
 */
public class GisQueryService implements IGisQueryService {
	/**
	 * 日志
	 */
	...
	public String isExhibitCargo(ExhibitionKeywordEntity exhibitionKeyword){
		String address = exhibitionKeyword.getExhibitionAddress();
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("address", address);
		jsonObject.put("type", "EXHIBITION_REGIONS");
		String body = jsonObject.toString();
		String responseBody = null;
		LOGGER.info(body);
		String url = PropertiesUtil.getKeyValue(WaybillConstants.ISEXHIBITCARGO_GIS_URL);
		try{
			...
			//获取返回的值
			boolean resultStatue = (Boolean) obj.get("resultStatue");
			if(resultStatue){
				responseBody = "1";
			}else{
				responseBody = "0";
	...
	public List<GisDepartmentDto> queryPickupOrgCodeByGis(GisPickupOrgDto dto){
		Map<String,Object> gisReturnMap=GisInterfaceUtil.postGisInterface(GisConstants.ARRIVE_DEPT_MATCH, convertToMap(dto,GisConstants.ARRIVE_DEPT_MATCH) );
		List<GisDepartmentDto> deptNos = null;
		if(null != gisReturnMap){
			List<Map<String,Object>> depts=(List<Map<String, Object>>) gisReturnMap.get("depts");
			//判空操作
			if(CollectionUtils.isNotEmpty(depts)){
				deptNos = new ArrayList<GisDepartmentDto>();
				//遍历map中的depts集合
				for (Map<String, Object> map : depts) {
					GisDepartmentDto dept = new GisDepartmentDto();
					//部门编码
					dept.setDeptNo(String.valueOf(map.get("deptNo")));
					dept.setIsAgentCollected((Boolean)map.get("isAgencydeliver"));
					deptNos.add(dept);
				}
			}
		}
	 ...
	 * @author 026123-foss-lifengteng
	 * @date 2013-1-4 下午4:42:14
	 */
	@SuppressWarnings("unchecked")
	public List<String> querySpecialAddressByGis(GisPickupOrgDto dto){
		Map<String,Object> gisReturnMap=GisInterfaceUtil.postGisInterface(GisConstants.ADDRESS_REMARK_SEARCH, convertToMap(dto,GisConstants.ARRIVE_DEPT_MATCH) );
		if(null != gisReturnMap){
			List<Map<String,String>> remarkList=(List<Map<String, String>>) gisReturnMap.get("remarks");
			if(CollectionUtils.isNotEmpty(remarkList)){
				List<String> remarks = new ArrayList<String>();
				for (Map<String, String> map : remarkList) {
					remarks.add(map.get("remarkType"));
				}
				return remarks;
			}else{
				return null;
	 ...
	 * 将封装参数的DTO转换为Map键值对
	 * @author 026123-foss-lifengteng
	 * @date 2012-12-27 上午10:46:24
	 */
	@SuppressWarnings("unchecked")
	private Map<String,String> convertToMap(GisPickupOrgDto dto,String netType){
		validateDtoData(dto,netType);//验证数据
		Map<String,String> map = null;
		try {
			map =  BeanUtils.describe(dto);
		} catch (IllegalAccessException e) {
			LOGGER.error(e.getMessage(), e);//处理异常
	 ...
	 * 判断DTO数据必填项
	 * @author 026123-foss-lifengteng
	 * @date 2012-12-27 上午11:22:45
	 */
	private void validateDtoData(GisPickupOrgDto dto,String netType){
		//客户省份
		if(StringUtil.isEmpty(dto.getProvince())){
			throw new SendSmsVoiceException(SendSmsVoiceException.STATIONPROVINCE_NULL);
		}
		//客户城市
		if(StringUtil.isEmpty(dto.getCity())){
			throw new SendSmsVoiceException(SendSmsVoiceException.STATIONCITY_NULL);
		}
		//客户区县
		if(StringUtil.isEmpty(dto.getCounty())){
			throw new SendSmsVoiceException(SendSmsVoiceException.STATIONDIST_NULL);
		}
		//地址
		if(StringUtil.isEmpty(dto.getOtherAddress())){
			throw new SendSmsVoiceException(SendSmsVoiceException.STATIONADDRESS_NULL);
		}
		//手机号码或电话至少有一个不能为空
		if(StringUtil.isEmpty(dto.getPhone()) && StringUtil.isEmpty(dto.getTel())){
			throw new SendSmsVoiceException(SendSmsVoiceException.STATIONTELORPHONE_NULL);
		}
		
		//若为到达网点匹配，则以下为必填；若为特殊地址备注则非必填
		if(GisConstants.ARRIVE_DEPT_MATCH.equals(netType)){
			//运输类型
			if(StringUtil.isEmpty(dto.getTransportway())){
				throw new SendSmsVoiceException(SendSmsVoiceException.STATIONTRANSPORTWAY_NULL);
			}
			//提货方式
			if(StringUtil.isEmpty(dto.getMatchtypes())){
				throw new SendSmsVoiceException(SendSmsVoiceException.STATIONMATCHTYPES_NULL);
			}
		}
	}
	...
	public HisSegMatchResponse queryStationInfo(HisSegMatchRequest request){
		//响应实体
		HisSegMatchResponse hisSegMatchResponse = null;
		List<HisSegMatchRequest> requestList = new ArrayList<HisSegMatchRequest>();
		requestList.add(request);
		Object requestStr=com.alibaba.fastjson.JSON.toJSON(requestList);
		try {
			//调用控制开关判断是否启用，启用时调用GIS接口查询提货网点信息,ACTIVE_GIS_CONTROL_SWITCH开关code
			String confValue = readConfValue("ACTIVE_GIS_CONTROL_SWITCH");
			//开关关闭时返回当前空对象
			if(StringUtils.isBlank(confValue)){
				return hisSegMatchResponse;
			}
			//调用gis接口
			GisServiceResponse responses = HttpClientUtil.postRequest(GisConstants.SALES_STATION_INFO_QUERY,String.valueOf(requestStr),GisServiceResponse.class);
			if(null!=responses.getResponses()){
				if(CollectionUtils.isNotEmpty(responses.getResponses())){
					hisSegMatchResponse  = responses.getResponses().get(0);
				}
			}
		}catch (Exception e) {
			LOGGER.error("调用gis接口异常",e.getMessage());
		}		
		return hisSegMatchResponse;
	}
	
	/**
	 * 通过调用GIS的历史经验分词匹配接口查询目的站信息的控制开关
	 * @param code
	 * @return
	 */
	public String readConfValue(String code){
		String confValue = "";
		String[] codes = new String[1];
		codes[0]=code;
		List<ConfigurationParamsEntity> configurationParamsEntitys = configurationParamsService.queryConfigurationParamsBatchByCode(codes);

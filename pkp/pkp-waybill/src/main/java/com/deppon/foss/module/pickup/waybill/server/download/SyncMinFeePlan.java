 ...
 */
public class SyncMinFeePlan extends iBatis3DaoImpl implements ISyncDataDao<MinFeePlanEntity>{
	...
	@Override
	public List<MinFeePlanEntity> getSyncData(Date fromDate, String userID,
			String regionID, String pagionation, int page) {
		DataBundle data;
		List<MinFeePlanEntity> list = new ArrayList<MinFeePlanEntity>();
		...
		data = priceDownLoadService.downMinFeePlanServerData(clientInfo);
		if(data != null){
			if(data.getObject() != null){
				list = ((List<MinFeePlanEntity>)data.getObject());
			}
		}
		return  list;
	}
	...
	@Override
	public List<MinFeePlanEntity> getSyncData(SyncDataRequest request) {
		return null;
	}

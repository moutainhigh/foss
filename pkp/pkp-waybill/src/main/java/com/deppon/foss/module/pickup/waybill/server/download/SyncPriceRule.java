 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncPriceRule.java
 * 
 * FILE NAME        	: SyncPriceRule.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncPriceRule extends iBatis3DaoImpl implements ISyncDataDao<PriceRuleEntity> {
	/**
	 *  价格 下载服务
	 */
	...
	public List getSyncData(Date fromDate,
		...
		List<PriceRuleEntity> list = new ArrayList<PriceRuleEntity>();
		...
		data = priceDownLoadService.downPriceRuleServerData(clientInfo);
		if(data != null){
			if(data.getObject()!=null){
				list = ((List<PriceRuleEntity>)data.getObject());
			}
		}
		return  list;
	}

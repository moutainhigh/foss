 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncLimitedWarrantyItems.java
 * 
 * FILE NAME        	: SyncLimitedWarrantyItems.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncLimitedWarrantyItems  extends iBatis3DaoImpl implements ISyncDataDao<LimitedWarrantyItemsEntity>{
	/**
	 * baseinfo 下载服务
	 */
	...
	public List<LimitedWarrantyItemsEntity> getSyncData(Date fromDate,
			String empCode, String regionID,String pagionation, int page) {
		List<LimitedWarrantyItemsEntity> list = new ArrayList<LimitedWarrantyItemsEntity>();
		...
		data = baseInfoDownloadService.downloadInsurGoodsData(clientInfo);
		if(data != null){
			if(data.getObject()!=null){
				list = ((List<LimitedWarrantyItemsEntity>)data.getObject());
			}
		}
		return  list;

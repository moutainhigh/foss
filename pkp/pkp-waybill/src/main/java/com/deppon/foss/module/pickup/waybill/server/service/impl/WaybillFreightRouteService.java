...
public class WaybillFreightRouteService implements IWaybillFreightRouteService {
	...
	public OrgInfoDto queryLodeDepartmentInfoOnline(boolean pickupCentralized,
		...
		boolean isTransSale = waybillManagerService.identityTransSale(orginalOrganizationCode);
		// 如果是集中开单则取集中开单组所属外场
		
		// 驻地营业部
		orgInfoDto.setDepartureTransCenter(waybillManagerService.queryDefaultTransCodeDept(orginalOrganizationCode,productCode, isTransSale));
	
		FreightRouteDto freightRouteDto = waybillManagerService.queryFreightRouteBySourceTarget(orginalOrganizationCode,destinationOrganizationCode, productCode, new Date());
		...
		if (freightRouteDto.getFreightRouteLinelist() != null) {
			...
			OrgAdministrativeInfoEntity firstDepartment = waybillManagerService.queryOrgInfoByCode(firstLoadOrgCode);
			...
			for (int i = 0; i < departmentInfoList.size(); i++) {
				orgAdministrativeInfoEntity = waybillManagerService.queryOrgInfoByCode(departmentInfoList.get(i));
				if (orgAdministrativeInfoEntity != null) {
					if (i != departmentInfoList.size() - 1) {
						wholeRouteLineNameBuffer.append(orgAdministrativeInfoEntity.getName()).append("-");
						wholeRouteLineCodeBuffer.append(departmentInfoList.get(i)).append("-");
					} else {
						wholeRouteLineNameBuffer.append(orgAdministrativeInfoEntity.getName());
						wholeRouteLineCodeBuffer.append(departmentInfoList.get(i));
					}
				} else {
					agencyBranchOrCompanyDto = waybillManagerService.queryAgencyBranchCompanyInfo(departmentInfoList.get(i));
					if (i != departmentInfoList.size() - 1) {
						wholeRouteLineNameBuffer.append(agencyBranchOrCompanyDto.getAgentDeptName()).append("-");
						wholeRouteLineCodeBuffer.append(departmentInfoList.get(i)).append("-");
					} else {
						wholeRouteLineNameBuffer.append(agencyBranchOrCompanyDto.getAgentDeptName());
						wholeRouteLineCodeBuffer.append(departmentInfoList.get(i));
					}
				}
			...
			// 最终配载部门
			OrgAdministrativeInfoEntity lastDepartment = waybillManagerService
...
//				if(lastDepartment.checkExpressSalesDepartment()){
//					expressLastLoadOrgCode = lastLoadCode;
//					expressLastLoadOrgName = lastLoadName;
//				}else{
//					expressLastLoadOrgCode = lastLoadCode;
//					expressLastLoadOrgName = lastLoadName;
...
//				}
				...
				for (int i = departmentInfoList.size() - 1; i > -1; i--) {
					...
					lastDepartment = waybillManagerService.queryOrgInfoByCode(lastLoadCode);
					if (lastDepartment == null) {
						continue;
					} else {
						lastLoadName = lastDepartment.getName();
				...
				// 配载线路拼最后一个代理网点
				agencyBranchOrCompanyDto = waybillManagerService
				...
				if (agencyBranchOrCompanyDto != null) {
					wholeRouteLineNameBuffer.append("-").append(
							agencyBranchOrCompanyDto.getAgentDeptName());
			...
			for (int i = departmentInfoList.size() - 1; i > -1; i--) {
				...
				lastOutLoadOrgDepartment = waybillManagerService
						.queryOrgInfoByCode(lastOutLoadOrgCode);

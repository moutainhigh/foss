 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncEffectiveRegionOrg.java
 * 
 * FILE NAME        	: SyncEffectiveRegionOrg.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
...
public class SyncEffectiveRegionOrg extends iBatis3DaoImpl implements ISyncDataDao<EffectiveRegionOrgEntity> {
	/**
	 *  价格 下载服务
	 */
	...
	public List<EffectiveRegionOrgEntity> getSyncData(Date fromDate,
			String empCode, String regionID,String pagionation, int page) {
		DataBundle data;
		List<EffectiveRegionOrgEntity> list = new ArrayList<EffectiveRegionOrgEntity> ();
		...
		data = priceDownLoadService.downEffectiveRegionOrgDetailServerData(clientInfo);
		if(data != null){
			if(data.getObject()!=null){
				list = ((List<EffectiveRegionOrgEntity>)data.getObject());
			}
		}
		return  list;
	}

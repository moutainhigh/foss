 ...
 *
 */
public class SyncPriceRegionAir extends iBatis3DaoImpl implements ISyncDataDao<PriceRegionAirEntity> {
	/**
	 *  价格 下载服务
	 */
	...
	public List getSyncData(Date fromDate,
		...
		List<PriceRegionAirEntity> list = new ArrayList<PriceRegionAirEntity>();
		DataBundle data;
		...
		data = priceDownLoadService.downPriceRegionAirServerData(clientInfo);
		if(data != null){
			if(data.getObject()!=null){
				list = ((List<PriceRegionAirEntity>)data.getObject());
			}
		}
		return  list;
	}

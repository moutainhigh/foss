 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncSaleDepartment.java
 * 
 * FILE NAME        	: SyncSaleDepartment.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncSaleDepartment extends iBatis3DaoImpl implements ISyncDataDao<SaleDepartmentEntity> {
	...
	public List<SaleDepartmentEntity> getSyncData(Date fromDate,
			String empCode, String regionID,String pagionation, int page) {
		List<SaleDepartmentEntity> list = new ArrayList<SaleDepartmentEntity>();
		...
		data = baseInfoDownloadService.downloadSalesDepartmentData(clientInfo);
		if(data != null){
			if(data.getObject() != null){
				list = ((List<SaleDepartmentEntity>)data.getObject());
			}
		}
		return  list;
	}

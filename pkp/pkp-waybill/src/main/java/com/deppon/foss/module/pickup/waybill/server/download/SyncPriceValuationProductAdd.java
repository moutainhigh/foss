 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncPriceValuationProductAdd.java
 * 
 * FILE NAME        	: SyncPriceValuationProductAdd.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncPriceValuationProductAdd extends iBatis3DaoImpl implements ISyncDataDao<PriceValuationProductAddDto> {
	/**
	 *  价格 下载服务
	 */
	...
	public List getSyncData(Date fromDate,
		...
		List<PriceValuationProductAddDto> priceValuationProductAddList = new ArrayList<PriceValuationProductAddDto>();
		List<PriceValuationEntity> list = new ArrayList<PriceValuationEntity> ();
		List<Map<String, List<PriceValuationProductAddDto>>> resultList = new ArrayList<Map<String, List<PriceValuationProductAddDto>>> ();
		DataBundle data;
		...
		data = priceDownLoadService
				.downPricingValuationProductValueAddServerData(clientInfo);
		if (data != null) {
			if (data.getObject() != null) {
				list = ((List<PriceValuationEntity>) data.getObject());
				if (list != null && list.size() > 0) {
					Map<String, List<PriceValuationProductAddDto>> map = new HashMap<String, List<PriceValuationProductAddDto>>();
					...
					for (PriceValuationEntity entity : list) {
						PriceValuationProductAddDto dto = new PriceValuationProductAddDto();
					...
					if (needDeleteLocalData != null
						...
						if (FossConstants.YES.equals(needDeleteLocalData)) {
						    ...
						    map.put(needDeleteLocalData,priceValuationProductAddList);
						}
					}

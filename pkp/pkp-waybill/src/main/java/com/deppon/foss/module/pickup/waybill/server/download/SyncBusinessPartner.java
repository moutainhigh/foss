 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncBusinessPartner.java
 * 
 * FILE NAME        	: SyncBusinessPartner.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncBusinessPartner extends iBatis3DaoImpl implements ISyncDataDao<BusinessPartnerEntity> {
	/**
	 *  baseinfo 下载服务
	 */
	...
	public List<BusinessPartnerEntity> getSyncData(Date fromDate, String empCode, 
			String regionID, String pagionation, int page) {
		DataBundle data;
		List<BusinessPartnerEntity> list = new ArrayList<BusinessPartnerEntity>();
		...
		data = baseInfoDownloadService.downloadBusinessPartnerData(clientInfo);
		if(data != null){
			if(data.getObject() != null){
				list = ((List<BusinessPartnerEntity>)data.getObject());
			}
		}
		return  list;
	}
	 ...
	 */
	public List<BusinessPartnerEntity> getSyncData(SyncDataRequest request) {
		return null;
	}
}
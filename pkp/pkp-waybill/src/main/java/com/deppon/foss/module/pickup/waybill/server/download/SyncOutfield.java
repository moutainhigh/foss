 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncOutfield.java
 * 
 * FILE NAME        	: SyncOutfield.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncOutfield extends iBatis3DaoImpl implements ISyncDataDao<OutfieldEntity> {
	/**
	 * baseinfo 下载服务
	 */
	...
	public List<OutfieldEntity> getSyncData(Date fromDate,
			String empCode, String regionID,String pagionation, int page) {
		List<OutfieldEntity> list = new ArrayList<OutfieldEntity>();
		...
		data = baseInfoDownloadService.downloadTransferCenterData(clientInfo);
		if(data != null){
			if(data.getObject() != null){
				list = ((List<OutfieldEntity>)data.getObject());
			}
		}
		return  list;

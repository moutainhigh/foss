 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncSalesProduct.java
 * 
 * FILE NAME        	: SyncSalesProduct.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncSalesProduct extends iBatis3DaoImpl implements ISyncDataDao<SalesProductEntity> {
	/**
	 * baseinfo 下载服务
	 */
	...
	public List<SalesProductEntity> getSyncData(Date fromDate,
			String empCode, String regionID,String pagionation, int page) {
		List<SalesProductEntity> list = new ArrayList<SalesProductEntity>();
		...
		data = baseInfoDownloadService.downloadSalesProductData(clientInfo);
		if(data != null){
			if(data.getObject() != null){
				list = ((List<SalesProductEntity>)data.getObject());
			}
		}
		return  list;
	}

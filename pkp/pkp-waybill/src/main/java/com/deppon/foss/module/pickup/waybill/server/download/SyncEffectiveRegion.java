 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncEffectiveRegion.java
 * 
 * FILE NAME        	: SyncEffectiveRegion.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncEffectiveRegion extends iBatis3DaoImpl implements ISyncDataDao<EffectiveRegionEntity> {
	/**
	 *  价格 下载服务
	 */
	...
	public List<EffectiveRegionEntity> getSyncData(Date fromDate,
			String empCode, String regionID,String pagionation, int page) {
		List<EffectiveRegionEntity> list = new ArrayList<EffectiveRegionEntity> ();
		DataBundle data;
		...
		data = priceDownLoadService.downEffectiveRegionServerData(clientInfo);
		if(data != null){
			if(data.getObject()!=null){
				list = ((List<EffectiveRegionEntity>)data.getObject());
			}
		}
		return  list;
	}

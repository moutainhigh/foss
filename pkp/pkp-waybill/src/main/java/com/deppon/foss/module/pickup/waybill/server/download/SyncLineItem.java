 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncLineItem.java
 * 
 * FILE NAME        	: SyncLineItem.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncLineItem extends iBatis3DaoImpl implements ISyncDataDao<LineItemEntity> {
	/**
	 * baseinfo 下载服务
	 */
	...
	public List<LineItemEntity> getSyncData(Date fromDate, String empCode,
			String regionID,String pagionation, int page) {
		List<LineItemEntity> list = new ArrayList<LineItemEntity>();
		...
		data = baseInfoDownloadService.downloadLineItemData(clientInfo);
		if(data != null){
			if(data.getObject() != null){
				list = ((List<LineItemEntity>)data.getObject());
			}
		}
		return  list;
	}

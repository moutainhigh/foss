 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncPrice.java
 * 
 * FILE NAME        	: SyncPrice.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncPrice extends iBatis3DaoImpl implements ISyncDataDao<PriceEntity> {
	/**
	 *  价格 下载服务
	 */
	...
	public List getSyncData(Date fromDate,
		...
		data = priceDownLoadService.downPriceEntity(clientInfo);
		if(data != null){
			if(data.getObject()!=null){
				list = ((List<PriceEntity>)data.getObject());
			}
		}
		return  list;
	}

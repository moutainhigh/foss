 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncEffectivePlan.java
 * 
 * FILE NAME        	: SyncEffectivePlan.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncEffectivePlan extends iBatis3DaoImpl implements ISyncDataDao<EffectivePlanEntity> {
	/**
	 *  价格 下载服务
	 */
	...
	public List getSyncData(Date fromDate, 
		...
		List<Map<String, List<EffectivePlanEntity>>> resultList = null;
		...
		data = priceDownLoadService.downEffectivePlanServerData(clientInfo);
		if(data != null){
			if(data.getObject()!=null){
				list = ((List<EffectivePlanEntity>)data.getObject());
				if (list != null && list.size() > 0) {
					Map<String, List<EffectivePlanEntity>> map = new HashMap<String, List<EffectivePlanEntity>>();
					String needDeleteLocalData = data.getNeedDeleteLocalData();
					if (needDeleteLocalData != null	&& !"".equals(needDeleteLocalData)) {
						//放原时效区域ID

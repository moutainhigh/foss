 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncFinOrg.java
 * 
 * FILE NAME        	: SyncFinOrg.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 * @return 
 */
public class SyncFinOrg extends iBatis3DaoImpl implements ISyncDataDao<FinancialOrganizationsEntity> {
	/**
	 * baseinfo 下载服务
	 */
	...
	public List<FinancialOrganizationsEntity> getSyncData(Date fromDate,
			String empCode, String regionID,String pagionation, int page) {
		DataBundle data;
		List<FinancialOrganizationsEntity> list = new ArrayList<FinancialOrganizationsEntity> ();
		...
		data = baseInfoDownloadService.downloadFinancialOrganizationsData(clientInfo);
		if(data != null){
			if(data.getObject()!=null){
				list = ((List<FinancialOrganizationsEntity>)data.getObject());
			}
		}
		return  list;
	}

...
import com.deppon.esb.pojo.transformer.jaxb.PartnerWaybillExpInfoRequestTrans;
 ...
 * @date 2016-1-15
 */
public class ExpWaybillInfoToPtpService implements IExpWaybillInfoToPtpService {
	
	private Logger logger = LoggerFactory.getLogger(ExpWaybillInfoToPtpService.class);
	
	private static String asynCode = "ESB_FOSS2ESB_PARTNER_WAYBILL_EXP" ;
	...
	@Override
	public void asynSendExpWaybillInfoToPtp(PtpWaybillDto ptpWaybillDto) {
		...
		PartnerWaybillExpInfoRequest request = new PartnerWaybillExpInfoRequest();
		...
		try {
			...
			try {
				...
				flag = true ;
				// 对异常进行处理
				logger.error("Foss推送快递消息至合伙人系统拷贝request异常，错误详情：", e);
				throw new WaybillSubmitException("Foss推送快递消息至合伙人系统拷贝request异常，错误详情："+System.err);
			}
			...
			try {
				...
				flag = true ;
				// 对异常进行处理
				logger.error("Foss推送消快递息至合伙人系统失败，错误详情：", e);
				throw new WaybillSubmitException("Foss推送快递消息至合伙人系统失败，错误详情："+ReflectionToStringBuilder.toString(e));
			}
			...
			} catch (WaybillSubmitException e) {
	 ...
	 * @return
	 */
	private AccessHeader createAccessHeader(PartnerWaybillExpInfoRequest dto) {
		AccessHeader header = new AccessHeader();
		//business code 根据esb提供
		//ESB_FOSS2ESB_RECEIVE_ORDERSTATUS
		logger.info("接送货推送的合伙人系统编码（快递）为："+asynCode+"版本号:"+version);
	 ...
	 * @date 2016-2-18
	 */
	private void addPartnerwaybillLog(PtpWaybillDto ptpWaybillDto,PartnerWaybillExpInfoRequest request,String errorMsg,boolean flag){
		try {
			PartnerWaybillExpInfoRequestTrans trans = PartnerWaybillExpInfoRequestTrans.class.newInstance();
			...
			logger.error("Foss推送快递消息至合伙人记录日志信息异常，错误详情：", e);
			throw new WaybillSubmitException("Foss推送快递消息至合伙人记录日志信息异常，错误详情："+System.err);
		}
	}

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/service/impl/SendSmsVoiceService.java
 * 
 * FILE NAME        	: SendSmsVoiceService.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 * 
 */
public class SendSmsVoiceService implements ISendSmsVoiceService {
	 private static final Logger LOGGER = LoggerFactory.getLogger(SendSmsVoiceService.class);
	 ...
	 */
	@Override
	public void sendShipperSms(WaybillEntity waybill){
		//广告
		String adContent = "广告内容待定";
		//发送内容
		NotificationEntity note = gainShipperNotification(waybill, adContent);
		//发送短信
		sendMessage(note);
	}
	 ...
	 */
	@Override
	public void sendConsigneeSms(WaybillEntity waybill){
		//发送内容
		NotificationEntity note = gainConsigneeNotification(waybill, null);
		//发送短信
		sendMessage(note);
	}
	 ...
	 * 封装给发货人发送短信的信息
	 * @author 026123-foss-lifengteng
	 * @date 2012-12-26 下午8:30:25
	 */
	private NotificationEntity gainShipperNotification(WaybillEntity waybill,String adContent){
		NotificationEntity entity = new NotificationEntity();
		 //运单号
		entity.setWaybillNo(waybill.getWaybillNo());
		//通知内容
		String content = querySmsTemplate(gainSmsVoiceParamDto(waybill,adContent),NotifyCustomerConstants.SMS_CODE_CONSIGNOR_TO_DOOR);
	 ...
	 * 封装给收货人发送短信的信息
	 * @author 026123-foss-lifengteng
	 * @date 2012-12-26 下午8:30:28
	 */
	private NotificationEntity gainConsigneeNotification(WaybillEntity waybill,String adContent){
		NotificationEntity entity = new NotificationEntity();
		 //运单号
		entity.setWaybillNo(waybill.getWaybillNo());
		//通知内容
		String content = querySmsTemplate(gainSmsVoiceParamDto(waybill,adContent),NotifyCustomerConstants.SMS_CODE_CONSIGNEE_SELF_PICKUP);
		entity.setNoticeContent(content);
	 ...
	 * 调用接口发送短信
	 * @author 026123-foss-lifengteng
	 * @date 2012-12-26 下午8:47:36
	 */
	public void sendMessage(NotificationEntity entity){
		 //通知类型
		entity.setNoticeType(NotifyCustomerConstants.NOTIFY_TYPE_SMS_NOTICE);
		 //操作人
		entity.setOperator(FossUserContextHelper.getUserName());
		 //操作人编码
		entity.setOperatorNo(FossUserContextHelper.getUserCode());
		 //操作部门
		...
		try {
			...
			throw new NotifyCustomerException(NotifyCustomerException.SENDMESSAGE_FAIL, new Object[]{messageBundle.getMessage(e.getErrorCode())},e);
		}
	}
	/**
	 * 给司机发送短信
	...
	public void sendDriverCodeSms(NotificationEntity2 notificationEntity){
		//发送内容
		LOGGER.info("sendSms start : jizhongwaybill pushmessage " + notificationEntity );
		// 设置操作时间
		notificationEntity.setOperateTime(new Date());
		//调用接口发送短信
		try {
			NotificationEntity e=new NotificationEntity();
			BeanUtils.copyProperties(notificationEntity,e);
			notifyCustomerService.sendMessage(e);
		} catch (NotifyCustomerException e) {
			LOGGER.error(e.getMessage(), e);
			throw new NotifyCustomerException(NotifyCustomerException.SENDMESSAGE_FAIL, new Object[]{messageBundle.getMessage(e.getErrorCode())},e);
		}
	 ...
	 * @author 026123-foss-lifengteng
	 * @date 2012-12-26 下午4:02:38
	 */
	 private String querySmsTemplate(SmsVoiceParamDto dto,String smsCode){
		// 返回短信
		String sms = ""; 
		//非空判断
		if (dto == null) {
	 ...
	 * 获取短信语音参数DTO
	 * @author 026123-foss-lifengteng
	 * @date 2012-12-26 下午7:26:35
	 */
	private SmsVoiceParamDto gainSmsVoiceParamDto(WaybillEntity waybill,String adContent){
		SmsVoiceParamDto sms = null;
		if(null != waybill){
			sms = new SmsVoiceParamDto();
			//发货人名称
			sms.setDeliveryCustomerContact(waybill.getDeliveryCustomerContact());
			//总件数
			sms.setGoodsQtyTotal(String.valueOf(waybill.getGoodsQtyTotal()));
			//收货人地址
			sms.setReceiveCustomerAddress(waybill.getReceiveCustomerAddress());
			//总费用
			sms.setTotalFee(String.valueOf(waybill.getTotalFee()));
			//运单号
			sms.setWaybillNo(waybill.getWaybillNo());
			//广告
			if(adContent != null){
				sms.setAdContent(StringUtil.defaultIfNull(adContent));
			}
			//根据部门编码查询部门信息
			OrgAdministrativeInfoEntity orgInfo = orgAdministrativeInfoService.queryOrgAdministrativeInfoByCode(waybill.getCustomerPickupOrgCode());
			if(null != orgInfo){
				//提货网点名称
				sms.setCustomerPickupOrg(StringUtil.defaultIfNull(orgInfo.getName()));
				//城市名
				sms.setDeliveryCustomerCity(StringUtil.defaultIfNull(orgInfo.getCityName()));
			}
		}
		return sms;
	}

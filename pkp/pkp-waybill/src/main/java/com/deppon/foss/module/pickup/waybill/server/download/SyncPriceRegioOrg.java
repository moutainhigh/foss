 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncPriceRegioOrg.java
 * 
 * FILE NAME        	: SyncPriceRegioOrg.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncPriceRegioOrg extends iBatis3DaoImpl implements ISyncDataDao<PriceRegioOrgnEntity> {
	/**
	 *  价格 下载服务
	 */
	...
	public List getSyncData(Date fromDate,
		...
		List<PriceRegioOrgnEntity> list = new ArrayList<PriceRegioOrgnEntity>();
		...
		data = priceDownLoadService.downPriceRegionOrgDetailServerData(clientInfo);
		if(data != null){
			if(data.getObject()!=null){
				list = ((List<PriceRegioOrgnEntity>)data.getObject());
			}
		}
		return  list;
	}

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncPriceValuationBasisAdd.java
 * 
 * FILE NAME        	: SyncPriceValuationBasisAdd.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncPriceValuationBasisAdd extends iBatis3DaoImpl implements ISyncDataDao<PriceValuationBasisAddDto> {
	...
	public List getSyncData(Date fromDate,
		...
		List<PriceValuationBasisAddDto> priceValuationBasisAddList = null;
		List<PriceValuationEntity> list = new ArrayList<PriceValuationEntity>();
		List<Map<String, List<PriceValuationBasisAddDto>>> resultList =new ArrayList<Map<String, List<PriceValuationBasisAddDto>>>();
		DataBundle data;
		...
		data = priceDownLoadService.downPricingValuationBasicValueAddServerData(clientInfo);
		if (data != null) {
			if (data.getObject() != null) {
				list = ((List<PriceValuationEntity>) data.getObject());
				if (list != null && list.size() > 0) {
					Map<String, List<PriceValuationBasisAddDto>> map = new HashMap<String, List<PriceValuationBasisAddDto>>();
					String needDeleteLocalData = data.getNeedDeleteLocalData();
					priceValuationBasisAddList = new ArrayList();
					for (PriceValuationEntity entity : list) {
						PriceValuationBasisAddDto dto = new PriceValuationBasisAddDto();
						BeanUtils.copyProperties(entity, dto);
						priceValuationBasisAddList.add(dto);
					}
					if (needDeleteLocalData != null
							&& !"".equals(needDeleteLocalData)) {
						// 放原时效区域ID
						if (FossConstants.YES.equals(needDeleteLocalData)) {
							map.put(regionID, priceValuationBasisAddList);
						} else{
						    map.put(needDeleteLocalData,priceValuationBasisAddList);
						}
					}

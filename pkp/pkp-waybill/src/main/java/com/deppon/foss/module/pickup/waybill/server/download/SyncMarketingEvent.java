 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncMarketingEvent.java
 * 
 * FILE NAME        	: SyncMarketingEvent.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 */
public class SyncMarketingEvent extends iBatis3DaoImpl implements ISyncDataDao<MarketingEventEntity> {
	/**
	 *  价格 下载服务
	 */
	...
	public List<MarketingEventEntity> getSyncData(Date fromDate,
			String empCode, String regionID,String pagionation, int page) {
		List<MarketingEventEntity> list = new ArrayList<MarketingEventEntity>();
		...
		data = priceDownLoadService.downMarketingEvent(clientInfo);
		if(data != null){
			if(data.getObject() != null){
				list = ((List<MarketingEventEntity>)data.getObject());
			}
		}
		return  list;
	}

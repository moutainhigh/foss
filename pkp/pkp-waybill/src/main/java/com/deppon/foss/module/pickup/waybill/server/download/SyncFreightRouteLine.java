 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/pickup/waybill/server/download/SyncFreightRouteLine.java
 * 
 * FILE NAME        	: SyncFreightRouteLine.java
 * 
 * AUTHOR			: FOSS接送货系统开发组
 * 
 ...
 *
 */
public class SyncFreightRouteLine extends iBatis3DaoImpl implements ISyncDataDao<FreightRouteLineEntity> {
	...
	public List<FreightRouteLineEntity> getSyncData(Date fromDate,
			String empCode, String regionID,String pagionation, int page) {
		List<FreightRouteLineEntity> list = new ArrayList<FreightRouteLineEntity>();
		...
		data = baseInfoDownloadService.downloadFreightRouteLineData(clientInfo);
		if(data != null){
			if(data.getObject()!=null){
				list = ((List<FreightRouteLineEntity>)data.getObject());
			}
		}
		return  list;
	}

 ...
 * @时间 2012-8-31
 */
waybill.getStore = function(storeId,model,fields,data) {
	var store = null;
...
//Ajax请求--文件表单的请求
waybill.requestFileUploadAjax = function(url,form,successFn,failFn)
{
	Ext.Ajax.request({
		url:url,
		form:form,
...
//Ajax请求--非json字符串
waybill.requestAjax = function(url,params,successFn,failFn)
{
	Ext.Ajax.request({
		url:url,
		params:params,
 ...
 * @时间 2012-3-15
 */
waybill.setReadOnly = function(readOnlyIdList){
 ...
 * @时间 2012-3-15
 */
waybill.setHiddenAndDestroy = function(hiddenIdList){
 ...
 * @时间 2012-3-15
 */
waybill.setHidden = function(hiddenIdList){
     for(var i=0;i<hiddenIdList.length;i++){
    	 Ext.getCmp(hiddenIdList[i]).hide();
     }
};
 ...
 * @时间 2012-3-21
 */
waybill.setDestroy = function(destoryIdList){
 ...
 * @时间 2012-3-21
 */
waybill.setDisabled =function(disabledIdList){
 ...
 * @时间 2012-3-22
 */
waybill.clearListeners =function(clearIdList){
 ...
 * @时间 2012-3-24
 */
waybill.isHaveEmpty = function(array){
	var boolen = false;
 ...
 * @时间 2012-4-16
 */
waybill.changeModelListToDataList = function(modelList){
 ...
 * @时间 2012-4-16
 */
waybill.copyModelListToDataList = function(modelList){
	var dataList = new Array();
	for(var i = 0;i<modelList.length;i++){
...
//警告
waybill.showWoringMessage = function(message,fun) {
	var len = message.length;
	Ext.Msg.show({
	    title:'FOSS提醒您:',//waybill.util.i18n('i18n.waybill-util.fossAlert'), 平台会对具体的大模块.小模块去映射i18n资源文件，util.js文件属于公共文件故不存在模块概念不被解析到，暂时先用中文
	    msg:message,
	    //cls:'mesbox',
	    width:110+len*15,
...
//是和否选择
waybill.showQuestionMes = function(message,fun) {
	var len = message.length;
	Ext.Msg.show({
	    title:'FOSS提醒您:',//waybill.util.i18n('i18n.waybill-util.fossAlert'),
	    width:110+len*15,
	    msg:'<div id="message">'+message+'</div>',
...
//ADD -ALL
waybill.addAll = function(list,all) {
...
waybill.changeCodeToName = function(list,code) {
	var name = '';
	for(var i = 0;i<list.length;i++){
...
waybill.changeCodeToNameStore = function(store,code) {
	var name = '';
	if(!Ext.isEmpty(store)){
 ...
 */
waybill.getDateBeforeAfter = function(date, day) {
	var d, s, t, t2;

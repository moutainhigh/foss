 ...
 *
 */
public class PreformatJRViewer extends javax.swing.JPanel implements JRHyperlinkListener {
	private static final Log log = LogFactory.getLog(PreformatJRViewer.class);
	...
	public PreformatJRViewer(JasperPrint jrPrint) throws Exception {
		this(jrPrint, null, null , null);
	}

	public PreformatJRViewer(JasperPrint fulljp, JasperPrint noimgjp, Locale locale) throws Exception {
		this(fulljp, noimgjp, locale, null);
	}

	public PreformatJRViewer(JasperPrint fulljp, JasperPrint noimgjp,
			Locale locale, ResourceBundle resBundle) throws Exception {
	...
	private JasperPrint getDisplayJasperPrint(){
		if(noImgJasperPrint!=null && chkShowImage!=null && !chkShowImage.isSelected()){
		...
		Thread thread = new Thread(new Runnable() {
			public void run() {
				try {
					...
					PreformatJRViewer.this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
//					JasperPrintManager.printReport(getDisplayJasperPrint(), true);
					...
					JOptionPane.showMessageDialog(PreformatJRViewer.this, getBundleString("error.printing"));
				} finally {
					PreformatJRViewer.this.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
	...
	protected void paintPage(Graphics2D grx) {
		...
		try {
			...
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					JOptionPane.showMessageDialog(PreformatJRViewer.this, getBundleString("error.displaying"));
				}
			});
		}
	}
	...
	static class PageRenderer extends JLabel {
		...
		PreformatJRViewer viewer = null;

		public PageRenderer(PreformatJRViewer viewer) {
			this.viewer = viewer;
		}

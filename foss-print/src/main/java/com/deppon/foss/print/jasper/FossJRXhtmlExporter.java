...
public class FossJRXhtmlExporter extends JRAbstractExporter
{
	private static final Log log = LogFactory.getLog(FossJRXhtmlExporter.class);
	...
	protected class ExporterContext extends BaseExporterContext implements JRHtmlExporterContext
	{
		...
		{
			return FossJRXhtmlExporter.this.getHyperlinkURL(link);
		}
	}
	 ...
	 *
	 */
	public FossJRXhtmlExporter(JasperReportsContext jasperReportsContext)
	{
	...
	public Map<String,byte[]> getImageNameToImageDataMap(){
		return this.imageNameToImageDataMap;
	}
	...
	public void exportReport() throws JRException
	{
		...
		try
		{
			...
			imageNameToImageDataMap = new HashMap();//(Map<String,byte[]>)parameters.get(JRHtmlExporterParameter.IMAGES_MAP);
		...
		if(renderer != null)
		{
			...
			if (renderer != null)
			{
				...
				else
				{
					...
					else if(image.getRenderable() instanceof BarbecueRenderer){
						//speical method to parse barcode image into base64 into html img src
						byte[] _imgdata = renderer.getImageData(jasperReportsContext);
						ByteArrayInputStream is = new ByteArrayInputStream(_imgdata); 
						ByteArrayOutputStream output = new ByteArrayOutputStream();  
				        try {
				            JimiReader reader = Jimi.createJimiReader(is);
				            Jimi.putImage("image/jpeg", reader.getImage(), output);  
				            output.flush();  
				            output.close();  
				            imagePath = "data:image/jpeg;base64,"+Base64.encodeBase64String(output.toByteArray());  
				        }catch (Exception e) { }
				        is.close();
				        output.close();
					}
					else
					{
						...
						// put images stream into base64 code of <img src/>
						byte[] _imgdata = renderer.getImageData(jasperReportsContext);
						ByteArrayInputStream is = new ByteArrayInputStream(_imgdata); 
						ByteArrayOutputStream output = new ByteArrayOutputStream();  
				        try {

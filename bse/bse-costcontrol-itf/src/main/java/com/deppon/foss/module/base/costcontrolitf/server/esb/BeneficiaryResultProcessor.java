...
import com.deppon.esb.inteface.domain.payment.PayeeInfo;
...
import com.deppon.esb.inteface.domain.payment.PayeeInfoProcessResult;
 ...
 * @date 2012-12-14 上午8:55:07
 * @since
 * @version
 */
public class BeneficiaryResultProcessor implements IProcess {

    /**
     * 日志
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(BeneficiaryResultProcessor.class);
	...
	public Object process(Object req) throws ESBBusinessException {
		...
		PayeeInfoNotificationRequest request = (PayeeInfoNotificationRequest) req;
		PayeeInfoNotificationResponse response = new PayeeInfoNotificationResponse();
		// 费控系统
		String user = "FIN";
		...
		for (PayeeInfo info : payeeInfoList) {
			...
			if (StringUtils.isNotBlank(info.getPayeeNo())) {
				// 收款方编码
				processResult.setPayeeNo(info.getPayeeNo().trim());
			} else {
				// 收款方编码
				processResult.setPayeeNo(info.getPayeeNo());
			}
			...
			if (StringUtil.equals(NumberConstants.NUMERAL_S_ONE,
					info.getOperateCode())) {// 新增信息
				try {
					int addResult = 0;
					addResult = payeeInfoService.addPayeeInfo(entity);
					if (addResult > 0) {// 保存成功
						processResult.setResult(true);
						// 成功数
						successCount++;
					} else {
						processResult.setReason("同步收款方失败，" + info.getPayeeNo()
								+ "在FOSS中已存在！");
			...
			if (StringUtil.equals(NumberConstants.NUMERAL_S_TWO,
					info.getOperateCode())) {// 修改信息
				try {
					int updateResult = payeeInfoService.updatePayeeInfo(entity);
					if (updateResult > 0) {
						processResult.setResult(true);
						// 成功数
						successCount++;

 ...
 * 
 * FILE PATH        	: src/test/java/com/deppon/foss/module/base/baseinfo/server/dao/RoleDaoTest.java
 * 
...
import com.deppon.foss.module.base.baseinfo.server.dao.impl.RoleDao;
...
public class RoleDaoTest {
	...
	// 先添加一个实体，用于测试
	RoleEntity addEntity = getEntity();
	RoleEntity result = roleDao.addRole(addEntity);
	...
	// 先添加一个实体，用于测试
	RoleEntity addEntity = getEntity();
	roleDao.addRole(addEntity);

	try{
	    RoleEntity result = roleDao.deleteRole(addEntity);
	    Assert.assertTrue(result != null);
    ...
    @Test
    public void testDeleteRoleMore(){
	// 先添加一个实体，用于测试
	RoleEntity addEntity =  getEntity();
	roleDao.addRole(addEntity);
	...
	try{
	    ...
	    RoleEntity result = roleDao.deleteRoleMore(codes, null);
	    Assert.assertTrue(result != null);
	...
	try{
	    RoleEntity result = roleDao.updateRole(addEntity);
	    Assert.assertTrue(result != null);	
    ...
    @Test
    public void testQueryRoleByCode(){
	// 先添加一个实体，用于测试
	RoleEntity addEntity = getEntity();
	roleDao.addRole(addEntity);
	...
	try{
	    ...
	    RoleEntity result = roleDao.queryRoleByCode(code);
	    Assert.assertTrue(result != null);
	}finally{
	    deleteById(addEntity.getId());
	}	
    ...
    @Test
    public void testQueryRoleBatchByCode(){
	// 先添加一个实体，用于测试
	RoleEntity addEntity = getEntity();
	roleDao.addRole(addEntity);
	...
	try{
	    ...
	    result = roleDao.queryRoleBatchByCode(codes);
	    Assert.assertTrue(result != null);
	}finally{
	    deleteById(addEntity.getId());
	}		
    ...
    @Test
    public void testQueryRoleExactByEntity(){
	// 先添加一个实体，用于测试
	RoleEntity addEntity = getEntity();
	roleDao.addRole(addEntity);
	...
	try{
	    ...
	    List<RoleEntity> result = null;
	    result = roleDao.queryRoleExactByEntity(entity, 0, 1);
	    Assert.assertTrue(result != null);
	}finally{
	    deleteById(addEntity.getId());
	}		
    ...
    @Test
    public void testQueryRoleExactByEntityCount(){
	// 先添加一个实体，用于测试
	RoleEntity addEntity = getEntity();
	roleDao.addRole(addEntity);
	...
	try{
	    ...
	    long result = 0;
	    result = roleDao.queryRoleExactByEntityCount(entity);
    ...
    @Test
    public void testQueryRoleByEntity(){
	// 先添加一个实体，用于测试
	RoleEntity addEntity = getEntity();
	roleDao.addRole(addEntity);
	...
	try{
	    ...
	    List<RoleEntity> result = roleDao.queryRoleByEntity(entity, 0, 1);
	    Assert.assertTrue(result != null);
    ...
    @Test
    public void testQueryRoleByEntityCount(){
	// 先添加一个实体，用于测试
	RoleEntity addEntity = getEntity();
	roleDao.addRole(addEntity);
	...
	try{
	    ...
	    long result = roleDao.queryRoleByEntityCount(entity);
	    Assert.assertTrue(result >= 0);
	...
	initTest();
	roleDao = (IRoleDao) SpringTestHelper.get()
		.getBeanByClass(RoleDao.class);
    }
	...
	// jdbc.execute("delete from T_BAS_ROLE");
    }
	...
	RoleEntity entity=(RoleEntity)createObj(RoleEntity.class.getName());
	entity.setCode(System.currentTimeMillis()+"");
	entity.setActive(FossConstants.ACTIVE);
	String id = UUIDUtils.getUUID();
	entity.setId(id);
	entity.setCreateUser("087584-todelete");
	return entity;
    }

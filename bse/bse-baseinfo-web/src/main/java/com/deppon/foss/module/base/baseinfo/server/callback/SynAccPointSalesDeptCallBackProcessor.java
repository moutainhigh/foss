...
import com.deppon.cubc.inteface.domain.SyncAccSalesDeptResponse;
...
import com.deppon.esb.pojo.transformer.jaxb.SyncAccSalesDeptResponseTrans;

public class SynAccPointSalesDeptCallBackProcessor implements ICallBackProcess {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(SynAccPointSalesDeptCallBackProcessor.class);

	@Override
	public void callback(Object response) throws ESBException {
		// TODO Auto-generated method stub
		if (null != response) {
			SyncAccSalesDeptResponse syncAccSalesDeptResponse = (SyncAccSalesDeptResponse) response;
			try {
				LOGGER.info("send AccSalesDept info,then receive callback info.同步接驳点与营业部映射关系到CUBC，收到回调信息：\n"+ 
				   new SyncAccSalesDeptResponseTrans().fromMessage(syncAccSalesDeptResponse));

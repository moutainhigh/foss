...
import com.deppon.esb.pojo.transformer.jaxb.SyncOrganizationResponseTrans;
...
import com.deppon.ows.inteface.domain.failinfo.FailInfo;
 ...
 */
public class OrgAdministrativeInfoResultCallBackProcessor implements
	ICallBackProcess {

    private static final Logger LOGGER = LoggerFactory
	    .getLogger(OrgAdministrativeInfoResultCallBackProcessor.class);

    @Override
    public void callback(Object response) throws ESBException {
	if (null != response) {
	    SyncOrganizationResponse responseObj = (SyncOrganizationResponse) response;
	    LOGGER.info("send OrgAdministrativeInfo info,then receive callback info.同步行政组织，收到回调信息：\n"
		    + new SyncOrganizationResponseTrans()
			    .fromMessage(responseObj));
	    List<FailInfo> failInfoLst = responseObj.getFailInfos();
	    for (FailInfo info : failInfoLst) {

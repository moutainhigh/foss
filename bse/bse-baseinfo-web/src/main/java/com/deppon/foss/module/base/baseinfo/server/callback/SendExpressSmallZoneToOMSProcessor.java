...
import com.deppon.esb.pojo.transformer.jaxb.SyncExpressSmallZoneResponseTrans;
import com.deppon.oms.inteface.domain.SyncExpressSmallZoneResponse;
 
/**
 * 同步快递收派小区到OMS系统的结果回调同步信息到数据库
 * @author 313353-foss-qiupeng
 * @date 2016-3-23 下午5:01:10
 */
public class SendExpressSmallZoneToOMSProcessor implements  ICallBackProcess {

    private static final Logger LOGGER = LoggerFactory.getLogger(SendExpressSmallZoneToOMSProcessor.class);
    
    @Override
    public void callback(Object response) throws ESBException {
    	if (null != response) {
			SyncExpressSmallZoneResponse syncExpressSmallZoneResponse = (SyncExpressSmallZoneResponse) response;
			LOGGER.info("send dictirct info,then receive callback info.同步快递收派小区，收到回调信息：\n"+ new SyncExpressSmallZoneResponseTrans().fromMessage(syncExpressSmallZoneResponse));
		}
    }

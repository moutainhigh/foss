//------------------------------------常量----------------------------------
querying.operatorCount = {defaultV:0,successV:1,failureV:-1};		//操作返回值 1为成功，-1为失败
querying.delAgencyType = 'MANY';									//删除的类型，批量
querying.delType = 'MANY';											//删除的类型，批量
querying.viewState = {add:'ADD',update:'UPDATE',view:'VIEW'};		// 查看状态viewState："ADD"新增,"UPDATE"修改,"VIEW"查看
querying.booleanType = {yes:'Y',no:'N'};							//booleanType  对应后台常量 "布尔类型"
querying.effectiveState = {active:'Y',inactive:'N'};				//booleanType  对应后台常量 "生效/未生效"
querying.booleanStr = {yes:'true',no:'false'};						//booleanStr   从复选框中得到值
querying.operateType = {save:'SAVE'};								//标识 是否 保存操作
querying.levelType = {p:'PARENT',c:'CHILDREN'};						//标识 是父容器还是子容器
/**.
 * <p>
 * AJAX请求<br/>
 * <p>
 ...
 * @时间 2012-12-07
 */
querying.requestAjaxJson = function(url,params,successFn,exceptionFn,failFn,async)
{
	Ext.Ajax.request({
		url:url,
 ...
 * @时间 2012-12-07
 */
querying.formReset = function(form,formRecord,grid,girdData){
 ...
 * @时间 2012-12-13
 */
querying.formSetReadOnly = function(flag,form) {
...
querying.formFieldSetReadOnly = function(flag,form) {
 ...
 * @时间 2012-12-13
 */
querying.showInfoMsg = function(message,fun) {
 ...
 * @时间 2012-12-13
 */
querying.operateWinBtn = function(win,viewState,operateType){
	//查看状态下 只有 取消按钮可用 [添加网点,取消]按钮分别占 0和1
	if(querying.viewState.view === viewState){
		var btnArr = win.query('button');
		for(var i = 0; i < btnArr.length;i++){
			btnArr[i].setDisabled(i != 1);
		}
	}else if(!Ext.isEmpty(operateType)&&querying.operateType.save === operateType){
		var btnArr = win.query('button');
		for(var i = 0; i < btnArr.length;i++){
			btnArr[i].setDisabled(i > 1);
		}
 ...
 * @时间 2012-8-31
 */
querying.getStore = function(storeId,model,fields,data) {
	var store = null;
...
//Ajax请求--文件表单的请求
querying.requestFileUploadAjax = function(url,form,successFn,failFn)
{
	Ext.Ajax.request({
		url:url,
		form:form,
...
//Ajax请求--非json字符串
querying.requestAjax = function(url,params,successFn,failFn)
{
	Ext.Ajax.request({
		url:url,
		params:params,
 ...
 * @时间 2012-3-15
 */
querying.setReadOnly = function(readOnlyIdList){
 ...
 * @时间 2012-3-15
 */
querying.setHiddenAndDestroy = function(hiddenIdList){
 ...
 * @时间 2012-3-15
 */
querying.setHidden = function(hiddenIdList){
     for(var i=0;i<hiddenIdList.length;i++){
    	 Ext.getCmp(hiddenIdList[i]).hide();
     }
};
 ...
 * @时间 2012-3-21
 */
querying.setDestroy = function(destoryIdList){
 ...
 * @时间 2012-3-21
 */
querying.setDisabled =function(disabledIdList){
 ...
 * @时间 2012-3-22
 */
querying.clearListeners =function(clearIdList){
 ...
 * @时间 2012-3-24
 */
querying.isHaveEmpty = function(array){
	var boolen = false;
 ...
 * @时间 2012-4-16
 */
querying.changeModelListToDataList = function(modelList){
 ...
 * @时间 2012-4-16
 */
querying.copyModelListToDataList = function(modelList){
	var dataList = new Array();
	for(var i = 0;i<modelList.length;i++){
...
//警告
querying.showWoringMessage = function(message,fun) {
	var len = message.length;
	Ext.Msg.show({
	    title:'FOSS提醒您:',
	    msg:message,
...
//是和否选择
querying.showQuestionMes = function(message,fun) {
	var len = message.length;
	Ext.Msg.show({
	    title:'FOSS提醒您:',
	    width:110+len*15,
...
//信息
querying.showInfoMes = function(message,fun) {
	var len = message.length;
	Ext.Msg.show({
	    title:querying.i18n('i18n.baseinfo-util.fossAlert'),
	    width:110+len*15,
	    msg:'<div id="message">'+message+'</div>',
	    buttons: Ext.Msg.OK,
	    icon: Ext.MessageBox.INFO,
...
//错误
querying.showErrorMes = function(message,fun) {
	var len = message.length;
	Ext.Msg.show({
	    title:querying.i18n('i18n.baseinfo-util.fossAlert'),
	    width:110+len*15,
...
//ADD -ALL
querying.addAll = function(list,all) {
...
querying.changeCodeToName = function(list,code) {
	var name = '';
	for(var i = 0;i<list.length;i++){
...
querying.changeCodeToNameStore = function(store,code) {
	var name = '';
	if(!Ext.isEmpty(store)){

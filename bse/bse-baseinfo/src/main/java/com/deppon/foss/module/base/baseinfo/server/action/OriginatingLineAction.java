 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/base/baseinfo/server/action/OriginatingLineAction.java
 * 
 * FILE NAME        	: OriginatingLineAction.java
 * 
 * AUTHOR			: FOSS综合管理开发组
 * 
 ...
 * @version
 */
public class OriginatingLineAction extends AbstractAction {
     ...
     */
    @JSON
    public String searchCurrentUserOrgResource(){
    	try {
...
//	    lineService.deleteLines(lineVo.getEntity().getIds(), userCode);
	...
	try {
	    ...
	    downloadFileName =new String(ColumnConstants.EXPROT_ORIGINATING_LINE_NAME.getBytes("UTF-8"),"iso-8859-1");
		//获取查询参数
	    LineEntity entity = lineVo.getEntity();
	    if(null == entity){
	    ...
	    //设置名称
	    exportSetting.setSheetName(ColumnConstants.EXPROT_ORIGINATING_LINE_NAME);
	    //设置下载最大条数

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/base/baseinfo/server/cache/PlatformEntityCacheProvider.java
 * 
 * FILE NAME        	: PlatformEntityCacheProvider.java
 * 
 * AUTHOR			: FOSS综合管理开发组
 * 
 ...
 * @version 1.0
 */
public class PlatformEntityCacheProvider implements ITTLCacheProvider<PlatformEntity> {
	...
	if (key.contains(SymbolConstants.EN_COLON)) {
	    ...
	    return platformDao.queryPlatformByCode(keys[0], keys[1]);
	} else { // 没有冒号的，说明是通过虚拟编码查询
	    return platformDao.queryPlatformByVirtualCode(key);
	}
    }

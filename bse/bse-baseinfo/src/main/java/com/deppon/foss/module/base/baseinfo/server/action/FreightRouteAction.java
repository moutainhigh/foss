 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/base/baseinfo/server/action/FreightRouteAction.java
 * 
 * FILE NAME        	: FreightRouteAction.java
 * 
 * AUTHOR			: FOSS综合管理开发组
 * 
...
import com.deppon.foss.module.base.baseinfo.api.shared.vo.FreightRouteVo;
 ...
 * </p>
 * <p style="display:none">
 * version:V1.0,author:094463-foss-xieyantao,date:2012-11-8 下午1:40:36
 * </p>
 * 
 * @author 094463-foss-xieyantao
 * @date 2012-11-8 下午1:40:36
 * @since
 * @version
 */
public class FreightRouteAction extends AbstractAction {
    
    /**
     * 日志.
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(FreightRouteAction.class);

    /**
     * serialVersionUID.
     */
    private static final long serialVersionUID = 1672976285303092132L;
     ...
     * 前台注入参数.
     */
    private FreightRouteVo freightRouteVo = new FreightRouteVo();
    ...
    public String queryCurrentUserOrg(){
    	try {
    		...
    		freightRouteVo.setUserOrgList(userOrgList);
     ...
     * @return 
     * @author 094463-foss-xieyantao
     * @date 2012-11-8 下午1:47:43
     * @see
     */
    @JSON
    public String addFreightRoute() {
	try {
	    UserEntity user = FossUserContext.getCurrentUser();// 获取当前登录用户
	    String userCode = user.getEmployee().getEmpCode();// 当前登录用户empcode
	    // 获取Form信息
	    FreightRouteEntity entity = freightRouteVo.getEntity();
	    entity.setCreateUser(userCode);
	    entity.setModifyUser(userCode);
	    if (StringUtil.equals("true", entity.getDefaultRoute())) {
		entity.setDefaultRoute(FossConstants.YES);
	    } else {
		entity.setDefaultRoute(FossConstants.NO);
	    }

	    // 保存成功返回一个对象
	    entity = freightRouteService.addFreightRoute(entity);
	    freightRouteVo.setEntity(entity);
	...
	try {
	    ...
	    freightRouteService.deleteFreightRouteList(freightRouteVo.getIds(), userCode);
     ...
     * @return 
     * @author 094463-foss-xieyantao
     * @date 2012-11-8 下午1:49:20
     * @see
     */
    @JSON
    public String updateFreightRoute() {
	try {
	    UserEntity user = FossUserContext.getCurrentUser();// 获取当前登录用户
	    String userCode = user.getEmployee().getEmpCode();// 当前登录用户empcode
	    // 获取Form信息
	    FreightRouteEntity entity = getFreightRouteVo().getEntity();
	    entity.setModifyUser(userCode);
	    if (StringUtil.equals("true", entity.getDefaultRoute())) {
		entity.setDefaultRoute(FossConstants.YES);
	    } else {
	    ...
	    //更新走货路径  更新成功返回一个对象
	    entity = freightRouteService.updateFreightRoute(entity);
	    getFreightRouteVo().setEntity(entity);
     ...
     */
    @JSON
    public String queryFreightRoute() {
	try {

	    List<FreightRouteEntity> freightRouteList = freightRouteService
		    .queryFreightRouteListByCondition(
			    freightRouteVo.getEntity(), this.getStart(),
			    this.getLimit());
	    freightRouteVo.setFreightRouteEntities(freightRouteList);

	    // 查询总记录数
	    Long totalCount = freightRouteService
		    .countFreightRouteListByCondition(freightRouteVo
			    .getEntity());
	    // 设置totalCount
    ...
    /**
     * <p>
     * 根据走货路径虚拟编码查询走货路径、走货路径线路、网点组信息
     * </p>.
     *
     * @return 
     * @author 094463-foss-xieyantao
     * @date 2012-12-13 下午5:52:40
     * @see
     */
    @JSON
    public String queryFreightRouteByVirtualCode() {
	try {
	    // 走货路径虚拟编码
	    String virtualCode = freightRouteVo.getFreightRouteVirtualCode();
	    // 走货路径信息
	    FreightRouteEntity entity = freightRouteService
		    .queryFreightRouteByVirtualCode(virtualCode);
	    // 网点组列表
	    List<NetGroupDto> netGroupDtos = netGroupService
		    .queryNetGroupListByFreightRoute(virtualCode);
	    // 走货路径线路信息列表
	    List<FreightRouteLineEntity> lineList = freightRouteLineService
		    .queryFreightRouteLineListByFreightRoute(virtualCode);
	    // 根据当前走货路径找出可以打木架的外场列表
	    Map<String, String> map = freightRouteService.queryPackingOrgs(virtualCode);
	    //设置 出发/到达站编码、名称集合
	    
	    freightRouteVo.setLineDeptList(getLineDeptDtos(map));
	    
	    freightRouteVo.setEntity(entity);
	    freightRouteVo.setNetGroupDtos(convertObjects(netGroupDtos));
	    freightRouteVo.setLineList(lineList);
    ...
    /**
     * <p>
     * 根据始发外场CODE查询对应的始发营业部
     * </p>.
     *
     * @return 
     * @author 094463-foss-xieyantao
     * @date 2012-12-17 下午2:27:42
     * @see
     */
    @JSON
    public String querySourceOrg() {
	try {
		List<NetGroupSiteDto> groupSiteList;
		//判断是否快递产品
		if(LineUtils.isExpOrRcpProduct(freightRouteVo.getProductCode())){
			groupSiteList = expresslineService.querySourceLineListByTransferCode(freightRouteVo
					    .getOrgCode(),freightRouteVo.getProductCode());
		}else{
			groupSiteList = lineService
				    .querySourceLineListByTransferCode(freightRouteVo
					    .getOrgCode(),freightRouteVo.getProductCode());
		}
	    freightRouteVo.setGroupSiteList(groupSiteList);
	    return returnSuccess();
     ...
     */
    @JSON
    public String queryTargetOrg() {
	try {
		List<NetGroupSiteDto> groupSiteList;
		//判断是否快递产品
	    if(LineUtils.isExpOrRcpProduct(freightRouteVo.getProductCode())){
	    	groupSiteList =expresslineService.queryTargetLineListByTransferCode(freightRouteVo
			    .getOrgCode());
	    }else{
	    	groupSiteList = lineService
	    		    .queryTargetLineListByTransferCode(freightRouteVo
	    			    .getOrgCode());
	    }
	    freightRouteVo.setGroupSiteList(groupSiteList);
	    return returnSuccess();
    ...
    /**
     * <p>
     * 通过中转线路虚拟编码查出该线路途径的外场列表
     * </p>.
     *
     * @return 
     * @author 094463-foss-xieyantao
     * @date 2012-12-18 上午9:15:37
     * @see
     */
    @JSON
    public String queryLineStations() {
	try {
	    LineStationsDto dto = lineService
		    .queryLineStations(freightRouteVo.getLineVirtualCode());
	    if(null != dto){
		//线路简码
		freightRouteVo.setLineSimpleCode(dto.getLineSimpleCode());
		//出发/到达站编码、名称集合
		freightRouteVo.setLineDeptList(getLineDeptDtos(dto.getStationMap()));
	    }
	    return returnSuccess();
	} catch (LineException e) {
	    LOGGER.debug(e.getMessage(), e);
    ...
    /**
     * <p>
     * 根据走货路径虚拟编码查询可以打木架的外场
     * </p>.
     *
     * @return 
     * @author 094463-foss-xieyantao
     * @date 2012-12-18 下午3:35:18
     * @see
     */
    @JSON
    public String queryPackingOrgs() {
	try {
	    Map<String, String> map = freightRouteService
		    .queryPackingOrgs(freightRouteVo
			    .getFreightRouteVirtualCode());
	    freightRouteVo.setLineDeptList(getLineDeptDtos(map));
	    return returnSuccess();
	} catch (FreightRouteException e) {
	    LOGGER.debug(e.getMessage(), e);
     ...
     */
    @JSON
    public String queryLineAging() {
	try {
	    //计算在一条线路上指定起点和终点的半截线路的卡车时效和普车时效
	    Long aging = lineService.calculateAging(freightRouteVo.getLineVirtualCode(), freightRouteVo.getSourceOrgCode(), freightRouteVo.getOrgCode(), freightRouteVo.getProductCode());
	    if(null != aging){
		//千分之小时转换成小时
		freightRouteVo.setAging(aging);
	    }else{
		freightRouteVo.setAging(0L);
	    }
	    return returnSuccess();
	} catch (LineException e) {
	    LOGGER.debug(e.getMessage(), e);
	...
	try {
	    ...
	    FreightRouteEntity entity = freightRouteService.valid(freightRouteVo.getFreightRouteVirtualCode(),userCode);
	    freightRouteVo.setEntity(entity);
	    return returnSuccess(MessageType.SAVE_SUCCESS);
	} catch (FreightRouteException e) {
	    LOGGER.debug(e.getMessage(), e);
     ...
     */
    @JSON
    public String updateInvalid(){
	try {
	    UserEntity user = FossUserContext.getCurrentUser();// 获取当前登录用户
	    String userCode = user.getEmployee().getEmpCode();// 当前登录用户empcode
	    
	    //使走货路径失效
	    FreightRouteEntity entity = freightRouteService.invalid(freightRouteVo.getFreightRouteVirtualCode(),userCode);
	    freightRouteVo.setEntity(entity);
	    return returnSuccess(MessageType.SAVE_SUCCESS);
	} catch (FreightRouteException e) {
	    LOGGER.debug(e.getMessage(), e);
     ...
     * @return 
     * @author 094463-foss-xieyantao
     * @date 2012-12-18 下午4:39:32
     * @see
     */
    private List<LineDeptDto> getLineDeptDtos(Map<String, String> map) {
	if (null != map) {
	    Set<String> codeSet = map.keySet();
	    List<LineDeptDto> list = new ArrayList<LineDeptDto>();
	    for (String code : codeSet) {
		LineDeptDto dto = new LineDeptDto();
		dto.setCode(code);
		dto.setName(map.get(code));
		list.add(dto);
	    }
	    return list;
	}else {
     ...
     * @return 
     * @author 094463-foss-xieyantao
     * @date 2012-12-18 下午11:56:49
     * @see
     */
    private NetWorkGroupDto convertObject(NetGroupDto dto){
	if(null != dto){
	    NetWorkGroupDto groupDto = new NetWorkGroupDto();
	    groupDto.setFreightRouteVirtualCode(dto
		    .getFreightRouteVirtualCode());
	    groupDto.setNetGroupName(dto.getNetGroupName());
	    groupDto.setSourceOrganizationCodeList(dto
		    .getSourceOrganizationCodeList());
	    groupDto.setSourceOrganizationNameList(dto
		    .getSourceOrganizationNameList());
	    groupDto.setSourceOrgName(dto.getSourceOrganizationName());
	    groupDto.setSourceOrgs(dto.getSourceOrgs());
	    groupDto.setTargetOrganizationCodeList(dto
		    .getTargetOrganizationCodeList());
	    groupDto.setTargetOrganizationNameList(dto
		    .getTargetOrganizationNameList());
	    groupDto.setTargetOrgName(dto.getTargetOrganizationName());
	    groupDto.setTargetOrgs(dto.getTargetOrgs());
     ...
     * @see
     */
    private List<NetWorkGroupDto> convertObjects(List<NetGroupDto> list){
	if(CollectionUtils.isNotEmpty(list)){
	    List<NetWorkGroupDto> dtoList = new ArrayList<NetWorkGroupDto>();
	    for(NetGroupDto dto : list){
		dtoList.add(convertObject(dto));
	    }
	    return dtoList;
	}else {
	    return null;
     ...
     * 设置 走货路径服务类.
     *
     * @param freightRouteService the new 走货路径服务类
     */
     ...
     * @return the 前台注入参数
     */
    public FreightRouteVo getFreightRouteVo() {
    	if(null == freightRouteVo){
    		freightRouteVo = new FreightRouteVo();
    	}
	return freightRouteVo;
    }

    /**
     * 设置 前台注入参数.
     *
     * @param freightRouteVo the new 前台注入参数
     */
    public void setFreightRouteVo(FreightRouteVo freightRouteVo) {
	this.freightRouteVo = freightRouteVo;
    }

    /**
     * 设置 走货路径线路服务接口.
     *
     * @param freightRouteLineService the new 走货路径线路服务接口
     */
     ...
     * 设置 网点组服务接口.
     *
     * @param netGroupService the new 网点组服务接口
     */
     ...
     * 设置 线路服务接口.
     *
     * @param lineService the new 线路服务接口
     */
     ...
     * @date 2013-1-18 下午11:56:49
     * @see
     */
    @JSON
    public String queryFreightRouteBySourceTarget() {
    	try {
    		List<FreightRouteLineDto> freightRouteLineDtoList =  freightRouteService.queryEnhanceFreightRouteBySourceTarget(freightRouteVo.getSourceCode(),freightRouteVo.getTargetCode(),freightRouteVo.getProductCode(),freightRouteVo.getTime());
        	freightRouteVo.setFreightRouteLineDtoList(freightRouteLineDtoList);
    	    return returnSuccess();
    	} catch (BusinessException e) {
    	    LOGGER.debug(e.getMessage(), e);
    	    return returnError(e);

 ...
 * 
 * FILE PATH        	: src/main/java/com/deppon/foss/module/base/baseinfo/server/action/FreightRouteLineAction.java
 * 
 * FILE NAME        	: FreightRouteLineAction.java
 * 
 * AUTHOR			: FOSS综合管理开发组
 * 
...
import com.deppon.foss.module.base.baseinfo.api.shared.vo.FreightRouteLineVo;
 ...
 * </p>
 * <p style="display:none">
 * version:V1.0,author:094463-foss-xieyantao,date:2012-12-5 下午4:31:44,content:
 * </p>
 * 
 * @author 094463-foss-xieyantao
 * @date 2012-12-5 下午4:31:44
 * @since
 * @version
 */
public class FreightRouteLineAction extends AbstractAction {
    
    /**
     * 日志.
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(FreightRouteLineAction.class);

    /**
     * serialVersionUID.
     */
    private static final long serialVersionUID = -3514770362714275018L;
     ...
     * 走货路径线路信息VO.
     */
    private FreightRouteLineVo freightRouteLineVo = new FreightRouteLineVo();

    /**
     * 获取 走货路径线路信息VO.
     *
     * @return the 走货路径线路信息VO
     */
    public FreightRouteLineVo getFreightRouteLineVo() {
	return freightRouteLineVo;
    }
     ...
     * 设置 走货路径线路信息VO.
     *
     * @param freightRouteLineVo the new 走货路径线路信息VO
     */
    public void setFreightRouteLineVo(FreightRouteLineVo freightRouteLineVo) {
	this.freightRouteLineVo = freightRouteLineVo;
    }
     ...
     */
    @JSON
    public String addFreightRouteLine() {
	try {
	    UserEntity user = FossUserContext.getCurrentUser();// 获取当前登录用户
	    String userCode = user.getEmployee().getEmpCode();// 当前登录用户empcode
	    // 获取Form信息
	    FreightRouteLineEntity entity = freightRouteLineVo.getEntity();
	    entity.setCreateUser(userCode);
	    entity.setModifyUser(userCode);

	    // 保存成功返回一个对象
	    entity = freightRouteLineService.addFreightRouteLine(entity);

	    if (null != entity) {
		// 根据当前走货路径找出可以打木架的外场列表
		Map<String, String> map = freightRouteService
			.queryPackingOrgs(entity.getFreightRouteVirtualCode());
		freightRouteLineVo.setLineDeptList(getLineDeptDtos(map));
	    }

	    freightRouteLineVo.setEntity(entity);
     ...
     */
    @JSON
    public String deleteFreightRouteLine() {
	try {
	    UserEntity user = FossUserContext.getCurrentUser();// 获取当前登录用户
	    String userCode = user.getEmployee().getEmpCode();// 当前登录用户empcode
	    FreightRouteLineEntity entity = freightRouteLineVo.getEntity();
	    if (null != entity) {
		entity.setModifyUser(userCode);
		freightRouteLineService.deleteFreightRouteLine(entity);
     ...
     */
    @JSON
    public String updateFreightRouteLine() {
	...
	try {
	    ...
	    entity.setModifyUser(userCode);
	    // 更新成功返回一个对象
	    entity = freightRouteLineService.updateFreightRouteLine(entity);
     ...
     */
    @JSON
    public String queryFreightRouteLine() {
	try {
	    // 获得走货路径的虚拟编码
	    String virtualCode = freightRouteLineVo
	    ...
	    freightRouteLineVo.setEntityList(lineList);

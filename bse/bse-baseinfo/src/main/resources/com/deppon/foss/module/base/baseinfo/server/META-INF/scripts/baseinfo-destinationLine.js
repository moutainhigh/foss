baseinfo.destinationLine.destinationOrg =null;
baseinfo.destinationLine.destinationOrgName =null;
baseinfo.destinationLine.orginalOrg =null;
baseinfo.destinationLine.orginalOrgName =null;
baseinfo.destinationLine.orgList =new Array();
/**
 * 获取数据权限
 */
baseinfo.destinationLine.init = function(){
	Ext.Ajax.request({
		url:baseinfo.realPath('queryCurrentUserOrgResource.action'),
		async:false,
		success:function(response){
			...
			if(!Ext.isEmpty(lineOrgDto)){
				//到达站
				baseinfo.destinationLine.destinationOrg =lineOrgDto.salesOrgs;
				baseinfo.destinationLine.destinationOrgName =lineOrgDto.salesOrgsName;
				//出发站
				baseinfo.destinationLine.orginalOrg =lineOrgDto.transferOrg;
				baseinfo.destinationLine.orginalOrgName  =lineOrgDto.transferOrgName;
				//数据权限
				baseinfo.destinationLine.orgList =lineOrgDto.orgList;
			}
		},
...
baseinfo.destinationLine.initForm =function(queryForm){
	//到达站
	...
	//到达部门常量不为空
	if(!Ext.isEmpty(baseinfo.destinationLine.destinationOrg)){
		a_destinationOrg.setCombValue(baseinfo.destinationLine.destinationOrgName,baseinfo.destinationLine.destinationOrg);
	}
	//出发部门常量不为空
	if(!Ext.isEmpty(baseinfo.destinationLine.orginalOrg)){
		a_orginalOrg.setCombValue(baseinfo.destinationLine.orginalOrgName,baseinfo.destinationLine.orginalOrg);
	}
}
/**
 * 到达线路
 ...
 */
//定义一个model
Ext.define('Foss.baseinfo.destinationLine.DestinationLineModel', {
...
//发车标准实体
Ext.define('Foss.baseinfo.destinationLine.StartStandardModel', {
    extend: 'Ext.data.Model',
    fields : [{
        name : 'id',
        type : 'string'
...
//创建一个到达线路的store
Ext.define('Foss.baseinfo.destinationLine.DestinationLineStore',{
	extend:'Ext.data.Store',
//	autoLoad: true,
	//页面条数定义
    pageSize: 10,
	//绑定model
    model: 'Foss.baseinfo.destinationLine.DestinationLineModel',
    proxy: {
        //以JSON的方式加载数据
        type : 'ajax',
        actionMethods:'POST',
        url: baseinfo.realPath('queryDestinationLine.action'),
		reader : {
			type : 'json',
	...
	listeners: {
		beforeload : function(store, operation, eOpts) {
			var queryForm = Ext.getCmp('Foss_baseinfo_destinationLine_QueryForm_Id').getForm();
			if (queryForm != null) {
...
//创建一个发车标准的store
Ext.define('Foss.baseinfo.destinationLine.StartStandardStore',{
	extend:'Ext.data.Store',
	//绑定model
	model: 'Foss.baseinfo.destinationLine.StartStandardModel',
    //构造函数
    constructor: function(config){
		var me = this,
			cfg = Ext.apply({}, config);
...
Ext.define('Foss.baseinfo.destinationLine.TransTypeStore',{
	extend: 'Ext.data.Store',
	fields: [
 ...
 * 查询表单
 */
Ext.define('Foss.baseinfo.destinationLine.QueryForm', {
	extend : 'Ext.form.Panel',
	title: baseinfo.destinationLine.i18n('foss.baseinfo.queryCondition'),//查询条件
	id : 'Foss_baseinfo_destinationLine_QueryForm_Id',
	frame: true,
	collapsible: true,
	layout:{
		type:'table',
	...
	items :[ {
		name : 'lineName',
		fieldLabel : baseinfo.destinationLine.i18n('foss.baseinfo.lineName'),
		columnWidth : .25
	}, {
		name : 'orginalOrganizationCode',
		fieldLabel : baseinfo.destinationLine.i18n('foss.baseinfo.orginalOrganizationCode'),
		columnWidth : .25,
        ...
        listeners:{
        	beforequery:function(queryEvent,eOpts){
        		var a_originalOrg =	Ext.getCmp('T_baseinfo-destinationLine_content').getQueryForm().getForm().findField('orginalOrganizationCode');
        		if(!Ext.isEmpty(baseinfo.destinationLine.orginalOrg)){
            		...
            		a_originalOrg.store.addListener('beforeload',function(store, operation, eOpts){
    					...
    					searchParams['commonOrgVo.deptCode']=baseinfo.destinationLine.orginalOrg;
    				});
		...
		fieldLabel : baseinfo.destinationLine.i18n('foss.baseinfo.destinationOrganizationCode'),
		columnWidth : .25,
		xtype : 'dynamicorgcombselector',
	    salesDepartment:'Y',
	    types : 'CPPX,LD,CPLD',
	    listeners:{
        	beforequery:function(queryEvent,eOpts){
        		var a_destinationOrg =	Ext.getCmp('T_baseinfo-destinationLine_content').getQueryForm().getForm().findField('destinationOrganizationCode');
        		if(!Ext.isEmpty(baseinfo.destinationLine.destinationOrg)){
        			...
        			a_destinationOrg.store.addListener('beforeload',function(store, operation, eOpts){
    					...
    					searchParams['commonOrgVo.deptCode']=baseinfo.destinationLine.destinationOrg;
    				});
		...
		name : 'simpleCode',
		fieldLabel : baseinfo.destinationLine.i18n('foss.baseinfo.simpleCode'),
		columnWidth : .25
	}, {
		name : 'organizationCode',
		fieldLabel : baseinfo.destinationLine.i18n('foss.baseinfo.organizationCode'),
		columnWidth : .25,
		maxLength:200,
        xtype : 'dynamicorgcombselector',
        type : 'ORG',
	...
	constructor : function(config) {
		...
		me.fbar = [{
			xtype : 'button', 
			width:70,
			text : baseinfo.destinationLine.i18n('foss.baseinfo.reset'),//重置
			disabled:!baseinfo.destinationLine.isPermission('destinationLine/destinationLineQueryButton'),
			...
			xtype : 'button', 
			width:70,
			text : baseinfo.destinationLine.i18n('foss.baseinfo.query'),//查询
			disabled:!baseinfo.destinationLine.isPermission('destinationLine/destinationLineQueryButton'),
			hidden:!baseinfo.destinationLine.isPermission('destinationLine/destinationLineQueryButton'),
			cls:'yellow_button',
			handler : function() {
				if(me.getForm().isValid()){
					//数据权限有值
					if(!Ext.isEmpty(baseinfo.destinationLine.orgList)){
						if(baseinfo.destinationLine.orgList.length >0){
							var flag =false;
							...
							var orgList =baseinfo.destinationLine.orgList;
					...
					Ext.getCmp('T_baseinfo-destinationLine_content').getDestinationLineGrid().getPagingToolbar().moveFirst()
				}
			}
		}]
		me.callParent([cfg]);
...
// 到达线路详细信息表单
Ext.define('Foss.baseinfo.destinationLine.DetailForm', {
	extend : 'Ext.form.Panel',
	frame: true,
	collapsible: true,
	defaults : {
	...
	items : [ {
			...
			name: 'distance',
	        fieldLabel: baseinfo.destinationLine.i18n('foss.baseinfo.distance'),//'线路距离(公里)'
	        allowBlank:false,
	        maxLength:200,
	        xtype : 'textfield'
		},{
			name: 'isDefaults',
	        fieldLabel: baseinfo.destinationLine.i18n('foss.baseinfo.isEndDefaults'),
	        allowBlank:true,
	        xtype : 'radiogroup',
	        layout:'column',
			defaultType: 'radio',
			...
			items: [{ 
					boxLabel  : baseinfo.destinationLine.i18n('foss.baseinfo.yes'), 
					columnWidth:.5,
					name      : 'isDefault',
					inputValue: 'Y'
				}, { 
					boxLabel  : baseinfo.destinationLine.i18n('foss.baseinfo.no'), 
					columnWidth:.5,
					name      : 'isDefault',
					inputValue: 'N',
					checked   : true
			...
			name: 'notes',//描述
	        fieldLabel: baseinfo.destinationLine.i18n('foss.baseinfo.notes'),
	        colspan : 2,
	        maxLength:1000,
	        width:450,
	        xtype : 'textareafield'
...
// 新增/修改到达线路表单
Ext.define('Foss.baseinfo.destinationLine.AddUpdateForm', {
	extend : 'Ext.form.Panel',
	frame: true,
	height : 330,
	collapsible: true,
    ...
    commitInfo : function(){
    	...
    	if(basicForm.isValid()){//校验form是否通过校验
			...
			if(me.isUpdate){
				...
				record = Ext.create('Foss.baseinfo.destinationLine.DestinationLineModel');
			}
			//将FORM中数据设置到MODEL里面
			...
			var url = null;
			if(me.isUpdate){
				url = baseinfo.realPath('updateDestinationLine.action');//请求到达线路修改
			}else{
				url = baseinfo.realPath('addDestinationLine.action');//请求到达线路新增
			}
			var infoGrid = Ext.getCmp('T_baseinfo-destinationLine_content').getDestinationLineGrid(); 
            Ext.Ajax.request({
				url:url,
				jsonData:jsonData,
				//成功
				success : function(response) {
					  ...
					  //打印成功消息
					  infoGrid.showWarningMsg(baseinfo.destinationLine.i18n('foss.baseinfo.notice'),json.message);
					  
					  if(Ext.isEmpty(json.lineVo.entity)){
						infoGrid.showWarningMsg(baseinfo.destinationLine.i18n('foss.baseinfo.notice'),baseinfo.destinationLine.i18n('foss.baseinfo.serverException'));//服务端有异常！
						return;
					  }
					  //将返回的值设置到window中
	...
	items : [ {
			...
			regexText:baseinfo.destinationLine.i18n('foss.baseinfo.regexTextMsg'),
	        xtype : 'textfield'
		},{
			name: 'organizationCode',
	        fieldLabel: baseinfo.destinationLine.i18n('foss.baseinfo.organizationCode'),
	        ...
	        listeners : {
	        	select:function(combo,records,eopts){
	        		//出发站名称
	        		var orginalOrganizationCode = combo.getRawValue();
	        		//设置出发部门名称
...
//	        leave : null,//可出发
			...
			name: 'distance',
	        fieldLabel: baseinfo.destinationLine.i18n('foss.baseinfo.distance'),
	        decimalPrecision:0,
	        step:1,
	        allowBlank:false,
	        maxValue:999999,
		...
		constructor : function(config) {
			var me = this, 
				cfg = Ext.apply({}, config);
			me.fbar = [{
				text :baseinfo.destinationLine.i18n('foss.baseinfo.cancel'),//取消
				handler :function(){
					me.up().close();
				}
			},{
				...
				handler :function(){
					if(me.isUpdate){//如果是修改，加载上一次修改的
						me.loadRecord(new Foss.baseinfo.destinationLine.DestinationLineModel(me.up('window').infoModel));
					}else{//如果是新增，直接reset
						me.getForm().reset();//表格重置
					}
				} 
			},{
				text : baseinfo.destinationLine.i18n('foss.baseinfo.save'),//保存
				cls:'yellow_button',
				handler :function(){
					me.commitInfo();
				} 
 ...
 * 发车标准-FORM
 */
Ext.define('Foss.baseinfo.destinationLine.StandardForm', {
	extend : 'Ext.form.Panel',
	frame: true,
	height:370,
	collapsible: true,
    ...
    commitInfo:function(){
    	...
    	if(basicForm.isValid()){//校验form是否通过校验
    	    //创建发车标准MODEL
    		var startStandardModel = Ext.create('Foss.baseinfo.destinationLine.StartStandardModel');
    		...
    		var infoGrid = Ext.getCmp('T_baseinfo-destinationLine_content').getDestinationLineGrid();
    		if(arriveT < leaveT){
    			infoGrid.showWarningMsg(baseinfo.destinationLine.i18n('foss.baseinfo.notice'),baseinfo.destinationLine.i18n('foss.baseinfo.arriveTimeNotice'));
    			return ;
    		}
    		me.getDockedItems()[1].items.items[2].setDisabled(true);//保存按钮不可用
	  		me.getDockedItems()[1].items.items[1].setDisabled(true);//重置按钮不可用
    		...
    		Ext.Ajax.request({
				...
				success : function(response) {
					  ...
					  //加到父元素grid的store中				  
					  me.up('window').parent.getStore().add(new Foss.baseinfo.destinationLine.StartStandardModel(startStandardModel));
		        ...
		        exception : function(response) {
		        	...
		        	me.getDockedItems()[1].items.items[1].setDisabled(false);//重置按钮可用
		              var json = Ext.decode(response.responseText);
		              if(Ext.isEmpty(json)){
		              	infoGrid.showWarningMsg(baseinfo.destinationLine.i18n('foss.baseinfo.notice'),baseinfo.destinationLine.i18n('foss.baseinfo.requestOvertime'));
		              }else{
		              	//失败消息
		              	infoGrid.showWarningMsg(baseinfo.destinationLine.i18n('foss.baseinfo.notice'),json.message);
		              }
    ...
    items : [{
			name: 'order',//班次
	        fieldLabel: baseinfo.destinationLine.i18n('foss.baseinfo.order'),
	        step:1,
	        allowBlank:false,
	        maxValue:999999,
	        minValue:1,
			...
			name: 'orginalOrganizationName',
			readOnly:true,
	        fieldLabel: baseinfo.destinationLine.i18n('foss.baseinfo.orginalOrganizationCode'),//只读不可编辑，数据从线路中获取
	        xtype : 'textfield'
		},{
			xtype: 'timefield',
			labelWidth:100,
		    fieldLabel: baseinfo.destinationLine.i18n('foss.baseinfo.leaveTime'),//准点出发时间
            name:'leaveTime',
            format:'H:i',//24小时制
            allowBlank:false
		},{
		    ...
		    items:[{
				...
				labelWidth:100,
				width:255,
		        fieldLabel: baseinfo.destinationLine.i18n('foss.baseinfo.arriveTime'),
		        format:'H:i',//24小时制
		        xtype : 'timefield'
		    },{
		    	name: 'arriveDay',
		        ...
		        xtype : 'label',
		        text:baseinfo.destinationLine.i18n('foss.baseinfo.day')//天
		    }]
		},{
			name: 'notes',//描述
	        fieldLabel: baseinfo.destinationLine.i18n('foss.baseinfo.notes'),
...
// 定义一个新增的窗口
Ext.define('Foss.baseinfo.destinationLine.AddWindow', {
	extend : 'Ext.window.Window',
//	id:'Foss_baseinfo_destinationLine_AddWindow_Id',
	title : baseinfo.destinationLine.i18n('foss.baseinfo.addDestinationLine'),
	width : 800,
	height : 700,
	isUpdate:false,//是否为修改，默认false
	parent : null, //父元素
	...
	getAddUpdateForm : function() {
		if (this.addUpdateForm == null) {
			this.addUpdateForm = Ext.create('Foss.baseinfo.destinationLine.AddUpdateForm');
	...
	getStartStandardGrid : function(){
		if(Ext.isEmpty(this.startStandardGrid)){
			this.startStandardGrid = Ext.create('Foss.baseinfo.destinationLine.StartStandardGrid');
	...
	getInfoGrid : function(){
		if(Ext.isEmpty(this.infoGrid)){
			this.infoGrid = Ext.getCmp('T_baseinfo-destinationLine_content').getDestinationLineGrid();
		}
	},
...
// 定义一个修改的窗口
Ext.define('Foss.baseinfo.destinationLine.UpdateWindow', {
	extend : 'Ext.window.Window',
//	id:'Foss_baseinfo_destinationLine_UpdateWindow_Id',
	title : baseinfo.destinationLine.i18n('foss.baseinfo.updateDestinationLine'),
	width : 800,
	height : 700,
...
//			me.getStartStandardGrid().getDockedItems()[1].items.items[0].setDisabled(true);//新增按钮不可用
		...
		beforeshow:function(me){//窗口显示之前事件
		    me.getAddUpdateForm().getForm().loadRecord(new Foss.baseinfo.destinationLine.DestinationLineModel(me.infoModel));//加载线路信息
		    //选择器赋值
...
// 定义一个表格列表
Ext.define('Foss.baseinfo.destinationLine.DestinationLineGrid',{
	extend : 'Ext.grid.Panel',
	// 增加表格列的分割线
	columnLines : true,
	id : 'Foss_baseinfo_destinationLine_DestinationLineGrid_Id',
	bodyCls : 'autoHeight',
	cls : 'autoHeight',
	// 表格对象增加一个边框
	frame : true,
	...
	columnLines : true,
	// 定义表格的标题
	title : baseinfo.destinationLine.i18n('foss.baseinfo.queryGrid'),
	collapsible : true,
	animCollapse : true,
	...
	plugins : [ {
		...
		rowsExpander : false,
		// 行体内容
		rowBodyElement : 'Foss.baseinfo.destinationLine.InfoPanel'
	} ],
	...
	getAddWindow : function() {
		if (Ext.isEmpty(this.addWindow)) {
			this.addWindow = Ext.create('Foss.baseinfo.destinationLine.AddWindow');
	...
	getUpdateWindow : function() {
		if (Ext.isEmpty(this.updateWindow)) {
			this.updateWindow = Ext.create('Foss.baseinfo.destinationLine.UpdateWindow');
	...
	deleteInfos : function(){
		...
		if(selections.length<1){//判断是否至少选中了一条
			me.showWarningMsg(baseinfo.destinationLine.i18n('foss.baseinfo.warnMsg'),baseinfo.destinationLine.i18n('foss.baseinfo.deleteNoticeMsg'));//请选择一条进行作废操作！
			return;//没有则提示并返回
		}
		
		Ext.Msg.confirm(baseinfo.destinationLine.i18n('foss.baseinfo.notice'),baseinfo.destinationLine.i18n('foss.baseinfo.deleteWarnMsg'),function(e){
			Ext.MessageBox.buttonText.yes = baseinfo.destinationLine.i18n('foss.baseinfo.confirm');
			Ext.MessageBox.buttonText.no = baseinfo.destinationLine.i18n('foss.baseinfo.cancel');
			if(e == 'yes'){//询问是否删除
				...
				var url = baseinfo.realPath('deleteDestinationLine.action');
	...
	ajaxRequest : function(url,jsonData){
		...
		Ext.Ajax.request({
			...
			success : function(response) {
                  ...
                  //打印成功消息
                  me.showWarningMsg(baseinfo.destinationLine.i18n('foss.baseinfo.notice'),json.message);
                },
            //保存失败
            exception : function(response) {
                  var json = Ext.decode(response.responseText);
	...
	columns : [{
				xtype : 'actioncolumn',
				width : 80,
				text : baseinfo.destinationLine.i18n('foss.baseinfo.operate'),
				align : 'center',
				items : [{
//							icon : '../images/baseinfo/edit.png',
                            iconCls:'deppon_icons_edit',
							tooltip : baseinfo.destinationLine.i18n('foss.baseinfo.edit'),
							disabled:!baseinfo.destinationLine.isPermission('destinationLine/destinationLineUpdateButton'),
							// 编辑事件
							handler : function(grid, rowIndex,colIndex) {
			    				...
			    				//进行Ajax请求
                                Ext.Ajax.request({
									url:baseinfo.realPath('queryDestinationLineStandard.action'),
									jsonData:jsonData,
									//作废成功
									success : function(response) {
					                  	var json = Ext.decode(response.responseText);
...
//							icon : '../images/baseinfo/delete.png',
							...
							tooltip : baseinfo.destinationLine.i18n('foss.baseinfo.void'),
							disabled:!baseinfo.destinationLine.isPermission('destinationLine/destinationLineVoidButton'),
							handler : function(grid, rowIndex,colIndex) {
								//作废操作提示窗口
								Ext.Msg.show({
									title:baseinfo.destinationLine.i18n('foss.baseinfo.notice'),
									msg: baseinfo.destinationLine.i18n('foss.baseinfo.deleteWarnMsg'),//'作废后不可恢复，确认要作废么？'
				...
				// 字段标题
				header : baseinfo.destinationLine.i18n('foss.baseinfo.lineName'),
				// 关联model中的字段名
				dataIndex : 'lineName',
				flex : 1
			}, {
				// 字段标题
				header : baseinfo.destinationLine.i18n('foss.baseinfo.simpleCode'),
				// 关联model中的字段名
				dataIndex : 'simpleCode',
				width : 100
			}, {
				// 字段标题
				header : baseinfo.destinationLine.i18n('foss.baseinfo.orginalOrganizationCode'),
				// 关联model中的字段名
				dataIndex : 'orginalOrganizationName',
				width : 120
			}, {
				// 字段标题
				header : baseinfo.destinationLine.i18n('foss.baseinfo.destinationOrganizationCode'),
				// 关联model中的字段名
				dataIndex : 'destinationOrganizationName',
				width : 120
			} , {
				// 字段标题
				header : baseinfo.destinationLine.i18n('foss.baseinfo.distance'),//'线路间距离(公里)'
				// 关联model中的字段名
				dataIndex : 'distance',
				width : 150
			}, {
				// 字段标题
				header : baseinfo.destinationLine.i18n('foss.baseinfo.organizationCode'),
				// 关联model中的字段名
				dataIndex : 'organizationName',
				width : 120
			}, {
				// 字段标题
				header : baseinfo.destinationLine.i18n('foss.baseinfo.isEndDefaults'),
				// 关联model中的字段名
				dataIndex : 'isDefault',
				width : 140,
				renderer:function(value){
				    if('N' == value){
				    	return baseinfo.destinationLine.i18n('foss.baseinfo.no');
				    }else if('Y' == value){
				        return baseinfo.destinationLine.i18n('foss.baseinfo.yes');
				    }
				}
			} ],
			//构造函数
	 		constructor : function(config) {
				var me = this, cfg = Ext.apply({}, config);
				me.store = Ext.create('Foss.baseinfo.destinationLine.DestinationLineStore');
				...
				me.dockedItems = [{
					...
					items : [{
						xtype : 'button',
						text : baseinfo.destinationLine.i18n('foss.baseinfo.add'),//新增
						disabled:!baseinfo.destinationLine.isPermission('destinationLine/destinationLineAddButton'),
						hidden:!baseinfo.destinationLine.isPermission('destinationLine/destinationLineAddButton'),
						width : 80,
						handler : function() {
						    //调用新增弹出窗口函数
...
//				            this.addWindow.resetWindow(model,'save');
						...
						xtype : 'button',
						text : baseinfo.destinationLine.i18n('foss.baseinfo.void'),//作废
						disabled:!baseinfo.destinationLine.isPermission('destinationLine/destinationLineVoidButton'),
						hidden:!baseinfo.destinationLine.isPermission('destinationLine/destinationLineVoidButton'),
						width : 80,
						handler : function() {
						    //调用删除函数
							me.deleteInfos();
						...
						xtype : 'button',
						text : baseinfo.destinationLine.i18n('foss.baseinfo.export'),//导出
						disabled:!baseinfo.destinationLine.isPermission('destinationLine/destinationLineExportButton'),
						hidden:!baseinfo.destinationLine.isPermission('destinationLine/destinationLineExportButton'),
						width : 80,
//						hidden :true,
						handler : function() {
							//获取Form表单
						    ...
						    if (queryForm != null) {
								...
								Ext.MessageBox.buttonText.yes = baseinfo.destinationLine.i18n('foss.baseinfo.confirm');  //"确定"
								Ext.MessageBox.buttonText.no = baseinfo.destinationLine.i18n('foss.baseinfo.cancel'); //"取消"
								...
								Ext.Msg.confirm( baseinfo.destinationLine.i18n('foss.baseinfo.tipInfo'), baseinfo.destinationLine.i18n('foss.baseinfo.exportMsg'), function(btn,text){
									if(btn == 'yes'){
										var params = {
												'lineVo.entity.lineName':queryParams.lineName,
												'lineVo.entity.orginalOrganizationCode':queryParams.orginalOrganizationCode,
										...
										Ext.Ajax.request({
											url:baseinfo.realPath('exportDestinationLines.action'),
											...
											failure:function(response){
												var result = Ext.decode(response.responseText);
												top.Ext.MessageBox.alert(baseinfo.destinationLine.i18n('foss.baseinfo.exportFailed'),result.message);
											}
										});
									}
								});
 ...
 * 新增发车标准信息
 */
Ext.define('Foss.baseinfo.destinationLine.StandardAddWindow',{
	extend : 'Ext.window.Window',
	title : baseinfo.destinationLine.i18n('foss.baseinfo.addStandard'),//新增发车标准
	closable : true,
	modal : true,
	isUpdate:false,//是否为修改，默认false
	parent:null,//父元素
...
//		this.getStandardForm().loadRecord(record);
    ...
    getStandardForm : function(){
    	if(Ext.isEmpty(this.standardForm)){
    		this.standardForm = Ext.create('Foss.baseinfo.destinationLine.StandardForm');
    	}
 ...
 * 修改发车标准信息
 */
Ext.define('Foss.baseinfo.destinationLine.StandardUpdateWindow',{
	extend : 'Ext.window.Window',
	title : baseinfo.destinationLine.i18n('foss.baseinfo.updateStandard'),
	closable : true,
	modal : true,
	isUpdate:true,//是否为修改
	parent:null,//父元素
	...
	listeners:{
		...
		beforeshow:function(me){//window显示所进行操作
			var record = new Foss.baseinfo.destinationLine.StartStandardModel(me.infoModel.data)
 ...
 * 定义发车标准列表
 */
Ext.define('Foss.baseinfo.destinationLine.StartStandardGrid',{
	extend : 'Ext.grid.Panel',
	// 增加表格列的分割线
	columnLines : true,
	// 表格对象增加一个边框
	...
	getAddStandardWindow : function(){
		if(Ext.isEmpty(this.addStandardwindow)){
			this.addStandardwindow = Ext.create('Foss.baseinfo.destinationLine.StandardAddWindow');
			//设置器父元素
	...
	getUpdateStandardWindow : function(){
		if(Ext.isEmpty(this.updateStandardWindow)){
			this.updateStandardWindow = Ext.create('Foss.baseinfo.destinationLine.StandardUpdateWindow');
			//设置器父元素
	...
	columns : [{
				...
				items : [{
						...
						},
						{
							...
							handler : function(grid, rowIndex,colIndex) {
								...
								Ext.Msg.show({
									title:baseinfo.destinationLine.i18n('foss.baseinfo.notice'),
									msg:baseinfo.destinationLine.i18n('foss.baseinfo.deleteWarnMsg'),
									buttons:Ext.Msg.YESNO,
									icon: Ext.Msg.QUESTION, 
									fn:function(btn){
										if(btn == 'yes'){
											...
											Ext.Ajax.request({
												...
												success : function(response) {
									                  ...
									                  //打印成功消息
									                  grid.up('grid').showWarningMsg(baseinfo.destinationLine.i18n('foss.baseinfo.notice'),json.message);
									                },
									            //保存失败
									            exception : function(response) {
									                  var json = Ext.decode(response.responseText);
				...
				// 字段标题
				header : baseinfo.destinationLine.i18n('foss.baseinfo.order'),
				// 关联model中的字段名
				dataIndex : 'order',
				flex : 1
			}, {
				// 字段标题
				header : baseinfo.destinationLine.i18n('foss.baseinfo.leaveTimeb'),
				// 关联model中的字段名
				dataIndex : 'leaveTime',
				width : 100
			}, {
				// 字段标题
				header : baseinfo.destinationLine.i18n('foss.baseinfo.arriveTime'),
				// 关联model中的字段名
				dataIndex : 'arriveTime',
				width : 120,
				renderer:function(value,metaData ,record){
					return value +'<font color="blue">'+'+'+record.get('arriveDay')+baseinfo.destinationLine.i18n('foss.baseinfo.day')+'</font>';
				}
			},{
				// 字段标题
				header : baseinfo.destinationLine.i18n('foss.baseinfo.airagencycompany.remark'),
				// 关联model中的字段名
				dataIndex : 'notes',
				width : 80
			} ],
			//构造函数
	 		constructor : function(config) {
				var me = this, cfg = Ext.apply({}, config);
				me.store = Ext.create('Foss.baseinfo.destinationLine.StartStandardStore');
				me.tbar = [{
					text :baseinfo.destinationLine.i18n('foss.baseinfo.addStandard'),
					disabled:true,
					handler :function(){
						//定义一个Model me.up('window').parent.up('window').infoModel.virtualCode
						var record = Ext.create('Foss.baseinfo.destinationLine.StartStandardModel');
						//出发站
 ...
 * 定义发车标准显示信息列表
 */
Ext.define('Foss.baseinfo.destinationLine.StartStandardInfoGrid',{
	extend : 'Ext.grid.Panel',
	// 增加表格列的分割线
	columnLines : true,
	// 表格对象增加一个边框
	...
	columnLines : true,
	// 定义表格的标题
	title : baseinfo.destinationLine.i18n('foss.baseinfo.standard'),
	collapsible : true,
	animCollapse : true,
	store : null,
	// 定义表格列信息
...
// 查看记录的详细信息
Ext.define('Foss.baseinfo.destinationLine.InfoPanel', {
	extend : 'Ext.panel.Panel',
	title : baseinfo.destinationLine.i18n('foss.baseinfo.detailInfo'),
	frame : true,
	infoForm : null,
	// 获取到达线路详细信息
	getInfoForm : function() {
		if (this.infoForm == null) {
			this.infoForm = Ext.create('Foss.baseinfo.destinationLine.DetailForm');
		}
	...
	getStartStandardGrid : function(){
		if(Ext.isEmpty(this.startStandardGrid)){
			this.startStandardGrid = Ext.create('Foss.baseinfo.destinationLine.StartStandardInfoGrid');
		}
		return this.startStandardGrid;
	},
	constructor : function(config) {
	...
	bindData : function(record) {
        ...
        Ext.Ajax.request({
			...
			success : function(response) {
              	var json = Ext.decode(response.responseText);
				//获取修改的Model信息
				var infoModel = new Foss.baseinfo.destinationLine.DestinationLineModel(json.lineVo.entity);
				// 绑定表格数据到表单上
...
Ext.onReady(function() {
	Ext.QuickTips.init();
	//初始化
	baseinfo.destinationLine.init();
    //查询FORM
	var queryForm = Ext.create('Foss.baseinfo.destinationLine.QueryForm');
	//初始化面板
	baseinfo.destinationLine.initForm(queryForm);
	//获取结果列表
	var queryResult = Ext.create('Foss.baseinfo.destinationLine.DestinationLineGrid');

	Ext.getCmp('T_baseinfo-destinationLine').add(Ext.create('Ext.panel.Panel', {
		id : 'T_baseinfo-destinationLine_content',

 ...
 * @version 1.0
 */
public class LoadNewLableAddService implements IBusinessService<String, RequestSealBindANDCheckOutEntity>{
	private static final Log LOG = LogFactory.getLog(LoadNewLableAddService.class);
	...
	public String service(AsyncMsg asyncMsg, RequestSealBindANDCheckOutEntity param) throws PdaBusiException {
		LOG.info("**************开始执行服务方法*************");
		ResponseJsonInfoEntity bean = itfSealInfomationService.bindSealForExpressService(param);
		String result = "";
		if(bean.getState()=="1"){
	...
	public String getOperType() {
		return LoadConstant.WK_LOAD_SAVE_LABLE.VERSION;
	}

...
public class AdapterCreateLoadTaskService implements IBusinessService<CreateLoadTaskResult, CreateLoadTask>{
	...
	private IPDALoadWKService pdaLoadWKService;
	
//	private IPDALoadService pdaLoadWKService;
	
	public void setPdaLoadWKService(IPDALoadWKService pdaLoadWKService) {
		this.pdaLoadWKService = pdaLoadWKService;
	}
	...
	public CreateLoadTask parseBody(AsyncMsg asyncMsg) throws PdaBusiException {
		CreateLoadTask model = JsonUtil.parseJsonToObject(CreateLoadTask.class,
				asyncMsg.getContent());
		LoadTaskCreateDto pdainfo = new LoadTaskCreateDto(); 
		// 操作人工号
		pdainfo.setOperatorNo(asyncMsg.getUserCode());
		// 操作部门编号
		pdainfo.setOperationOrgCode(asyncMsg.getDeptCode());
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		// 操作时间
		pdainfo.setOperationTime(sdf.format(new Date()));
		// 操作设备编码
		pdainfo.setOperationDeviceCode(asyncMsg.getPdaCode());
		// 版本
		pdainfo.setDataVersion(asyncMsg.getPgmVer());
		
		model.setLoadTaskCreateDto(pdainfo);
	...
	public CreateLoadTaskResult service(AsyncMsg asyncMsg, CreateLoadTask param)
		...
		this.validate(param);
		//封装参数
		WKLoadTaskDto model = new WKLoadTaskDto();
		...
		model.setSendType(Integer.parseInt(param.getSendType()));
		...
		try {
			if(param.getLoadType().equals(TransferPDADictConstants.LOAD_TASK_TYPE_DELIVER)){
				...
				model.setLoadTaskCreateDto(param.getLoadTaskCreateDto());
				res = pdaLoadWKService.createLoadTask(model);
			}
	...
	public String getOperType() {
		return LoadConstant.CRETE_LOAD_TASK.VERSION;
	}

 ...
 * @since
 */
public class RfshSalesDepartmentTaskService implements IBusinessService<List<SalesDepartmentCrgDetail>, QrySalesDepartmentInfo>{
	private IPDADeliverLoadService pdaDeliverLoadService;
	private IPDATransferLoadService pdaTransferLoadService;
	private static final Log LOG = LogFactory.getLog(RfshSalesDepartmentTaskService.class);
	/**
	 * 
	...
	public List<SalesDepartmentCrgDetail> service(AsyncMsg asyncMsg, QrySalesDepartmentInfo param)
			throws PdaBusiException {
		this.validate(param);
		List<LoadSaleGoodsDetailDto> res = null;
		long start = System.currentTimeMillis();
		try {
			if(param.getLoadType().equals(TransferPDADictConstants.LOAD_TASK_TYPE_DELIVER)){
				res =  pdaDeliverLoadService.refrushSaleLoadTaskDetail(param.getTaskCode());
			}else{
				res =  pdaTransferLoadService.refrushSaleLoadTaskDetail(param.getTaskCode());
			}
			...
			for (LoadSaleGoodsDetailDto loadTaskDetailDto : res) {
				...
				if(sers.size()>0){
					...
					for(int i=0;i<sers.size();i++){
						...
						if(isPackArea){
							packNos[index-1]=BitUtil.setBit(packNos[index-1],serialNo%Constant.SERIALNUM==0?32:serialNo%Constant.SERIALNUM);
						}
						if(isModify){
	...
	public String getOperType() {
		return LoadConstant.OPER_TYPE_SALES_DEPARTMENT_TASK_RFSH.VERSION;
	}

 ...
 * @since
 */
public class RfshDeliveryLoadTaskService implements IBusinessService<List<LoadCrgDetail>, QryLoadInfo>{
	private IPDADeliverLoadService pdaDeliverLoadService;
	public final static int NUM = 32;
	private static final Log LOG = LogFactory.getLog(RfshDeliveryLoadTaskService.class);
	/**
	 * 
	...
	@Transactional
	@Override
	public List<LoadCrgDetail> service(AsyncMsg asyncMsg, QryLoadInfo param)
			throws PdaBusiException {
		this.validate(param);
		...
		try {
			if(param.getLoadType().equals(TransferPDADictConstants.LOAD_TASK_TYPE_DELIVER)){
				res =  pdaDeliverLoadService.refrushLoadTaskDetail(param.getTaskCode());
			}
		...
		if(res!=null&&!res.isEmpty()){
			...
			for (LoadGoodsDetailDto loadTaskDetailDto : res) {
				...
				//货物位置
				detail.setFreightPosition("派送库区");
				for (PDAGoodsSerialNoDto seriaNoDto : loadTaskDetailDto.getSerialNos()) {
					if("待叉区".equals(seriaNoDto.getGoodsPosition())){
						detail.setFreightPosition("待叉区");
					}else if("未卸".equals(seriaNoDto.getGoodsPosition())){
						detail.setFreightPosition("未卸");
						break;
					}
				}
				//封装流水号
				...
				if(sers.size()>0){
					...
					for(int i=0;i<sers.size();i++){
						...
						if(isValArea){
							valNos[index-1]=BitUtil.setBit(valNos[index-1],serialNo%NUM==0?NUM:serialNo%NUM);
						}
						if(isPackArea){
							packNos[index-1]=BitUtil.setBit(packNos[index-1],serialNo%NUM==0?NUM:serialNo%NUM);
						}
						if(isModify){
				...
				detail.setCustDistName(loadTaskDetailDto.getReceiveCustDistName());//零担派送装车刷新新增  行政区域
				details.add(detail);
			}
		}
	...
	public String getOperType() {
		return LoadConstant.OPER_TYPE_LOAD_TASK_DISP.VERSION;
	}

 ...
 * @version
 */
public class CheckSealsService implements IBusinessService<Void,CheckLoadSeals>{
	...
	@Transactional
	@Override
	public Void service(AsyncMsg asyncMsg, CheckLoadSeals param)
			throws PdaBusiException {
		...
		//保存封签检查信息分录信息
		for(SealDestDetail sealDestDetail:param.getSealDestDetails()){
			sealDestDetail.setTruckCode(param.getTruckCode());
			sealDestDetail.setScanId(param.getId());
			sealDestDetail.setRemark(param.getCheckOrgMemo());
			loadDao.saveSealDestDetail(sealDestDetail);
		}
		//保存封签检查信息
		loadDao.saveCheckSeals(param);
		try {
			//调用foss接口	
		      pdaVehicleSealService.insertSealDest (param.getStatus(),  param.getTruckCode(), 
	...
	public String getOperType() {
		return UnLoadConstant.OPER_TYPE_UNLD_SEALS_CHK.VERSION;
	}
	//PDA20140327版本同步改异步	
	@Override

...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.DepartassemblyDeptEntity;
...
public class ExpressDepartassemblyDataVerGenService extends
		AbstractBaseDataVerGenService<DepartassemblyDeptEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		...
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_DEPARTDEPT.toUpperCase() + ".txt";
		return filePath;
	}

	@Override
	public String dealLocalDatas(Set<DepartassemblyDeptEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (DepartassemblyDeptEntity entity : localDatas) {
			buffer.append(isNull(entity.getId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getDepartDeptId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getAssemblyDeptId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getAssemblyType())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getIsDefault())).append(Constant.NEWLINE);
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_DEPARTDEPT + Constant.UNDERLINE_DELIMITER

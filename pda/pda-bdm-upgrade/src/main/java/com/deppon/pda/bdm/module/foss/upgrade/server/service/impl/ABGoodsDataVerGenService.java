...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.ABGoodsEntity;
...
public class ABGoodsDataVerGenService extends
		AbstractBaseDataVerGenService<ABGoodsEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_ABGOODS.toUpperCase() + ".txt";
		return filePath;
	}
	 ...
	 */
	@Override
	public String dealLocalDatas(Set<ABGoodsEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (ABGoodsEntity entity : localDatas) {
			// AB货ID
			buffer.append(isNull(entity.getId())).append(Constant.VERTICAL_LINE);
			// AB货key值
			buffer.append(isNull(entity.getAbKey())).append(Constant.VERTICAL_LINE);
			// AB货值
			buffer.append(isNull(entity.getAbValue())).append(Constant.VERTICAL_LINE);
			// AB货状态
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_ABGOODS + Constant.UNDERLINE_DELIMITER
	...
	public String getBaseDataClassName() {
		...
		return ABGoodsEntity.class.getName();
	}

...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.StoreEntity;
...
public class ExpressStoreDataVerGenService extends
		AbstractBaseDataVerGenService<StoreEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		...
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_STORE.toUpperCase() + ".txt";
		return filePath;
	}

	@Override
	public String dealLocalDatas(Set<StoreEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (StoreEntity storeEntity : localDatas) {
			//wwn添加类型Goodstype
			buffer.append(isNull(storeEntity.getId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(storeEntity.getDeptCode())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(storeEntity.getStoreNumber())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(storeEntity.getStoreLocType())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(storeEntity.getAsterisk_code())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(storeEntity.getLadingStatCode())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(storeEntity.getGoodstype())).append(Constant.NEWLINE);
			//buffer.append(isNull(storeEntity.getOperFlag())).append(Constant.NEWLINE);
		}
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_STORE + Constant.UNDERLINE_DELIMITER

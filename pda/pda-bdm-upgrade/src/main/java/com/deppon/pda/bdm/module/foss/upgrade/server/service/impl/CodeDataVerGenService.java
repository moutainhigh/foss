...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.CodeEntity;
...
public class CodeDataVerGenService extends
		AbstractBaseDataVerGenService<CodeEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		...
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_CODE.toUpperCase() + ".txt";
		return filePath;
	}

	@Override
	public String dealLocalDatas(Set<CodeEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		for (CodeEntity codeEntity : localDatas) {
			buffer.append(isNull(codeEntity.getId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(codeEntity.getCodeType())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(codeEntity.getCodeNo())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(codeEntity.getCodeValue())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(codeEntity.getSeq())).append(Constant.NEWLINE);
			//buffer.append(isNull(codeEntity.getOperFlag())).append(Constant.NEWLINE);
		}
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_CODE + Constant.UNDERLINE_DELIMITER

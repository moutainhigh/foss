...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.PriceRegionOrgEntity;
...
public class PriceRegionOrgDataVerGenService extends
		AbstractBaseDataVerGenService<PriceRegionOrgEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_PRICE_REGION_ORG.toUpperCase() + ".txt";
		return filePath;
	}
	 ...
	 */
	@Override
	public String dealLocalDatas(Set<PriceRegionOrgEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (PriceRegionOrgEntity entity : localDatas) {
			...
			buffer.append(isNull(entity.getInclude_org_id())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getInclude_org_code())).append(Constant.VERTICAL_LINE);
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_PRICE_REGION_ORG

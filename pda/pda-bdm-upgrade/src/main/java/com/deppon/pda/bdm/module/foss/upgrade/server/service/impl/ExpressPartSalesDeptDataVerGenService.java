...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.ExpressPartSalesDeptEntity;
import com.deppon.pda.bdm.module.foss.upgrade.shared.vo.QueryLocalListDto;

/**
 * 
  * @ClassName ExpressPartSalesDeptDataVerGenService 
  * @Description TODO 同步用户基础数据服务类
  * @author mt 
  * @date 2013-8-16 上午10:29:06
 */
public class ExpressPartSalesDeptDataVerGenService extends
		AbstractBaseDataVerGenService<ExpressPartSalesDeptEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		...
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_EXPRESS_PART_SALES_DEPT.toUpperCase() + ".txt";
		return filePath;
	}

	@Override
	public String dealLocalDatas(Set<ExpressPartSalesDeptEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (ExpressPartSalesDeptEntity expressPSDEntity : localDatas) {
			buffer.append(isNull(expressPSDEntity.getId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(expressPSDEntity.getPartCode())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(expressPSDEntity.getSalesdeptCode())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(expressPSDEntity.getVersionNo())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(expressPSDEntity.getOperFlag())).append(Constant.NEWLINE);
		}
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_EXPRESS_PART_SALES_DEPT + Constant.UNDERLINE_DELIMITER
	...
	public QueryLocalListDto initBaseDataParam(AsyncMsg asyncMsg,QueryLocalListDto dto){
		 ...
		 * 获取快递员所在点部
		 */
		String deptId = CodeParseUtil.getDeptId(asyncMsg.getUserCode());
		if(deptId != null){
			String deptCode = CodeParseUtil.getDeptCode(deptId, asyncMsg.getUserCode(), "");

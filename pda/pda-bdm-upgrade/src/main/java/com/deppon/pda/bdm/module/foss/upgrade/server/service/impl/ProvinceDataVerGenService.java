...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.ProvinceEntity;
...
public class ProvinceDataVerGenService extends
		AbstractBaseDataVerGenService<ProvinceEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		...
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_PROVINCE.toUpperCase() + ".txt";
		return filePath;
	}

	@Override
	public String dealLocalDatas(Set<ProvinceEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (ProvinceEntity provinceEntity : localDatas) {
			buffer.append(isNull(provinceEntity.getId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(provinceEntity.getProvince())).append(Constant.NEWLINE);
			//buffer.append(isNull(provinceEntity.getOperFlag())).append(Constant.NEWLINE);
		}
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_PROVINCE + Constant.UNDERLINE_DELIMITER

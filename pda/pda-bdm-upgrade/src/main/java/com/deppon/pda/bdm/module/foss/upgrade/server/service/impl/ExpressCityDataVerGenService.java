...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.ExpressCityEntity;

/**
 * 
  * @ClassName ExpressCityDataVerGenService 
  * @Description TODO 同步用户基础数据服务类
  * @author mt 
  * @date 2013-8-16 上午10:07:40
 */
public class ExpressCityDataVerGenService extends
		AbstractBaseDataVerGenService<ExpressCityEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		...
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_EXPRESS_CITY.toUpperCase() + ".txt";
		return filePath;
	}

	@Override
	public String dealLocalDatas(Set<ExpressCityEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (ExpressCityEntity expressCityEntity : localDatas) {
			buffer.append(isNull(expressCityEntity.getId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(expressCityEntity.getDistrictCode())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(expressCityEntity.getType())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(expressCityEntity.getVersionNo())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(expressCityEntity.getOperFlag())).append(Constant.NEWLINE);
		}
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_EXPRESS_CITY + Constant.UNDERLINE_DELIMITER

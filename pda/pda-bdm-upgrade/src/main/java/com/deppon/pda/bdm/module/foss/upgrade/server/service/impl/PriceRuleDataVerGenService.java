...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.PriceRuleEntity;
...
public class PriceRuleDataVerGenService extends
		AbstractBaseDataVerGenService<PriceRuleEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_PRICE_RULE.toUpperCase() + ".txt";
		return filePath;
	}
	 ...
	 */
	@Override
	public String dealLocalDatas(Set<PriceRuleEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (PriceRuleEntity entity : localDatas) {
			...
			buffer.append(isNull(entity.getExpression())).append(Constant.VERTICAL_LINE);
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_PRICE_RULE + Constant.UNDERLINE_DELIMITER

...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.ProductEntity;
...
public class ProductDataVerGenService extends
		AbstractBaseDataVerGenService<ProductEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_PRODUCT.toUpperCase() + ".txt";
		return filePath;
	}
	 ...
	 */
	@Override
	public String dealLocalDatas(Set<ProductEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (ProductEntity entity : localDatas) {
			...
			buffer.append(isNull(entity.getTransport_type())).append(Constant.VERTICAL_LINE);
			if (entity.getLevels() != null) {
				buffer.append(entity.getLevels()).append(Constant.VERTICAL_LINE);
			} else {
				buffer.append(Constant.BLANK).append(Constant.VERTICAL_LINE);
			}
			buffer.append(isNull(entity.getParent_code())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getRef_id())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getShort_name())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getPriority())).append(Constant.VERTICAL_LINE);
			buffer.append(entity.getSeq()).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getDest_net_type())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getDescRiption())).append(Constant.NEWLINE);
			//目前不需要以下字段
			/*
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_PRODUCT + Constant.UNDERLINE_DELIMITER

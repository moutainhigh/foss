...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.PriceRegionEntity;
...
public class PriceRegionDataVerGenService extends
		AbstractBaseDataVerGenService<PriceRegionEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_PRICE_REGION.toUpperCase() + ".txt";
		return filePath;
	}
	 ...
	 */
	@Override
	public String dealLocalDatas(Set<PriceRegionEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (PriceRegionEntity entity : localDatas) {
			...
			buffer.append(isNull(entity.getNation_code())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getProv_code())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getCity_code())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getCounty_code())).append(Constant.VERTICAL_LINE);
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_PRICE_REGION + Constant.UNDERLINE_DELIMITER

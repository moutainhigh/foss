...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.ProhibitedGoodsEntity;
...
public class ProhibitedGoodsDataVerGenService extends
		AbstractBaseDataVerGenService<ProhibitedGoodsEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_PROHIBITED_GOODS.toUpperCase() + ".txt";
		return filePath;
	}
	 ...
	 */
	@Override
	public String dealLocalDatas(Set<ProhibitedGoodsEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (ProhibitedGoodsEntity entity : localDatas) {
			buffer.append(isNull(entity.getId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getDanType())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getDanName())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getDanLevel())).append(Constant.VERTICAL_LINE);
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_PROHIBITED_GOODS
	...
	public String getBaseDataClassName() {
		...
		return ProhibitedGoodsEntity.class.getName();
	}

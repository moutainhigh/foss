...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.CountyEntity;
...
public class CountyDataVerGenService extends
		AbstractBaseDataVerGenService<CountyEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		...
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_COUNTY.toUpperCase() + ".txt";
		return filePath;
	}

	@Override
	public String dealLocalDatas(Set<CountyEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (CountyEntity countyEntity : localDatas) {
			buffer.append(isNull(countyEntity.getId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(countyEntity.getProvince())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(countyEntity.getCity())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(countyEntity.getCounty())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(countyEntity.getAreaCode())).append(Constant.NEWLINE);
			//buffer.append(isNull(countyEntity.getOperFlag())).append(Constant.NEWLINE);
		}
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_COUNTY + Constant.UNDERLINE_DELIMITER

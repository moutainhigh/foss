...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.CityEntity;
...
public class CityDataVerGenService extends
		AbstractBaseDataVerGenService<CityEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		...
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_CITY.toUpperCase() + ".txt";
		return filePath;
	}

	@Override
	public String dealLocalDatas(Set<CityEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (CityEntity cityEntity : localDatas) {
			buffer.append(isNull(cityEntity.getId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(cityEntity.getProvince())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(cityEntity.getCity())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(cityEntity.getAreaCode())).append(Constant.NEWLINE);
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_CITY + Constant.UNDERLINE_DELIMITER

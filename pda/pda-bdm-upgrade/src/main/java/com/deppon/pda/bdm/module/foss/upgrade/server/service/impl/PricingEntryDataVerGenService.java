...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.PricingEntryEntity;
...
public class PricingEntryDataVerGenService extends
		AbstractBaseDataVerGenService<PricingEntryEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_PRICING_ENTRY.toUpperCase() + ".txt";
		return filePath;
	}
	 ...
	 */
	@Override
	public String dealLocalDatas(Set<PricingEntryEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		// 把字段值封装到StringBuffer中
		for (PricingEntryEntity entity : localDatas) {
			...
			buffer.append(isNull(entity.getRef_code())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getBelong_price_entry_id())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(entity.getBelong_price_entry_code())).append(Constant.VERTICAL_LINE);
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_PRICING_ENTRY

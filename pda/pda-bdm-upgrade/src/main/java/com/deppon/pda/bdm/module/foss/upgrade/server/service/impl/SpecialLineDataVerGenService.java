...
import com.deppon.pda.bdm.module.foss.upgrade.shared.domain.SpecialLineEntity;
...
public class SpecialLineDataVerGenService extends
		AbstractBaseDataVerGenService<SpecialLineEntity> {
	...
	public String bulidAllPath(String currVer, File file) {
		...
		String filePath = file.getParent() + File.separator + currVer
				...
				+ Constant.TABLE_NAME_LINE.toUpperCase() + ".txt";
		return filePath;
	}

	@Override
	public String dealLocalDatas(Set<SpecialLineEntity> localDatas) {
		StringBuffer buffer = new StringBuffer();
		for (SpecialLineEntity lineEntity : localDatas) {
			buffer.append(isNull(lineEntity.getId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(lineEntity.getArriveDeptId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(lineEntity.getDepartDeptId())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(lineEntity.getOperFlag())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(lineEntity.getUpdTime())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(lineEntity.getActiveTime())).append(Constant.VERTICAL_LINE);
			buffer.append(isNull(lineEntity.getVersion())).append(Constant.NEWLINE);
			//buffer.append(isNull(lineEntity.getIsActive())).append(Constant.NEWLINE);
		}
	...
	public void zipAllBaseDataFiles(String filePath, String buffer) {
		...
		String[] filed = PropertyConstant.PRO_DATA_MAP.get(
				Constant.TABLE_NAME_LINE + Constant.UNDERLINE_DELIMITER

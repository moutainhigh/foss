 ...
 * 
 * @package com.deppon.pda.bdm.module.foss.accept.server.service.impl 
 * @file UpdateBillingService.java 
 * @description 修改开单信息
 * @author zhangzhenxian
 * @created 2014-02-13 下午2:34:33    
 * @version 1.0
 */
public class UpdateBillingService implements IBusinessService<Void, BillingScanEntity> {
	...
	public Void service(AsyncMsg asyncMsg, BillingScanEntity billingScan) throws PdaBusiException {
		...
		if (appreciationService != null && !appreciationService.isEmpty()) {
			log.debug("---保存修改后的运单增值服务项总条数：" + appreciationService.size() + "---");
			for (ValueAddServiceEntity valueAddService : appreciationService) {
				if (valueAddService != null) {
					...
					log.debug("---保存修改后的运单增值服务项，第"+ (appreciationService.indexOf(valueAddService) + 1 ) +"条开始---");
					acctDao.saveBillVolAddService(valueAddService);
				}
			}
			log.debug("---保存修改后的运单增值服务项结束---");
		}
...
//				valueAddServiceDto.set
		...
		log.debug("---调用FOSS修改运动接口开始---");
		ResultDto resultDto= null;
		try {
			long startTime = System.currentTimeMillis();
			resultDto= pdaWaybillService.updateWaybillByPDA(waybillPdaDto);
			log.debug("---调用FOSS修改运动接口返回结果："+resultDto.getCode()+"---");
			long endTime = System.currentTimeMillis();
			QueueMonitorInfo.addTotalFossTime(endTime-startTime);
			log.info("[asyncinfo]修改运动消耗时间:"+(endTime-startTime)+"ms");
		} catch (BusinessException e) {
			throw new FossInterfaceException(e.getCause(),e.getErrorCode());
		}
		log.debug("---调用FOSS修改运动接口结束---");
		return null;
	}
	...
	public String getOperType() {
		return AcceptConstant.OPER_TYPE_ACCT_BILLING_UPDATE.VERSION;
	}

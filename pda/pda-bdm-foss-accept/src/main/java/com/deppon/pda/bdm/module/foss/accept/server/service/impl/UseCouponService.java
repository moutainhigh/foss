 ...
 * @description 使用优惠券服务类
 * @author ChenLiang
 * @created 2012-12-31 下午2:24:02    
 * @version 1.0
 */
public class UseCouponService implements IBusinessService<UseCoponDetailEntity, UseCouponsEntity> {
	...
	public UseCoponDetailEntity service(AsyncMsg asyncMsg, UseCouponsEntity useCoupons) throws PdaBusiException {
		...
		try {
			log.debug("---调用FOSS使用优惠劵接口开始---");
			long startTime = System.currentTimeMillis();
			couponInfoResult = pdaWaybillService.validateCoupon(couponInfo);
			long endTime = System.currentTimeMillis();
			QueueMonitorInfo.addTotalFossTime(endTime-startTime);
			log.info("[asyncinfo]使用优惠券接口消耗时间:"+(endTime-startTime)+"ms");
		} catch (BusinessException e) {
			throw new FossInterfaceException(e.getCause(),e.getErrorCode());
		}
		log.debug("---调用FOSS使用优惠劵接口结束---");
	...
	public String getOperType() {
		return AcceptConstant.OPER_TYPE_ACCT_USE_COUPON.VERSION;
	}

 ...
 * @version 1.0
 */
public class KdCountFreightService  implements IBusinessService<List<FreightResEntity>, CountFreightEntity> {
	...
	public List<FreightResEntity> service(AsyncMsg asyncMsg, CountFreightEntity countFreight) throws PdaBusiException {
		...
		try {
			lists = pdaPriceService.queryExpressBillCalculate(billCalculateDto);
		...
		if(isFlag){
		    ...
		    //如果产品编号为电商尊享,不变,其他则为标准快递 
		    if("EPEP".equals(countFreight.getProductCode())){
		     billCalculateDto.setProductCode(countFreight.getProductCode());
		    }else{
		     billCalculateDto.setProductCode("PACKAGE");
		    }
		    log.info("---原件返单第二次调用FOSS快递运单费用计算接口开始---");
		    List<PdaResultBillCalculateDto> listsSeconds = null;
		    try {
		        listsSeconds = pdaPriceService.queryExpressBillCalculate(billCalculateDto);
	        } catch (BusinessException e) {
	            throw new FossInterfaceException(e.getCause(),e.getErrorCode());
	        }
	...
	public String getOperType() {
		return AcceptConstant.OPER_TYPE_ACCT_KD_COUNT_FREIGHT.VERSION;
	}

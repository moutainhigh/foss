...
(function(){
	...
	login.queryDictionaryInfo = function(isAsync) {
		//Ajax请求业务字典信息
		Ext.Ajax.request({
			url:'../dict/searchAllDataDictionary.action',
			method: 'POST',
			async: isAsync || false,
...
Ext.define('FossUserContext', {
	...
	getCurrentUserDept: function(){
		if(FossUserContext.getCurrentUserEmp()){
			return login.currentUser.employee.department;			
		}
		return null;
	},
	//获得当前用户所拥有角色的编码集合
...
Ext.define('FossDataDictionary', {
	...
	getDataByTermsCodes: function(termsCodes){
		...
		for(var i=0;i<termsCodes.length;i++){
			data.push(FossDataDictionary.getDataByTermsCode(termsCodes[i]));
		}
		return data;
	},
	/**
	...
	getDataDictionaryStore: function(termsCode, id, anyRecords, valueCodes){
		var data = FossDataDictionary.getDataByTermsCode(termsCode, valueCodes);
		if(!Ext.isEmpty(data)){
	...
	getDataDictionaryCombo: function(termsCode, config, anyRecords, id, valueCodes){
		...
		var store = FossDataDictionary.getDataDictionaryStore(termsCode, id, anyRecords, valueCodes),

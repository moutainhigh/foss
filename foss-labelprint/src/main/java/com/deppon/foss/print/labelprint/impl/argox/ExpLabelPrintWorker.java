package com.deppon.foss.print.labelprint.impl.argox;
...
/**
 * Argox Comm Label Print Program
...
^XZ
			*/
			...
			//^FO5,6^GB469,308,4^FS
			printStrTemplate.append("^FO5,0^GB469,308,4^FS\r\n");
			//^FO10,218^GB458,0,8^FS
			printStrTemplate.append("^FO10,202^GB458,0,4^FS\r\n");
			//^FO19,251^GB113,0,8^FS
			printStrTemplate.append("^FO19,235^GB113,0,4^FS\r\n");
			//^FO62,7^GB0,213,8^FS
			printStrTemplate.append("^FO62,1^GB0,213,4^FS\r\n");
			//^FO68,122^GB403,0,8^FS
			printStrTemplate.append("^FO68,106^GB403,0,4^FS\r\n");
			//获取字段进行打印
			//^FO31,223^A:,20,20^FD德邦物流^FS^FO31,227^AZN,20,20^FD德邦快递^FS
			printStrTemplate.append("^FO31,207^A:,20,20^FD德邦快递^FS\r\n");

			//^FO63,20^A:,28,24^FD【送】^FS
			printStrTemplate.append("^FO63,4^A:,28,24^FD"+labelForm.getIsDelivery().trim()+"^FS\r\n");
			...
			{
				printStrTemplate.append("^FO140,36^GB110,0,65^FS\r\n");
				printStrTemplate.append("^FO145,51^A:,58,48^LRY^FD"+labelForm.getFinalOutfield()+"^FS\r\n");
			} 
			else if(labelForm.getFinalOutfield().length()==3){
				printStrTemplate.append("^FO140,36^GB170,0,65^FS\r\n");
				printStrTemplate.append("^FO145,51^A:,58,48^LRY^FD"+labelForm.getFinalOutfield()+"^FS\r\n");
			}
			else if(labelForm.getFinalOutfield().length()>3 &&labelForm.getFinalOutfield().length()<=5){
				printStrTemplate.append("^FO140,36^GB270,0,65^FS\r\n");
				printStrTemplate.append("^FO145,51^A:,58,48^LRY^FD"+labelForm.getFinalOutfield()+"^FS\r\n");
			}else if(labelForm.getFinalOutfield().length()>=6){
				printStrTemplate.append("^FO140,36^GB270,0,65^FS\r\n");
				printStrTemplate.append("^FO145,51^A:,58,48^LRY^FD"+labelForm.getFinalOutfield().substring(0, 5)+"^FS\r\n");
			}
			//^FO145,146^A:,58,48^FD广州转运场^FS
			//第二外场打印
			if (labelForm.getSecLoadOrgName().length() < 6)
			{
				printStrTemplate.append("^FO145,130^A:,58,48^FD"+labelForm.getSecLoadOrgName()+"^FS\r\n");
			} else if(labelForm.getSecLoadOrgName().length()>=6){
				printStrTemplate.append("^FO145,130^A:,58,48^FD"+labelForm.getSecLoadOrgName().substring(0, 5)+"^FS\r\n");
			}
		...
		//出发城市打印
			...
			{
				printStrTemplate.append("^FO19,8^FPV,0^A:,45,35^FD"+labelForm.getLeaveCity()+"^FS\r\n");
			} else if(labelForm.getLeaveCity().length() >=5){
				printStrTemplate.append("^FO19,8^FPV,0^A:,45,35^FD"+labelForm.getLeaveCity().substring(0, 4)+"^FS\r\n");
			}
			...
			//^FO35,257^AD^FD220125^FS
			printStrTemplate.append("^FO35,241^AD^FD"+labelForm.getOptNum()+"^FS\r\n");
			//^FO13,277^AD^FD2015-01-05^FS
			printStrTemplate.append("^FO13,261^AD^FD"+labelForm.getPrintDate()+"^FS\r\n");
			//^FO326,23^A:,25,25^FD001/02^FS
			printStrTemplate.append("^FO360,7^AD26,26^FD"+outPieces+"^FS\r\n");
			
			
			//^FO426,245^A:,46,46^FD@^FS
			if(labelForm.getIsNonStop() != null && "Y".equals(labelForm.getIsNonStop())){
				printStrTemplate.append("^FO426,229^A:,46,46^FD@^FS\r\n");
		}
...
//			^FD>;123654789987456213^FS
			//^FO158,223^BY2,3,52^BCN,60,Y,N^FD>;520351685900010000^FS
			printStrTemplate.append("^FO150,207^BY2,3,52^BCN,60,Y,N^FD>;"+barcode+"^FS\r\n");//条形码

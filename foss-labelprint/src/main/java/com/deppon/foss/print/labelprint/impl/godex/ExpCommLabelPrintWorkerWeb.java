...
//			printStrTemplate.append("^Y96,N,8,1\r\n");
			...
			printStrTemplate.append("H95,6,1,1,310,700,10\r\n");//画矩形框
			...
			printStrTemplate.append("AZ,95,30,2,1,0,1,德邦快递\r\n");//打印公司LOGO
			...
			if("Y".equals(labelForm.getBigCustomer()) && "Y".equals(labelForm.getIsNonStop())){
				printStrTemplate.append("AE,115,550,1,2,1,1,@\r\n");//打印@字样
				printStrTemplate.append("H35,595,1,1,50,78,2\r\n");//画VIP矩形框
				printStrTemplate.append("AE,85,600,1,1,1,1,VIP\r\n");//打印VIP字样
			}
			//打印大客户标记
			else if("Y".equals(labelForm.getBigCustomer())){
				printStrTemplate.append("H35,590,1,1,50,78,2\r\n");//画VIP矩形框
				printStrTemplate.append("AE,85,595,1,1,1,1,VIP\r\n");//打印VIP字样
			}
			
		   //快递营业部门是否是出发快递营业部：如果是打印出 @
			else if("Y".equals(labelForm.getIsNonStop())){
				printStrTemplate.append("AE,115,605,1,2,1,1,@\r\n");//打印@字样
			}
			//wutao === end
			
			//如果最终外场名称长度小于6个则字体大号
			if(labelForm.getFinalOutfield()!=null && labelForm.getFinalOutfield().length()<=5){
				printStrTemplate.append("AZ,380,110,3,2,1,1,"+labelForm.getFinalOutfield()+signFlag+"\r\n");//最终外场名称
			}else if(labelForm.getFinalOutfield().length() >5 && labelForm.getFinalOutfield().length() <= 10){
				printStrTemplate.append("AZ,380,110,2,2,1,1,"+labelForm.getFinalOutfield()+signFlag+"\r\n");//最终外场名称
			}else if(labelForm.getFinalOutfield().length()>10){
				printStrTemplate.append("AZ,380,110,1,2,1,1,"+labelForm.getFinalOutfield().substring(0,10)+signFlag+"\r\n");//最终外场名称
			}
...
//			件数/总件数
			...
			//printStrTemplate.append("AE,80,580,1,1,1,1,"+labelForm.getGoodsType()+"\r\n");//货物类型
...
//			始发城市
			...
			if(labelForm.getDestination().length() <6) {
				printStrTemplate.append("AZ,300,115,3,2,1,1,"+labelForm.getDestination()+"\r\n");//180
			}else if(labelForm.getDestination().length() >=6 && labelForm.getDestination().length() < 8){
				printStrTemplate.append("AZ,300,115,2,2,1,1,"+labelForm.getDestination()+"\r\n");
			}else {
				printStrTemplate.append("AZ,300,115,1,2,1,1,"+labelForm.getDestination()+"\r\n");//218
			}
			
			//第二城市外场
			//wutao 打印不出第二城市外场 原因：godex打印机，居然用的是斑马的打印指令
//			if(labelForm.getSecLoadOrgName().length() <= 5) {
//				printStrTemplate.append("A330,95,1,8,3,2,N,\""+labelForm.getSecLoadOrgName()+"\"\r\n");//180
//			}else if(labelForm.getSecLoadOrgName().length() > 5 && labelForm.getSecLoadOrgName().length() <= 10){
//				printStrTemplate.append("A330,95,1,8,2,1,N,\""+labelForm.getSecLoadOrgName()+"\"\r\n");//218
//			}else if(labelForm.getSecLoadOrgName().length()>10){
...
//			}
			printStrTemplate.append("AE,340,495,2,4,2,1,K\r\n");//单号1                           
			printStrTemplate.append("AE,335,545,1,2,2,1,"+labelForm.getWaybillNum1()+"\r\n");//单号1
			printStrTemplate.append("AE,265,545,1,2,2,1,"+labelForm.getWaybillNum2()+"\r\n");//单号2
			...
			if(labelForm.getTransportProperty().length()==4){
				...
				printStrTemplate.append("AE,165,500,1,1,1,1,"+labelForm.getTransportProperty().substring(0,1)+"\r\n");
				printStrTemplate.append("AE,165,515,1,1,1,1,"+labelForm.getTransportProperty().substring(1,2)+"\r\n");
				printStrTemplate.append("AE,165,530,1,1,1,1,"+labelForm.getTransportProperty().substring(2,4)+"\r\n");
				printStrTemplate.append("AZ,165,570,2,2,1,1,"+labelForm.getTransportProperty().substring(4,7)+"\r\n");
			}
			else{

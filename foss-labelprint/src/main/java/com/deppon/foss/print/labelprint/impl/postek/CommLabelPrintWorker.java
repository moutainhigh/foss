...
//		}
		...
		for (int i = 1; i <= barcodeList.size(); i++){
			...
			printStrTemplate.append("A115,42,1,8,1,1,N,\"德邦物流\"\r\n");//打印公司LOGO
			     }
			...
			printStrTemplate.append("A95,630,1,4,2,2,N,\"展\"\r\n");}
			...
			//wutao === end
			if(_finalOutField.length()<8){
				printStrTemplate.append("A428,90,1,8,3,2,N,\""+_finalOutField+"\"\r\n");//最终外场名称
			}else if(_finalOutField.length() >= 8 && _finalOutField.length() <= 10 ){
			...
			//如果是大客户，则往上移
			if("Y".equals(labelForm.getBigCustomer())) {
				printStrTemplate.append("A120,495,1,4,1,1,N,\""+labelForm.getGoodsType()+"\"\r\n");//货物类型
			}else{
				printStrTemplate.append("A80,495,1,4,1,1,N,\""+labelForm.getGoodsType()+"\"\r\n");//货物类型
			}			
			...
			if(labelForm.getTransportProperty().length()==4){
				//DMANA-8166 FOSS将PDA信息同步到GPS接口优化start liutao 2015/01/05
				 if("汽运偏线".equals(labelForm.getTransportProperty())){
					   printStrTemplate.append("A178,465,1,8,2,2,N,\""+labelForm.getTransportProperty()+"\"\r\n");
					 }else{
					   printStrTemplate.append("A178,465,1,8,2,2,R,\""+labelForm.getTransportProperty()+"\"\r\n");
					 }
				...
				if(labelForm.getTransportProperty().contains("(")){
					if(labelForm.getTransportProperty().indexOf('(')<=4){
						printStrTemplate.append("A178,465,1,8,1,1,N,\""+labelForm.getTransportProperty().substring(0,4)+"\"\r\n");
						printStrTemplate.append("A178,570,1,4,1,1,N,\""+labelForm.getTransportProperty().substring(4,5)+"\"\r\n");
						printStrTemplate.append("A178,585,1,8,1,1,N,\""+labelForm.getTransportProperty().substring(5,7)+"\"\r\n");
						printStrTemplate.append("A178,635,1,4,1,1,N,\""+labelForm.getTransportProperty().substring(7,8)+"\"\r\n");
					    //printStrTemplate.append("A178,465,1,8,1,1,N,\""+labelForm.getTransportProperty()+"\"\r\n");
					}else{
						printStrTemplate.append("A178,465,1,8,1,1,N,\""+labelForm.getTransportProperty().substring(0,6)+"\"\r\n");
						printStrTemplate.append("A178,620,1,4,1,1,N,\""+labelForm.getTransportProperty().substring(6,7)+"\"\r\n");
						printStrTemplate.append("A178,635,1,8,1,1,N,\""+labelForm.getTransportProperty().substring(7,8)+"\"\r\n");
						printStrTemplate.append("A178,660,1,4,1,1,N,\""+labelForm.getTransportProperty().substring(8,9)+"\"\r\n");
					 ...
					 */
					printStrTemplate.append("A178,465,1,8,1,1,R,\""+labelForm.getTransportProperty()+"\"\r\n");
				}
			}

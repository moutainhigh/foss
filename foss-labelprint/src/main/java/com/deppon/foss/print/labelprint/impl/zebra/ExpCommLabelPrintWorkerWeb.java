...
//				outPacking = totalpieces + " " + labelForm.getPacking();
			...
			printStrTemplate.append("X130,1,10,440,700\r\n");//画矩形框
			printStrTemplate.append("LO346,1,3,700\r\n");//第一条竖线
			
			//printStrTemplate.append("LO346,1,90,444\r\n");//第二条竖线
			...
			printStrTemplate.append("A115,30,1,8,1,1,N,\"德邦快递\"\r\n");//打印公司LOGO
		   ...
		   //快递营业部门是否是出发快递营业部：如果是打印出 @
			...
			if(labelForm.getFinalOutfield().length()<=5){
				printStrTemplate.append("A425,90,1,8,2,2,R,\""+labelForm.getFinalOutfield()+signFlag+"\"\r\n");//最终外场名称
			}else if(labelForm.getFinalOutfield().length() > 5 && labelForm.getFinalOutfield().length() <= 10 ){
				printStrTemplate.append("A432,95,1,8,2,1,R,\""+labelForm.getFinalOutfield()+signFlag+"\"\r\n");//最终外场名称
			}else if(labelForm.getFinalOutfield().length()>10){
				printStrTemplate.append("A425,90,1,8,2,1,R,\""+labelForm.getFinalOutfield().substring(0,10)+signFlag+"\"\r\n");//最终外场名称
			}
			...
			if(labelForm.getTotalPieces() > 0
				...
				printStrTemplate.append("A430,480,1,8,2,1,N,\""+outPieces+"\"\r\n");
			}else if(labelForm.getTotalPieces() >= 10
					&& labelForm.getTotalPieces() < 100){
				printStrTemplate.append("A430,470,1,8,2,1,N,\""+outPieces+"\"\r\n");
			}else if(labelForm.getTotalPieces() >= 100
					&& labelForm.getTotalPieces() < 1000) {
				printStrTemplate.append("A430,464,1,8,2,1,N,\""+outPieces+"\"\r\n");
			}else if(labelForm.getTotalPieces() >= 1000
					&& labelForm.getTotalPieces() < 10000){
				printStrTemplate.append("A430,440,1,8,2,1,N,\""+outPieces+"\"\r\n");
			}else {
				printStrTemplate.append("A430,440,1,8,1,1,N,\""+outPieces+"\"\r\n");
			}
...
//			printStrTemplate.append("A380,446,1,8,1,1,N,\""+outPacking+"\"\r\n");//总件数 包装
			...
			if (labelForm.getLeaveCity().length() < 3)
			{
				...
				printStrTemplate.append("A320,72,1,8,1,1,N,\"" + "—" + "\"\r\n");
				printStrTemplate.append("A300,27,1,8,1,1,N,\""+outleaveCity+"\"\r\n");
			}else{
				printStrTemplate.append("A336,10,1,8,1,1,N,\"" + labelForm.getLeaveCity().substring(0, 2) + "\"\r\n");
				printStrTemplate.append("A320,72,1,8,1,1,N,\"" + "—" + "\"\r\n");
...
//			//目的站
			if(labelForm.getDestination().length() <= 5) {
				printStrTemplate.append("A330,95,1,8,2,2,R,\""+labelForm.getDestination()+"\"\r\n");//180
			}else if(labelForm.getDestination().length() > 6 && labelForm.getDestination().length() < 11){
				printStrTemplate.append("A320,95,1,8,1,1,R,\""+labelForm.getDestination()+"\"\r\n");//218
			}else if(labelForm.getDestination().length()>10){
				printStrTemplate.append("A320,95,1,8,1,1,R,\""+labelForm.getDestination().substring(0,11)+"\"\r\n");//218
			}

			//第二城市外场
//			if(labelForm.getSecLoadOrgName().length() <= 5) {
//				printStrTemplate.append("A330,95,1,8,2,2,N,\""+labelForm.getSecLoadOrgName()+"\"\r\n");//180
//			}else if(labelForm.getSecLoadOrgName().length() > 5 && labelForm.getSecLoadOrgName().length() < 11){
...
//			}
			printStrTemplate.append("A306,455,1,4,4,3,N,\"K\"\r\n");//打印K字

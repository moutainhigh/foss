 ...
 *  
 *  FILE PATH     : src/main/java/com/deppon/foss/module/transfer/scheduling/server/job/ForecastTransferCenterArriveJob.java
 *  
 *  FILE NAME          :ForecastTransferCenterArriveJob.java
 * 
 *  AUTHOR  : FOSS中转系统开发组
 *  
 *  TIME              : 
 ...
 */
public class ForecastTransferCenterArriveJob extends GridJob{
	
	private static final Logger LOGGER = LoggerFactory.getLogger(ForecastTransferCenterArriveJob.class);
...
//			1、取出上次成功执行JOB后的业务截止时间
			jobProcess = tfrCommonService.queryJobInfo(TfrJobBusinessTypeEnum.FORECAST_TRANSFERCENTER_ARRIVE, scheduledFireTime, threadNo, threadCount);
			
//			2、执行业务任务
			jobStartTime = Calendar.getInstance().getTime();
//			 2.1、执行预测总方法,预测所有到达货量
			forecastService.forecastTransferCenterTotal(ForecastConstants.FORECAST_ARRIVE, scheduledFireTime);

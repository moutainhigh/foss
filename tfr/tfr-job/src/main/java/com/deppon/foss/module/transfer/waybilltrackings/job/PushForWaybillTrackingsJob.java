...
public class PushForWaybillTrackingsJob extends BaseStateFulJob {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(PushForWaybillTrackingsJob.class);
	
	private static final String EXCEPTION_REMARK = "任务执行失败";

	/**  
	 * 车辆出发到达的正反操作之后，将运单异步存入轨迹表，以便后续推送给快递100（可选）.  
	 * @see com.deppon.foss.module.transfer.job.BaseStateFulJob#doExecute(org.quartz.JobExecutionContext)  
	 */
	@Override
	protected void doExecute(JobExecutionContext context)
			throws JobExecutionException {
		LOGGER.info("************************ 开始###执行异步存入运单轨迹（出发、到达、取消出发、取消到达） JOB ************************");

		//任务执行实体
		TfrJobProcessEntity jobProcess = new TfrJobProcessEntity();
		//轨迹存入service
		IPushForWaybillTrackingsService pushForWaybillTrackingsService = getBean("pushForWaybillTrackingsService", IPushForWaybillTrackingsService.class);
		//提供中转模块中的通用服务
		...
		try {
			...
			//调用具体服务
			pushForWaybillTrackingsService.pushForWaybillTrackings();
		...
		LOGGER.info("************************ 结束###执行异步存入运单轨迹（出发、到达、取消出发、取消到达） JOB ************************");

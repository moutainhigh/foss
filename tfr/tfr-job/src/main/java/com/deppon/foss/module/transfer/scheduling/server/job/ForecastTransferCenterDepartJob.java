 ...
 */
public class ForecastTransferCenterDepartJob extends GridJob{
	
	private static final Logger LOGGER = LoggerFactory.getLogger(ForecastTransferCenterDepartJob.class);
...
//			1、取出上次成功执行JOB后的业务截止时间
			jobProcess = tfrCommonService.queryJobInfo(TfrJobBusinessTypeEnum.FORECAST_TRANSFERCENTER_DEPART, scheduledFireTime, threadNo, threadCount);
			
//			2、执行业务任务
			jobStartTime = Calendar.getInstance().getTime();
//			 2.1、执行预测总方法,预测所有出发货量
			forecastService.forecastTransferCenterTotal(ForecastConstants.FORECAST_DEPART, scheduledFireTime);

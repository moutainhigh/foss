 ...
 */
public class TruckTaskCallESBJob  extends BaseStateFulJob {
	private final static Logger LOGGER = LoggerFactory.getLogger(TruckTaskCallESBJob.class);
	...
	protected void doExecute(JobExecutionContext context) throws JobExecutionException {
		LOGGER.error("************************ 开始执行任务车辆调用ESBJOB ************************");
		ITruckTaskCallESBService truckTaskCallESBService = getBean("truckTaskCallESBService", ITruckTaskCallESBService.class);
...
//			1、取出上次成功执行JOB后的业务截止时间
			jobProcess = tfrCommonService.queryJobInfo(TfrJobBusinessTypeEnum.VEHICLE_TASK_CALL_ESB, scheduledFireTime, threadNo, threadCount);
...
//			2、执行业务任务
			...
			LOGGER.error("任务车辆调用ESB接口时间:"+df.format(queryBeginTime)+"-"+df.format(scheduledFireTime));
			truckTaskCallESBService.callEsb(queryBeginTime, scheduledFireTime,threadNo,threadCount);
...
//			3、更新任务完成状态及时间
			...
			LOGGER.error("TruckTaskCallESBJob error", e);

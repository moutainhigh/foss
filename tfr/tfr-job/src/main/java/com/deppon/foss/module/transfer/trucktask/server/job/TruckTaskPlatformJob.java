 ...
 *  
 *  FILE PATH     : src/main/java/com/deppon/foss/module/transfer/trucktask/server/job/TruckTaskStockJob.java
 *  
 *  FILE NAME          :TruckTaskStockJob.java
 * 
 *  AUTHOR  : FOSS中转系统开发组
 *  
 *  TIME              : 
...
/**
 * 车辆出发时更新月台的信息，车辆到达时删除该任务关联的月台的信息
 * @author foss-liubinbin
 * @date 2012-11-27 下午4:16:57
 */
public class TruckTaskPlatformJob extends BaseStateFulJob{
	
	private final static Logger LOGGER = LoggerFactory.getLogger(TruckTaskPlatformJob.class);
	...
	protected void doExecute(JobExecutionContext context) throws JobExecutionException {
		LOGGER.trace("************************ 车辆出发时更新月台的信息，车辆到达时删除该任务关联的月台的信息JOB ************************");
...
//			1、取出上次成功执行JOB后的业务截止时间
			jobProcess = tfrCommonService.queryLastExecutedJobInfo(TfrJobBusinessTypeEnum.TRUCK_TASK_FLAG_PLATFORM, scheduledFireTime, threadNo, threadCount);
			
//			2、执行业务任务
			jobStartTime = Calendar.getInstance().getTime();
			
			sharedService.refreshDataForActionTask(DepartureConstant.TASK_FLAG_PLATFORM);
...
//			记录出错日志
			...
			jobProcessLogEntity.setBizName(TfrJobBusinessTypeEnum.TRUCK_TASK_FLAG_PLATFORM.getBizName());
			jobProcessLogEntity.setBizCode(TfrJobBusinessTypeEnum.TRUCK_TASK_FLAG_PLATFORM.getBizCode());
		...
		LOGGER.trace("************************ 车辆出发时更新月台的信息，车辆到达时删除该任务关联的月台的信息 JOB************************");
	}
}
...
public class UnloadDiffReportJob extends BaseStateFulJob{
...
//			1、取出上次成功执行JOB后的业务截止时间
			jobProcess = tfrCommonService.queryJobInfo(TfrJobBusinessTypeEnum.UNLOAD_DIFF_REPORT_OA, scheduledFireTime, threadNo, threadCount);
			
//			2、执行业务任务
			jobStartTime = Calendar.getInstance().getTime();
			unloadDiffReportService.executeUnloadDiffReportTask(threadNo, threadCount);
			jobEndTime = new Date();

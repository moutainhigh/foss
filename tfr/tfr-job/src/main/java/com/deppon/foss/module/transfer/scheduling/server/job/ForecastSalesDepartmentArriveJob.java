 ...
 *  
 *  FILE PATH     : src/main/java/com/deppon/foss/module/transfer/scheduling/server/job/ForecastSalesDepartmentArriveJob.java
 *  
 *  FILE NAME          :ForecastSalesDepartmentArriveJob.java
 * 
 *  AUTHOR  : FOSS中转系统开发组
 *  
 *  TIME              : 
 ...
 */
public class ForecastSalesDepartmentArriveJob extends GridJob{
	
	private static final Logger LOGGER = LoggerFactory.getLogger(ForecastSalesDepartmentArriveJob.class);
	...
	protected void doExecute(JobExecutionContext context) throws JobExecutionException {
		LOGGER.debug("************************ 开始执行营业部到达货量预测 JOB ************************");
...
//			1、取出上次成功执行JOB后的业务截止时间
			jobProcess = tfrCommonService.queryJobInfo(TfrJobBusinessTypeEnum.FORECAST_SALESDEPARTMENT_ARRIVE, scheduledFireTime, threadNo, threadCount);
			
//			2、执行业务任务
			jobStartTime = Calendar.getInstance().getTime();
//			 2.1、执行预测总方法,预测所有到达货量
			forecastService.forecastSalesDepartmentTotal(ForecastConstants.FORECAST_ARRIVE, scheduledFireTime);
...
//			记录出错日志
			...
			jobProcessLogEntity.setBizName(TfrJobBusinessTypeEnum.FORECAST_SALESDEPARTMENT_ARRIVE.getBizName());
			jobProcessLogEntity.setBizCode(TfrJobBusinessTypeEnum.FORECAST_SALESDEPARTMENT_ARRIVE.getBizCode());
		...
		LOGGER.debug("************************ 结束执行营业部到达货量预测 JOB ************************");
	}
}
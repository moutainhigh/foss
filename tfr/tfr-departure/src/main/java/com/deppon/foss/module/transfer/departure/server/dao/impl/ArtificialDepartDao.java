 ...
 *  
 *  FILE PATH     : src/main/java/com/deppon/foss/module/transfer/departure/server/dao/impl/ArtificialDepartDao.java
 *  
 *  FILE NAME          :ArtificialDepartDao.java
 * 
 *  AUTHOR  : FOSS中转系统开发组
 *  
 *  TIME              : 
 ...
 * 
 * 任务车辆放行 系统放行权限：
 * 公司车、短途班车外请车统一由调度来进行放行操作
 * ，长途外请车由外场来放行（长途外请车在外场报到
 * ，公司车及短途班车外请车在车队报到）。
 * 对应公司车辆及短途班车外请车辆的临时放行为车队调度处理；
 * 长途外请车辆的放行由外场操作员办理
 ...
 * ，LMS完成年审/季审/保养月计划后
 * ，调用接口且将信息发送给FOSS系统；如果LMS计划有变动的
 * ，LMS调用该接口，更新车辆信息。
 * 车队调度查看FOSS（本地）已获取的车辆信息
 * ，手动在系统上做临时放行确认操作。
 * FOSS提供车辆信息接口（年审/季审/保养
 * ），LMS完成年审/季审/保养月计划后
 * ，调用接口且将信息发送给FOSS系统；
 ...
 * 车队调度查看FOSS
 * （本地）已获取的车辆信息，手动在系统上做临时放行确认操作。
 * 
 * @author foss-liubinbin(for IBM)
 * @date 2013-3-19 上午10:31:26
 */
public class ArtificialDepartDao extends iBatis3DaoImpl implements
		IArtificialDepartDao {
	private static final String NAMESPACE = "tfr-artificial.";
	/**
	 * 查询临时任务,分页
	 * 
	 * @author foss-liubinbin(for IBM)
	...
	@Override public List<TruckDepartEntity> queryTemporaryAssignments(
		...
		List<TruckDepartEntity> artificialList = this.getSqlSession()
	 ...
	 * @date 2012-10-29 下午5:22:40
	 */
	@Override public List<LmsTruckDepartPlanEntity> queryTemporaryAssignments(
			QueryDepartEntity queryEntity) {
		return (List<LmsTruckDepartPlanEntity>) this.getSqlSession()
				.selectList(NAMESPACE + "queryTemporaryAssignments",
						queryEntity);
	}
	/**
	 ...
	 */
	@Override public void updateLMS(
			LmsTruckDepartPlanEntity lmsTruckDepartPlanEntity) {
		this.getSqlSession().update(NAMESPACE + "updateLMS",
				lmsTruckDepartPlanEntity);
	}
	/**
	 * 通过车牌号查找公司司机的信息（放行计划）
	 * 
	 ...
	 * @date 2012-10-29 下午5:22:40
	 */
	@Override public List<RelationInfoEntity> queryDriverInfoByVehicleNoFormDepartPlan(
			QueryDepartEntity queryEntity) {
		List<RelationInfoEntity> artificialList = this.getSqlSession()
				.selectList(
						NAMESPACE + "queryDriverInfoByVehicleNoFormDepartPlan",
						queryEntity);
		return artificialList;
	}
	/**
	 ...
	 * @date 2012-10-29 下午5:22:40
	 */
	@Override public List<RelationInfoEntity> queryDriverInfoByVehicleNoFormBSE(
			QueryDepartEntity queryEntity) {
		List<RelationInfoEntity> artificialList = this.getSqlSession()
				.selectList(NAMESPACE + "queryDriverInfoByVehicleNoFormBSE",
						queryEntity);
		return artificialList;
	}
	...
	@Override
	public List<RelationInfoEntity> queryDriverInfoByTruckTaskDetailId(
			QueryDepartEntity queryEntity) {
		List<RelationInfoEntity> artificialList = this.getSqlSession()
				.selectList(NAMESPACE + "queryDriverInfoByTruckTaskDetailId",
						queryEntity);
		return artificialList;
	}
}
	...
	columns: [{
		...
		items: [{
                ...
                handler: function(grid, rowIndex, colIndex) {		
					...
					var data = Ext.Date.format(new Date(new Date()),'Y-');
					var dataYear = data.substr(2, 3);
					tmpForm.getForm().findField("externalBillNo").setValue(dataYear);
					Ext.getCmp('attention').setFieldStyle({color:'red'});
...
Ext.define('Foss.uninputedpartial.form.Viewpartialline',{
			...
			//录入外发单情况,需要验证运单号是否已录入validateWaybillNo:yes
				...
				Ext.Ajax.request({
        			...
        			success : function(response) {
        				...
        				if(billInfo.otherFee==null){//chigo 保证默认为0
        					tmpForm.getForm().findField('otherFee').setValue(0);
        				}else{
        					
        					tmpForm.getForm().findField('otherFee').setValue(billInfo.otherFee);
        				}
        				//给外发代理费用和送货费赋值
						...
						tmpForm.getForm().findField('externalAgencyFee').setReadOnly(true);
						tmpForm.getForm().findField('deliveryFee').setReadOnly(true);
						tmpForm.getForm().findField('totalFee').setReadOnly(true);
		...
		items : [{				
				...
				items: [{
							...
							xtype: 'numberfield',
							//hideTrigger: true,
							fieldLabel: partialline.uninputedpartial.i18n('Foss.partialline.form.Viewpartialline.fieldLabel.otherFee'),// '其他费用',
							name: 'otherFee',
							value:0,
//							minValue:0, //chigo保证能为负数
							maxLength:10,
							allowBlank:false,
							columnWidth:.30,
							listeners: {
							...
							columnWidth: .60,
							id:'attention',
							value:partialline.uninputedpartial.i18n('Foss.partialline.form.Viewpartialline.fieldLabel.attention')//注意：无特殊原因请勿去掉系统默认外发单年份前缀，否则可能影响外发结款。
							...
							allowBlank:false,
							items: [
					            { boxLabel: partialline.uninputedpartial.i18n('Foss.partialline.form.Viewpartialline.fieldLabel.isWriteOff.yes'), name: 'isWriteOff', inputValue: 'Y' },
					            { boxLabel: partialline.uninputedpartial.i18n('Foss.partialline.form.Viewpartialline.fieldLabel.isWriteOff.no'), name: 'isWriteOff', inputValue: 'N', checked: true}
					        ],
					        listeners:{
				...
				items: [{
							...
							fieldLabel: partialline.uninputedpartial.i18n('Foss.partialline.form.Viewpartialline.fieldLabel.receiveCustomerContact'),// '收货联系人',
							name: 'receiveCustomerContact',
							readOnly:true,
							columnWidth:.33,
							tipConfig: {
								title: partialline.uninputedpartial.i18n('Foss.partialline.form.Viewpartialline.fieldLabel.receiveCustomerContact'),//'收货联系人',
								height: 150,
								width: 500,
								//是否随鼠标滑动
								trackMouse: true,
		...
		items : [{
			...
			disabled:partialline.uninputedpartial.isPermission('partialline/addExternalBillButton')==true?false:true,
			hidden:partialline.uninputedpartial.isPermission('partialline/addExternalBillButton')==true?false:true,
			...
			handler: function() {
				...
				if (form.isValid()) { 
	        		...
	        		if(Ext.getCmp('externalBillNo').getValue().length<=5){
	        			  Ext.Msg.alert('提示信息', '该外发单号小于或等于5，保存失败!');
	        			  return ;
	        		}else{

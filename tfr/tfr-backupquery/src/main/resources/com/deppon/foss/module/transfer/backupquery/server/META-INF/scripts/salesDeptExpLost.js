Ext.define('Foss.stock.salesDeptExpLost.queryForm', {
	id : 'Foss_backupquery_salesDeptExpLost_queryForm_id',
	...
	buttons : [{
		...
		handler : function() {
			var form = this.up('form').getForm();
			if (form.isValid()) {
				backupquery.salesDeptExpLost.pagingBar.moveFirst();
			}
		}
	}],
...
Ext.define('Foss.stock.salesDeptExpLost.serialNoStore', {
	...
	proxy : {
		type : 'ajax',
		actionMethods : 'POST',
		url : backupquery.realPath('queryReportSerialNo.action'),
		reader : {
			type : 'json',
...
Ext.define('Foss.stock.salesDeptExpLost.gridStore', {
	...
	proxy : {
		type : 'ajax',
		actionMethods : 'POST',
		url : backupquery.realPath('queryReportWaybillNoPaging.action'),
		reader : {
			type : 'json',
	...
	listeners : {
		'beforeload' : function(store, operation, eOpts) {
			var queryForm = Ext.getCmp('Foss_backupquery_salesDeptExpLost_queryForm_id').getForm();
...
Ext.define('Foss.stock.salesDeptExpLost.resultGrid', {
	...
	constructor : function(config) {
		...
		me.tbar = [{
			...
			text : '导出',
			handler : function() {
				var form = Ext.getCmp('Foss_backupquery_salesDeptExpLost_queryForm_id').getForm(), 
				...
				Ext.Ajax.request({
					url : backupquery.realPath('exportReport.action'),
		...
		backupquery.salesDeptExpLost.pagingBar = me.bbar;
		me.callParent([cfg]);
	}
});
...
Ext.onReady(function() {
	...
	Ext.create('Ext.panel.Panel', {
		id : 'T_backupquery-salesDeptExpLostIndex_content',
		cls : "panelContentNToolbar",
		bodyCls : 'panelContent-body',
		layout : 'auto',
		items : [queryForm, resultGrid],
		renderTo : 'T_backupquery-salesDeptExpLostIndex-body'
	});
});
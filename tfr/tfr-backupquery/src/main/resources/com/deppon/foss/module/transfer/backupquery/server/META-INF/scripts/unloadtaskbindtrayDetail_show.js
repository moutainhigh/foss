...
Ext.define('Foss.unload.unloadtaskbindtrayDetail.QueryUnloadBindTrayDetailStore', {
	...
	proxy : {
		...
		actionMethods:'POST',
		// 请求的url
		url : backupquery.realPath('queryUnloadbindTrayDetailByUnloadTaskNo.action'),
		// 定义一个读取器
		reader : {
			// 以JSON的方式读取
			type : 'json',
	...
	constructor: function(config){
		...
		me.tbar = [{
			...
			handler : function(){
				...
				Ext.Ajax.request({
					url : backupquery.realPath('exportUnloadTaskbindTrayDetailExcel.action'),
					method : 'POST',
...
Ext.onReady(function() {
	...
	Ext.Ajax.request({
		url : backupquery.realPath('queryUnloadTaskbindTray.action'),
		params : params,
		success : function(response){
			...
			{
				Ext.getCmp('Foss_backupquery_unloadtaskbindtrayDetail_totalPieces').setValue(basicInfo[0].unLoadScanPieces);
				
			}else{
				
				Ext.getCmp('Foss_backupquery_unloadtaskbindtrayDetail_totalPieces').setValue(0);
			}
			//显示绑定件数
			if(basicInfo[0].bindPieces!=null)
			{
				Ext.getCmp('Foss_backupquery_unloadtaskbindtrayDetail_bindPieces').setValue(basicInfo[0].bindPieces);
				
			}else{
				
				Ext.getCmp('Foss_backupquery_unloadtaskbindtrayDetail_bindPieces').setValue(0);
			}
			//显示叉车扫描件数
			if(basicInfo[0].scanPieces!=null)
			{
				Ext.getCmp('Foss_backupquery_unloadtaskbindtrayDetail_scanPieces').setValue(basicInfo[0].scanPieces);
				
			}else{
				
				Ext.getCmp('Foss_backupquery_unloadtaskbindtrayDetail_scanPieces').setValue(0);
			}
			//显示托盘绑定率
			if(basicInfo[0].bindRate!=null)
			{
				Ext.getCmp('Foss_backupquery_unloadtaskbindtrayDetail_bindRate').setValue(basicInfo[0].bindRate);
				
			}else{
				
				Ext.getCmp('Foss_backupquery_unloadtaskbindtrayDetail_bindRate').setValue(0);
			}

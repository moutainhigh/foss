 ...
 * 
 * 类名称：FindLostWarningDataJob
 * 
 * 创建人：lyz 263072
 * 
 * 创建时间：2015-8-13 上午10:06:03
 * 
 * 版权所有：上海德邦物流有限公司
 */
public class FindLostWarningDataJob extends BaseStateFulJob {
	...
	protected void doExecute(JobExecutionContext paramJobExecutionContext)
		...
		try{
			...
			jobProcess = tfrCommonService.queryJobInfo(TfrJobBusinessTypeEnum.PUSH_FINDLOSTGOODS_QMS_JOB, scheduledFireTime, threadNo, threadCount);
			...
			/*** 执行丢货找到数据上报业务 ***/
			LOGGER.info("丢货预警丢货找到数据上报job开始");
			findLostService.findLostGoods();
			//初始化结束时间
			jobEndTime = new Date();
			...
			jobProcessLogEntity.setBizName(TfrJobBusinessTypeEnum.PUSH_FINDLOSTGOODS_QMS_JOB.getBizName());
			jobProcessLogEntity.setBizCode(TfrJobBusinessTypeEnum.PUSH_FINDLOSTGOODS_QMS_JOB.getBizCode());

...
/**
 * @ClassName: ForecastAction 
 * @Description: 货量预测action类
 * @author yuyongxiang-134019-yuyongxiang@deppon.com
 * @date 2014年3月8日 下午3:10:15 
 *
 */
public class ForecastAction extends AbstractAction {
	 ...
	 * 货量预测Service
	 */
	private transient IForecastService forecastServicePlatform;
	/**
	 * 综合配置参数相关Service
	 */
	 ...
	 * 设置 货量预测Service.
	 *
	 * @param forecastServicePlatform the new 货量预测Service
	 */
	public void setForecastServicePlatform(IForecastService forecastServicePlatform) {
		this.forecastServicePlatform = forecastServicePlatform;
	}
	...
	* @throws
	...
	public String queryTimeList() {
		try {
			...
			List<Date> forecastDateList = forecastServicePlatform.queryForecastTimeList(forecastVO.getForecastQuantityEntity());
	...
	/**
	 * 
	* @Title: queryForecastByPage 
	* @Description:  货量预测总表LIST查询
	* @author yuyongxiang-134019-yuyongxiang@deppon.com
	* @date 2014年3月8日 下午3:12:32 
	* @param @return    设定文件 
	* @return String    返回类型 
	* @throws
	 */
	@JSON
	public String queryForecastByPage() {
		try {
			/**
			 * 分页查询以目的地为单位的信息
			 */
			String org = forecastVO.getForecastQuantityEntity().getBelongOrgCode();
			forecastVO.getForecastQuantityEntity().setBelongOrgCode(findTransforCenter(org));
			List<ForecastQuantityEntity> forecastQuantityList = forecastServicePlatform.queryByPage(forecastVO.getForecastQuantityEntity(), start, limit);
			 ...
			 * 查询全部区域数量(前台以此进行分页)
			 */
//			int count = forecastServicePlatform.queryByPageCount(forecastVO.getForecastQuantityEntity());
//			this.setTotalCount((long) count);
	 ...
	 * 
	* @Title: queryBilling 
	* @Description: 货量预测开单表LIST查询
	* @author yuyongxiang-134019-yuyongxiang@deppon.com
	* @date 2014年3月8日 下午3:12:52 
	* @param @return    设定文件 
	* @return String    返回类型 
	* @throws
	 */
	public String queryBilling() {
		try {
			 ...
			 * 查询开单部分的货量预测信息
			 */
			List<BillingEntity> billingList = forecastServicePlatform.queryBillingList(forecastVO.getBillingEntity());
	...
	/**
	 * 
	 * @Title: queryInTransit 
	 * @Description: 货量预测在途表LIST查询
	 * @author yuyongxiang-134019-yuyongxiang@deppon.com
	 * @date 2014年3月8日 下午3:13:04 
	 * @param @return    设定文件 
	 * @return String    返回类型 
	 * @throws
	 */
	public String queryInTransit() {
		try {
			 ...
			 * 查询在途部分的货量预测信息
			 */
			List<InTransitEntity> inTransitList = forecastServicePlatform.queryInTransitList(forecastVO.getInTransitEntity());
	...
	public String queryChangeQuantity() {
		try {
			 ...
			 * 查询合入的条目
			 */
			changeQuantityList.addAll(forecastServicePlatform.queryChangeInByDate(forecastVO.getForecastQuantityEntity().getForecastTime(), forecastVO.getForecastQuantityEntity().getBelongOrgCode(), forecastVO.getForecastQuantityEntity().getRelevantOrgCode()));
			/**
			 * 查询合出的条目
			 */
			changeQuantityList.addAll(forecastServicePlatform.queryChangeOutByDate(forecastVO.getForecastQuantityEntity().getForecastTime(), forecastVO.getForecastQuantityEntity().getBelongOrgCode(), forecastVO.getForecastQuantityEntity().getRelevantOrgCode()));
	...
	public String changeQuantity() {
		try {
			 ...
			 */
			forecastServicePlatform.changeQuantity(forecastVO.getForecastQuantityList(), forecastVO.getChangeQuantityEntity());
			return returnSuccess();
			/**
			 * 捕获中转异常
			 */
	...
	public String showChartSpecPath() {
		try {
			 ...
			 * 查询某一线路信息
			 */
			List<ForecastQuantityEntity> forecastQuantityList = forecastServicePlatform.queryForecastQuantityList(forecastVO.getForecastQuantityEntity());
			if (forecastQuantityList.size() > 0) {
				...
				if (ifSameDay != 1) {
					 ...
					 */
					RealWeightAndVolumeDto realWeightAndVolumeDto = forecastServicePlatform.countRealWeightAndVolume(forecastVO.getForecastQuantityEntity());
	...
	public String showChartAllPath() {
		try {
			 ...
			 * 获取开始时间点
			 */
			starts = querySysConf(starts, DictionaryConstants.SYSTEM_CONFIG_PARM__TFR, ConfigurationParamsConstants.TFR_PARM__FORECAST_START, forecastVO.getForecastQuantityEntity().getBelongOrgCode()).toString();
//			ConfigurationParamsEntity entityStart = configurationParamsService.queryConfigurationParamsByOrgCode(DictionaryConstants.SYSTEM_CONFIG_PARM__TFR, ConfigurationParamsConstants.TFR_PARM__FORECAST_START, forecastVO.getForecastQuantityEntity().getBelongOrgCode());
			/**
			 * 如果不为空则赋值
			 */
			 ...
			 */
			List<ForecastQuantityEntity> forecastQuantityList = forecastServicePlatform.queryTotalList(forecastVO.getForecastQuantityEntity());
			if (forecastQuantityList.size() > 0) {
				List<ShowChartDto> showChartDtoList = transferList(forecastQuantityList);
				/**
				 * 获取当天货量预测的周期开始时间,持续时间,间隔时间
				 ...
				 * 获取持续天数
				 */
				day = Integer.valueOf(querySysConf(day, DictionaryConstants.SYSTEM_CONFIG_PARM__TFR, ConfigurationParamsConstants.TFR_PARM__FORECAST_DURATION, forecastVO.getForecastQuantityEntity().getBelongOrgCode()).toString());
//				ConfigurationParamsEntity entityDuration = configurationParamsService.queryConfigurationParamsByOrgCode(DictionaryConstants.SYSTEM_CONFIG_PARM__TFR, ConfigurationParamsConstants.TFR_PARM__FORECAST_DURATION, forecastVO.getForecastQuantityEntity().getBelongOrgCode());
				/**
				 * 如果不为空,则赋值
				 */
				 ...
				 */
				int executeGapHour = 1;
				executeGapHour = Integer.valueOf(querySysConf(executeGapHour, DictionaryConstants.SYSTEM_CONFIG_PARM__TFR, ConfigurationParamsConstants.TFR_PARM__FORECAST_GAP, forecastVO.getForecastQuantityEntity().getBelongOrgCode()).toString());
//				ConfigurationParamsEntity entityGap = configurationParamsService.queryConfigurationParamsByOrgCode(DictionaryConstants.SYSTEM_CONFIG_PARM__TFR, ConfigurationParamsConstants.TFR_PARM__FORECAST_GAP, forecastVO.getForecastQuantityEntity().getBelongOrgCode());
				/**
				 * 如果不为空,则赋值
				 */
			/*	if (entityGap != null && StringUtils.isNotEmpty(entityGap.getConfValue())) {
				...
				warnWeight = new BigDecimal(querySysConf(warnWeight, DictionaryConstants.SYSTEM_CONFIG_PARM__TFR, ConfigurationParamsConstants.TFR_PARM__WARN_WEIGHT, orgCode).toString());
//				ConfigurationParamsEntity entityWeight = configurationParamsService.queryConfigurationParamsByOrgCode(DictionaryConstants.SYSTEM_CONFIG_PARM__TFR, ConfigurationParamsConstants.TFR_PARM__WARN_WEIGHT, orgCode);
				...
				warnVolume = new BigDecimal(querySysConf(warnVolume, DictionaryConstants.SYSTEM_CONFIG_PARM__TFR, ConfigurationParamsConstants.TFR_PARM__WARN_VOLUME, orgCode).toString());
//				ConfigurationParamsEntity entityVolume = configurationParamsService.queryConfigurationParamsByOrgCode(DictionaryConstants.SYSTEM_CONFIG_PARM__TFR, ConfigurationParamsConstants.TFR_PARM__WARN_VOLUME, orgCode);
	...
	/**
	 * author 2016年12月23日10:00:41 wwb 311396
	 * sonar优化，改为传入参数，有配置存在，返回配置，无配置存在返回传入的参数
	 * @param obj
	 * @param parmMouudle
	 * @param parmCode
	 * @param orgCode
	 * @return
	 */
	private Object querySysConf(Object obj, String parmMouudle,String parmCode,String orgCode){
		ConfigurationParamsEntity conf = configurationParamsService.queryConfigurationParamsByOrgCode(parmMouudle, parmCode, orgCode);
		if(conf != null && StringUtils.isNotEmpty(conf.getConfValue())){
	...
	public String showChartSpecHour() {
		try {
			...
			for (int i = 0; i <= forecastVO.getDay(); i++) {
				...
				ForecastQuantityEntity forecastQuantityEntity = forecastServicePlatform.querySpecHourList(forecastVO.getForecastQuantityEntity());
	...
	public String queryDepartureType() {
		try {
			 ...
			 * 根据出发,到达部门获取发车计划类型
			 */
			String departureType = forecastServicePlatform.queryDepartureType(origOrgCode, destOrgCode);
	...
	public String forecastExportExcel() {
		try {
			 ...
			 * 设置文件名
			 */
			fileName = forecastServicePlatform.encodeFileName("货量预测");
			/**
			 * 获取文件内容流
			 */
			...
			String org = forecastVO.getForecastQuantityEntity().getBelongOrgCode();
			forecastVO.getForecastQuantityEntity().setBelongOrgCode(findTransforCenter(org));
			excelStream = forecastServicePlatform.exportExcelStream(forecastVO.getForecastQuantityEntity());
			return returnSuccess();
			/**
			 * 捕获中转异常
			 */
	...
	public String detailExportExcel() {
		try {
			 ...
			 * 设置文件名
			 */
			fileName = forecastServicePlatform.encodeFileName("货量预测明细");
			/**
			 * 获取文件内容流
			 */
			excelStream = forecastServicePlatform.detailExportExcelStream(forecastVO.getForecastQuantityEntity().getForecastQuantityId());
			return returnSuccess();
			/**
			 * 捕获中转异常
			 */
	...
	public String queryStatisticalInquiriesIndex(){
		...
		if (orgEntity != null) {
			parentTfrCtrCode = orgEntity.getCode();
		}
		forecastVO.setParentTfrCtrCode(parentTfrCtrCode);
		return SUCCESS;
	}
...
//			forecastJOBService.zyxTest();
			...
			forecastVO = forecastServicePlatform.queryStatisticalInquiries(forecastVO);
	...
	public String queryStatisticalInquiriesExcelStream(){
		try {
			 ...
			 * 设置文件名
			 */
			fileName = forecastServicePlatform.encodeFileName("统计货量查询");
			...
			forecastVO.setLimit(Integer.MAX_VALUE);
			
			/**
			 * 获取文件内容流
			 */
			excelStream = forecastServicePlatform.queryStatisticalInquiriesExcelStream(forecastVO);
			return returnSuccess();
			/**
			 * 捕获中转异常
			 */
	 ...
	 * 根据车次号获取当前运单的所有流水
	 * @author 163580
	 * @date 2014-4-3 上午10:15:09
	 * @return
	 * @see
	 */
	@JSON
	public String querySerialnoByVehicleassembleNo() {
		try {
			//check
			if(null == forecastVO.getStatisticalInquiriesDto()){
				throw new TfrBusinessException("请输入正确的参数!");
			...
			//根据车次号获取当前运单的所有流水
			forecastVO = forecastServicePlatform.querySerialnoByVehicleassembleNo(forecastVO);

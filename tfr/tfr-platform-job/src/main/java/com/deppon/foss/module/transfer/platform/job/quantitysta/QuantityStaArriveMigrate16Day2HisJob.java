...
public class QuantityStaArriveMigrate16Day2HisJob extends BaseStateFulJob {

	private static final Logger LOGGER = Logger
			.getLogger(QuantityStaArriveMigrate16Day2HisJob.class);
	...
	protected void doExecute(JobExecutionContext context)
		...
		try {
			...
			processEntity = tfrCommonService.queryJobInfo(
					TfrJobBusinessTypeEnum.QUANTITY_STA_ARRIVE_HIS,
			...
			quantityStaArriveService.migrateFcst2His();
			...
			LOGGER.info("*****迁移预测货量-到达货量job执行失败*****", e);
			processEntity.setStatus(TransferConstants.JOB_FAILURE);

			// 记录失败日志
			TfrJobProcessLogEntity logEntity = new TfrJobProcessLogEntity();
			logEntity.setBizName(TfrJobBusinessTypeEnum.QUANTITY_STA_ARRIVE_HIS
					.getBizName());
			logEntity.setBizCode(TfrJobBusinessTypeEnum.QUANTITY_STA_ARRIVE_HIS
					.getBizCode());

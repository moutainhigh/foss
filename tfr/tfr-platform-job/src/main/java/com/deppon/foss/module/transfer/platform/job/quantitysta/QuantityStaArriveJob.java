...
public class QuantityStaArriveJob extends BaseStateFulJob {

	private static final Logger LOGGER = Logger
			.getLogger(QuantityStaArriveJob.class);
	...
	protected void doExecute(JobExecutionContext context)
		...
		LOGGER.info("*****货量预测-实际货量-到达货量job于"
		...
		try {
			...
			processEntity = tfrCommonService.queryJobInfo(
					TfrJobBusinessTypeEnum.QUANTITY_STA_ARRIVE,
			...
			quantityStaArriveService.generateArrive(jobStartTime, threadCount,
					threadNo);
			...
			LOGGER.info("*****货量预测-实际货量-到达货量job执行失败*****", e);
			processEntity.setStatus(TransferConstants.JOB_FAILURE);

			// 记录失败日志
			TfrJobProcessLogEntity logEntity = new TfrJobProcessLogEntity();
			logEntity.setBizName(TfrJobBusinessTypeEnum.QUANTITY_STA_ARRIVE
					.getBizName());
			logEntity.setBizCode(TfrJobBusinessTypeEnum.QUANTITY_STA_ARRIVE
					.getBizCode());

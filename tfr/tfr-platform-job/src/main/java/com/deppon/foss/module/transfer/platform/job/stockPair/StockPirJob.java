package com.deppon.foss.module.transfer.platform.job.stockPair;
...
/**
* @description 库存副表的定时任务
* @version 1.0
* @author 14022-foss-songjie
* @update 2014年3月8日 下午5:13:15
*/
public class StockPirJob extends GridJob implements StatefulJob {
	private final static Logger LOGGER = LoggerFactory.getLogger(StockPirJob.class);
	...
	* @author 14022-foss-songjie
	* @update 2014年3月4日 上午9:01:02
	* @version V1.0
	*/
	@Override
	protected void doExecute(JobExecutionContext context)
			throws JobExecutionException {
		LOGGER.info("---库存副表JOB开始---");
		TfrJobProcessEntity jobProcess = new TfrJobProcessEntity();
		Date jobStartTime = null;
		Date jobEndTime = null;
		int threadNo = 0;
		int threadCount = 1;
		IStockPairService stockPairService = this.getBean("stockPairService", IStockPairService.class);
		...
		try{
			...
			jobProcess = tfrCommonService.queryJobInfo(TfrJobBusinessTypeEnum.STOCK_PAIR_JOB, scheduledFireTime, threadNo, threadCount);
			//job开始执行时间
			jobStartTime = Calendar.getInstance().getTime();
			//执行任务(0点采集数据,实际日期应当减一天)
			Date queryDate = DateUtils.addDayToDate(scheduledFireTime, -1);
			stockPairService.doStockPairJob(queryDate,threadNo,threadCount);
			stockPairService.doStockPairJobContinue(queryDate);
			//job结束执行时间
			...
			LOGGER.error("StockPirJob error", e);
			jobProcess.setStatus(TransferConstants.JOB_FAILURE);
//			记录出错日志
			TfrJobProcessLogEntity jobProcessLogEntity = new TfrJobProcessLogEntity();
			jobProcessLogEntity.setBizName(TfrJobBusinessTypeEnum.STOCK_PAIR_JOB.getBizName());
			jobProcessLogEntity.setBizCode(TfrJobBusinessTypeEnum.STOCK_PAIR_JOB.getBizCode());

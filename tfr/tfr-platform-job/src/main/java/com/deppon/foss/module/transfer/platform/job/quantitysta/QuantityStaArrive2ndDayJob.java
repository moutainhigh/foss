...
public class QuantityStaArrive2ndDayJob extends BaseStateFulJob {

	private static final Logger LOGGER = Logger
			.getLogger(QuantityStaArrive2ndDayJob.class);
	...
	protected void doExecute(JobExecutionContext context)
		...
		LOGGER.info("*****货量预测-实际货量-第2天到达货量job于"
		...
		try {
			...
			LOGGER.info("*****货量预测-实际货量-第2天到达货量job执行失败*****", e);
			processEntity.setStatus(TransferConstants.JOB_FAILURE);

			// 记录失败日志
			TfrJobProcessLogEntity logEntity = new TfrJobProcessLogEntity();
			logEntity
					.setBizName(TfrJobBusinessTypeEnum.QUANTITY_STA_ARRIVE_2ND_DAY
							.getBizName());
			logEntity
					.setBizCode(TfrJobBusinessTypeEnum.QUANTITY_STA_ARRIVE_2ND_DAY
							.getBizCode());

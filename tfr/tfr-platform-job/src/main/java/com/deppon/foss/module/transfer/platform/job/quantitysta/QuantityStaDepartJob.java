...
public class QuantityStaDepartJob extends BaseStateFulJob {

	private static final Logger LOGGER = Logger
			.getLogger(QuantityStaDepartJob.class);
	...
	protected void doExecute(JobExecutionContext context)
		...
		LOGGER.info("*****货量预测-实际货量-出发货量job于"
		...
		try {
			...
			processEntity = tfrCommonService.queryJobInfo(
					TfrJobBusinessTypeEnum.QUANTITY_STA_DEPART,
			...
			quantityStaDepartService.generateDepart(jobStartTime, threadCount,
					threadNo);
			...
			LOGGER.info("*****货量预测-实际货量-出发货量job执行失败*****", e);
			processEntity.setStatus(TransferConstants.JOB_FAILURE);

			// 记录失败日志
			TfrJobProcessLogEntity logEntity = new TfrJobProcessLogEntity();
			logEntity.setBizName(TfrJobBusinessTypeEnum.QUANTITY_STA_DEPART
					.getBizName());
			logEntity.setBizCode(TfrJobBusinessTypeEnum.QUANTITY_STA_DEPART
					.getBizCode());

...
/**
* @description 仓库饱和度job
* @version 1.0
* @author 14022-foss-songjie
* @update 2014年3月27日 下午4:24:08
*/
public class StockSaturationJob extends GridJob implements StatefulJob {
	private final static Logger LOGGER = LoggerFactory.getLogger(StockSaturationJob.class);
	...
	*/
	...
	protected void doExecute(JobExecutionContext context)
		...
		try{
			...
			jobProcess = tfrCommonService.queryJobInfo(TfrJobBusinessTypeEnum.STOCK_SATURATION_JOB, scheduledFireTime, threadNo, threadCount);
			//job开始执行时间
			jobStartTime = Calendar.getInstance().getTime();
			//执行任务
			stockSaturationService.doStockSaturationJob(scheduledFireTime, threadNo, threadCount);
			//job结束执行时间
...
//			记录出错日志
			...
			jobProcessLogEntity.setBizName(TfrJobBusinessTypeEnum.STOCK_SATURATION_JOB.getBizName());
			jobProcessLogEntity.setBizCode(TfrJobBusinessTypeEnum.STOCK_SATURATION_JOB.getBizCode());

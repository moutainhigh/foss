package com.deppon.foss.module.transfer.platform.job.sendRate;
...
/**
* @description 派送率的定时任务
* @version 1.0
* @author 14022-foss-songjie
* @update 2014年3月8日 下午5:11:44
*/
public class SendRateJob extends GridJob implements StatefulJob {
	private final static Logger LOGGER = LoggerFactory.getLogger(SendRateJob.class);
	...
	*/
	...
	protected void doExecute(JobExecutionContext context)
		...
		LOGGER.info("---派送率的定时任务 JOB开始---");
		TfrJobProcessEntity jobProcess = new TfrJobProcessEntity();
		Date jobStartTime = null;
		Date jobEndTime = null;
		int threadNo = 0;
		int threadCount = 1;
		ISendRateService sendRateService = this.getBean("sendRateService", ISendRateService.class);
		...
		try{
			...
			jobProcess = tfrCommonService.queryJobInfo(TfrJobBusinessTypeEnum.SEND_RATE_JOB, scheduledFireTime, threadNo, threadCount);
			//job开始执行时间
			jobStartTime = Calendar.getInstance().getTime();
			//执行任务
			sendRateService.doSendRateJobList(scheduledFireTime, threadNo, threadCount);
			//job结束执行时间
			...
			LOGGER.error("SendRateJob error", e);
			jobProcess.setStatus(TransferConstants.JOB_FAILURE);
//			记录出错日志
			TfrJobProcessLogEntity jobProcessLogEntity = new TfrJobProcessLogEntity();
			jobProcessLogEntity.setBizName(TfrJobBusinessTypeEnum.SEND_RATE_JOB.getBizName());
			jobProcessLogEntity.setBizCode(TfrJobBusinessTypeEnum.SEND_RATE_JOB.getBizCode());

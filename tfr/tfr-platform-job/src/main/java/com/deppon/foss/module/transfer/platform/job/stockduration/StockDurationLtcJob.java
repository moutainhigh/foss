...
public class StockDurationLtcJob extends BaseStateFulJob {
	...
	protected void doExecute(JobExecutionContext context)
		...
		try {
			...
			processEntity = tfrCommonService.queryJobInfo(
					TfrJobBusinessTypeEnum.STOCK_DURATION_LTC, scheduledFireTime,
					threadNo, threadCount);

			Date bizStartTime = new Date();

			stockDurationService.generateLtc(threadCount, threadNo);
			...
			logEntity
					.setBizName(TfrJobBusinessTypeEnum.STOCK_DURATION_LTC.getBizName());
			logEntity
					.setBizCode(TfrJobBusinessTypeEnum.STOCK_DURATION_LTC.getBizCode());

...
public class QuantityStaDepartFcstMigrate2HisJob extends BaseStateFulJob {

	private static final Logger LOGGER = Logger
			.getLogger(QuantityStaDepartFcstMigrate2HisJob.class);
	...
	protected void doExecute(JobExecutionContext context)
		...
		try {
			...
			processEntity = tfrCommonService.queryJobInfo(
					TfrJobBusinessTypeEnum.QUANTITY_STA_DEPART_FCST_HIS,
			...
			quantityStaDepartService.migrate16Day2His();
			...
			LOGGER.info("*****迁移实际货量-出发货量job执行失败*****", e);
			processEntity.setStatus(TransferConstants.JOB_FAILURE);

			// 记录失败日志
			TfrJobProcessLogEntity logEntity = new TfrJobProcessLogEntity();
			logEntity.setBizName(TfrJobBusinessTypeEnum.QUANTITY_STA_DEPART_FCST_HIS
					.getBizName());
			logEntity.setBizCode(TfrJobBusinessTypeEnum.QUANTITY_STA_DEPART_FCST_HIS
					.getBizCode());

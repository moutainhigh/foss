 ...
 *  
 *  FILE PATH     : src/main/java/com/deppon/foss/module/transfer/scheduling/server/job/ForecastTransferCenterDepartJob.java
 *  
 *  FILE NAME          :ForecastTransferCenterDepartJob.java
 * 
 *  AUTHOR  : FOSS中转系统开发组
 *  
 *  TIME              : 
 ...
 */
public class ForecastTransferCenterArriveJob extends BaseStateFulJob {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(ForecastTransferCenterArriveJob.class);

	/**
	 * 
	 * @Title: doExecute 
	 * @Description: JOB定时执行货量预测JOB 1、按配置时间执行JOB 不用关心上次执行是否成功 2、更新此JOB的执行时间和状态
	 * @author yuyongxiang-134019-yuyongxiang@deppon.com
	 * @date 2014年3月22日 上午9:28:09
	 *
	 * @param context
	 * @throws JobExecutionException    设定文件 
	 *
	...
	protected void doExecute(JobExecutionContext context)
		...
		LOGGER.debug("************************ 开始执行外场到达货量预测 JOB ************************");
		TfrJobProcessEntity jobProcess = new TfrJobProcessEntity();
		Date jobStartTime = null;
		Date jobEndTime = null;
		int threadNo = 0;
		int threadCount = 0;
		IForecastJOBService forecastJOBService = getBean("forecastJOBService",
				IForecastJOBService.class);
		ITfrCommonService tfrCommonService = getBean("tfrCommonService",
		...
		try {
			...
			jobProcess = tfrCommonService.queryJobInfo(
					TfrJobBusinessTypeEnum.FORECAST_TRANSFERCENTER_ARRIVE,
					scheduledFireTime, threadNo, threadCount);

			// 2、执行业务任务
			jobStartTime = Calendar.getInstance().getTime();
			// 2.1、执行预测总方法,预测所有到达货量
			forecastJOBService.forecastTransferCenterTotal(
					ForecastConstants.FORECAST_ARRIVE,
					String.valueOf(threadNo), String.valueOf(threadCount),
					scheduledFireTime);
			...
			jobProcessLogEntity
					.setBizName(TfrJobBusinessTypeEnum.FORECAST_TRANSFERCENTER_ARRIVE
							.getBizName());
			jobProcessLogEntity
					.setBizCode(TfrJobBusinessTypeEnum.FORECAST_TRANSFERCENTER_ARRIVE
							.getBizCode());

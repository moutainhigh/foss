 ...
 * 营业部交接单模块action类
 * @author 360903
 * @date 2016年11月17日 15:07:26
 */
public class HandOverBillSaleAction extends AbstractAction{
	...
	public String handOverBillAddNewIndex(){
		try{
			...
			*/
			//获取当前大部门的code
			//sonar 注释 311396 返回值未使用 2016年12月20日16:02:36 
			/*String superOrgCode = */
	...
	public String saveHandOverBill(){
		...
		try{
			//默认为营业部交接
			newHandOverBillDto.getHandOverBillEntity().setHandOverType(LoadConstants.HANDOVER_TYPE_SALES_DEPARTMENT);
			newHandOverBillDto.getHandOverBillEntity().setGoodsType(LoadConstants.LOAD_TASK_TYPE_ALL);
			newHandOverBillDto.getHandOverBillEntity().setLoadEndTime(new Date());
			//保存时重新生成交接单号
	...
	public String queryHandOverBillList(){
		...
		queryHandOverBillConditionDto.setHandOverType(LoadConstants.HANDOVER_TYPE_SALES_DEPARTMENT);
		//调用service得到查询结果
	 ...
	 * 查询交接单页面跳转，同时获取大部门code
	 * @author 360903
	 * @date 2016年11月17日 13:43:13
	 */
	@JSON
	public String handOverBillSaleQueryIndex(){
		try{
			String superOrgCode = handOverBillService.querySuperiorOrgCode();
			handOverBillVo.setSuperOrgCode(superOrgCode);
		}catch(BusinessException e){

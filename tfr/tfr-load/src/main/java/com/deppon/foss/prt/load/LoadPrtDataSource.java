 ...
 ******************************************************************************/
package com.deppon.foss.prt.load;
...
public class LoadPrtDataSource implements MultiJasperDataSource {
	...
	class MyJasperDataSource implements JasperDataSource {
		...
		public Map<String, Object> createParameterDataSource(JasperContext jasperContext) {
			...
			List<SealOrigDetailEntity> sealOrigDetails = vehicleSealService.getSealOrigDetailsByVehicleNo(vehicleNo);
			//封签
			...
			//转货类型
			String tranGoodsType = handOverBill.getTranGoodsType();
			if(StringUtils.equals(tranGoodsType, LoadConstants.TRANSITGOODS_TYPE_TRAN)){
				tranGoodsType = "转货";
			}else if(StringUtils.equals(tranGoodsType, LoadConstants.TRANSITGOODS_TYPE_TAKE)){
				tranGoodsType = "带货";
			}else if(StringUtil.equals(handOverType, "快递空运交接单")){
				tranGoodsType = "";
				
			}
			parameter.put("tranGoodsType", tranGoodsType);
			//打印部门

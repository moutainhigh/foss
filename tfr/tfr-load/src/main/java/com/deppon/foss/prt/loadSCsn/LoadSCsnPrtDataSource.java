 ...
 ******************************************************************************/
package com.deppon.foss.prt.loadSCsn;
 ...
 */
public class LoadSCsnPrtDataSource implements MultiJasperDataSource {
	/****
	 * 批量打印的jasperDataSource
	 * @author 205109-zenghaibin
	 * @date 2015-08-08 09:34:22
	...
	class MyJasperDataSource implements JasperDataSource {
		...
		public Map<String, Object> createParameterDataSource(JasperContext jasperContext) {
			...
			//车牌号
			parameter.put("vehicleNo", connectionBill.getVehicleNo());
			//备注
		...
		public List<Map<String, Object>> createDetailDataSource(JasperContext jasperContext) {
			...
			List<HandOverBillDetailDto> printData = new ArrayList<HandOverBillDetailDto>(connectionBillNoDetails.size());
			for(HandOverBillDetailDto connectionBillNoDetail : connectionBillNoDetails){
				if(StringUtils.equals(connectionBillNoDetail.getTransProperty(), "精准空运")) {
					printData.add(connectionBillNoDetail);
			...
			for(HandOverBillDetailDto connectionBillNoDetail : connectionBillNoDetails){
				if(!StringUtils.equals(connectionBillNoDetail.getTransProperty(), "精准空运") 
						&& StringUtils.equals(connectionBillNoDetail.getIsPreciousGoods(), "Y")) {
					printData.add(connectionBillNoDetail);
					flag ++;
				}
			}
			...
			for(HandOverBillDetailDto handover : printData){
				...
				for(int i = 0; i < connectionBillNoDetails.size(); i++){
					HandOverBillDetailDto connectionBillNoDetail = connectionBillNoDetails.get(i);
					if(StringUtils.equals(connectionBillNoDetail.getWaybillNo(), handover.getWaybillNo())) {
						connectionBillNoDetails.remove(i);
						break;
					}
				}
			}
			printData.addAll(connectionBillNoDetails);
			connectionBillNoDetails = printData;
			...
			for(HandOverBillDetailDto connectionBillNoDetail : connectionBillNoDetails){
				//运单号
				String wayBillNo = connectionBillNoDetail.getWaybillNo();
				//如果为空, 就空一行
				if(StringUtils.isEmpty(wayBillNo)) {
					continue;
				}
				//获取交接流水
				List<HandOverBillSerialNoDetailEntity> connectionBillSerialNoDetails = connectionBillService.queryWaybillDetailByNos(connectionBillNo,wayBillNo);
				//交接流水
				StringBuffer serialNos = new StringBuffer();
				//将交接单流水以"1/2/3/4/5"这样的方式输出
				for(HandOverBillSerialNoDetailEntity connectionBillSerialNoDetail : connectionBillSerialNoDetails) {
					serialNos.append(Integer.valueOf(connectionBillSerialNoDetail.getSerialNo())).append("/");
				}
				...
				//运输性质(简称)
				m.put("transProperty", productMap.get(connectionBillNoDetail.getTransProperty()));
				//运单号
				m.put("waybillNo", connectionBillNoDetail.getWaybillNo());
				//收货人
				m.put("consignee", connectionBillNoDetail.getConsignee());
				//收货人电话
				m.put("receiveCustomerMobilephone", connectionBillNoDetail.getReceiveCustomerMobilephone());
				if(connectionBillNoDetail.getConsignee() == null && connectionBillNoDetail.getReceiveCustomerMobilephone() == null) {
					m.put("consigneeinfo", null);
				} else {
					StringBuffer consigneeinfo = new StringBuffer();
					if(StringUtils.isNotEmpty(connectionBillNoDetail.getConsignee())) {
						consigneeinfo.append(connectionBillNoDetail.getConsignee());
					}
					if(StringUtils.isNotEmpty(connectionBillNoDetail.getReceiveCustomerMobilephone())) {
						if(StringUtils.isNotEmpty(connectionBillNoDetail.getConsignee())) {
							consigneeinfo.append("/");
						}
						consigneeinfo.append(connectionBillNoDetail.getReceiveCustomerMobilephone());
					}
					m.put("consigneeinfo", consigneeinfo.toString());
				}
				//收货部门
				m.put("receiveOrgName", connectionBillNoDetail.getReceiveOrgName());
				//件数
				m.put("pieces", connectionBillNoDetail.getPieces());
				//开单件数
				m.put("waybillPieces", connectionBillNoDetail.getWaybillPieces());
				if(connectionBillNoDetail.getPieces() == null && connectionBillNoDetail.getWaybillPieces() == null) {
					m.put("pieceinfo", null);
				} else {
					m.put("pieceinfo", connectionBillNoDetail.getPieces() + "/" + connectionBillNoDetail.getWaybillPieces());
				}
				//重量
				m.put("weight", connectionBillNoDetail.getWeight());
				//目的站
				m.put("destination", connectionBillNoDetail.getDestination());
				//实际体积
				m.put("cubage", connectionBillNoDetail.getCubage());
				//运单备注
				m.put("waybillNote", connectionBillNoDetail.getWaybillNote());
				//品名
				m.put("goodsName", connectionBillNoDetail.getGoodsName());
				//包装
				m.put("packing", connectionBillNoDetail.getPacking());
				if(connectionBillNoDetail.getGoodsName() == null && connectionBillNoDetail.getPacking() == null) {
					m.put("packinginfo", null);
				} else {
					m.put("packinginfo", connectionBillNoDetail.getGoodsName() + "/" + connectionBillNoDetail.getPacking());
				}
				//提货方式
				m.put("receiveMethod", pickupGoodsDictMap.get(connectionBillNoDetail.getReceiveMethod()));
				data.add(m);
			}
			return data;
		}

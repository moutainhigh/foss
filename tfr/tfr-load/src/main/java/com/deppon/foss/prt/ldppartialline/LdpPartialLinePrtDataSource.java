 ...
 *  
 *  FILE PATH     : src/main/java/com/deppon/foss/prt/partialline/PartialLinePrtDataSource.java
 *  
 ...
 ******************************************************************************/
package com.deppon.foss.prt.ldppartialline;
 ...
 */
public class LdpPartialLinePrtDataSource implements MultiJasperDataSource {
	
	/****
	 * 批量打印的jasperDataSource
	 * @author ibm-zhangyixin
	 * @date 2013-3-14 下午4:01:12
	...
	class MyJasperDataSource implements JasperDataSource {
		...
		public List<Map<String, Object>> createDetailDataSource(JasperContext jasperContext) {
			...
			paymentMode.add(DictionaryConstants.PAYMENT_MODE);
			//从数据字典中取付款方式
			...
			for(HandOverBillDetailDto handOverBillDetail : handOverBillDetails){
				...
				//269701--foss--lln--20151010--begin
				//根据运单号获取代理单号
				//String agentWaybillNo = handOverBillService.getAgentWaybillNoByWaybillNo(wayBillNo);
				//获取交接单流水
				 ...
				 * 快递代理外发清单打印
				 * 在德邦单号与代理单号之间增加流水号字段，
				 * 同一单号不同流水号显示多行。打印件数信息显示为1，
				 * 重量体积信息显示为已录入的实际重量体积，如无，则为空。
				 */
...
//				StringBuffer serialNos = new StringBuffer();
				...
				for(HandOverBillSerialNoDetailEntity handOverBillSerialNoDetail : handOverBillSerialNoDetails) {
					//serialNos.append(handOverBillSerialNoDetail.getSerialNo()).append("/");
					Map<String, Object> m = new HashMap<String, Object>();
					//流水号
					m.put("serialNos", handOverBillSerialNoDetail.getSerialNo());
					//运单号
					m.put("waybillNo", handOverBillDetail.getWaybillNo());
					//一票多件时，一个运单对应多个代理单号
					//根据运单号和流水号获取代理单号
					agentWaybillNo = handOverBillService.getAgentWaybillNoByWaybillNo(wayBillNo,handOverBillSerialNoDetail.getSerialNo());
					//代理单号
					m.put("agentWaybillNo", agentWaybillNo);
					//收货人
					m.put("consignee", handOverBillDetail.getConsignee());
					//收货部门
					m.put("receiveOrgName", handOverBillDetail.getReceiveOrgName());
					//件数
					m.put("pieces", 1);//handOverBillDetail.getPieces()
					//开单件数
					m.put("waybillPieces", handOverBillDetail.getWaybillPieces());
					BigDecimal weight = handOverBillDetail.getWeightAc();
					BigDecimal volumn = handOverBillDetail.getCubageAc();
					/*		//modified by 257220 优先获取录入的子母件或一票多件重量体积
					//获取重新录入的重量体积信息
					InputWeightVolumnEntity inputWeightVolumnEntity = inputWeightVolumnService.queryInputWeightVolumnByWaybillNo(handOverBillDetail.getWaybillNo(),handOverBillSerialNoDetail.getSerialNo());
					if(inputWeightVolumnEntity != null){
						weight = inputWeightVolumnEntity.getWeight();
						volumn = inputWeightVolumnEntity.getVolumn();
					}*/
					//重量
					m.put("weight", weight.divide(handOverBillDetail.getPieces(), 2, BigDecimal.ROUND_HALF_UP));
					//目的站
					m.put("destination", handOverBillDetail.getDestination());
					//实际体积
					m.put("cubage", volumn.divide(handOverBillDetail.getPieces(), 2, BigDecimal.ROUND_HALF_UP));
					//运单备注
					...
					//代收货款金额
					//数据库中保存的是元
					BigDecimal codAmount = handOverBillDetail.getCodAmount();
					if(codAmount == null) {
						codAmount = BigDecimal.ZERO;
					}
					m.put("codAmount", codAmount);
					
					BigDecimal totMoney = toPayAmount.add(codAmount);
					m.put("totMoney", totMoney);

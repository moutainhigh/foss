...
Ext.define('Foss.load.complementLog.queryForm', {
			...
			buttons : [{
				...
				handler : function() {
					var form = this.up('form').getForm();
					if (form.isValid()) {
						load.complementLog.pagingBar.moveFirst();
					}
				}
			}],
...
Ext.define('Foss.load.complementLog.complementLogStore', {
	...
	proxy : {
		type : 'ajax',
		actionMethods : 'POST',
		url : load.realPath('queryComplementLogList.action'),
		reader : {
			type : 'json',
...
Ext.define('Foss.load.complementLog.complementLogGrid', {
			...
			constructor : function(config) {
				...
				load.complementLog.pagingBar = me.bbar;
				me.callParent([cfg]);
			},
			columns : [{
						dataIndex : 'waybillNo',
...
Ext.onReady(function() {
			...
			Ext.create('Ext.panel.Panel', {
						id : 'T_load-complementLogIndex_content',
						cls : "panelContentNToolbar",
						bodyCls : 'panelContent-body',
						layout : 'auto',
						items : [queryForm, queryResult],
						renderTo : 'T_load-complementLogIndex-body'
					});
		});
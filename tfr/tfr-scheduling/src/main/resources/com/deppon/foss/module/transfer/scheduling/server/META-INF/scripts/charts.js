...
Ext.define('Foss.forecastQuantity.OperatePanel',{
			...
			this.countMenssage = Ext.create('Ext.container.Container', {
				...
				items: [{
					...
					items: [{
						...
						xtype:'label',
						text:scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.warnLine'),
						padding:'10 0 0 0',
						columnWidth: .13
					}]
				},{
	...
	constructor: function(config){
		...
		me.items = [{
			...
			xtype:'label',
			text:scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.showDay'),
			padding:'9 0 0 0',
			html:'&nbsp;&nbsp;',
			columnWidth:.1
			...
			xtype:'label',
			text:scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.day'),
			padding:'10 0 0 0',
			columnWidth:.03
			
		},{
			xtype:'button',
			text:scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.asVolume'),
			columnWidth:.07,
			maxHeight:27,
			handler:function(){
				me.queryByVolume();
			...
			xtype:'button',
			text:scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.asWeight'),
			columnWidth:.07,
			maxHeight:27,
			handler:function(){
				me.queryByWeight();
			...
			xtype:'label',
			text:scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.realNumber'),
			padding:'10 0 0 0',
			columnWidth:.05
			...
			xtype:'label',
			text:scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.forecastGoods'),
			padding:'10 0 0 0',
			columnWidth:.08
		}];
		me.callParent([cfg]);
...
Ext.define('Foss.forecastQuantity.LineGoodCharts.OperatePanel',{
	...
	queryByVolume: function(){
		...
		this.buttonHandler({
			total: 'totalVolume',
			title: scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.volumeUnit'),
			real: 'realVolume',
	...
	queryByWeight: function(){
		...
		this.buttonHandler({
			total: 'totalWeight',
			title: scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.weightUnit'),
			real: 'realWeight',
			isCount: record.get('relevantOrgCode')?false:true,
			warn: 'warnWeight'
		});
	...
	buttonHandler: function(param){
		...
		if(dayNumber == null){
			Ext.MessageBox.alert(scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.hint'),scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.notEmpty'));
			return;
		}
		if(dayNumber%1!=0){
			Ext.MessageBox.alert(scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.hint'),scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.onlyInt'));
			return;
		}
		if(dayNumber>7||dayNumber<0) {
			Ext.MessageBox.alert(scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.hint'),scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.zeroToSeven'));
			return;
		}
...
Ext.define('Foss.forecastQuantity.LineGoodStore', {
	...
	proxy : {
		type : 'ajax',
		actionMethods: 'POST',
		url : scheduling.realPath('showChartSpecPath.action'),
		reader : {
			type : 'json',
...
Ext.define('Foss.forecastQuantity.CountLineGoodStore', {
	...
	proxy : {
		type : 'ajax',
		actionMethods: 'POST',
		url : scheduling.realPath('showChartAllPath.action'),
		reader : {
			type : 'json',
			root : 'forecastVO.showChartDto'
		}
...
Ext.define('Foss.forecastQuantity.LineGood.Chart',{
	...
	constructor: function(config){
		...
		me.axes = [ {
			...
			title: scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.time')+'('+Ext.Date.format(statisticsDate, 'Y-m-d')+')',
			label: {
				font: '11px Arial',
		...
		if(isCount){
			me.series.push({
				...
				tips: {
					...
					height: 25,
					renderer: function(storeItem, item) {
						this.setTitle(scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.warnLine') + storeItem.get(warn));
					}
				},
				highlight: {
                    size: 7,
		...
		if(lineView){
			me.series.push({
				...
				tips: {
					...
					height: 25,
					renderer: function(storeItem, item) {
						this.setTitle(scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.actual') + storeItem.get(real));
					}
				},
				highlight: {
                    size: 7,
...
Ext.define('Foss.forecastQuantity.LineGoodChartsWindow',{
	...
	bindData: function(record){
		...
		if(record.get('relevantOrgCode')){
			title = record.get('belongOrgCodeName') + "-" +record.get('relevantOrgCodeName') +' '+record.get('rawValue')+ scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.lineChart');
			me.getOperatePanel().getCountMenssage().getLayout().setActiveItem(1);
		}else{
			title = record.get('rawValue')+ scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.totalChart');
...
Ext.define('Foss.forecastQuantity.HourGoodVolumeStore', {
	...
	proxy : {
		type : 'ajax',
		actionMethods: 'POST',
		url : scheduling.realPath('showChartSpecHour.action'),
		reader : {
			type : 'json',
			root : 'forecastVO.showChartDto'
		}
...
Ext.define('Foss.forecastQuantity.HourGoodChart.Chart',{
	...
	constructor: function(config){
		...
		me.axes = [ {
			...
			title: scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.time'),
			label: {
				font: '11px Arial',
				renderer: function(value) {
					if(Ext.isEmpty(value)){
		...
		me.series = [{
			...
			tips: {
				...
				height: 25,
				renderer: function(storeItem, item) {
					this.setTitle(scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.actual') + storeItem.get(real) + scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.weightUnit'));
				}
			},
			highlight: {
                    size: 7,
			...
			tips: {
				...
				renderer: function(storeItem, item) {
					//提示文本
					this.setTitle(scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.forecast') + storeItem.get(total) + scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.weightUnit'));
				}
			},
			renderer: function(sprite, storeItem, src, i, store) {
				//更改柱子的背景颜色
...
Ext.define('Foss.forecastQuantity.HourGoodChartWindow',{
	...
	bindData: function(record,currentRecord){
		...
		me.setTitle(title+ ' ' +currentRecord.get('rawValue') + scheduling.forecastQuantity.i18n('foss.scheduling.forecastQuantity.charts.goodsChart'));
		me.getOperatePanel().getCountMenssage().getLayout().setActiveItem(1);

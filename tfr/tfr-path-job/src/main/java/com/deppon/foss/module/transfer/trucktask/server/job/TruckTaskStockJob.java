...
/**
 * 车辆出发、到达、取消出发、取消到达时需要更新交接单配载单的状态
 * 车辆出发需要发送消息到营业部
 * @author foss-liubinbin
 * @date 2012-11-27 下午4:16:57
 */
public class TruckTaskStockJob extends GridJob implements StatefulJob{
	
	private final static Logger LOGGER = LoggerFactory.getLogger(TruckTaskStockJob.class);

	/** 
	 * JOB定时执行车辆出发、到达、取消出发、取消到达时需要更新交接单配载单的状态
     * 车辆出发需要发送消息到营业部
	...
	protected void doExecute(JobExecutionContext context) throws JobExecutionException {
		LOGGER.trace("************************ 开始执行车辆出发、到达、取消出发、取消到达时需要更新交接单配载单的状态JOB ************************");
...
//			1、取出上次成功执行JOB后的业务截止时间
			jobProcess = tfrCommonService.queryLastExecutedJobInfo(TfrJobBusinessTypeEnum.TRUCK_TASK_STOCK_JOB, scheduledFireTime, threadNo, threadCount);
...
//			 2.2、执行成功后，若存在明细都已处理完毕的清仓差异报告，需更新这些清仓报告状态为TransferConstants.STOCK_CHECKING_REPORT_DONE
			sharedService.refreshDataForActionTask(DepartureConstant.TASK_FLAG_INSTOCK);
...
//			记录出错日志
		...
		LOGGER.trace("************************ 结束执行车辆出发、到达、取消出发、取消到达时需要更新交接单配载单的状态 JOB ************************");
	}
}
 ...
 *  
 *  FILE PATH     : src/main/java/com/deppon/foss/module/transfer/trucktask/server/job/TruckTaskTransferCenterJob.java
 *  
 *  FILE NAME          :TruckTaskTransferCenterJob.java
 * 
 *  AUTHOR  : FOSS中转系统开发组
 *  
 *  TIME              : 
...
/**
 * 车辆出发、到达、取消出发、取消到达时需要更新走货路径的状态
 * @author foss-liubinbin
 * @date 2012-11-27 下午4:16:57
 */
public class TruckTaskTransferCenterJob extends GridJob implements StatefulJob{
	
	private final static Logger LOGGER = LoggerFactory.getLogger(TruckTaskTransferCenterJob.class);

	/** 
	 * JOB定时执行车辆出发、到达、取消出发、取消到达时需要更新走货路径的状态
	...
	protected void doExecute(JobExecutionContext context) throws JobExecutionException {
		LOGGER.trace("************************ 开始执行车辆出发、到达、取消出发、取消到达时需要更新走货路径的状态JOB ************************");
...
//			1、取出上次成功执行JOB后的业务截止时间
			jobProcess = tfrCommonService.queryLastExecutedJobInfo(TfrJobBusinessTypeEnum.TRUCK_TASK_TRANSFER_CENTER_JOB, scheduledFireTime, threadNo, threadCount);
...
//			 2.2、执行成功后，若存在明细都已处理完毕的清仓差异报告，需更新这些清仓报告状态为TransferConstants.STOCK_CHECKING_REPORT_DONE
			sharedService.refreshDataForActionTask(DepartureConstant.TASK_FLAG_CALCULATE_TRANSPORT_PATH);
...
//			记录出错日志
			...
			jobProcessLogEntity.setBizName(TfrJobBusinessTypeEnum.TRUCK_TASK_TRANSFER_CENTER_JOB.getBizName());
			jobProcessLogEntity.setBizCode(TfrJobBusinessTypeEnum.TRUCK_TASK_TRANSFER_CENTER_JOB.getBizCode());
		...
		LOGGER.trace("************************ 结束执行车辆出发、到达、取消出发、取消到达时需要更新走货路径的状态JOB ************************");
	}
}
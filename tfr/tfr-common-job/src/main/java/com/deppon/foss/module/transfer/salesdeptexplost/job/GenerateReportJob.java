...
public class GenerateReportJob extends BaseStateFulJob {
	...
	protected void doExecute(JobExecutionContext context)
		...
		try {
			...
			processEntity = tfrCommonService.queryJobInfo(
					TfrJobBusinessTypeEnum.SALES_DEPT_EXP_LOST_REPORT,
					scheduledFireTime, threadNo, threadCount);

			Date bizStartTime = new Date();

			salesDeptExpLostService.generateReport(threadCount, threadNo);
			...
			logEntity.setBizName(TfrJobBusinessTypeEnum.SALES_DEPT_EXP_LOST_REPORT
					.getBizName());
			logEntity.setBizCode(TfrJobBusinessTypeEnum.SALES_DEPT_EXP_LOST_REPORT
					.getBizCode());

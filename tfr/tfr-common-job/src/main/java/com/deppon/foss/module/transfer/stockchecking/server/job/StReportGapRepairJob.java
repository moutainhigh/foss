 ...
 * 3、由于数据量比较大，在数据库中多保留一个执行间隔时间参数，到达此时间后，结束执行JOB
 * 4、以后再考虑是否分片
 * </pre>
 * @author foss-wuyingjie
 * @date 2013-07-03 上午8:16:57
 */
public class StReportGapRepairJob extends BaseStateFulJob{
	...
	protected void doExecute(JobExecutionContext context) throws JobExecutionException {
		LOGGER.trace("************************ 开始执行清仓、卸车差异修复 JOB ************************");
		...
		try {
			...
			timeInterval = Integer.valueOf((String) context.getMergedJobDataMap().get("timeInterval")).intValue();
			Date scheduledFireTime = context.getScheduledFireTime();
			
//			1、取出上次成功执行JOB后的业务截止时间
			jobProcess = tfrCommonService.queryJobInfo(TfrJobBusinessTypeEnum.ST_REPORT_GAP_REPAIR, scheduledFireTime, threadNo, threadCount);
			
//			2、执行业务任务
			jobStartTime = Calendar.getInstance().getTime();
			stReportService.executeStReportGapRepair(jobProcess.getBizEndTime(), scheduledFireTime, threadNo, threadCount, timeInterval);
...
//			3、更新任务完成状态及时间
			...
			LOGGER.error("GapRepairJob error", e);
...
//			记录出错日志
		...
		LOGGER.trace("************************ 结束执行清仓、卸车差异修复 JOB ************************");
	}
}
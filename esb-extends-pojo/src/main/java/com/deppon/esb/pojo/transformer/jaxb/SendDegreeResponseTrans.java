...
/**
 * SendDegreeResponse的转换类.
 * 
 * @author HuangHua
 */
public class SendDegreeResponseTrans implements IMessageTransform<SendDegreeResponse> {
	 ...
	 */
	public SendDegreeResponse toMessage(String string) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
		...
		try {
			...
			JAXBElement<SendDegreeResponse> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	 ...
	 */
	public String fromMessage(SendDegreeResponse value) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
				throw new ESBConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<SendDegreeResponse> element = new com.deppon.uums.inteface.domain.usermanagement.ObjectFactory().createSendDegreeResponse(value);

...
/**
 * SendDegreeRequest的转换类.
 * 
 * @author HuangHua
 */
public class SendDegreeRequestTrans implements IMessageTransform<SendDegreeRequest> {
	 ...
	 */
	public SendDegreeRequest toMessage(String string) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
		...
		try {
			...
			JAXBElement<SendDegreeRequest> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	 ...
	 */
	public String fromMessage(SendDegreeRequest value) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
				throw new ESBConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<SendDegreeRequest> element = new com.deppon.uums.inteface.domain.usermanagement.ObjectFactory().createSendDegreeRequest(value);

...
public class ThrowfreightRequestTrans implements IMessageTransform<ThrowfreightRequest> {
	...
	/** The Constant CLZZ. */
	private static final Class<ThrowfreightRequest> CLZZ =  ThrowfreightRequest.class;

	/** The log. */
	private static Log log = LogFactory.getLog(ThrowfreightRequest.class);
	...
	public ThrowfreightRequest toMessage(String string) throws ESBConvertException {
		...
		try {
			...
			JAXBElement<ThrowfreightRequest> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	 ...
	 */
	public String fromMessage(ThrowfreightRequest value) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
				throw new ESBConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<ThrowfreightRequest> element = new com.deppon.esb.inteface.domain.wfs.ObjectFactory().createThrowFreightRequest(value);

...
/**
 * SendPositionRequest的转换类.
 * 
 * @author HuangHua
 */
public class SendPositionRequestTrans implements IMessageTransform<SendPositionRequest> {
	 ...
	 */
	public SendPositionRequest toMessage(String string) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
		...
		try {
			...
			JAXBElement<SendPositionRequest> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	 ...
	 */
	public String fromMessage(SendPositionRequest value) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
				throw new ESBConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<SendPositionRequest> element = new com.deppon.uums.inteface.domain.usermanagement.ObjectFactory().createSendPositionRequest(value);

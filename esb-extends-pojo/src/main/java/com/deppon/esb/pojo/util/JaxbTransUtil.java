...
/**
 * 转换类生成器，如果需要生成的文件已经存在，会自动跳过。 注意： 1.会自动扫描生成文件夹下的所有文件，因为有些问题不是顶层的业务对象，
 * 因此在生成之后会报错，需要手动删除报错的文件 2.某些命名不规范的java对象可能会无报错，需要手动修正（如ESBHeader）
 * 
 * @author HuangHua
 * 
 */
public class JaxbTransUtil {
	...
	/** The log. */
	private static Log log = LogFactory.getLog(JaxbTransUtil.class);
	// 需要扫描的文件路径
	/** The Constant FILE_PATH. */
	private static final String FILE_PATH = "O:/trunk/CODE/esb-extends/esb-extends-pojo/target/generated-sources/xjc";
	// 需要保存的文件路径
	/** The Constant SAVE_FILE_PATH_PREFIX. */
	private static final String SAVE_FILE_PATH_PREFIX = System
			.getProperty("user.dir")
			+ "/src/main/java/com/deppon/esb/pojo/transformer/jaxb/";
	// 文件后缀，如xxxTrans.java
	/** The Constant SUFFIX. */
	...
	private static final String MODEL_FILE = "O:/trunk/CODE/esb-extends/esb-extends-pojo/src/main/resources/jaxb-model.txt";

	/** The Constant EXCLUDE_FILE_NAME. */
	private static final String[] EXCLUDE_FILE_NAME = new String[] {
	...
	public static void main(String[] a) {
		...
		log.info("tran file size :" + list.size());
	...
	public static String fillParam(String content, File file) {
		String regStr = "target\\generated-sources\\xjc";
		...
		String packageName = fullClassName.substring(0,
				fullClassName.lastIndexOf(className) - 1);
		// 替换
		...
		for (File f : files) {
			...
			if (f.isDirectory()) {
				...
				if (f.getAbsolutePath().endsWith(filter)) {
					boolean flag = true;
					for (int i = 0; i < EXCLUDE_FILE_NAME.length; i++) {
						if (EXCLUDE_FILE_NAME[i].equals(f.getName())) {
							flag = false;
						}
					}
					if (flag) {

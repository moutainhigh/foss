...
/**
 * sendWscWayBillRequest的转换类.
 * 
 * @author yangqiang
 */
public class SendWscWayBillRequestTrans implements IMessageTransform<SendWscWayBillRequest> {

	/** The Constant CLZZ. */
	private static final Class<SendWscWayBillRequest> CLZZ =  SendWscWayBillRequest.class;

	/** The log. */
	private static Log log = LogFactory.getLog(SendWscWayBillRequest.class);
	 ...
	 */
	public SendWscWayBillRequest toMessage(String string) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
		...
		try {
			...
			JAXBElement<SendWscWayBillRequest> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	 ...
	 */
	public String fromMessage(SendWscWayBillRequest value) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
				throw new ESBConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<SendWscWayBillRequest> element = new ObjectFactory().createSendWscWayBillRequest(value);

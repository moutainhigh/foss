...
public class ThrowfreightResponseTrans implements IMessageTransform<ThrowfreightResponse> {
	...
	/** The Constant CLZZ. */
	private static final Class<ThrowfreightResponse> CLZZ =  ThrowfreightResponse.class;

	/** The log. */
	private static Log log = LogFactory.getLog(ThrowfreightResponse.class);
	...
	public ThrowfreightResponse toMessage(String string) throws ESBConvertException {
		...
		try {
			...
			JAXBElement<ThrowfreightResponse> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	 ...
	 */
	public String fromMessage(ThrowfreightResponse value) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
				throw new ESBConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<ThrowfreightResponse> element = new com.deppon.esb.inteface.domain.wfs.ObjectFactory().createThrowFreightResponse(value);

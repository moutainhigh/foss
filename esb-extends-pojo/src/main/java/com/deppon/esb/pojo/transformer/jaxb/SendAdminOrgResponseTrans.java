...
/**
 * SendAdminOrgResponse的转换类.
 * 
 * @author HuangHua
 */
public class SendAdminOrgResponseTrans implements IMessageTransform<SendAdminOrgResponse> {
	 ...
	 */
	public SendAdminOrgResponse toMessage(String string) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
		...
		try {
			...
			JAXBElement<SendAdminOrgResponse> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	 ...
	 */
	public String fromMessage(SendAdminOrgResponse value) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
				throw new ESBConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<SendAdminOrgResponse> element = new com.deppon.uums.inteface.domain.usermanagement.ObjectFactory().createSendAdminOrgResponse(value);

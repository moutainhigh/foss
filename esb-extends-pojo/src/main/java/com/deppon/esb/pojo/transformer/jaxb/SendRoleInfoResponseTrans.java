...
/**
 * SendRoleInfoResponse的转换类.
 * 
 * @author HuangHua
 */
public class SendRoleInfoResponseTrans implements IMessageTransform<SendRoleInfoResponse> {
	 ...
	 */
	public SendRoleInfoResponse toMessage(String string) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
		...
		try {
			...
			JAXBElement<SendRoleInfoResponse> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	 ...
	 */
	public String fromMessage(SendRoleInfoResponse value) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
				throw new ESBConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<SendRoleInfoResponse> element = new com.deppon.uums.inteface.domain.usermanagement.ObjectFactory().createSendRoleInfoResponse(value);

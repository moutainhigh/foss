...
/**
 * SendAdminOrgRequest的转换类.
 * 
 * @author HuangHua
 */
public class SendAdminOrgRequestTrans implements IMessageTransform<SendAdminOrgRequest> {
	 ...
	 */
	public SendAdminOrgRequest toMessage(String string) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
		...
		try {
			...
			JAXBElement<SendAdminOrgRequest> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	 ...
	 */
	public String fromMessage(SendAdminOrgRequest value) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
				throw new ESBConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<SendAdminOrgRequest> element = new com.deppon.uums.inteface.domain.usermanagement.ObjectFactory().createSendAdminOrgRequest(value);

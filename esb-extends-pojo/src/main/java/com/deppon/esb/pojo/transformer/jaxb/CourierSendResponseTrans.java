 ...
 * @author HuangHua
 */
public class CourierSendResponseTrans implements IMessageTransform<CourierSend2OmsResponse> {

	/** The Constant CLZZ. */
	private static final Class<CourierSend2OmsResponse> CLZZ =  CourierSend2OmsResponse.class;

	/** The log. */
	private static Log log = LogFactory.getLog(CourierSend2OmsResponse.class);
	 ...
	 */
	public CourierSend2OmsResponse toMessage(String string) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
		...
		try {
			...
			JAXBElement<CourierSend2OmsResponse> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	 ...
	 */
	public String fromMessage(CourierSend2OmsResponse value) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
				throw new ESBConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<CourierSend2OmsResponse> element = new com.deppon.esb.inteface.domain.oms.ObjectFactory().createCourierSend2OmsResponse(value);

...
/**
 * SyncDistrictResponse的转换类.
 * 
 * @author HuangHua
 */
public class SyncDistrictResponseTrans implements IMessageTransform<SyncDistrictResponse> {
	 ...
	 */
	public SyncDistrictResponse toMessage(String string) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
		...
		try {
			...
			JAXBElement<SyncDistrictResponse> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	 ...
	 */
	public String fromMessage(SyncDistrictResponse value) throws ESBConvertException {
		if(context == null){
			initContext();//再次尝试一次
			if (context == null) {
				throw new ESBConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<SyncDistrictResponse> element = new com.deppon.ows.inteface.domain.ObjectFactory().createSyncDistrictResponse(value);

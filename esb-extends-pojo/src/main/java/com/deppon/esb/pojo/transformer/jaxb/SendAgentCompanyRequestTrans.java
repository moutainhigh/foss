...
public class SendAgentCompanyRequestTrans implements
		IMessageTransform<SendAgentCompanyRequest> {
	/** The Constant CLZZ. */
	private static final Class<SendAgentCompanyRequest> CLZZ =SendAgentCompanyRequest.class;
	/** The log. */
	private static Log log = LogFactory.getLog(SendAgentCompanyRequest.class);
	...
	public SendAgentCompanyRequest toMessage(String string) throws ESBConvertException {
		...
		try {
			...
			JAXBElement<SendAgentCompanyRequest> element = unmarshaller.unmarshal(new StreamSource(stream),CLZZ);
	 ...
	 */
	@Override
	public String fromMessage(SendAgentCompanyRequest message)
			throws ESBConvertException {
		if(null ==context){
			//再初始化一遍
			initContext();
		...
		try {
			...
			JAXBElement<SendAgentCompanyRequest> element = new com.deppon.foss.inteface.domain.usermanagements.ObjectFactory().createSendAgentCompanyRequest(message);

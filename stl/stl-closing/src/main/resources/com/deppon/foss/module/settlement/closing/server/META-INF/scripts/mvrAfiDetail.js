closing.mvrAfiDetail.MVR_AFI_DETAIL_QUERY_MAX = 31;  //时间间隔最大不超过31天

/**
 * 查询
 */
closing.mvrAfiDetail.queryReport = function(form,me){
	//判断是否合法
	if(form.isValid()){
		var grid = Ext.getCmp('T_closing-mvrAfiDetail_content').getGrid();
		//设置查询条件
		closing.mvrAfiDetail.startDate = form.findField('startDate').getValue();
		closing.mvrAfiDetail.endDate= form.findField('endDate').getValue();
		closing.mvrAfiDetail.productType= form.findField('productType').getValue();
		closing.mvrAfiDetail.commoncustomerselector= form.findField('commoncustomerselector').getValue();
		closing.mvrAfiDetail.commonvehagencycompselector= form.findField('commonvehagencycompselector').getValue();
		closing.mvrAfiDetail.commonairlinesselector= form.findField('commonairlinesselector').getValue();
		closing.mvrAfiDetail.commonairagencycompanyselector= form.findField('commonairagencycompanyselector').getValue();
		closing.mvrAfiDetail.origOrgCode= form.findField('origOrgCode').getValue();
		closing.mvrAfiDetail.destOrgCode= form.findField('destOrgCode').getValue();
		closing.mvrAfiDetail.typeCode= form.findField('typeCode').getValue();
		closing.mvrAfiDetail.subTypeCode= form.findField('subTypeCode').getValue();
		
		closing.mvrAfiDetail.customerCode = "";
		if(closing.mvrAfiDetail.commoncustomerselector != null && closing.mvrAfiDetail.commoncustomerselector != ""){
			closing.mvrAfiDetail.customerCode = closing.mvrAfiDetail.commoncustomerselector;
		}else if(closing.mvrAfiDetail.commonvehagencycompselector != null && closing.mvrAfiDetail.commonvehagencycompselector != ""){
			closing.mvrAfiDetail.customerCode = closing.mvrAfiDetail.commonvehagencycompselector;
		}else if(closing.mvrAfiDetail.commonairlinesselector != null && closing.mvrAfiDetail.commonairlinesselector != ""){
			closing.mvrAfiDetail.customerCode = closing.mvrAfiDetail.commonairlinesselector;
		}else if(closing.mvrAfiDetail.commonairagencycompanyselector != null && closing.mvrAfiDetail.commonairagencycompanyselector != ""){
			closing.mvrAfiDetail.customerCode = closing.mvrAfiDetail.commonairagencycompanyselector;
		}else{
			closing.mvrAfiDetail.customerCode = "";
		}
		if(closing.mvrAfiDetail.startDate==null || closing.mvrAfiDetail.startDate==''){
			Ext.Msg.alert('温馨提示','开始日期不能为空');
			return false;
		}

		if(closing.mvrAfiDetail.endDate==null || closing.mvrAfiDetail.endDate==''){
			Ext.Msg.alert('温馨提示','结束日期不能为空');
			return false;
		}
		var compareTwoDate = stl.compareTwoDate(closing.mvrAfiDetail.startDate,closing.mvrAfiDetail.endDate);
		if(compareTwoDate>closing.mvrAfiDetail.MVR_AFI_DETAIL_QUERY_MAX){
 ...
 * 重置
 */
closing.mvrAfiDetail.reset = function(){
 ...
 * 导出
 */
closing.mvrAfiDetail.exportRfo = function(){

	var me = this;
	//获取主面板、查询GRID
	var mainPane = Ext.getCmp('T_closing-mvrAfiDetail_content');
	...
	Ext.Msg.confirm('温馨提示','确定要导出报表?',function(btn,text){
		if('yes' == btn){
			...
			var params  = {
				'vo.dto.startDate':closing.mvrAfiDetail.startDate,	
				'vo.dto.endDate':closing.mvrAfiDetail.endDate,	
				'vo.dto.productType':closing.mvrAfiDetail.productType,
				'vo.dto.customerCode':closing.mvrAfiDetail.customerCode,
				'vo.dto.origOrgCode':closing.mvrAfiDetail.origOrgCode,
				'vo.dto.destOrgCode':closing.mvrAfiDetail.destOrgCode,
				'vo.dto.typeCode':closing.mvrAfiDetail.typeCode,
				'vo.dto.subTypeCode':closing.mvrAfiDetail.subTypeCode,
				'vo.dto.rptType':mvrDetail.rptType_AFI
			};
			...
			Ext.Ajax.request({
				...
				success:function(response){
					...
					if(result.syncExport){
						...
						//创建一个form
						if(!Ext.fly('exportmvrAfiDetailForm')){
						...
						Ext.Ajax.request({
							...
							form: Ext.fly('exportmvrAfiDetailForm'),
							params:params,
							method:'post',
							isUpload: true,
							timeout:3*60*1000,
...
Ext.define('Foss.mvrAfiDetail.Store',{
	...
	listeners:{
		'beforeLoad':function(store, operation, eOpts){
			var form = Ext.getCmp('T_closing-mvrAfiDetail_content').getForm().getForm();
			//如果用户直接点击分页查询，即没点击查询按钮，则在此处获取查询条件
			if(Ext.isEmpty(closing.mvrAfiDetail.startDate)){
				closing.mvrAfiDetail.startDate = form.findField('startDate').getValue();
				closing.mvrAfiDetail.endDate= form.findField('endDate').getValue();
				closing.mvrAfiDetail.productType= form.findField('productType').getValue();
				closing.mvrAfiDetail.commoncustomerselector= form.findField('commoncustomerselector').getValue();
...
Ext.define('Foss.mvrAfiDetail.QueryForm',{
	...
	items:[{
		...
		items:[{
			text:'重置',
			columnWidth:.08,
			handler:closing.mvrAfiDetail.reset
		},{
			xtype:'container',
			border:false,
			html:'&nbsp;',
			...
			handler:function(){
    			...
    			var me = this;
    			closing.mvrAfiDetail.queryReport(form,me)
    		}
		}]
  	}]
});
	...
	getExportButton:function(){
		...
		if(Ext.isEmpty(me.exportButton)){
			me.exportButton = Ext.create('Ext.Button',{
				text:'导出',
				height:20,
				handler:closing.mvrAfiDetail.exportRfo,
				disabled:!closing.mvrAfiDetail.isPermission('/stl-web/closing/exportMvrDetail.action'),
				hidden:!closing.mvrAfiDetail.isPermission('/stl-web/closing/exportMvrDetail.action')
			});
		}
...
Ext.onReady(function() {
	...
	Ext.create('Ext.panel.Panel', {
		id : 'T_closing-mvrAfiDetail_content',
		...
		layout : 'auto',
		items : [form,grid],
		renderTo : 'T_closing-mvrAfiDetail-body'
	});
});
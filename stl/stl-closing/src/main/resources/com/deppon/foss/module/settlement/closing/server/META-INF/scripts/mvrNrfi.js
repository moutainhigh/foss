 ...
 * 获取上个月期间
 */
closing.mvrNrfi.getLastMonthPeriod = function(){
	var now  = new Date();
 ...
 * 声明账期model
 */
Ext.define('Foss.mvrNrfi.PeriodModel',{
 ...
 * 声明账期store
 */
Ext.define('Foss.mvrNrfi.PeriodStore',{
	extend:'Ext.data.Store',
	model:'Foss.mvrNrfi.PeriodModel',
	proxy:{
		type:'ajax',
...
closing.mvrNrfi.queryMvrNrfiByConditions = function(me){
	var m =Ext.getCmp('T_closing-mvrNrfi_content');
...
closing.mvrNrfi.exportMvrNrfi = function(){
	//获取主面板、查询GRID
	var mainPane = Ext.getCmp('T_closing-mvrNrfi_content');
	...
	Ext.Msg.confirm('温馨提示','确定导出始发专线往来月报表吗?',function(btn,text){
		if('yes' == btn){
			...
			//创建一个form
			if(!Ext.fly('exportMvrNrfiForm')){
				var frm = document.createElement('form');
				frm.id = 'exportMvrNrfiForm';
			...
			//导出Ajax请求
			Ext.Ajax.request({
				url:closing.realPath('mvrNrfiExport.action'), 
				form: Ext.fly('exportMvrNrfiForm'),
				params:params,
				method:'post',
				isUpload: true,
 ...
 * 获取期间控件下拉框Store
 */
closing.mvrNrfi.getComboPeriodStore = function() {
	var now = new Date();
	...
	return Ext.create('Ext.data.Store', {
				model : 'Foss.closing.mvrNrfiComboModel',
				data : periods
			});
}
 ...
 * 获取业务类型下拉框Store
 */
closing.mvrNrfi.getComboProductTypeStore = function() {
...
// 下拉框(combo)数据模型
Ext.define('Foss.closing.mvrNrfiComboModel', {
			extend : 'Ext.data.Model',
			fields : [{
						/* 显示名 */
						name : 'name',
...
// 空运月报表数据模型
Ext.define('Foss.closing.mvrNrfiModel', {
			extend : 'Ext.data.Model',
			fields : [
			          {name:'id',                   type:'string'},
...
//Store
Ext.define('Foss.closing.mvrNrfiStore',{
	extend:'Ext.data.Store',
	model:'Foss.closing.mvrNrfiModel',  
	pageSize:100,
	proxy : {
		type : 'ajax',
		actionMethods : 'post',
		url :closing.realPath('queryMvrNrfi.action'),
		timeout:10*60*1000,
		reader : {
			type : 'json',
	...
	constructor:function(config){
		...
		me.listeners = {
	   		'beforeload': function(store, operation, eOpts){
        			var searchParams;
        			var form = Ext.getCmp('T_closing-mvrNrfi_content').getQueryTab();
...
//定义查询Form
Ext.define('Foss.closing.mvrNrfiQueryForm', {
			...
			items : [{
				...
				fieldLabel:'查询期间',
				queryMode: 'remote', 
		    	store:Ext.create('Foss.mvrNrfi.PeriodStore'),
				displayField:'name',
				valueField:'name',
				allowBlank : false,
				columnWidth : .3
				...
				store:FossDataDictionary.getDataDictionaryStore('SETTLEMENT__CUSTOMER_TYPE',null,null,
						[closing.mvrNrfi.SETTLEMENT__CUSTOMER_TYPE__LINE_CUSTOMER,
						 closing.mvrNrfi.SETTLEMENT__CUSTOMER_TYPE__AIR,
						 closing.mvrNrfi.SETTLEMENT__CUSTOMER_TYPE__AIR_AGENCY]),
				queryModel:'local',
				displayField:'valueName',
				valueField:'valueCode',
				value:closing.mvrNrfi.SETTLEMENT__CUSTOMER_TYPE__LINE_CUSTOMER,
		    	columnWidth:.3,
		    	listeners:{
				...
				items : [{
							...
							handler:function(){
								...
								if(form.getForm().isValid()){
									closing.mvrNrfi.queryMvrNrfiByConditions(me);
								}else{
...
// 空运月报表查询Grid
Ext.define('Foss.closing.mvrNrfiQueryGrid', {
	...
	getExportButton:function(){
		...
		if(Ext.isEmpty(me.exportButton)){
			me.exportButton = Ext.create('Ext.Button',{
				text:'导出',
				height:20,
				handler:closing.mvrNrfi.exportMvrNrfi,
				disabled:!closing.mvrNrfi.isPermission('/stl-web/closing/mvrNrfiExport.action'),
				hidden:!closing.mvrNrfi.isPermission('/stl-web/closing/mvrNrfiExport.action')
			});
		}
	...
	constructor : function(config) {
		...
		me.store = Ext.create('Foss.closing.mvrNrfiStore');
...
Ext.onReady(function() {
			...
			if (Ext.getCmp('T_closing-mvrNrfi_content')) {
				return;
			}

			// 查询FORM
			var queryForm = Ext.create('Foss.closing.mvrNrfiQueryForm');
			
			//显示grid
			var queryGrid = Ext.create('Foss.closing.mvrNrfiQueryGrid');

			// 显示到JSP页面
			Ext.create('Ext.panel.Panel', {
						id : 'T_closing-mvrNrfi_content',
						...
						renderTo : 'T_closing-mvrNrfi-body'
					});
		});
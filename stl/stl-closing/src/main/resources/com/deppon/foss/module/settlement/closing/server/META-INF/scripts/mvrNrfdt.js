 ...
 * 声明账期model
 */
Ext.define('Foss.mvrNrfdt.PeriodModel',{
 ...
 * 声明账期store
 */
Ext.define('Foss.mvrNrfdt.PeriodStore',{
	extend:'Ext.data.Store',
	model:'Foss.mvrNrfdt.PeriodModel',
	proxy:{
		type:'ajax',
 ...
 * 查询方法
 */
closing.mvrNrfdt.queryMvrNrfdt=function(form,me){
	var m=Ext.getCmp('T_closing-mvrNrfdt_content');
	if(m){
 ...
 * 导出专线到达月报表
 */
closing.mvrNrfdt.exportMvrNrfdt = function(){
	//获取主面板、查询GRID
	var mainPane = Ext.getCmp('T_closing-mvrNrfdt_content');
	...
	Ext.Msg.confirm('温馨提示','确定导出02到达月报表吗?',function(btn,text){
		if('yes' == btn){
			...
			Ext.apply(params,{ 
				'mvrNrfdtVo.productCodeList':stl.convertProductCode(mainPane.getQueryForm().getForm().findField('mvrNrfdtVo.productCodeList').getValue())
			});
			
			//创建一个form
			if(!Ext.fly('exportMvrNrfdtForm')){
				var frm = document.createElement('form');
				frm.id = 'exportMvrNrfdtForm';
			...
			//导出Ajax请求
			Ext.Ajax.request({
				url:closing.realPath('mvrNrfdtExport.action'), 
				form: Ext.fly('exportMvrNrfdtForm'),
				params:params,
				method:'post',
				isUpload: true,
 ...
 * 获取期间控件下拉框Store
 */
closing.mvrNrfdt.getComboPeriodStore = function() {
 ...
 * 获取业务类型下拉框Store
 */
closing.mvrNrfdt.getComboProductTypeStore = function() {
...
// 下拉框(combo)数据模型
Ext.define('Foss.closing.MvrNrfdtComboModel', {
			extend : 'Ext.data.Model',
			fields : [{
						/* 显示名 */
						name : 'name',
...
// 专线到达月报表数据模型
Ext.define('Foss.closing.MvrNrfdtModel', {
			extend : 'Ext.data.Model',
			fields : [
			    {name:'id',type:'string'},
			    ...
			    {name:'cptDestPayApply',             type:'decimal'},
...
//Store
Ext.define('Foss.closing.MvrNrfdtStore',{
	extend:'Ext.data.Store',
	model:'Foss.closing.MvrNrfdtModel',  
	pageSize:100,
	proxy : {
		type : 'ajax',
		actionMethods : 'post',
		url :closing.realPath('queryMvrNrfdt.action'),
		timeout:10*60*1000,
		reader : {
			type : 'json',
	...
	constructor:function(config){
		...
		me.listeners = {
	   		...
	   		var queryForm = Ext.getCmp('T_closing-mvrNrfdt_content').getQueryForm();
			if (queryForm) {
				var form = queryForm.getForm();
				var params = form.getValues();
				
				Ext.apply(params,{ 
					'mvrNrfdtVo.productCodeList':stl.convertProductCode(form.findField('mvrNrfdtVo.productCodeList').getValue())
				});
...
// 定义查询Form
Ext.define('Foss.closing.MvrNrfdtQueryForm', {
			...
			items : [{
				...
				triggerAction : 'all',
				editable : false,
				value : closing.mvrNrfdt.getComboPeriodStore().getAt(1)
						.get('value'),
				store : closing.mvrNrfdt.getComboPeriodStore(),*/
				queryMode: 'remote', 
		    	store:Ext.create('Foss.mvrNrfdt.PeriodStore'),
				displayField:'name',
				valueField:'name',
				allowBlank : false,
				columnWidth : .33
				...
				triggerAction : 'all',
				editable : false,
				value : closing.mvrNrfdt.getComboProductTypeStore().first()
						.get('code'),
				store : closing.mvrNrfdt.getComboProductTypeStore(),
				columnWidth : .33
			}, {
				...
				items : [{
							...
							handler:function(){
								...
								var me = this;
								if(form.isValid()){
									closing.mvrNrfdt.queryMvrNrfdt(form,me);
								}else{
...
// 空运月报表查询Grid
Ext.define('Foss.closing.MvrNrfdtQueryGrid', {
	...
	getExportButton:function(){
		...
		if(Ext.isEmpty(me.exportButton)){
			me.exportButton = Ext.create('Ext.Button',{
				text:'导出',
				height:20,
				handler:closing.mvrNrfdt.exportMvrNrfdt,
				disabled:!closing.mvrNrfdt.isPermission('/stl-web/closing/mvrNrfdtExport.action'),
				hidden:!closing.mvrNrfdt.isPermission('/stl-web/closing/mvrNrfdtExport.action')
			});
		}
	...
	constructor : function(config) {
		...
		me.store = Ext.create('Foss.closing.MvrNrfdtStore');
...
Ext.onReady(function() {
			...
			if (Ext.getCmp('T_closing-mvrNrfdt_content')) {
				return;
			}

			// 查询FORM
			var queryForm = Ext.create('Foss.closing.MvrNrfdtQueryForm');
			
			//显示grid
			var queryGrid = Ext.create('Foss.closing.MvrNrfdtQueryGrid');

			// 显示到JSP页面
			Ext.create('Ext.panel.Panel', {
						id : 'T_closing-mvrNrfdt_content',
						...
						renderTo : 'T_closing-mvrNrfdt-body'
					});
		});
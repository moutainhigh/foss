...
public class MvrSnrfdtEntityAction extends AbstractAction{

	/** The Constant LOGGER. */
	private static final Logger LOGGER = LogManager
			.getLogger(MvrSnrfdtEntityAction.class);
	
	/** The Constant serialVersionUID. */
	private static final long serialVersionUID = 1L;
	
	/** 02特殊到达月报表Service. */
	private IMvrSnrfdtEntityService mvrSnrfdtEntityService;
	 ...
	 * @return the string
	 * @author shenweihua
	 * @date 2014-06-12 下午3:39:28
	 */
	@JSON
	public String querySnrfdtByConditions() {
		...
		try {
			...
			// 查询专线到达报表合计
			MvrNrfdtDto totalDto = mvrSnrfdtEntityService
			...
			if (null != totalDto
				...
				// 查询专线到达报表
				mvrNrfdtEntityList = mvrSnrfdtEntityService.selectByConditions(dto,
						start, limit);
	 ...
	 * @return
	 */
	public String exportSnrfdtByConditions() {
		...
		try {
			...
			// 02到达月报表_+账期构成
			try {
				String exportExeclName = "02特殊到达月报表" + mvrNrfdtVo.getPeriod();				
			...
			for (int i = SettlementReportNumber.ELEVEN; i <= SettlementReportNumber.THIRTY_TWO; i++) {
				map.put(i, "float");
			}
			...
			// 记录日志
			LOGGER.error("02特殊到达月报表导出异常：" + e.getErrorCode(), e);
			// 异常返回
			return returnError("02特殊到达月报表导出异常：" + e.getErrorCode());
			// 异常处理
		} catch (Exception e) {
			// 记录日志
			LOGGER.error("02特殊到达月报表导出异常：" + e.getMessage(), e);
			// 异常返回
			return returnError("02特殊到达月报表导出异常：" + e.getMessage());
			// 异常处理
		}
	}
	...
	private ExportResource exportRfdResource(List<MvrNrfdtEntity> mvrNrfdtEntityList) {
		...
		HeaderRows rowContent4 = new HeaderRows(SettlementReportNumber.ZERO, SettlementReportNumber.ONE, SettlementReportNumber.ELEVEN, SettlementReportNumber.TWELVE, "预收客户"); // 4
		HeaderRows rowContent5 = new HeaderRows(SettlementReportNumber.ZERO, SettlementReportNumber.ONE, SettlementReportNumber.FIFTEEN, SettlementReportNumber.TWENTY_TWO, "理赔"); // 8
		HeaderRows rowContent7 = new HeaderRows(SettlementReportNumber.ZERO, SettlementReportNumber.ONE, SettlementReportNumber.TWENTY_THREE, SettlementReportNumber.THIRTY, "退运费"); // 8
		HeaderRows rowContent8 = new HeaderRows(SettlementReportNumber.ZERO, SettlementReportNumber.ONE, SettlementReportNumber.THIRTY_ONE, SettlementReportNumber.THIRTY_TWO, "服务补救"); // 2
	...
	public void setMvrSnrfdtEntityService(
			IMvrSnrfdtEntityService mvrSnrfdtEntityService) {
		this.mvrSnrfdtEntityService = mvrSnrfdtEntityService;
	}

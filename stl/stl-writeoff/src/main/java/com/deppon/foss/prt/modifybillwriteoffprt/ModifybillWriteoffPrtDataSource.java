package com.deppon.foss.prt.modifybillwriteoffprt;
 ...
 * @date 2013-4-11 下午5:07:11
 */
public class ModifybillWriteoffPrtDataSource implements JasperDataSource {
	...
	public Map<String, Object> createParameterDataSource(
		...
		if (jasperContext != null && jasperContext.getParamMap() != null) {
			...
			if (StringUtils.isBlank(waybillChangeId)) {
				throw new SettlementException("更改单打印时，传入的更改单参数错误 id为空！");
			}
			//获取更改单Service实例
			...
			if(null==dto){
				//查询参数不正确
				throw new SettlementException("更改单打印时，根据传入的更改单id,未找到更改单的记录！");
			}

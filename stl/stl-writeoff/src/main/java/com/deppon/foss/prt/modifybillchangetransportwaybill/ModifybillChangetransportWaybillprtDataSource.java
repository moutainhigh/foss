package com.deppon.foss.prt.modifybillchangetransportwaybill;
 ...
 */
public class ModifybillChangetransportWaybillprtDataSource implements JasperDataSource {
...
// 注入日志
	...
	public Map<String, Object> createParameterDataSource(
		...
		if (jasperContext != null && jasperContext.getParamMap() != null) {
			...
			if (StringUtils.isBlank(waybillChangeId)) {
				throw new SettlementException("更改单转运单打印，传入的更改单参数错误 id为空！");
			}
			//获取更改单Service实例
			...
			LOGGER.info("根据ID获取更改单service");
			//根据ID获取更改单service
			dto = waybillChangeService.queryWaybillRfcPrintDtoByRfcid(waybillChangeId);
			if(null==dto){
				//查询参数不正确
				throw new SettlementException("更改单转运单打印，根据传入的更改单id:"+waybillChangeId+"未找到更改单的记录！");
			}
	...
	public List<Map<String, Object>> createDetailDataSource(
		...
		LOGGER.info("更改单转运单打印明细,开始填充数据开始.......");

...
writeoff.statementPrint = function(){
	...
	Ext.MessageBox.show({
      ...
      fn: function(e){
      	...
      	if(e=='yes'){
      		//默认显示列
				var arrayColumns = ['sourceBillNo','waybillNo','billParentType','billType',
				...
				targetObject.columnName1 = '<ext:i18nForJsp key="foss.stl.writeoff.statementAdd.number"/>';
				targetObject.columnName2 = '<ext:i18nForJsp key="foss.stl.writeoff.common.waybillNo"/>';
				targetObject.columnName3 = '<ext:i18nForJsp key="foss.stl.writeoff.statementAdd.billParentType"/>';
				targetObject.columnName4 = '<ext:i18nForJsp key="foss.stl.writeoff.common.billType"/>';
				targetObject.columnName5 = '<ext:i18nForJsp key="foss.stl.writeoff.common.customerCode"/>';
				targetObject.columnName6 = '<ext:i18nForJsp key="foss.stl.writeoff.common.customerName"/>';
				targetObject.columnName7 = '<ext:i18nForJsp key="foss.stl.writeoff.common.totalAmount"/>';
				targetObject.columnName8 = '<ext:i18nForJsp key="foss.stl.writeoff.common.verifyAmount"/>';
				targetObject.columnName9 = '<ext:i18nForJsp key="foss.stl.writeoff.common.originalUnverifyAmount"/>';
				targetObject.columnName10 = '<ext:i18nForJsp key="foss.stl.writeoff.common.orgCode"/>';
				targetObject.columnName11 = '<ext:i18nForJsp key="foss.stl.writeoff.common.orgName"/>';
				targetObject.columnName12 = '<ext:i18nForJsp key="foss.stl.writeoff.common.businessDate"/>';
				targetObject.columnName13 = '<ext:i18nForJsp key="foss.stl.writeoff.common.accountDate"/>';
				targetObject.columnName14 = '<ext:i18nForJsp key="foss.stl.writeoff.statementAdd.paymentType"/>';
				targetObject.columnName15 = '<ext:i18nForJsp key="foss.stl.writeoff.statementAdd.productCode"/>';
				//设置该按钮灰掉
				me.disable(false);
				//10秒后自动解除灰掉效果
				setTimeout(function() {
...
writeoff.printStatementReceipt = function(){
	...
	Ext.Ajax.request({
		...
		success:function(response){
			...
			//调用打印
			do_printpreview('statementconfreceipt',{'statementBillNo':statementBillId,'createUserName':stl.emp.empName},ContextPath.STL_WEB);
		},
		exception:function(response){
			var result = Ext.decode(response.responseText);	
			Ext.Msg.alert('<ext:i18nForJsp key="foss.stl.writeoff.common.alert"/>',result.message);
...
Ext.define('Foss.statementbill.CustomerTypeStore',{
	...
	data:{
		'items':[
			{customerTypeCode:'01',customerTypeName:'<ext:i18nForJsp key="foss.stl.writeoff.common.customerDetial"/>'},
			{customerTypeCode:'02',customerTypeName:'<ext:i18nForJsp key="foss.stl.writeoff.common.vehAgencyDetial"/>'},
			{customerTypeCode:'03',customerTypeName:'<ext:i18nForJsp key="foss.stl.writeoff.common.airAgencyDetial"/>'},
			{customerTypeCode:'04',customerTypeName:'<ext:i18nForJsp key="foss.stl.writeoff.common.airDetial"/>'}
		]
	},
	proxy:{
		type:'memory',
...
* 待进对账单的明细store
*/
Ext.define('Foss.statementbill.StatementPreEntryEditStore',{
	extend:'Ext.data.Store',
...
Ext.define('Foss.statementbill.RepaymentForm',{
  ...
  defaults:{
	...
	items:[{
		...
		allowBlank : false,
		style:'margin-left:100px',
		store:FossDataDictionary.getDataDictionaryStore(settlementDict.SETTLEMENT__PAYMENT_TYPE,null,null,[writeoff.SETTLEMENT__PAYMENT_TYPE__CASH,writeoff.SETTLEMENT__PAYMENT_TYPE__CARD,writeoff.SETTLEMENT__PAYMENT_TYPE__TELEGRAPH_TRANSFER,writeoff.SETTLEMENT__PAYMENT_TYPE__NOTE]),
		queryModel:'local',
...
* 待进对账单明细记录
*/
Ext.define('Foss.statementbill.StatementPreEntryEditGrid', {
	extend:'Ext.grid.Panel',
  title:'<ext:i18nForJsp key="foss.stl.writeoff.statementAdd.periodBeginEntry"/>',
  height:330,
  hidden:true,
  collapsible: true,
	animCollapse: true,
  ...
  bodyCls: 'autoHeight',
	cls: 'autoHeight',
  store: Ext.create('Foss.statementbill.StatementPreEntryEditStore'),
  viewConfig:{
		enableTextSelection : true//设置行可以选择，进而可以复制
	},
  columns: [{ 
  xtype:'actioncolumn',
		...
		items:[{
			...
			tooltip:'<ext:i18nForJsp key="foss.stl.writeoff.statementAdd.addToEntry"/>',
			getClass:function(v,m,r,rowIndex){
				if(!writeoff.isPermission('/stl-web/writeoff/addStatementDetail.action')){
					return 'statementBill_hide';
				}else {
					return 'statementBill_add';
				}
			},
			handler:function(grid, rowIndex, colIndex){
				writeoff.statementAddEntryEdit(grid, rowIndex, colIndex);
			}	
		}]
	},{
		header:'<ext:i18nForJsp key="foss.stl.writeoff.statementAdd.number"/>',
...
Ext.onReady(function(){
	...
	if(Ext.isEmpty(writeoff.statementEntryEditGrid)){
		writeoff.statementEntryEditGrid =  Ext.create('Foss.statementbill.StatementEntryEditGrid',{
									//批量删除
										...
										items:[{
											...
											handler:function(){
												...
												if(baseInfo.getRecord().get('confirmStatus')==writeoff.STATEMENTCONFIRMSTATUS_Y){
													Ext.Msg.alert('<ext:i18nForJsp key="foss.stl.writeoff.common.alert"/>','<ext:i18nForJsp key="foss.stl.writeoff.statementCommon.auditToAddStatementEntryWarning"/>');
													return false;
												}

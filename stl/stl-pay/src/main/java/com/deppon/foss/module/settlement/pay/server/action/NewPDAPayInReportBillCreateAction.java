...
/**
 * 生成PDA收入报表，和PDA收入报表明细-零担
 * 
 * @author 045738-foss-maojianqiang
 * @date 2012-12-18 上午11:51:41
 */
public class NewPDAPayInReportBillCreateAction extends AbstractAction {
	 ...
	 * 注入日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(NewPDAPayInReportBillCreateAction.class);
	 ...
	 * 注入PDA司机缴款服务
	 */
	private INewPDAPayInReportBillCreateService newPDAPayInReportBillCreateService;
	...
	public String queryReceiptSendGoodsInfo() {
		try{
			...
			//调用service进行查询
			DriverCollectionRptDto resultDto = newPDAPayInReportBillCreateService.queryReceiptSendGoodsInfo(vo.getDto(), currentInfo);
			//将dto放到vo中，传递到前台
			vo.setDto(resultDto);
	...
	public String addReceiveReportBill() {
		try{
			...
			CurrentInfo currentInfo = FossUserContext.getCurrentInfo();
			//调用service进行查询
			DriverCollectionRptDto resultDto = newPDAPayInReportBillCreateService.addReceiveReportBill(vo.getDto(), currentInfo);
			//将dto放到vo中，传递到前台
			vo.setDto(resultDto);
			return returnSuccess();
		}catch(BusinessException e){
	...
	public String queryReceiveReportBill() {
		try{
			...
			//调用service进行查询
			DriverCollectionRptDto resultDto = newPDAPayInReportBillCreateService.queryReceiveReportBill(vo.getDto(), currentInfo,getStart(),getLimit());
			//将dto放到vo中，传递到前台
			vo.setDto(resultDto);
			//设置分页总条数
	...
	public String queryReportDetail(){
		try{
			...
			//调用service进行查询
			DriverCollectionRptDto resultDto = newPDAPayInReportBillCreateService.queryReportDetail(vo.getDto().getReportNo(),vo.getDto().getReportBeginDate());
			//将dto放到vo中，传递到前台
			vo.setDto(resultDto);
			return returnSuccess();
		}catch(BusinessException e){
	...
	public void setNewPDAPayInReportBillCreateService(
			INewPDAPayInReportBillCreateService newPDAPayInReportBillCreateService) {
		this.newPDAPayInReportBillCreateService = newPDAPayInReportBillCreateService;
	}

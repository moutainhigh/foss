 ...
 */
public class PayToFSSCSendService implements IPayToFSSCSendService {

	/**
	 * Logger 预付单服务
	 */
	private static final Logger LOGGER = LogManager
			.getLogger(PayToFSSCSendService.class);
	/**
	 * 服务编码
	 */
	private static final String FSSC_DEAL_PAY_WORKFLOW = "ESB_FOSS2ESB_GENERATE_PAY_WORKFLOW";
	 ...
	 * @see com.deppon.foss.module.settlement.pay.api.server.service.IPayToFSSCSendService#PayToCostcontrol(com.deppon.foss.module.settlement.pay.api.shared.dto.PayToCostcontrolDto, com.deppon.foss.module.frameworkimpl.shared.domain.CurrentInfo)
	 */
	@Override
    public void payToCostcontrol(PayToCostcontrolDto dto) {
		LOGGER.info("发送给财务共享开始！");
		//校验dto
		validatorParams(dto);
		...
		request.setAccountNumber(dto.getPayBillPayeeCode());//收款人编码
		...
		request.getExpenseDetail().addAll(dto.getExpenseDetailToFSSC());//明细
		request.setCurrency(dto.getCurrency());//币种  默认为人民币
		request.setExchangeRate(dto.getExchangeRate());//汇率
		request.setIsAutoAbatementLoan(dto.getIsAutoAbatementLoan());//是否冲借支
        if (CollectionUtils.isNotEmpty((dto.getStowageDetail()))) {
			request.getStowageDetail().addAll(dto.getStowageDetail());//配载单信息
		}
        //是否保理
        if (null != dto.getFactoring() && StringUtils.isNotEmpty(dto.getFactoring())) {
            request.setFactoring(dto.getFactoring());//设置是否保理
            if (dto.getFactoring().equals(FossConstants.YES)) {
                request.setCusCode(dto.getCusCode());//设置贷款编码
                request.setCustomerCode(dto.getCustomerCode());//设置客户编码
                request.setCustomerName(dto.getCustomerName());//设置客户名称
            }
        } else {
            request.setFactoring(FossConstants.NO);
        }
        //批次号规则
		...
		try {
            ...
            LOGGER.error("信息发送异常，调用esb接口失败" + e.getMessage());
            throw new SettlementException("信息发送异常，调用esb接口失败" + e);
		}
        LOGGER.info("批次号：" + dto.getBatchNo() + "发送给财务共享结束！");
	}
	...
	private AccessHeader buildHeader(String batchNo) {
		...
		//服务编码
		header.setEsbServiceCode(FSSC_DEAL_PAY_WORKFLOW);
		// 处理工作流审批结果
    ...
    private void validatorParams(PayToCostcontrolDto dto) {
        ...
        //校验是否冲借支
        if (StringUtils.isBlank(dto.getIsAutoAbatementLoan())) {
            //抛出异常
            throw new SettlementException("传入是否自动冲借支标志为空！");
        }
    }

    private void validaExtracted(PayToCostcontrolDto dto) {
		//校验省份编码
		...
		//校验费用明细
        if (CollectionUtils.isEmpty(dto.getExpenseDetailToFSSC())) {
			//抛出异常
			throw new SettlementException("传入费用明细为空！");
		}
		//增加校验 明细总条数不超过3000条 由于财务共享那边有限制，故而foss也要加
        final int numberOfThreeThousand = 3000;
        if (dto.getExpenseDetailToFSSC().size() > numberOfThreeThousand) {
			//抛出异常
			throw new SettlementException("传入费用明细总条数不能超过3000条！");
		}
	 ...
	 * @see com.deppon.foss.module.settlement.pay.api.server.service.IPayToFSSCSendService#sharePayableForBad(java.util.List, java.lang.String)
	 */
    public void sharePayableForBad(List<ExpenseDetail> detail, BillPayableEntity entity, String payDesc) {
        ...
        if (CollectionUtils.isNotEmpty(orgList)) {
            ...
            for (int i = 0; i < orgList.size(); i++) {
				...
				d.setBillNumber(entity.getWaybillNo());//明细单号

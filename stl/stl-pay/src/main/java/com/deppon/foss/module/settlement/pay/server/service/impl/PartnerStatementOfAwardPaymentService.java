 ...
 * @date 2016-2-25 下午5:18:19    
 */
public class PartnerStatementOfAwardPaymentService implements
		IPartnerStatementOfAwardPaymentService {
	//声明日志
	private static final Logger logger = LogManager.getLogger(PartnerStatementOfAwardPaymentService.class);
	 ...
	 * 注入合伙人奖罚对账单付款DAO
	 */
	private IPartnerStatementOfAwardPaymentDao partnerStatementOfAwardPaymentDao;
	...
	@Transactional
	public String partnerStatementOfAwardToPayment(BillPaymentEntity paymentEntity, String statementBillNo,CurrentInfo currentInfo) {
		logger.info("进入奖罚对账单付款service: 金额 " + paymentEntity.getAmount());
		...
		//根据对账单号查询对账单
		PartnerStatementOfAwardEntity partnerStatementOfAwardEntity = partnerStatementOfAwardMService.queryPStatementsByStaNoNPage(statementBillNo);	
		//判断页面传递付款金额与对账单付款金额是否一致
		if (paymentEntity.getAmount().compareTo(partnerStatementOfAwardEntity.getUnpaidAmount())!=0) {
			throw new SettlementException("明细金额发生变化,请关闭查看明细页面,重新付款！");
		}
		//对账单确认状态判断
		...
		//判断剩余应收与剩余应付金额差是否大于0，大于0则不能付款
		if (partnerStatementOfAwardEntity.getPeriodUnverifyRecAmount().compareTo(partnerStatementOfAwardEntity.getPeriodUnverifyPayAmount())>0) {
			throw new SettlementException("对账单单据类型是应收对账单不可以付款！");
		}
		//判断未还总额
		if(partnerStatementOfAwardEntity.getUnpaidAmount().compareTo(BigDecimal.ZERO)==0){
			throw new SettlementException("未核销总额为0，不能进行付款！");
		}
		
		//对账单核销操作--应收冲应付
		BillWriteoffOperationDto writeoffResultDto = writeoffStatement(partnerStatementOfAwardEntity, currentInfo);
		// 声明当前日期
		Date now = new Date();
		// 获取单据编号 （此处单号是手动生成付款单）
		...
		//新增付款单明细
		partnerStatementOfAwardPaymentDao.addBillPaymentD(paymentNum,statementBillNo);
		...
		//更新付款单状态、付款金额、版本号
		partnerStatementOfAwardPaymentDao.payForBillPayable(payableMap);
		
		//设置已核销金额
		partnerStatementOfAwardEntity.setPaidAmount(partnerStatementOfAwardEntity.getUnpaidAmount());
		...
		//调用修改对账单接口
		partnerStatementOfAwardMService.updateStatementForWriteOff(partnerStatementOfAwardEntity,currentInfo);
		
		// 获取传递给财务自助参数
		PayToCostcontrolDto dto = buildPayToFSSCDto(paymentEntity,payableList,partnerStatementOfAwardEntity,currentInfo);
...
//			}
			...
			validaMethod(paymentEntity);
		}
	}


	private void validaMethod(BillPaymentEntity paymentEntity) {
	...
	public static String getWorkFlowTypeByBillType(String number,String billType,String payType){
		//声明工作流类型
		String workFlowType = SettlementESBDictionaryConstants.COST_CONTROL_WORKFLOW_PARTNER_AWARD_PAY;//合伙人奖罚--付款
		return workFlowType;
	}
	...
	private void addBillPayment(BillPaymentEntity entity, CurrentInfo currentInfo) {
		...
		entity.setIsInit(FossConstants.NO);
		//保存数据后返回值
		billPaymentService.addBillPAwardPayment(entity);
	 ...
	 * @date 2014-06-17
	 */
	private PayToCostcontrolDto buildPayToFSSCDto(BillPaymentEntity bill, List<BillPayableDto> payableList,PartnerStatementOfAwardEntity partnerStatementOfAwardEntity,
		...
		// 声明传递给共享的dto
				...
				d.setBillNumber(partnerStatementOfAwardEntity.getStatementBillNo());//明细单号
				d.setExpenseExplain(PAYDESC);//费用说明
				d.setExpensesMoney(partnerStatementOfAwardEntity.getPeriodAmount());//明细金额
				//转化给对应共享那边费用明细
				String payType = convertBillType(payable.getPayableNo(),payable.getBillType());
				d.setExpensesType(payType);
				d.setPayBillExpDate(partnerStatementOfAwardEntity.getCreateTime());//业务发生日期
	...
	public static String convertBillType(String number,String billType){
		//生命费用明细类型
		String payType = SettlementESBDictionaryConstants.COST_CONTROL_PAY_DETAIL_PARTNER_AWARD;
		return payType;
	}
	...
	public void setPartnerStatementOfAwardPaymentDao(
			IPartnerStatementOfAwardPaymentDao partnerStatementOfAwardPaymentDao) {
		this.partnerStatementOfAwardPaymentDao = partnerStatementOfAwardPaymentDao;
	}

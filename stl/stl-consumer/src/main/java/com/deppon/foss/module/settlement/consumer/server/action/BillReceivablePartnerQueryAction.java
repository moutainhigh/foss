 ...
 */
public class BillReceivablePartnerQueryAction extends AbstractAction {

	/** 实体序列号. */
	private static final long serialVersionUID = -475408607567583013L;

	/** 声明日志对象. */
	private static final Logger LOGGER = LoggerFactory.getLogger(BillReceivablePartnerQueryAction.class);
	...
	@JSON
	public String queryPartnerReceivableBillByWayBillNo() {
		try {
			// 判断页面传过来的运单号数组是否有值
			...
			if (CollectionUtils.isNotEmpty(wayBillNoList)) {
				// 按运单单号List查询
				billReceivableBillNoList = billReceivablePartnerService.queryByWaybillNosAndOrgCodes(wayBillNoList, userOrgCodesList, null,cInfo);
			}
			...
			if (CollectionUtils.isNotEmpty(receivableBillNoList)) {
				// 以应收单单号做参数查询
				list = billReceivablePartnerService.queryByReceivableNosAndOrgCodes(receivableBillNoList, null,cInfo);
			}
	 ...
	 */
	@JSON
	public String queryPartnerReceivableBillBySourceBillNo() {
		try {
			// 判断页面传过来的来源单号数组是否有值
			...
			if(CollectionUtils.isNotEmpty(sourceBillNoList)){
				// 按运单单号List查询
				list = billReceivablePartnerService.queryBySourceBillNOsAndOrgCodes(sourceBillNoList, userOrgCodesList, null,null,cInfo);
			}
	 ...
	 */
	@JSON
	public String queryPartnerReceivableBillByDate() {
		try {
			...
			billReceivableDto.setIsPartner(FossConstants.YES);
	...
	public String exportPartnerBillReceivable() {
		...
		try {
			...
			if (SettlementConstants.TAB_QUERY_BY_WAYBILL_NO
				...
				if (CollectionUtils.isNotEmpty(wayBillNoList)) {
					// 按运单单号List查询
					billReceivableBillNoList = billReceivablePartnerService.queryByWaybillNosAndOrgCodes(wayBillNoList, null, null,cInfo);
				}
				// 存放由应收单单号查询出来的应收单实体对象集合
				...
				if (CollectionUtils.isNotEmpty(receivableBillNoList)) {
					// 以应收单单号做参数查询
					list = billReceivablePartnerService.queryByReceivableNOs(receivableBillNoList, "");
				}
				// 声明一个总的List来存放按照运单号的查询结果和应收单号查询的结果
				List<BillReceivableEntity> listAll = new ArrayList<BillReceivableEntity>();
				// 只有有一个查询结果不为空才能做 Collection的并集
			...
			// 按日期查询
				...
				billReceivableVO.getDto().setIsPartner(FossConstants.YES);
				if (SettlementConstants.TAB_DATE_TYPE_FOR_BUSINESS

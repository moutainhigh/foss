CREATE OR REPLACE PACKAGE PKG_STL_VCH_DAILY_LAND_COD IS
...
END PKG_STL_VCH_DAILY_LAND_COD;
/
CREATE OR REPLACE PACKAGE BODY PKG_STL_VCH_DAILY_LAND_COD IS
	...
	BEGIN
		...
		INSERT INTO STV.T_STL_DVD
			...
			SELECT SYS_GUID(),
						 P_PERIOD,
						 PKG_STL_VCH_COMMON.LAND_COD_CH_POD, --业务场景
						 RCV.CUSTOMER_CODE,
						 ...
						 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_DEST_ORG_CODE，RCV.DEST_ORG_CODE),
			 ...
			 WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RR --还款冲应收
						 AND RCV.BILL_TYPE = PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DLC --落地配代理代收货款应收单
		...
		INSERT INTO STV.T_STL_DVD
			...
			SELECT SYS_GUID(),
						 P_PERIOD,
						 PKG_STL_VCH_COMMON.LAND_COD_CD_POD, --业务场景
						 RCV.CUSTOMER_CODE,
						 RCV.CUSTOMER_NAME, --客户名称
						 RCV.CUSTOMER_TYPE, --客户类型
						 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE),
          ...
          AND RCV.IS_RED_BACK = PKG_STL_COMMON.IS_RED_BACK_NO
			 WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RR --还款冲应收
						 AND RCV.BILL_TYPE = PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DLC --落地配代理代收货款应收
		...
		INSERT INTO STV.T_STL_DVD
			...
			SELECT SYS_GUID(),
						 P_PERIOD,
						 PKG_STL_VCH_COMMON.LAND_COD_POD_WO_COD, --业务场景
						 RCV.CUSTOMER_CODE,
						 RCV.CUSTOMER_NAME, --客户名称
						 RCV.CUSTOMER_TYPE, --客户类型
						 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE),
		...
		INSERT INTO STV.T_STL_DVD
			...
			SELECT SYS_GUID(),
						 P_PERIOD,
						 PKG_STL_VCH_COMMON.LAND_COD_NPOD_WO_COD, --业务场景
						 RCV.CUSTOMER_CODE,
						 RCV.CUSTOMER_NAME, --客户名称
						 RCV.CUSTOMER_TYPE, --客户类型
						 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE),
		...
		--5. 落地配签收时未核销代收货款
		/**
    按签收表中的签收时间来统计，取关联运单生成的（应收单金额减去对应核销单的累计金额），应收单的单据子类型为：
    代收货款应收（产品类型为：包裹）、落地配代理代收货款应收，且签收时间＞核销单的记账时间   
    */
		DECLARE
		...
		BEGIN
			FOR V_POD_ROW IN CUR_POD LOOP
				...
				INSERT INTO STV.T_STL_DVD
					...
					SELECT SYS_GUID(),
								 P_PERIOD,
								 PKG_STL_VCH_COMMON.LAND_COD_POD_NWO_COD, --业务场景
								 RCV.CUSTOMER_CODE,
								 RCV.CUSTOMER_NAME, --客户名称
								 RCV.CUSTOMER_TYPE, --客户类型
								 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE),
					 ...
					 WHERE RCV.ID IN
								 (SELECT ID
										FROM (SELECT *
													 ...
													 ORDER BY T.ACCOUNT_DATE DESC, T.CREATE_TIME DESC,
																		T.IS_RED_BACK  ASC)
									 WHERE ROWNUM <= 1)
		...
		BEGIN
			FOR V_POD_ROW IN CUR_UPD LOOP
				...
				INSERT INTO STV.T_STL_DVD
					...
					SELECT SYS_GUID(),
								 P_PERIOD,
								 PKG_STL_VCH_COMMON.LAND_COD_NPOD_NWO_COD, --业务场景
								 RCV.CUSTOMER_CODE,
								 RCV.CUSTOMER_NAME, --客户名称
								 RCV.CUSTOMER_TYPE, --客户类型
								 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE),
		...
		INSERT INTO STV.T_STL_DVD
			...
			SELECT SYS_GUID(),
						 P_PERIOD,
						 PKG_STL_VCH_COMMON.LAND_COD_CH_NPOD, --业务场景
						 RCV.CUSTOMER_CODE,
						 RCV.CUSTOMER_NAME, --客户名称
						 RCV.CUSTOMER_TYPE, --客户类型
						 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE),
		...
		INSERT INTO STV.T_STL_DVD
			...
			SELECT SYS_GUID(),
						 P_PERIOD,
						 PKG_STL_VCH_COMMON.LAND_COD_CD_NPOD, --业务场景
						 RCV.CUSTOMER_CODE,
						 RCV.CUSTOMER_NAME, --客户名称
						 RCV.CUSTOMER_TYPE, --客户类型
						 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE),
		...
		INSERT INTO STV.T_STL_DVD
			...
			SELECT SYS_GUID(), --ID
						 ...
						 RCV.CUSTOMER_NAME, --客户名称
						 RCV.CUSTOMER_TYPE, --客户类型
						 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE), --出发部门编码
						 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_NAME，RCV.ORIG_ORG_NAME), --始发部门名称
          ...
          AND PAY.IS_RED_BACK = PKG_STL_COMMON.IS_RED_BACK_NO--应付单
			 WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RP --应收冲应付
						 AND
						 PAY.BILL_TYPE = PKG_STL_COMMON.PAYABLE_BILL_TYPE_LANDSTOWAGE --落地配外发成本
						 AND RCV.BILL_TYPE = PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DLC --落地配代理代收货款应收
						 AND (SELECT NVL(SUM(DECODE(T.POD_TYPE,
																				PKG_STL_COMMON.POD__POD_TYPE__POD,
																				1,
		...
		INSERT INTO STV.T_STL_DVD
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.LAND_COD_WO_OTH_PAY_COD, --业务场景:落地配应付到达代理成本冲应收代收货款（未签收）
						 RCV.CUSTOMER_CODE, --客户编码
						 RCV.CUSTOMER_NAME, --客户名称
						 RCV.CUSTOMER_TYPE, --客户类型
						 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE), --出发部门编码
          ...
          AND PAY.IS_RED_BACK = PKG_STL_COMMON.IS_RED_BACK_NO --应付单
			 WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RP --应收冲应付
						 AND PAY.BILL_TYPE = PKG_STL_COMMON.PAYABLE_BILLTYPE_LAND_OTHER --落地配其它应付
						 AND RCV.BILL_TYPE = PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DLC --落地配代理代收货款应收
						 AND (SELECT NVL(SUM(DECODE(T.POD_TYPE,
																				PKG_STL_COMMON.POD__POD_TYPE__POD,
																				1,
		...
		INSERT INTO STV.T_STL_DVD
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.LAND_COD_WO_AGENCY_PAY_NPOD, --业务场景:落地配应付到达代理成本冲应收代收货款（未签收）
						 RCV.CUSTOMER_CODE, --客户编码
						 RCV.CUSTOMER_NAME, --客户名称
						 RCV.CUSTOMER_TYPE, --客户类型
						 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE), --出发部门编码
          ...
          AND PAY.IS_RED_BACK = PKG_STL_COMMON.IS_RED_BACK_NO --应付单
			 WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RP --应收冲应付
						 AND
						 PAY.BILL_TYPE = PKG_STL_COMMON.PAYABLE_BILL_TYPE_LANDSTOWAGE --落地配到达代理应付
						 AND RCV.BILL_TYPE = PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DLC --落地配代理代收货款应收
						 AND (SELECT NVL(SUM(DECODE(T.POD_TYPE,
																				PKG_STL_COMMON.POD__POD_TYPE__POD,
																				1,
		...
		INSERT INTO STV.T_STL_DVD
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.LAND_COD_WO_OTH_NPOD, --业务场景:落地配应付到达代理成本冲应收代收货款（未签收）
						 RCV.CUSTOMER_CODE, --客户编码
						 RCV.CUSTOMER_NAME, --客户名称
						 RCV.CUSTOMER_TYPE, --客户类型
						 DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE), --出发部门编码
          ...
          AND PAY.IS_RED_BACK = PKG_STL_COMMON.IS_RED_BACK_NO --应付单
			 WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RP --应收冲应付
						 AND PAY.BILL_TYPE = PKG_STL_COMMON.PAYABLE_BILLTYPE_LAND_OTHER --落地配其它应付
						 AND RCV.BILL_TYPE = PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DLC --应收单的单据子类型为代收货款应收（产品类型为：精准落地配）、落地配代理代收货款应收
						 AND (SELECT NVL(SUM(DECODE(T.POD_TYPE,
																				PKG_STL_COMMON.POD__POD_TYPE__POD,
																				1,
																				PKG_STL_COMMON.POD__POD_TYPE__UPD,

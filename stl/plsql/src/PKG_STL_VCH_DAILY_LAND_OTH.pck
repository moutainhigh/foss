CREATE OR REPLACE PACKAGE PKG_STL_VCH_DAILY_LAND_OTH IS
...
END PKG_STL_VCH_DAILY_LAND_OTH;
/
CREATE OR REPLACE PACKAGE BODY PKG_STL_VCH_DAILY_LAND_OTH IS
  ...
  FUNCTION FUNC_PROCESS_DAILY_DETAIL(P_PERIOD     VARCHAR2, -- 账期yyyymmdd，例子20121221
   ...
   IS
  BEGIN
    --1）空运其他应收录入:按落地配其他应收单的记账日期来统计，取落地配其他应收单的金额
    INSERT INTO STV.T_STL_DVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.LAND_OTH_ENTRY, --业务场景
       ...
       WHERE RCV.BILL_TYPE = PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_LR --落地配其他应收
    ...
    INSERT INTO STV.T_STL_DVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.LAND_OTH_RCV_CH, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE) , --始发部门编码
    ...
    INSERT INTO STV.T_STL_DVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.LAND_OTH_RCV_CD, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE) , --始发部门编码
       ...
       WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RR --还款冲应收
             ...
             AND RP.ACCOUNT_DATE < P_END_DATE;
    RETURN TRUE;

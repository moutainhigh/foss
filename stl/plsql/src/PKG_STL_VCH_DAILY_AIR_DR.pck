...
CREATE OR REPLACE PACKAGE BODY PKG_STL_VCH_DAILY_AIR_DR IS
	...
	FUNCTION FUNC_PROCESS_DAILY_DETAIL(P_PERIOD     VARCHAR2, -- 账期yyyymmdd，例子20121221
	 ...
	 IS
	BEGIN
		--1)预收空运代理现金:按预收单的记账日期来统计，预收单的付款方式为：现金，预收单运输类型为：空运代理
		INSERT INTO STV.T_STL_DVD
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.AIR_DR_CH, --业务场景
		...
		--2)预收空运代理银行:按预收单的记账日期来统计，预收单的付款方式为：银行卡、电汇、支票、网上支付，预收单运输类型为：空运代理
		INSERT INTO STV.T_STL_DVD
			(ID,
			 PERIOD,
			 BUSINESS_CASE,
		...
		--应收单的单据子类型为到达应收、空运到达代理应收，预收单运输类型为：空运代理，取核销明细的金
		--额，反核销时生成的负数也统计
		INSERT INTO STV.T_STL_DVD
			(ID,
			 PERIOD,
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.AIR_DR_DEST_RCV_POD, --业务场景
		...
		INSERT INTO STV.T_STL_DVD
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.AIR_DR_DEST_RCV_NPOD, --业务场景
						 RCV.CUSTOMER_CODE, --客户编码
						 RCV.CUSTOMER_NAME, --客户名称
						 RCV.CUSTOMER_TYPE, --客户类型
						 RCV.ORIG_ORG_CODE, --始发部门编码
		...
		--5）预收空运代理冲其他应收
		--按核销单的记账日期来统计，核销类型为：预收冲应收，应收单的单据子类型为空运其他应收，
		--预收单运输类型为：空运代理，取核销明细的金额，反核销时生成的负数也统计
		INSERT INTO STV.T_STL_DVD
			(ID,
			 PERIOD,
			 BUSINESS_CASE,
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.AIR_DR_WO_OTHER_RCV, --业务场景
						 RCV.CUSTOMER_CODE, --客户编码
						 RCV.CUSTOMER_NAME, --客户名称
						 RCV.CUSTOMER_TYPE, --客户类型
						 RCV.ORIG_ORG_CODE, --始发部门编码
		...
		--6)预收空运代理冲应收代收货款已签收
		--按核销单的记账日期来统计，核销类型为：预收冲应收，关联判断核销时点应收单的签收状态为已签收，
		--应收单的单据子类型为代收货款应收（产品类型为：精准空运）、空运代理代收货款应收，预收单运输
		--类型为：空运代理，取核销明细的金额，反核销时生成的负数也统计
		INSERT INTO STV.T_STL_DVD
			(ID,
			 PERIOD,
			 BUSINESS_CASE,
    ...
    --8）空运退预收付款申请：
    --按付款单的记账日期来统计，关联里面的付款明细单据子类型为：预收单，预收单运输类型为：空运代理，
    --统计对应的付款单明细金额之和
    INSERT INTO STV.T_STL_DVD
      (ID,
       PERIOD,

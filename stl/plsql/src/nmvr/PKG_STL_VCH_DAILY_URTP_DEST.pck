CREATE OR REPLACE PACKAGE STV.PKG_STL_VCH_DAILY_URTP_DEST IS

  -- ==============================================================
  -- Author  : ZBW
  -- Created : 2013-11-06
  -- Purpose : 每日凭证偏线_还款运单总运费（到付）【02】
  -- ==============================================================

  -----------------------------------------------------------------
  -- 偏线_还款运单总运费（到付）【02】处理日明细
  -----------------------------------------------------------------
...
END PKG_STL_VCH_DAILY_URTP_DEST;
/
CREATE OR REPLACE PACKAGE BODY STV.PKG_STL_VCH_DAILY_URTP_DEST IS

  -----------------------------------------------------------------
  -- 偏线--还款运单总运费（到付）【02】处理日明细
  -----------------------------------------------------------------
  FUNCTION FUNC_PROCESS_DAILY_DETAIL(P_PERIOD     VARCHAR2, -- 账期yyyymmdd，例子20121221
                                     P_BEGIN_DATE DATE, -- 开始时间 2012-12-21
                                     P_END_DATE   DATE -- 结束时间 2012-12-22
  ...
  BEGIN
    ...
    INSERT INTO STV.T_STL_NDVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.URTP_DEST_CH_NPOD, --业务场景
    ...
    INSERT INTO STV.T_STL_NDVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.URTP_DEST_CD_NPOD, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,
    ...
    INSERT INTO STV.T_STL_NDVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.URTP_DEST_CH_POD, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,
    ...
    INSERT INTO STV.T_STL_NDVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.URTP_DEST_CD_POD, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,

CREATE OR REPLACE PACKAGE PKG_STL_VCH_DAILY_UPDT IS

	-- ==============================================================
	-- Author  : ZBW
	-- Created : 2013-11-06
	-- Purpose : 每日凭证反签收运单【02】
	-- ==============================================================
...
END PKG_STL_VCH_DAILY_UPDT;
/
CREATE OR REPLACE PACKAGE BODY PKG_STL_VCH_DAILY_UPDT IS
	...
	BEGIN
		...
		INSERT INTO STV.T_STL_NDVD
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.UPDT_CASH_COLLECTED, --业务场景
		...
		INSERT INTO STV.T_STL_NDVD
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.UPDT_ORIG_RCV_WO, --业务场景
						 CUSTOMER_CODE, --客户编码
						 CUSTOMER_NAME, --客户名称
						 CUSTOMER_TYPE, --客户类型
		...
		BEGIN
			...
			FOR V_POD_ROW IN CUR_POD LOOP
				...
				INSERT INTO STV.T_STL_NDVD
					...
					SELECT SYS_GUID(), --ID
								 P_PERIOD, --期间
								 PKG_STL_VCH_COMMON.UPDT_ORIG_RCV_NWO, --业务场景
								 CUSTOMER_CODE, --客户编码
								 CUSTOMER_NAME, --客户名称
								 CUSTOMER_TYPE, --客户类型
								 ORIG_ORG_CODE, --始发部门编码 
		...
		INSERT INTO STV.T_STL_NDVD
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.UPDT_DEST_RCV_WO, --业务场景
						 CUSTOMER_CODE, --客户编码
						 CUSTOMER_NAME, --客户名称
						 CUSTOMER_TYPE, --客户类型
						 ORIG_ORG_CODE, --始发部门编码 
		...
		BEGIN
			...
			FOR V_POD_ROW IN CUR_POD LOOP
				...
				INSERT INTO STV.T_STL_NDVD
					...
					SELECT SYS_GUID(), --ID
								 P_PERIOD, --期间
								 PKG_STL_VCH_COMMON.UPDT_DEST_RCV_NWO, --业务场景
								 CUSTOMER_CODE, --客户编码
								 CUSTOMER_NAME, --客户名称
								 CUSTOMER_TYPE, --客户类型
								 ORIG_ORG_CODE, --始发部门编码 

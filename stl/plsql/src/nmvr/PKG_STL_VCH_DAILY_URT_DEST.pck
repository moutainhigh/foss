CREATE OR REPLACE PACKAGE STV.PKG_STL_VCH_DAILY_URT_DEST IS

  -- ==============================================================
  -- Author  : ZBW
  -- Created : 2013-11-06
  -- Purpose : 每日凭证还款运单总运费（到付）【02】
  -- ==============================================================
...
END PKG_STL_VCH_DAILY_URT_DEST;
/
CREATE OR REPLACE PACKAGE BODY STV.PKG_STL_VCH_DAILY_URT_DEST IS
  ...
  BEGIN
    ...
    INSERT INTO STV.T_STL_NDVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.URT_DEST_CH_NPOD, --业务场景
       ...
       WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RR --还款冲应收
             AND RCV.BILL_TYPE IN (PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DEST,
                                   ...
                                   PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DL       ) --到达应收、偏线到达应收、空运到达应收
    ...
    INSERT INTO STV.T_STL_NDVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.URT_DEST_CD_NPOD, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,
    ...
    INSERT INTO STV.T_STL_NDVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.URT_DEST_CH_POD, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,
    ...
    INSERT INTO STV.T_STL_NDVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.URT_DEST_CD_POD, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,
       ...
       SELECT SYS_GUID(), --ID
        ...
        JOIN STL.T_STL_BILL_RECEIVABLE RCV
          ON RCV.RECEIVABLE_NO = WO.END_NO
             AND RCV.IS_RED_BACK = PKG_STL_COMMON.IS_RED_BACK_NO
       WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RR --还款冲应收
             AND RCV.BILL_TYPE IN (PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DEST                                  ) --到达应收、偏线到达应收、空运到达应收
             AND RCV.SOURCE_BILL_TYPE =
             PKG_STL_COMMON.RECEIVABLE_SOURCE_BILL_TYPE_W --来源于运单
             AND RPMT.PAYMENT_TYPE IN
             (PKG_STL_COMMON.PAYMENT_TYPE_CARD,

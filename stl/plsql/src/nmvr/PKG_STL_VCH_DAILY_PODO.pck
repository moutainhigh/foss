...
CREATE OR REPLACE PACKAGE BODY PKG_STL_VCH_DAILY_PODO IS
	...
	BEGIN
		...
		INSERT INTO STV.T_STL_NDVD
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.PODO_CASH_COLLECTED, --业务场景
			 ...
			 WHERE POD.POD_TYPE = PKG_STL_COMMON.POD__POD_TYPE__POD --签收
						 ...
						 AND CASH.INVOICE_MARK = PKG_STL_COMMON.INVOICE_MARK_ONE; --收款小于等于签收时间   
	
		--2)签收运单【01】——签收时始发应收已核销金额 
		INSERT INTO STV.T_STL_NDVD
			(ID,
			 PERIOD,
			 BUSINESS_CASE,
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.PODO_ORIG_RCV_WO, --业务场景
						 CUSTOMER_CODE, --客户编码
						 CUSTOMER_NAME, --客户名称
						 CUSTOMER_TYPE, --客户类型
				...
				FROM (SELECT RCV.CUSTOMER_CODE, --客户编码
							 ...
							 WHERE POD.POD_TYPE = PKG_STL_COMMON.POD__POD_TYPE__POD --签收
										 ...
										 AND RCV.INVOICE_MARK = PKG_STL_COMMON.INVOICE_MARK_ONE -- 发票标记为01
										 AND POD.POD_DATE >= P_BEGIN_DATE
										 AND POD.POD_DATE < P_END_DATE
		...
		BEGIN
			...
			FOR V_POD_ROW IN CUR_POD LOOP
				...
				INSERT INTO STV.T_STL_NDVD
					...
					SELECT SYS_GUID(), --ID
								 P_PERIOD, --期间
								 PKG_STL_VCH_COMMON.PODO_ORIG_RCV_NWO, --业务场景
								 CUSTOMER_CODE, --客户编码
								 CUSTOMER_NAME, --客户名称
								 CUSTOMER_TYPE, --客户类型
								 ORIG_ORG_CODE, --始发部门编码 
		...
		INSERT INTO STV.T_STL_NDVD
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.PODO_DEST_RCV_WO, --业务场景
						 CUSTOMER_CODE, --客户编码
						 CUSTOMER_NAME, --客户名称
						 CUSTOMER_TYPE, --客户类型
						 ORIG_ORG_CODE, --始发部门编码 
		...
		BEGIN
			...
			FOR V_POD_ROW IN CUR_POD LOOP
				...
				INSERT INTO STV.T_STL_NDVD
					...
					SELECT SYS_GUID(), --ID
								 P_PERIOD, --期间
								 PKG_STL_VCH_COMMON.PODO_DEST_RCV_NWO, --业务场景
								 CUSTOMER_CODE, --客户编码
								 CUSTOMER_NAME, --客户名称
								 CUSTOMER_TYPE, --客户类型
								 ORIG_ORG_CODE, --始发部门编码 

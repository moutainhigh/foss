CREATE OR REPLACE PACKAGE STV.PKG_STL_VCH_DAILY_UROP_DEST IS

  -- ==============================================================
  -- Author  : ZBW
  -- Created : 2013-11-06
  -- Purpose : 每日凭证偏线_还款运单总运费（到付）【01】
  -- ==============================================================

  -----------------------------------------------------------------
  -- 偏线_还款运单总运费（到付）【01】处理日明细
  -----------------------------------------------------------------
...
END PKG_STL_VCH_DAILY_UROP_DEST;
/
CREATE OR REPLACE PACKAGE BODY STV.PKG_STL_VCH_DAILY_UROP_DEST IS

  -----------------------------------------------------------------
  -- 偏线--还款运单总运费（到付）【01】处理日明细
  -----------------------------------------------------------------
  FUNCTION FUNC_PROCESS_DAILY_DETAIL(P_PERIOD     VARCHAR2, -- 账期yyyymmdd，例子20121221
                                     P_BEGIN_DATE DATE, -- 开始时间 2012-12-21
                                     P_END_DATE   DATE -- 结束时间 2012-12-22
  ...
  BEGIN
   --1)偏线--还款运单总运费（到付）-还款现金未签收
    INSERT INTO STV.T_STL_NDVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.UROP_DEST_CH_NPOD, --业务场景
       ...
       WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RR --还款冲应收
             ...
             AND RCV.PRODUCT_CODE = PKG_STL_COMMON.PRODUCT_CODE_PLF --汽运偏线
             AND RCV.BILL_TYPE IN (PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DEST,
             PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_PARTIAL)
             ;

    --2)偏线--还款运单总运费（到付）-还款银行未签收
    INSERT INTO STV.T_STL_NDVD
      (ID,
       PERIOD,
       BUSINESS_CASE,
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.UROP_DEST_CD_NPOD, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,
    ...
    INSERT INTO STV.T_STL_NDVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.UROP_DEST_CH_POD, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,
    ...
    INSERT INTO STV.T_STL_NDVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.UROP_DEST_CD_POD, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,
    ...
    INSERT INTO STV.T_STL_NDVDI
      ...
      (ID,
       ...
       SELECT SYS_GUID(), --ID
             ...
             DECODE(RCV.PRODUCT_CODE,
                    PKG_STL_COMMON.PRODUCT_CODE_PKG,
                    RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE), --到达部门编码
             DECODE(RCV.PRODUCT_CODE,
                    PKG_STL_COMMON.PRODUCT_CODE_PKG,
                    RCV.EXPRESS_ORIG_ORG_NAME，RCV.ORIG_ORG_NAME), --到达部门名称
             PKG_STL_COMMON.INVOICE_MARK_ONE,
             PKG_STL_VCH_COMMON.VOUCHER_ORG_TYPE_DEST,
             PKG_STL_VCH_COMMON.VOUCHER_ORG_TYPE_ORIG
        FROM STL.T_STL_BILL_WRITEOFF WO

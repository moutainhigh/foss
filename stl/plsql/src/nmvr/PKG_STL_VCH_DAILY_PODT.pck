CREATE OR REPLACE PACKAGE PKG_STL_VCH_DAILY_PODT IS
...
END PKG_STL_VCH_DAILY_PODT;
/
CREATE OR REPLACE PACKAGE BODY PKG_STL_VCH_DAILY_PODT IS
	...
	BEGIN
		...
		INSERT INTO STV.T_STL_NDVD
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.PODT_CASH_COLLECTED, --业务场景
			 ...
			 WHERE POD.POD_TYPE = PKG_STL_COMMON.POD__POD_TYPE__POD --签收
						 ...
						 AND CASH.ACCOUNT_DATE <= POD.POD_DATE --收款小于等于签收时间
						 AND CASH.INVOICE_MARK = PKG_STL_COMMON.INVOICE_MARK_TWO;
	
		--2)签收运单【02】——签收时始发应收已核销金额 
		INSERT INTO STV.T_STL_NDVD
			(ID,
			 PERIOD,
			 BUSINESS_CASE,
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.PODT_ORIG_RCV_WO, --业务场景
						 CUSTOMER_CODE, --客户编码
						 CUSTOMER_NAME, --客户名称
						 CUSTOMER_TYPE, --客户类型
				...
				FROM (SELECT RCV.CUSTOMER_CODE, --客户编码
							 ...
							 WHERE POD.POD_TYPE = PKG_STL_COMMON.POD__POD_TYPE__POD --签收
										 ...
										 AND WO.ACCOUNT_DATE <= POD.POD_DATE --核销时间于小等于还款时间
										 AND RCV.INVOICE_MARK = PKG_STL_COMMON.INVOICE_MARK_TWO -- 发票标记为02
										 AND POD.POD_DATE >= P_BEGIN_DATE
										 AND POD.POD_DATE < P_END_DATE
		...
		BEGIN
			...
			FOR V_POD_ROW IN CUR_POD LOOP
				...
				INSERT INTO STV.T_STL_NDVD
					...
					SELECT SYS_GUID(), --ID
								 P_PERIOD, --期间
								 PKG_STL_VCH_COMMON.PODT_ORIG_RCV_NWO, --业务场景
								 CUSTOMER_CODE, --客户编码
								 CUSTOMER_NAME, --客户名称
								 CUSTOMER_TYPE, --客户类型
								 ORIG_ORG_CODE, --始发部门编码 
		...
		INSERT INTO STV.T_STL_NDVD
			...
			SELECT SYS_GUID(), --ID
						 P_PERIOD, --期间
						 PKG_STL_VCH_COMMON.PODT_DEST_RCV_WO, --业务场景
						 CUSTOMER_CODE, --客户编码
						 CUSTOMER_NAME, --客户名称
						 CUSTOMER_TYPE, --客户类型
						 ORIG_ORG_CODE, --始发部门编码 
				...
				FROM (SELECT RCV.CUSTOMER_CODE, --客户编码
							 ...
							 WHERE POD.POD_TYPE = PKG_STL_COMMON.POD__POD_TYPE__POD --签收
										 ...
										 (PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DEST,
													...
													PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DL) --到达应收、偏线到达应收、空运到达应收、到达落地配代理应收
										 AND WO.ACCOUNT_DATE <= POD.POD_DATE --核销时间于小还款时间
		...
		BEGIN
			...
			FOR V_POD_ROW IN CUR_POD LOOP
				...
				INSERT INTO STV.T_STL_NDVD
					...
					SELECT SYS_GUID(), --ID
								 P_PERIOD, --期间
								 PKG_STL_VCH_COMMON.PODT_DEST_RCV_NWO, --业务场景
								 CUSTOMER_CODE, --客户编码
								 CUSTOMER_NAME, --客户名称
								 CUSTOMER_TYPE, --客户类型
								 ORIG_ORG_CODE, --始发部门编码 
																		...
																		FROM (SELECT *
																					 ...
																					 WHERE T.WAYBILL_NO =
																								 ...
																								 (PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DEST,
																									PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_PARTIAL,
																									PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_AA,
																									PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DL) --增加到达落地配代理应收
																					 ORDER BY T.ACCOUNT_DATE DESC,
																										T.CREATE_TIME  DESC,
																										T.IS_RED_BACK  ASC)
																	 WHERE ROWNUM <= 1)

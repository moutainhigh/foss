...
CREATE OR REPLACE PACKAGE BODY PKG_STL_VCH_DAILY_LAND_PR IS
  ...
  BEGIN
    ...
    --按核销单的记账日期来统计，关联判断核销时点应收单的签收状态为已签收，
    -- 应收单的单据子类型为应收（产品类型为：包裹）、落地配代理应收，应付单的单据子类型为：落地配代理应付
    ...
    INSERT INTO STV.T_STL_DVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.LAND_PR_AGENCY_WO_DEST_RCV_POD, --业务场景
    ...
    --按核销单的记账日期来统计，关联判断核销时点应收单的签收状态为未签收，
    --应收单的单据子类型为应收（产品类型为：包裹）、落地配代理应收，应付单的单据子类型为：落地配代理应付，
    --取核销明细的金额，反核销时生成的负数也统计
    INSERT INTO STV.T_STL_DVD
      (ID,
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.LAND_PR_AGENCY_WO_DEST_RCV_NP, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE) , --始发部门编码
       ...
       WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RP --应收冲应付
             ...
             AND RCV.BILL_TYPE IN (PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DEST,--到达应收
             PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DL) --到达落地配代理应收
             AND PA.BILL_TYPE = PKG_STL_COMMON.PAYABLE_BILL_TYPE_LANDSTOWAGE -- 落地配外发成本
             AND WO.ACCOUNT_DATE >= P_BEGIN_DATE
             AND WO.ACCOUNT_DATE < P_END_DATE;

    --3）其他应付冲应收到付运费已签收：
    --按核销单的记账日期来统计，关联判断核销时点应收单的签收状态为已签收，
    --应收单的单据子类型为应收（产品类型为：包裹）、落地配代理应收，应付单的单据子类型为：落地配其他应付，
    --取核销明细的金额，反核销时生成的负数也统计
    INSERT INTO STV.T_STL_DVD
      (ID,
       PERIOD,
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.LAND_PR_OT_WO_DEST_RCV_POD, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE) , --始发部门编码
       ...
       WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RP --应收冲应付
             ...
             AND RCV.BILL_TYPE IN (PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DEST, --到达应收
                 PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DL) --到达落地配代理应收
             AND PA.BILL_TYPE = PKG_STL_COMMON.PAYABLE_BILLTYPE_LAND_OTHER -- 落落地配其他应付
             AND WO.ACCOUNT_DATE >= P_BEGIN_DATE
             AND WO.ACCOUNT_DATE < P_END_DATE;
    ...
    INSERT INTO STV.T_STL_DVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.LAND_PR_OTH_WO_DEST_RCV_NPOD, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE) , --始发部门编码
       ...
       WHERE WO.WRITEOFF_TYPE = PKG_STL_COMMON.WRITEOFF_WRITEOFF_TYPE_RP --应收冲应付
             AND (SELECT NVL(SUM(DECODE(T.POD_TYPE,
                 ...
                 AND RCV.BILL_TYPE IN (PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DEST, --到达应收
                     PKG_STL_COMMON.RECEIVABLE_BILL_TYPE_DL) --到达落地配代理应收
             AND PA.BILL_TYPE = PKG_STL_COMMON.PAYABLE_BILLTYPE_LAND_OTHER -- 落地配其他应付
             AND WO.ACCOUNT_DATE >= P_BEGIN_DATE
             AND WO.ACCOUNT_DATE < P_END_DATE;
    ...
    INSERT INTO STV.T_STL_DVD
      ...
      SELECT SYS_GUID(), --ID
             P_PERIOD, --期间
             PKG_STL_VCH_COMMON.LAND_PR_OTH_WO_OTH_RCV, --业务场景
             RCV.CUSTOMER_CODE, --客户编码
             RCV.CUSTOMER_NAME, --客户名称
             RCV.CUSTOMER_TYPE, --客户类型
             DECODE(RCV.PRODUCT_CODE,PKG_STL_COMMON.PRODUCT_CODE_PKG,RCV.EXPRESS_ORIG_ORG_CODE，RCV.ORIG_ORG_CODE) , --始发部门编码

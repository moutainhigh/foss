...
import com.deppon.foss.module.settlement.agency.api.shared.vo.LandSystemReportQueryVo;
 ...
 * @date 2013-08-08 3:51:47 PM
 */
public class LandSystemReportQueryAction extends AbstractAction {
	 ...
	 * 日志对象
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(LandSystemReportQueryAction.class);
	 ...
	 * 快递代理全盘报表Vo
	 */
	private LandSystemReportQueryVo vo;
	
	/**
	 * 导出excel名称
	 */
	private static final String  EXPROTNAME = "快递代理全盘报表";
	 ...
	 * @author foss-guxinhua
	 * @param
	 * @date 2013-08-08 8:33:15 PM
	 * @return
	 */
	@JSON
	public String queryLandSystemReportAction() {
		try {
			...
			// 判断action类型
			if (null == vo.getQueryType()) {
				// 如果没有值则抛出异常
				throw new SettlementException("查询类型异常");
			}
			// 如果传过来的是运单单号类型则按照按运单单号查询
			if (SettlementConstants.TAB_QUERY_BY_WAYBILL_NO.equals(vo.getQueryType())) {
				// 判断传入的运单号是否为空
				...
				// 执行查询操作
				List<LdpAgencySystemReportResultDto> list = ldpAgencySystemReportQueryService.queryLdpAgencySystemReportByDto(start,limit, vo.getDto(), cInfo);
				// 对List非空判断
			...
			if (SettlementConstants.TAB_QUERY_BY_DATE.equals(vo.getQueryType())) {
				...
				long count = ldpAgencySystemReportQueryService.queryTotalRecordsInDBByDto(vo.getDto(),cInfo);
				// 对Dto进行非空判断及判断总记录条数是否大于0
				if (count > 0) {
					// 如果不为空，设置数据总条数
					setTotalCount(count);
					// 执行查询操作
					List<LdpAgencySystemReportResultDto> list = ldpAgencySystemReportQueryService.queryLdpAgencySystemReportByDto(getStart(),getLimit(), vo.getDto(), cInfo);
					// 非空判断查询结果
					if (CollectionUtils.isNotEmpty(list)) {
						// 把结果集传回页面显示
						vo.setList(list);
	 ...
	 * @date 2013-08-08 上午11:15:41
	 * @return
	 */
	public String exportLandSystemReport(){
		try {
			List<LdpAgencySystemReportResultDto> list = null;
			...
			if (SettlementConstants.TAB_QUERY_BY_WAYBILL_NO.equals(vo.getQueryType())) {
				...
				// 执行查询操作
				list = ldpAgencySystemReportQueryService.queryLdpAgencySystemReportByDto( getStart(),getLimit(), vo.getDto(),cInfo);
			}
			// 如果传过来的是Dto则按照按Dto查询
			if (SettlementConstants.TAB_QUERY_BY_DATE.equals(vo.getQueryType())) {
				// 开始日期非空校验
			...
			// 设置名称
			exportSetting.setSheetName(EXPROTNAME);
			// 创建导出工具类
	 ...
	 * @date 2013-8-8 上午10:42:55
	 */
	private ExportResource getExportResource(List<LdpAgencySystemReportResultDto> queryList) {
		// 导出excel数据
		...
		if (CollectionUtils.isNotEmpty(queryList)) {
			...
			// 循环结果集
			for (LdpAgencySystemReportResultDto entity : queryList) {
	...
	/**
	 * @param ldpAgencySystemReportQueryService
	 */
	public void setLdpAgencySystemReportQueryService(
			ILdpAgencySystemReportQueryService ldpAgencySystemReportQueryService) {
		this.ldpAgencySystemReportQueryService = ldpAgencySystemReportQueryService;
	}

	/**
	 * @return vo
	 */
	public LandSystemReportQueryVo getVo() {
		return vo;
	}
	 ...
	 * @param vo
	 */
	public void setVo(LandSystemReportQueryVo vo) {
		this.vo = vo;
	}

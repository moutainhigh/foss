 ...
 * @version
 */
public class BillPARecAndPayImportAction extends AbstractAction{
	 ...
	 * 注入导入偏线应收、应付的service
	 */
	private IBillPARecAndPayImportService billPARecAndPayImportService;
	...
	private static final Logger logger = LogManager
			.getLogger(BillPARecAndPayImportAction.class);
	 ...
	 * @see
	 */
	public String PAImportExcel() throws Exception{
		try{
			//获取并设置应收应付的信息
			billRecAndPayImportVo.setBillRecAndPayImportDto(billPARecAndPayImportService.
	 ...
	 */
	@JSON
	public String savePAImportPayAndRec(){
		try{
			//获取参数
			...
			//保存应收信息、应付信息
    	    billPARecAndPayImportService.savePAImportPayAndRec(billRecAndPayImportDto,currentInfo);
		}catch(BusinessException e){
			logger.error(e.getMessage(),e);
	 ...
	 */
	public void setBillPARecAndPayImportService(
			IBillPARecAndPayImportService billRecAndPayImportService) {
		this.billPARecAndPayImportService = billRecAndPayImportService;
	}

 ...
 */
public class BillPaymentOnlineResultProcessor implements IProcess {

	/**
	 * 日志
	 */
	private static final Logger logger = LogManager
			.getLogger(BillPaymentOnlineResultProcessor.class);
	...
	public Object process(Object obj) throws ESBBusinessException {
		...
		//获取返回信息
		PaymentResultNotificationRequest request = (PaymentResultNotificationRequest) obj;
		// 返回Foss审批处理状态到费控
		PaymentResultNotificationResponse response = new PaymentResultNotificationResponse();
		// 对结果循环做处理,将结果放入DTO中
		if (request != null) {
			//日志记录接口参数信息
	 ...
	 */
	private void valiadteParams(PaymentResultNotificationRequest request){
		//校验金额必须大于0
